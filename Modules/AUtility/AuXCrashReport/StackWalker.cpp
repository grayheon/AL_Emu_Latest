 ...
 *                        it uses LPTSTR instead of LPCTSTR as first paremeter)
...
void StackWalker::OnLoadModule(LPCTSTR img, LPCTSTR mod, DWORD64 baseAddr, DWORD size, DWORD result, LPCTSTR symType, LPCTSTR pdbName, ULONGLONG fileVersion)
{
...
{
	FILE *fpLogFile = NULL;
	fopen_s( &fpLogFile, m_szLogFileName, "a" );
	
	//fwrite( buffer, STACKWALK_MAX_NAMELEN, 1, fpLogFile );
	fprintf_s( fpLogFile, buffer );
