...
#define FILENAME_DUMPTYPE_CONFIG	"\\DumpConfig.ini"
...
BOOL CheckCurrentThreadCallStack( LPCTSTR pszDirName, DWORD nSize )
{ 
	AuStackWalker sw; 

	if ( !sw.SetLogFileName( pszDirName, nSize ) )
		return FALSE;
...
///////////////////////////////////////////////////////////////////////////////
// DumpMiniDump
static void DumpMiniDump(HANDLE hFile, PEXCEPTION_POINTERS excpInfo, MINIDUMP_TYPE DumpType = MiniDumpNormal)
{
	if (excpInfo == NULL) 
	{
								...
								//@} Jaewon
{
	...
	//////////////////////////////////////////////////////////////////////////
	// 서버인 경우 시간에 따른 덤프파일 생성
	if(g_iServerClientType != ASSM_SERVER_TYPE_INVALID)	// 서버인 경우에만
	{
		// 현재 시간을 가져옴
		...
		// DUMP 폴더 생성
		GetCurrentDirectory( sizeof(szModuleName), szModuleName );
		lstrcat( szModuleName, "\\DUMP" );
		::CreateDirectory( szModuleName, NULL );
		TCHAR szTimeDmpDirName[128];
		ZeroMemory(szTimeDmpDirName, sizeof(szTimeDmpDirName));
		_sntprintf(szTimeDmpDirName, sizeof(szTimeDmpDirName), _T("\\%d_%d_%d_%d_%d"), 
			systemTime.wYear,
			systemTime.wMonth,
			systemTime.wDay,
			systemTime.wHour,
		...
		lstrcat( szModuleName, szTimeDmpDirName );
		::CreateDirectory( szModuleName, NULL );

		// 콜스택 로그파일 작성
		if ( !CheckCurrentThreadCallStack( szModuleName , sizeof(szModuleName) ) )
			OutputDebugString( _T("[ERROR] CheckCurrentThreadCallStack()\n") );

		// .exe, .pdb파일 복사 시작
		TCHAR szCurTimeDir[520];
		ZeroMemory(szCurTimeDir, sizeof(szCurTimeDir));
		CopyMemory( szCurTimeDir, szModuleName, sizeof(szModuleName) );

		TCHAR szCurFileName[520];
		TCHAR szDesFileName[520];
		ZeroMemory(szCurFileName, sizeof(szCurFileName));
		ZeroMemory(szDesFileName, sizeof(szDesFileName));

		GetModuleFileName( NULL, szModuleName, sizeof(szModuleName) );
		CopyMemory( szCurFileName, szModuleName, sizeof(szCurFileName) );

		CopyMemory( szDesFileName, szCurTimeDir, sizeof(szCurTimeDir) );
		lstrcat( szDesFileName, "\\" );
		lstrcat( szDesFileName, pszFilePart );
		CopyFile( szCurFileName, szDesFileName, FALSE );

		// .exe, .pdb파일의 경로상에 "."문자가 있으면 문제발생
		TCHAR* pszCurFileName = strtok( szCurFileName, "." );
		TCHAR* pszDesFileName = strtok( szDesFileName, "." );
		lstrcat( pszCurFileName, ".pdb" );
		lstrcat( szDesFileName, ".pdb" );
		CopyFile( szCurFileName, szDesFileName, FALSE );
...
#define WIDTH 15
		...
		sprintf_s(FileName, sizeof(FileName), szCurTimeDir);
		lstrcat( FileName, TEXT("\\MemoryState.txt") );
		m_LogFile.open( FileName, std::ios_base::out | std::ios_base::app);

		char strCharBuff[1024] = { 0, };

		sprintf_s(strCharBuff, sizeof(strCharBuff), TEXT("There is  %*ld percent of memory in use.\n"),
			WIDTH, statex.dwMemoryLoad);
		m_LogFile << strCharBuff;
		sprintf_s(strCharBuff, sizeof(strCharBuff), TEXT("There are %*I64d total physical memory.\n"),
			WIDTH, statex.ullTotalPhys);
		m_LogFile << strCharBuff;
		sprintf_s(strCharBuff, sizeof(strCharBuff), TEXT("There are %*I64d free physical memory.\n"),
			WIDTH, statex.ullAvailPhys);
		m_LogFile << strCharBuff;
		sprintf_s(strCharBuff, sizeof(strCharBuff), TEXT("There are %*I64d total paging file.\n"),
			WIDTH, statex.ullTotalPageFile);
		m_LogFile << strCharBuff;
		sprintf_s(strCharBuff, sizeof(strCharBuff), TEXT("There are %*I64d free paging file.\n"),
			WIDTH, statex.ullAvailPageFile);
		m_LogFile << strCharBuff;
		sprintf_s(strCharBuff, sizeof(strCharBuff), TEXT("There are %*I64d total virtual memory.\n"),
			WIDTH, statex.ullTotalVirtual);
		m_LogFile << strCharBuff;
		sprintf_s(strCharBuff, sizeof(strCharBuff), TEXT("There are %*I64d free virtual memory.\n"),
		...
		// 덤프 생성 시작
		TCHAR szTimeDmpFileName[128];
		ZeroMemory(szTimeDmpFileName, sizeof(szTimeDmpFileName));
		
		_sntprintf(szTimeDmpFileName, sizeof(szTimeDmpFileName), _T("\\Dump_%d_%d_%d_%d_%d.dmp"), 
			systemTime.wYear,
			systemTime.wMonth,
			systemTime.wDay,
			systemTime.wHour,
		...
		lstrcat( szCurTimeDir, szTimeDmpFileName );

		ZeroMemory( szModuleName, sizeof(szModuleName) );
		CopyMemory( szModuleName, szCurTimeDir, sizeof(szCurTimeDir) );
	}
	...
	StreamReadDumpConfig(FILENAME_DUMPTYPE_CONFIG);

	// Write the minidump to the file
	if (hMiniDumpFile != INVALID_HANDLE_VALUE)
	{
		DumpMiniDump(hMiniDumpFile, pExceptPtrs, g_eMiniDumpType);
...
bool	StreamReadDumpConfig( char* pszFileName )
{
	...
	GetCurrentDirectory( sizeof(szModuleName), szModuleName );
	lstrcat( szModuleName, pszFileName );

	FILE	*pFile = fopen( szModuleName , "rb" );
	if( !pFile )
	{
		printf( "%s 파일을 읽을수 없습니다" , szModuleName );
		return false;
	}
	...
	fread( (void*)&szBuff, sizeof(lDumpType), 1, pFile );

	lDumpType = atoi(szBuff);
