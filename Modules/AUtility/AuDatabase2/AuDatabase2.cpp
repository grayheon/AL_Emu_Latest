...
INT8 AuDatabase2::ExecuteQuery()
	...
	if (!m_pStatement->Execute())
	{
		...
		sprintf_s(strCharBuff, sizeof(strCharBuff), "%s[%d] Execute Error - [%s]\n", __FUNCTION__, __LINE__, szQuery);
		AuLogFile_s("LOG\\LoginQueryError.txt", strCharBuff);

		if(!m_pDatabase->IsAlive())
		{
			CHAR strCharBuff[256] = { 0, };
			sprintf_s(strCharBuff, sizeof(strCharBuff), "%s[%d] is not opend\n", __FUNCTION__, __LINE__);
			AuLogFile_s("LOG\\LoginQueryError.txt", strCharBuff);
		}
...
BOOL AuDatabase2::IsConnect()
{
	if(m_pDatabase)
	{
		return m_pDatabase->IsAlive();
	}
