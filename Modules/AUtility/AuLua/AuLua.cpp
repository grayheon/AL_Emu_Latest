...
void AuLua::PushInteger(int value)
{
	lua_pushinteger( m_pScriptContext, value );
}

void AuLua::PushNumber64( __int64 nValue )
{
	lua_pushnumber( m_pScriptContext, (lua_Number)nValue );
}

void AuLua::CallLuaFunction( const char* pLuaFunctionName )
{
	lua_pushstring( m_pScriptContext, pLuaFunctionName );
	...
	if( lua_pcall( m_pScriptContext, 0, 0, 0 ) )
	{
		if( m_fucErrorHandler )
		{
			...
			sprintf_s( strDebug, sizeof( char ) * 1024, "Lua Error - String Run\nString : %s\nError Message : %s\n", pLuaFunctionName, luaL_checkstring( m_pScriptContext, -1 ) );
			m_fucErrorHandler( strDebug );
		}
	}
}

void AuLua::CallLuaFunction( const char* pLuaFunctionName, int nParam )
{
	lua_getglobal( m_pScriptContext, pLuaFunctionName );
	lua_pushnumber( m_pScriptContext, nParam );

	if( lua_pcall( m_pScriptContext, 1, 0, 0 ) )
	{
		if( m_fucErrorHandler )
		{
			char strDebug[ 1024 ] = { 0, };
...
void AuLua::CallLuaFunction( const char* pLuaFunctionName, char* pParam )
{
	lua_getglobal( m_pScriptContext, pLuaFunctionName );
	lua_pushstring( m_pScriptContext, pParam );
...
void AuLua::CallLuaFunction( const char* pLuaFunctionName, char* pParam, int nParam )
{
	lua_getglobal( m_pScriptContext, pLuaFunctionName );
	lua_pushstring( m_pScriptContext, pParam );
	lua_pushnumber( m_pScriptContext, nParam );

	if( lua_pcall( m_pScriptContext, 2, 0, 0 ) )
	{
		if( m_fucErrorHandler )
		{
			char strDebug[ 1024 ] = { 0, };
...
int AuLua::CallLuaFunctionInt( const char* pLuaFunctionName )
{
	lua_pushstring( m_pScriptContext, pLuaFunctionName );
	lua_rawget( m_pScriptContext, LUA_GLOBALSINDEX );
	lua_call( m_pScriptContext, 0, 1 );

	int nResult = ( int )lua_tonumber( m_pScriptContext, -1 );
	return nResult;
}

float AuLua::CallLuaFunctionFloat( const char* pLuaFunctionName )
{
	lua_pushstring( m_pScriptContext, pLuaFunctionName );
	lua_rawget( m_pScriptContext, LUA_GLOBALSINDEX );
	lua_call( m_pScriptContext, 0, 1 );

	float fResult = ( float )lua_tonumber( m_pScriptContext, -1 );
	return fResult;
}

std::string AuLua::CallLuaFunctionString( const char* pLuaFunctionName )
{
	lua_pushstring( m_pScriptContext, pLuaFunctionName );
	lua_rawget( m_pScriptContext, LUA_GLOBALSINDEX );
	lua_call( m_pScriptContext, 0, 1 );

	size_t nLen = sizeof( pLuaFunctionName );
	std::string strResult = lua_tolstring( m_pScriptContext, -1, &nLen );
	return strResult;
}
