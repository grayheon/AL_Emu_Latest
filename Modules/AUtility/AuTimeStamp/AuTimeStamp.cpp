...
UINT32 AuTimeStamp::ConvertMSSqlTimeToTimeStamp(TCHAR *szMSSqlTime)
{
	if(!szMSSqlTime)
		return 0;

	int iLength = (int)strlen(szMSSqlTime);
	if(iLength >= 4)
		szMSSqlTime[iLength-4] = '\0';

	// 졸래 하드코딩 해준다.
	// 포맷은 ####/##/## ##:##:## 이거다. 고로 삑살나면 return 0;
	if(strlen(szMSSqlTime) != AUTIMESTAMP_SIZE_ORACLE_TIME_STRING)
		return 0;
	...
	memcpy(szTmp, szMSSqlTime, 4);
	szTmp[4] = '\0';
	timeptr.tm_year = atoi(szTmp) - 1900;		// 1900 Base

	memcpy(szTmp, &szMSSqlTime[5], 2);
	szTmp[2] = '\0';
	timeptr.tm_mon = atoi(szTmp) - 1;			// January == 0 이므로 1 빼줌.

	memcpy(szTmp, &szMSSqlTime[8], 2);
	szTmp[2] = '\0';
	timeptr.tm_mday = atoi(szTmp);

	memcpy(szTmp, &szMSSqlTime[11], 2);
	szTmp[2] = '\0';
	timeptr.tm_hour = atoi(szTmp);

	memcpy(szTmp, &szMSSqlTime[14], 2);
	szTmp[2] = '\0';
	timeptr.tm_min = atoi(szTmp);

	memcpy(szTmp, &szMSSqlTime[17], 2);
	szTmp[2] = '\0';
