...
BOOL AgsmArchlord::OnAddModule()
{
	...
	if (!m_pcsAgpmArchlord || !m_pcsAgsmGuild || !m_pcsAgpmItem || !m_pcsAgpmGrid || 
		...
		!m_pcsAgsmDeath || !m_pcsAgpmEventBinding || !m_pcsAgpmConfig)
		return FALSE;
...
{
	// 2011.10.06. silvermoo
	// 육성서버에서는 아크로드 결정전을 진행하지 않음
	if ( m_pcsAgpmConfig->IsPromotionServer() )
		return ;

	// NONE 상태가 아니면 아크로드 결정전 시작 불가 // 운영자명령어 잘못된 사용으로 인한 문제수정 [ 2011-07-27 silvermoo ]
	if ( AGPMARCHLORD_STEP_NONE != m_pcsAgpmArchlord->GetCurrentStep() )
		return ;

	// 아크로드 시스템 시작
	EnumCallback(AGSMARCHLORD_CB_READY_ARCHLORD_SIEGEWAR, NULL, NULL);
...
{	
	// 아크로드길드나 디카인과의 전투중일때는 다음 단계 진행 명령어를 사용하면 안된다 [ 2011-07-27 silvermoo ]
	if ( AGPMARCHLORD_STEP_ARCHLORD == m_pcsAgpmArchlord->GetCurrentStep() )
		return;

	// (임시) 공성전상태에서는 다음 단계 진행 명령어를 사용하면 안된다 [ 2011-07-27 silvermoo ]
...
BOOL AgsmArchlord::SetArachlord(AgpdCharacter *pcsCharacter)
{
	...
	{
		INT16 nPacketLength = 0;
		PVOID pvPacket = m_pcsAgpmArchlord->MakePacketNoticeNewArchlord(&nPacketLength);
		if (pvPacket && nPacketLength > sizeof(PACKET_HEADER))
			SendPacketAllUser(pvPacket, nPacketLength);
	}
...
BOOL AgsmArchlord::SetCallbackReadyArchlordSiegeWar(ApModuleDefaultCallBack pfCallback, PVOID pClass)
{
	return SetCallback(AGSMARCHLORD_CB_READY_ARCHLORD_SIEGEWAR, pfCallback, pClass);
}
