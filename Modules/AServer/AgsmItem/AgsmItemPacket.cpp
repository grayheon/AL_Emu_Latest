...
// 		}
		ulUseReverseOrbTime	= AGPMITEM_REVERSE_ORB_REUSE_INTERVAL - (GetClockCount() - pcsAgpdItemADChar->m_ulUseReverseOrbTime);
	}
...
BOOL AgsmItem::SendPacketItemAll(AgpdCharacter *pcsCharacter, UINT32 ulNID, BOOL bGroupNID, INT16 nFlag)
{
	...
	i = 0;
	for (pcsGridItem = m_pagpmGrid->GetItemSequence(&pcsItemADChar->m_csChargingInventoryGrid, &i);
		pcsGridItem;
		pcsGridItem = m_pagpmGrid->GetItemSequence(&pcsItemADChar->m_csChargingInventoryGrid, &i))
	{
		AgpdItem *pcsItem = m_pagpmItem->GetItem( pcsGridItem );
		if (!pcsItem)
			continue;
...
BOOL AgsmItem::SendPacketUseItemSuccess(AgpdCharacter *pcsCharacter, AgpdItem *pcsItem)
{
	...
	PVOID pvExtraData	= NULL;

	pvExtraData = m_pagpmItem->MakePacketExtra(pcsItem, &nPacketLength);
...
// 	UINT32	lUseReverseOrbTime	= ((AgpdItemTemplateUsable *) pcsReverseOrbTemplate)->m_ulUseInterval - (GetClockCount() - pcsAttachData->m_ulUseReverseOrbTime);
	...
	UINT32	lUseReverseOrbTime	= AGPMITEM_REVERSE_ORB_REUSE_INTERVAL - (GetClockCount() - pcsAttachData->m_ulUseReverseOrbTime);
...
BOOL AgsmItem::SendPacketItemInformation(AgpdCharacter *pcsCharacter, AgpdItem* pcsItem, AgpdItemTemplate *pcsItemTemplate, INT32 lType)
{
	if(!pcsCharacter)
		return FALSE;
	...
	if(pcsItem)
	{
		pvPacket = m_pagpmItem->MakePacketItemInforMationResult(pcsItem, lType, &nPacketLength);
		if(!pvPacket || nPacketLength < 1)
			return FALSE;
	}
	else if(pcsItemTemplate)
	{
		pvPacket = m_pagpmItem->MakePacketItemInforMationResult(pcsItemTemplate, lType, &nPacketLength);
		if(!pvPacket || nPacketLength < 1)
			return FALSE;
	}
