...
BOOL AgsmConfig::LoadConfig()
{
	...
	szTmpValue = pagsdServer->GetExtraValue(AGSMSERVER_EXTRA_KEY_IS_PROMOTION_SERVER);
	if (NULL != szTmpValue)
		m_pcsAgpmConfig->SetPromotionServer( ((strnicmp("y", szTmpValue, 1)==0)?TRUE:FALSE) );
	else
		m_pcsAgpmConfig->SetPromotionServer(FALSE);
