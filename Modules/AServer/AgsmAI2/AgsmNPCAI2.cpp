...
/*
			...
			if( m_pcsAgpmCharacter->IsPC( pcsListCharacter ) )
			{
				...
				{
					// 악당 상태더라도 쉬라인 전투지역에서는 공격받으면 안된다
					if ( m_pcsAgpmCharacter->GetRaceFromTemplate(pcsAgpdNPCCharacter->m_pcsCharacterTemplate) == m_pcsAgpmCharacter->GetRaceFromTemplate(pcsListCharacter->m_pcsCharacterTemplate)
						&& m_pcsAgpmShrineBattle->IsInShrineRegionByIndex(pcsListCharacter) )
						continue;
				}
			}
*/
			//쉬라인 지역이라면 
			if(m_pcsAgpmShrineBattle->IsInShrineRegionByIndex(pcsListCharacter))
			{
				// 같은 종족은 공격하지않는다.
				if ( m_pcsAgpmCharacter->GetRaceFromTemplate(pcsAgpdNPCCharacter->m_pcsCharacterTemplate) == m_pcsAgpmCharacter->GetRaceFromTemplate(pcsListCharacter->m_pcsCharacterTemplate) )
					continue;
			}
			else//그 외지역은 악당 레벨이 일정 이하면 공격하지 않는다.
			{
				if( m_pcsAgpmCharacter->IsPC( pcsListCharacter ) )
				{
					if (AGPMCHAR_MURDERER_LEVEL1_POINT > m_pcsAgpmCharacter->GetMurdererLevel(pcsListCharacter))
