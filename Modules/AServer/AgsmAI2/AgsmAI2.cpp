...
const float			AGSMAI2_Summons_Max_Follow	= 2500.0f;		// 소환수가 주인에게 강제로 따라가게끔 하는 거리. 2007.07.25. steeple
...
const float			AGSMAI2_Remove_Distance_Fixed_Summons		= 3000.0f;		// 고정형 소환수가 주인으로부터 멀어져 소환해제 되는 거리
...
BOOL AgsmAI2::OnInit()
{
	...
	if(!m_pagpmBattleGround || !m_pagpmMonsterPath || !m_pcsAgsmDeath || !m_pcsAgpmShrineBattle)
		return FALSE;
...
AgpdCharacter* AgsmAI2::GetTargetPC( AgpdCharacter *pcsAgpdMobCharacter, AgpdAI2ADChar *pcsAI2ADChar, AuPOS csMobPos, float fPreemptiveRange )
{
	...
	for (int i = 0; i < lNumCID; ++i)
	{
		...
		if( lAgro == 0 )
		{
				...
				if( fTempX*fTempX + fTempZ*fTempZ < fPreemptiveRange*fPreemptiveRange )
				{
					...
					else
					{
						...
						if( TRUE == CheckPreemptive( pcsAgpdMobCharacter, pcsListCharacter, pcsAI2ADChar ) )
						{
							...
							if(pcsAI2ADChar->m_lTargetID != 0)
							{
								// 원래 타겟캐릭터가 있으면 그냥 계속 때린다
								pcsAgroTargetCharacter = m_pcsAgpmCharacter->GetCharacter(pcsAI2ADChar->m_lTargetID);
			...
			{
				// 선공몹이 아니더라도 마지막에 어그로를 주었던 시간보다 정해진 시간이 지나지 않았으면 계속 공격한다
				UINT32	ulCurrentTime	= AuTimeStamp::GetCurrentTimeStamp();
				if( pcsAgpdMobCharacter->m_ulLastAddAgroDate + AGSMAI2_MIN_TARGET_KEEPING_TIME_SECOND > ulCurrentTime 
					&& NULL == pcsTargetCharacter)
				{
					pcsTargetCharacter = pcsListCharacter;
				}
			}
	...
	if (AGPMSUMMONS_PROPENSITY_ATTACK == ePropensity)
	{
		...
		if(TargetCID.IsValidIterator(iter) == TRUE)
		{
			while(iter != TargetCID.end())
			{
				...
				INT32 lRange = 0;	// IsInRange가 FALSE일 때만 값이 설정된다
				if ( FALSE == m_pcsAgpmFactors->IsInRange(&pcsSummons->m_stPos, &pcsTargetCharacter->m_stPos, &pcsSummons->m_csFactor, &pcsTargetCharacter->m_csFactor, NULL, &lRange) )
				{
					++iter;
					continue;
				}
...
// 		return TRUE;
	...
	//////////////////////////////////////////////////////////////////////////
	// 고정형 소환수는 주인과의 거리가 멀어지면 소환해제 한다
	if ( m_pcsAgpmCharacter->IsStatusFixed(pcsSummons) )
	{
		INT32 lDistance = 0;	// IsInRange가 FALSE일 때만 값이 설정된다
		if ( FALSE == m_pcsAgpmFactors->IsInRange(&pcsOwner->m_stPos, &pcsSummons->m_stPos, 0, 0, NULL, &lDistance) )
		{
			if ( AGSMAI2_Remove_Distance_Fixed_Summons <= lDistance)
			{
				m_pcsAgsmDeath->SetDead(pcsSummons);
				return TRUE;
			}
		}
	}
	...
	if(pcsTarget && pcsTarget->m_lID != pcsOwner->m_lID && pcsTarget->m_lID != pcsSummons->m_lID)
	{
		...
		{
			if( FALSE == m_pcsAgpmCharacter->IsStatusFixed(pcsSummons) )
			{
				// 고정형 소환수는 이동하면 안된다
...
INT32 AgsmAI2::CheckUseSkill( AgpdCharacter *pcsAgpdCharacter, AgpdAI2ADChar *pcsAgpdAI2ADChar, UINT32 lClockCount )
{
	...
	if( pcsAgpdAI2ADChar && pcsAgpdAI2ADChar->m_pcsAgpdAI2Template )
	{
		...
		// STLPort 문제로 계속 서버 죽어서 배열쓰도록 변경 2012-04-19 silvermoo
...
BOOL AgsmAI2::LootItem(AgpdCharacter *pcsAgpdCharacter, AgpdAI2ADChar *pcsAgpdAI2ADChar, UINT32 lClockCount)
{
	...
	if(pcsAgpdAI2ADChar->m_pcsAgpdAI2Template->m_csLootItemInfo.m_fLootItemRange > 0)
	{
		...
		if(nItemCount > 0)
		{
			...
			{
				AgpdItem *pcsLootItem = m_pcsAgpmItem->GetItem((INT32)(arNearItemList[i]));
				if(pcsLootItem)
				{
