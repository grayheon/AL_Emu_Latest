...
AuPOS AgsmSummons::GetFrontofCharacterPos(AgpdCharacter* pcsTarget, double dWidth)
{
	...
	stDirection.x = (FLOAT)(stDirection.x * dWidth);
	stDirection.y = (FLOAT)(stDirection.y * dWidth);
	stDirection.z = (FLOAT)(stDirection.z * dWidth);
...
BOOL AgsmSummons::SendSummonsCheckResult(UINT32 ulNID, eAgpmSummonsSummonsCheckResult eCheckResult)
{
	...
	switch(eCheckResult)
	{
		...
		case AGPMSUMMONS_SUMMONS_CHECK_RESULT_FAILURE_SUMMON_ITEM:
			lCode = AGPMSYSTEMMESSAGE_CODE_CANNOT_USE_BY_STATUS;
			break;
	}
