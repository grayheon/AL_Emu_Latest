...
int AgsmNpcManager::LuaRegister(lua_State* pLuaState)
{
	if(pLuaState)
	{
		...
		lua_register(pLuaState, "RequestJoinWorldChampionShip",		AgsmNpcManager::RequestJoinWorldChampionShip);
		lua_register(pLuaState, "EnterWorldChampionShip",			AgsmNpcManager::EnterWorldChampionShip);
		...
		lua_register(pLuaState, "AddExp",							AgsmNpcManager::AddExp);
...
int AgsmNpcManager::IsCriminal( lua_State* L )
{
	...
	INT32 iBadGuy = 0;
	// 악당이지만 현상수배자가 아니면 포탈이용이 가능하여 악당인 유저는 포탈이용제약을 하기로함.
	if( pThis->m_pagpmCharacter->HasPenalty(pcsCharacter,AGPMCHAR_PENALTY_POTAL) )
		iBadGuy = 1;
...
int AgsmNpcManager::MoveIncludeParty(lua_State* L)
{
	...
	if ( pThis->m_pagsmCharacter->MoveCharacterIncludeParty(pcsCharacter, PosX, PosZ) )
	{
		lMoveResult = 1;
	}

	lua_pushinteger(L, lMoveResult);
...
int AgsmNpcManager::AddMoney(lua_State* L)
{
	...
	INT32 CID = (INT32)lua_tonumber(L, -2);
	INT32 MONEY = (INT32)lua_tonumber(L, -1);
...
int AgsmNpcManager::AddExpByPercent(lua_State* L)
{
	...
	INT32 CID = (INT32)lua_tonumber(L, -2);
	INT32 PERCENT = (INT32)lua_tonumber(L, -1);
	...
	INT64 lLevelUpExp = pThis->m_pagpmCharacter->GetLevelUpExp(pcsCharacter);
	INT64 lExp = pThis->m_pagpmCharacter->GetExp(pcsCharacter);
	INT32 nRemainExpPercent = (INT32)(100 - ( (double)lExp / (double)lLevelUpExp * 100.0));

	if ( PERCENT <= nRemainExpPercent )
	{
		// 더해줄 경험치만큼 더해줘도 레벨업하지않으면 그냥 더하고 끝냄
	...
	{
		// 경험치량이 많아서 레벨업하게 되면 남는경험치를 다시 계산해서 더해줌
		while ( 0 < PERCENT)
		{
			INT64 lAddExp = (INT64) ( (double)PERCENT * (double)pThis->m_pagpmCharacter->GetLevelUpExp(pcsCharacter) / 100.0 );
			BOOL bResult = pThis->m_pagsmDeath->AddBonusExpToChar(pcsCharacter, NULL, lAddExp, FALSE, FALSE, TRUE);
			lua_pushnumber(L, bResult);
			PERCENT -= nRemainExpPercent;
			nRemainExpPercent = 100;
...
int AgsmNpcManager::AddExp(lua_State* L)
{
	AgsmNpcManager* pThis = AgsmNpcManager::GetInstance();

	INT32 CID = (INT32)lua_tonumber(L, -2);
	INT32 EXP = (INT32)lua_tonumber(L, -1);
	...
	BOOL bResult = pThis->m_pagsmDeath->AddBonusExpToChar(pcsCharacter, NULL, EXP, FALSE, FALSE, TRUE);
