...
BOOL AgsmBilling::SendBuyCashItemGlobal(AgpdCharacter *pAgpdCharacter, INT32 lProductID, CHAR *pszDesc, INT64 llMoney, stCashItemBuyList &sList)
{
	...
	DWORD AccessIP = inet_addr((const char*)&pAgsdCharacter->m_strIPAddress[0]);

	return m_pAgsmBillingGlobal->BuyProduct(pAgsdCharacter->m_szAccountID, lProductID, szItemName, ullListSeq, Class, Level, pAgpdCharacter->m_szID, pAgsdServer->m_lServerID, (double)llMoney, lType, AccessIP);
}
...
BOOL AgsmBilling::OnInquireCash(long ReturnCode, CHAR* AccountID, double WCoin, double PCoin )
{
	...
	PACKET_BILLINGINFO_CASHINFO pPacket(pcsCharacter->m_lID, (INT64)pCash.m_WCoin, (INT64)pCash.m_PCoin);
