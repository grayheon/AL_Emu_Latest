...
public:
	...
	BOOL				CheckSpecialStatusRate(AgpdSkill* pcsSkill);
...
public:
	...
	BOOL				EndRaceBuffedSkill(ApBase* pcsBase, BOOL bCreateTempSkill);//JK_쉬라인추가
	...
	BOOL				SetDamageOnlyTemporary(AgpdSkill* pcsSkill);		// 스킬자체는 성공이지만, 데미지만 적용하고 추가효과가 확률로 인해 실패할때
	BOOL				IsSetDamageOnlyTemporary(AgpdSkill* pcsSkill);		
	...
	static BOOL			CBPreCheckEnableJanusTalisman(PVOID pData, PVOID pClass, PVOID pCustData);
...
//	BOOL				WriteMasteryLog(ApBase* pcsBase);
	...
	BOOL CalcSkillOptionIgnoreStatus(AgpdSkill* pcsSkill, AgpdCharacter* pcsCharacter, BOOL bAdd);
};
