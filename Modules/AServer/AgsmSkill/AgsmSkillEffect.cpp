...
BOOL AgsmSkillEffect::ProcessSkillEffect(AgpdSkill *pcsSkill, ApBase *pcsTarget, INT16 nIndex, BOOL bProcessInterval)
{
	...
	for (int i = 0; i < nRepeatCount; ++i)
	{
		if (bProcessInterval)
		{
			...
			if (((AgpdSkillTemplate *) pcsSkill->m_pcsTemplate)->m_lProcessIntervalEffectType[nIndex] & AGPMSKILL_EFFECT_IGNORE_STATUS_TYPE)
				bRetval |= ProcessSkillEffectIgnoreStatus(pcsSkill, pcsTarget, nIndex, bProcessInterval);
		...
		else
		{
			...
			if (((AgpdSkillTemplate *) pcsSkill->m_pcsTemplate)->m_lEffectType[nIndex] & AGPMSKILL_EFFECT_IGNORE_STATUS_TYPE)
				bRetval |= ProcessSkillEffectIgnoreStatus(pcsSkill, pcsTarget, nIndex, bProcessInterval);
...
BOOL AgsmSkillEffect::ProcessSkillEffectTransformTargetType5Cancel(AgpdSkill* pcsSkill, AgpdCharacter* pcsCharacter)
{
	...
	BOOL	bResult = FALSE;
	bResult = m_pagpmCharacter->RestoreEvolutionCharacter(pcsCharacter, pcsTransformTemplate);
	if (!bResult)
		return FALSE;
	...
	if (pcsTarget->m_eType == APBASE_TYPE_CHARACTER)
	{
		...
		if(lSpecialStatus & AGPMSKILL_SPECIAL_STATUS_STUN)
		{
			...
			if(((AgpdCharacter*)pcsTarget)->m_ulSpecialStatus & AGPDCHAR_SPECIAL_STATUS_STUN_PROTECT)
			{
				...
				if(pcsAgsdTarget)
					lAdjustProbability += m_pagsmCombat->GetStunDefenceRate( (AgpdCharacter*)pcsSkill->m_pcsBase, (AgpdCharacter*)pcsTarget );
			}
...
*/
	...
	if(pcsEquipItem->m_eStatus == AGPDITEM_STATUS_EQUIP)
	{
		...
		// 여기까지 왔으면 아이템의 상태를 AGPMITEM_STATUS_DISARMAMENT 상태로 바꿔준다.
...
BOOL AgsmSkillEffect::ProcessSkillEffectDOT(AgpdSkill *pcsSkill, ApBase *pcsTarget, INT16 nIndex, BOOL bProcessInterval)
{
	...
	// 도트효과가 적용되어야 하는지 확률을 체크한다
	// skill_const의 special_status_rate를 이용한다
	if (!bProcessInterval) // 최초 도트 적용시에만 체크해야하므로...
	{
		if (FALSE == m_pagsmSkill->CheckSpecialStatusRate(pcsSkill))
		{
			// 도트는 실패해도 일반 데미지는 적용해야 하므로 TRUE 리턴
			return TRUE;
		}
	}
...
BOOL AgsmSkillEffect::ProcessSkillEffect2Summons(AgpdSkill* pcsSkill, ApBase* pcsTarget, INT16 nIndex, BOOL bProcessInterval)
{
	...
	if(bProcessInterval)
	{
		...
		if(pcsSkillTemplate->m_nProcessIntervalEffectDetailType[AGPMSKILL_EFFECT_DETAIL_SUMMONS] & AGPMSKILL_EFFECT_DETAIL_SUMMONS_TYPE11)
			bResult |= ProcessSkillEffect2SummonsType11(pcsSkill, pcsTarget, bProcessInterval);
	}
	else
	{
		...
		if(pcsSkillTemplate->m_nEffectDetailType[AGPMSKILL_EFFECT_DETAIL_SUMMONS] & AGPMSKILL_EFFECT_DETAIL_SUMMONS_TYPE11)
			bResult |= ProcessSkillEffect2SummonsType11(pcsSkill, pcsTarget, bProcessInterval);
	}
...
BOOL AgsmSkillEffect::ProcessSkillEffect2SummonsType1(AgpdSkill* pcsSkill, ApBase* pcsTarget, BOOL bProcessInterval)
{
	...
	INT32 lItemSummonsCount = m_pagpmSummons->GetItemSummonsCount( (AgpdCharacter*)pcsSkill->m_pcsBase );
	if (lItemSummonsCount > 0 
		&& m_pagpmSummons->GetSummonsCount((AgpdCharacter*)pcsSkill->m_pcsBase) < m_pagpmSummons->GetMaxSummonsCount((AgpdCharacter*)pcsSkill->m_pcsBase) )
		m_pagpmSummons->AddMaxSummonsCount((AgpdCharacter*)pcsSkill->m_pcsBase, lSummonsCount);
...
BOOL AgsmSkillEffect::ProcessSkillEffect2DisturbCharacterAction(AgpdSkill *pcsSkill, ApBase *pcsTarget, INT16 nIndex, BOOL bProcessInterval)
{
	...
	// 데미지만 입히고 스킬효과는 적용되지 않는 상태라면 처리하지 않는다
...
BOOL AgsmSkillEffect::ProcessSkillEffectIgnoreStatus( AgpdSkill *pcsSkill, ApBase *pcsTarget, INT16 nIndex, BOOL bProcessInterval )
{
	if (!pcsSkill || !pcsSkill->m_pcsTemplate || !pcsTarget)
		return FALSE;
	...
	BOOL bResult = m_pagsmSkill->CalcSkillOptionIgnoreStatus(pcsSkill, (AgpdCharacter*)pcsTarget, TRUE);

	return bResult;
}

BOOL AgsmSkillEffect::ProcessSkillEffect2SummonsType11( AgpdSkill* pcsSkill, ApBase* pcsTarget, BOOL bProcessInterval )
{
	if(!m_pagsmSummons)
		return FALSE;
	...
	INT32 lCreateNum = m_pagsmSummons->ProcessSummons(static_cast<AgpdCharacter*>(pcsTarget), lSummonsTID, ulDuration, lSummonsCount, lSkillLevel,
		AGPMSUMMONS_TYPE_ITEM, AGPMSUMMONS_PROPENSITY_DEFENSE, pcsSkill->m_bForceAttack);
