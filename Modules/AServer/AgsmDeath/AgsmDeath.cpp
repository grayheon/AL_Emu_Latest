...
BOOL AgsmDeath::ProcessPinchCharacterDeath(AgpdCharacter *pAttackChar, AgpdCharacter *pTargetChar)
{
	...
	if(pcsAgpdAI2ADChar && pcsAgpdAI2ADChar->m_pcsAgpdAI2Template)
	{
		...
		{
			if(pcsAgpdAI2ADChar->m_csPinchInfo.pcsPinchCharcter && pcsAgpdAI2ADChar->m_csPinchInfo.pcsPinchCharcter->m_pcsCharacterTemplate)
			{
				if( pcsAgpdAI2ADChar->m_csPinchInfo.pcsPinchCharcter->IsDead() )
				{
					// 본체가 먼저 죽어있으면 처리하지 않는다
					char strCharBuff[256] = { 0, };
					sprintf_s(strCharBuff, sizeof(strCharBuff), "[ERROR] Attack CHARID : %d \n",
						pAttackChar->m_szID);
					AuLogFile_s(_T("LOG\\PinchMonsterError.txt"), strCharBuff);
					return TRUE;
				}

				AgpdAI2ADChar *pcsAgpdAI2PinchChar = m_pagpmAI2->GetCharacterData(pcsAgpdAI2ADChar->m_csPinchInfo.pcsPinchCharcter);
				if(pcsAgpdAI2PinchChar && pcsAgpdAI2PinchChar->m_pcsAgpdAI2Template)
				{
					if( pcsAgpdAI2ADChar->m_csPinchInfo.pcsPinchCharcter->IsDead() )
					{
						// 본체가 먼저 죽어있으면 처리하지 않는다
	...
	*/
	...
	if(TRUE && !bIsQuest)	// 해당 아이템이 사용중인지 아닌지 체크한다.
	{
		...
		if((ulPCRoomType & AGPDPCROOMTYPE_HANGAME_TPACK) || (ulPCRoomType & AGPDPCROOMTYPE_WEBZEN_GPREMIUM)) 
			llBonusExpByCash += (INT64)m_pagpmConfig->GetTPackExpRatio();
	...
	// 야누스 타입 체크
	if( pcsCharacter->m_eJanusType != AGPDCHAR_JANUS_TALISMAN_NONE
	...
	m_pagsmCharacter->WriteExpLog(pcsCharacter, (INT32)llBonusExp, bIsQuest); // arycoat 2010.07.12
...
BOOL AgsmDeath::ConAgsdDeath(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	pagsdDeath->m_tPvPRaceWinPointGiveTime = 0;
