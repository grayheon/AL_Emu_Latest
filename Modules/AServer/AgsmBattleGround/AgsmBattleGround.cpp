...
#define SEND_RACE_PVP_INFO_MIN_TIME 3

AgsmBattleGround* AgsmBattleGround::m_pInstance = NULL;

AgsmBattleGround::AgsmBattleGround()
	: m_ulClockCount(0), pLuaState(0), m_lSendRacePvPInfoMinTime(0)
{
	m_pInstance = this;
	m_Mutex.Init();
...
BOOL AgsmBattleGround::OnInit()
{
	...
	m_pcsAgsmWarInfo        = (AgsmWarInfo *)GetModule("AgsmWarInfo");
	...
	if(!m_pagpmBattleGround || !m_pagpmCharacter || !m_pagsmPvP || !m_pagpmEventBinding
		...
		|| !m_papmMap || !m_pagpmEventTeleport || !m_pcsAgsmWarInfo
		)
		return FALSE;
	...
	if(pLuaState)
	{
		...
		lua_register(pLuaState, "SendBattleGroundWarInfo",	AgsmBattleGround::SendBattleGroundWarInfo);
		lua_register(pLuaState, "SendBattleGroundTimeStep",	AgsmBattleGround::SendBattleGroundTimeStep);
...
BOOL AgsmBattleGround::OnTimer(UINT32 ulClockCount)
{
	...
	if(m_pBattleGroundState == 2 || m_pBattleGroundState == 3)
	{
		++m_lSendRacePvPInfoMinTime;
		if(m_lSendRacePvPInfoMinTime >= SEND_RACE_PVP_INFO_MIN_TIME)
		{
			m_lSendRacePvPInfoMinTime = 0;
			m_pcsAgsmWarInfo->SendRacePvPInfoToAll(BattleGround);
		}
	}
...
BOOL AgsmBattleGround::OnDead( AgpdCharacter* pcsCharacter, AgpdCharacter* pcsTarget )
{
	...
	m_pagsmPvP->ProcessCharisma(pcsCharacter, pcsTarget);
...
int AgsmBattleGround::SetBattleGroundState(lua_State* L)
{
	...
	{
	case 2:
		pThis->m_pcsAgsmWarInfo->StartWar(BattleGround);
		break;
	...
	case 3: // Start...
		{
			...
			pThis->m_pStepCheckVector.clear();
			pThis->m_cHeroDeathFlag = 0;
			...
			for(AuXmlElement* pElem = pNode->FirstChildElement("Group"); pElem; pElem = pElem->NextSiblingElement())
			{
				...
				pSpawn.strSpawnGroupName =  (CHAR*)pSpawn.pElem->Attribute("Name");
				pThis->m_pStepCheckVector.push_back(pSpawn);
...
int AgsmBattleGround::SendBattleGroundTimeStep(lua_State* L)
{
	AgsmBattleGround* pThis = AgsmBattleGround::GetInstance();
	int iTimeStep	= (int)lua_tointeger(L, -1);
	
	pThis->m_pcsAgsmWarInfo->SendTimeStep(BattleGround, iTimeStep);
	return 0;
}

int AgsmBattleGround::SendBattleGroundWarInfo(lua_State* L)
{
	AgsmBattleGround* pThis = AgsmBattleGround::GetInstance();
	int iStep	= (int)lua_tointeger(L, -2);
	int iMessage	= (int)lua_tointeger(L, -1);

	pThis->m_pcsAgsmWarInfo->SendWarInfo(BattleGround, (INT8)--iStep, (INT8)--iMessage);
	return 0;
}
		...
		if(pThis->GetSpawnGroupCharCount((CHAR*)it->strSpawnGroupName.c_str()) == 0 )
		{
			...
			if(szNotice)
			{
				...
				if(pElem2)
				{
					...
					const char *szStep = it->pElem->Attribute("Step");
					if(szStep)
					{
						int iStep = atoi(it->pElem->Attribute("Step"));
						pThis->m_pcsAgsmWarInfo->SendWarInfo(BattleGround, --iStep,0);
					}
				}
			}
...
/*
AgsmPVP ·Î ÇØ´ç ³»¿ë ¿Å°ÜÁü. 20101207@supertj
...
void AgsmBattleGround::CheckStepMonster(AgpdCharacter* pcsTarget)
{
	AgpdSpawnADChar* pagpdSpawnADChar = m_pcsAgpmEventSpawn->GetCharacterData(pcsTarget);
	if(!pagpdSpawnADChar || !pagpdSpawnADChar->m_pstSpawn || !pagpdSpawnADChar->m_pstSpawn->m_pstParent)
		return;
	if(m_pBattleGroundState != BATTLEGROUND_STATE_ING) return;

	string strSpawnGrup(pagpdSpawnADChar->m_pstSpawn->m_pstParent->m_szName);

	for(CSpawnVector::iterator it = m_pStepCheckVector.begin(); it != m_pStepCheckVector.end(); ++it)
	{
		if(it->pElem == NULL) continue;
		if(it->strSpawnGroupName == strSpawnGrup)
		{
			if(GetSpawnGroupCharCount((CHAR*)strSpawnGrup.c_str()) == 0 )
			{
				const CHAR *szStep = it->pElem->Attribute("Step");
				if(szStep)
				{   
					INT8 iStep = atoi(szStep);
					m_pcsAgsmWarInfo->SendWarInfo(BattleGround, iStep - 1, 1);	

					it->pElem = it->pElem->NextSiblingElement();
					if(!it->pElem) return;
					it->strSpawnGroupName = it->pElem->Attribute("Name");
				}
			}
			return;
		}
...
void AgsmBattleGround::AbilityMonsterRemove(AgpdCharacter* pcsCharacter, AgpdCharacter* pcsTarget)
{
	...
	if(NULL == pagpdSpawnADChar->m_pstSpawn)
		return;

	if(NULL == pagpdSpawnADChar->m_pstSpawn->m_pstParent)
		return;
	...
	if(pcsCharacter)
	{
		...
		if(lSiegeWarType == AGPDSPAWN_BATTLEGROUND_ABILITY)
		{
			...
			if(pElem2)
			{
				...
				PACKET_BATTLEGROUND_NOTICE pPacketBattleGroundNotice(strNotice, dwColor, 5, FALSE);
			...
			for(CSpawnVector::iterator it = m_pBossVector.begin(); it != m_pBossVector.end(); ++it)
			{
				...
				if(pNoticeElem)
				{
					...
					PACKET_BATTLEGROUND_NOTICE pPacketBattleGroundNotice(strNotice2, dwColor, 5, FALSE);
					SendPacketAllUser(pPacketBattleGroundNotice);

					const CHAR *strTemp = pBossElem->Attribute("AddInfo");
					if(strTemp)
					{
						INT8 cUpdateNum = atoi(strTemp);
						INT8 cTempFlag = 0x01 << cUpdateNum;
						m_cHeroDeathFlag |= cTempFlag;
						m_pcsAgsmWarInfo->SendAddInfo(BattleGround, m_cHeroDeathFlag, cUpdateNum);
					}

				}
				
				m_pagsmServerStatus->MakeAndSendGameInfoPacket(m_pagsmServerStatus->GetConnectedSocketIndex(), 
															   pcsCharacter->m_szID, 
															   m_pagpmCharacter->GetCharacterRaceString(pcsCharacter), 
...
BOOL AgsmBattleGround::WriteBattleGroundLog(eAGPDLOGTYPE_ETC eType, AgpdCharacter *pcsKillCharacter, AgpdCharacter *pcsTarget)
{
	...
	_stprintf(szValue, _T("(%s) ½Â=%s, ÆÐ=%s"), pAgsdCharacter->m_szServerName,
...
BOOL AgsmBattleGround::SendBattleGroundNotice(AgpdCharacter *pcsCharacter, CHAR* szNotice, INT32 lNoticeType, CHAR* szDirectNotice)
{
	...
	if(pElem2)
	{
		...
		//m_pcsAgsmWarInfo->SendWarInfo(BattleGround,1,0,1);
...
BOOL AgsmBattleGround::CBEnterGameWorld(PVOID pData, PVOID pClass, PVOID pCustData)
{
	if(!pData || !pClass)
		return FALSE;
	...
	if(pThis->m_pBattleGroundState <= BATTLEGROUND_STATE_NORMAL
		|| pThis->m_pBattleGroundState == BATTLEGROUND_STATE_AFTER)
		return TRUE;
	...
	pThis->m_pcsAgsmWarInfo->SendLastInfoForChar(pcsCharacter);
...
BOOL AgsmBattleGround::OnPvPDead(AgpdCharacter* pcsAttackCharacter, AgpdCharacter* pcsDeadCharacter)
{
	if(!pcsAttackCharacter || !pcsDeadCharacter)  return FALSE;


	if( !m_pagpmBattleGround->IsInBattleGround(pcsDeadCharacter) || !m_pagpmBattleGround->IsInBattleGround(pcsAttackCharacter)
		|| m_pagpmCharacter->IsSameRace(pcsDeadCharacter, pcsAttackCharacter))
		return FALSE;

	if(m_pBattleGroundState == 2 || m_pBattleGroundState == 3)
	{
		m_pcsAgsmWarInfo->ProcessPvPResult(BattleGround, pcsAttackCharacter, pcsDeadCharacter);
	}
	return TRUE;
}
