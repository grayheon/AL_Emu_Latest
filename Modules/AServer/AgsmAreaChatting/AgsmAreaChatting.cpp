...
BOOL AgsmAreaChatting::OnInit()
{
	...
	if(!m_pagpmBattleGround || !m_pagsmSystemMessage || !m_pagpmShrineBattle)
		return FALSE;
...
BOOL AgsmAreaChatting::CBRegionChange(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	if (nPrevRegion >= 0 && nPrevRegion < AGSMAREACHATTING_MAX_REGIONTEMPLATE)
		...
		pThis->m_acsRegionAdmin[nPrevRegion].RemoveObject(pcsCharacter->m_lID);
	...
	if (nRegionIndex >= 0 && nRegionIndex < AGSMAREACHATTING_MAX_REGIONTEMPLATE)
		...
		pThis->m_acsRegionAdmin[nRegionIndex].AddObject(&pcsCharacter->m_lID, pcsCharacter->m_lID);
...
BOOL AgsmAreaChatting::CBRemoveCharacterFromMap(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	if (nRegionIndex >= 0 && nRegionIndex < AGSMAREACHATTING_MAX_REGIONTEMPLATE)
		...
		pThis->m_acsRegionAdmin[nRegionIndex].RemoveObject(pcsCharacter->m_lID);
...
BOOL AgsmAreaChatting::CBRecvChatMessage(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	//JK_쉬라인추가
	if(pThis->m_pagpmShrineBattle->IsInShrineRegionByIndex(pcsCharacter))
	{
		if( pThis->m_pagsmSystemMessage )
			pThis->m_pagsmSystemMessage->SendSystemMessage( pcsCharacter , AGPMSYSTEMMESSAGE_CODE_CANNOT_USE_BY_STATUS );
							...
							case AGSMAREACHAT_TYPE_REGION:
								{
									...
									PVOID	pvPacket		= pThis->MakeAreaChatPacket(pcsBuffer, pcsCharacter, &nPacketLength, NULL, AGPDCHATTING_TYPE_REGION);
							...
							case AGSMAREACHAT_TYPE_RACE:
								{
									...
									{
										INT16	nPacketLength	= 0;
										PVOID	pvPacket		= pThis->MakeAreaChatPacket(pcsBuffer, pcsCharacter, &nPacketLength, pcsItem->m_pcsItemTemplate->m_szName, AGPDCHATTING_TYPE_RACE);
							...
							case AGSMAREACHAT_TYPE_ALL:
								{
									...
									{
										INT16	nPacketLength	= 0;
										PVOID	pvPacket		= pThis->MakeAreaChatPacket(pcsBuffer, pcsCharacter, &nPacketLength, pcsItem->m_pcsItemTemplate->m_szName, AGPDCHATTING_TYPE_ALL);
							...
							case AGSMAREACHAT_TYPE_ARCHLORD:
								{
									...
									{
										INT16	nPacketLength	= 0;
										PVOID	pvPacket		= pThis->MakeAreaChatPacket(pcsBuffer, pcsCharacter, &nPacketLength, ServerStr().GetStr(STI_ARCHLORD_ENG_UPPER), AGPDCHATTING_TYPE_ARCHLORD);
...
try
{
	...
	INT32	*pnCID	= (INT32 *)m_acsRegionAdmin[nRegionIndex].GetObjectSequence(&lIndex);
...
*/
	while (pnCID)
	{
		...
		UINT32 pulNID = m_pcsAgsmCharacter->GetCharDPNID(pCharacter);

		SendPacket(pvPacket, nPacketLength, pulNID);
		pnCID	= (INT32 *)m_acsRegionAdmin[nRegionIndex].GetObjectSequence(&lIndex);
	}
}
catch(...)
{
...
BOOL AgsmAreaChatting::BroadCastMessage(AgsmChatReceiveBuffer *pcsBuffer)
{
	...
	if (!pcsBuffer->bIsNotify)
	{
		...
		INT16	nPacketLength	= 0;
		PVOID	pvPacket		= MakeAreaChatPacket(FALSE, pcsBuffer->szSenderName, pcsBuffer->szServerName, pcsBuffer->szPacketBuffer, pcsBuffer->lSize, &nPacketLength, szItemName, AGPDCHATTING_TYPE_ALL);
