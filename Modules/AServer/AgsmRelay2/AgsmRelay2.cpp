...
BOOL AgsmRelay2::OnAddModule()
	...
	if (m_pAgsmCharacter)
		...
		if (!m_pAgsmCharacter->SetCallbackBankUpdate(CBAccountWorldUpdate, this))
			return FALSE;
		if(!m_pAgsmCharacter->SetCallbackUseEscapeDate(CBUseEscapeDateUpdate, this))
			return FALSE;
...
BOOL AgsmRelay2::OnReceive(UINT32 ulType, PVOID pvPacket, INT16 nSize, UINT32 ulNID, DispatchArg *pstCheckArg)
	...
	switch (nParam)
		...
		case AGSMRELAY_PARAM_CHARGAMEOUT:
			...
			break;
		case AGSMRELAY_PARAM_USE_ESCAPE_UPDATE:
			OnParamUseEscapeUpdate(nParam, pvPacketEmb, ulNID);
			break;
		...
		case AGSMRELAY_PARAM_CHARACTER_CREATION_DATE:
			...
			break;
		case AGSMRELAY_PARAM_COUPON:
			OnParamCoupon((PACKET_HEADER*)pvPacket, ulNID);
			break;
		//JK_특성화서버
		case AGSMRELAY_PARAM_SERVERMOVE:
			OnParamServerMove((PACKET_HEADER*)pvPacket, ulNID);
			break;
		case AGSMRELAY_PARAM_PVP:
			OnParamPvP((PACKET_HEADER*)pvPacket, ulNID);
			break;
		default:
...
BOOL AgsmRelay2::SetCallbackUseEscapeDateUpdate(ApModuleDefaultCallBack pfCallback, PVOID pClass)
{
	return SetCallback(AGSMRELAY_PARAM_USE_ESCAPE_UPDATE, pfCallback, pClass);
}
...
BOOL AgsmRelay2::SetCallbackOperation()
	...
	if (!SetCallbackCustom(CBOperationCustom, this)
		...
		|| !SetCallbackUseEscapeDateUpdate(CBOperation, this)
