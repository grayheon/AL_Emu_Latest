...
void AgsmRelay2::InitPacketQuest()
	...
	m_csPacketQuest.SetFieldType(AUTYPE_INT16,		1,								// eAgsmRelay2Operation
								 ...
								 AUTYPE_INT32,		1,								// bRegisterJournal;
...
BOOL AgsmRelay2::OnParamQuestFlag(INT16 nParam, PVOID pvPacket, UINT32 ulNID)
	...
	if(pszQuestBlock1)
		memcpy(pcsQuestFlag->m_szQuestBlock1, pszQuestBlock1, AGSDQUEST_MAX_FLAG);

	if(pszQuestBlock2)
		memcpy(pcsQuestFlag->m_szQuestBlock2, pszQuestBlock2, AGSDQUEST_MAX_FLAG);

	if(pszQuestBlock3)
		memcpy(pcsQuestFlag->m_szQuestBlock3, pszQuestBlock3, AGSDQUEST_MAX_FLAG);
...
BOOL AgsmRelay2::CBCurrentQuest(PVOID pData, PVOID pClass, PVOID pCustData)
	...
	csQuest.m_bRegisterJournal = pQuest->bRegisterJournal;
//	strcpy(csQuest.m_szNPC, pQuest->szNPCName);
	...
	switch (nOperation)
		...
		case AGSMQUEST_DB_CURRENT_QUEST_JOURNAL_REGISTER:
			csQuest.m_eOperation = AGSMDATABASE_OPERATION_EXECUTE;
			break;
...
BOOL AgsmRelay2::CBQuestFlag(PVOID pData, PVOID pClass, PVOID pCustData)
	...
	BYTE szQuestBlock1[AGSDQUEST_MAX_FLAG + 1] = { 0, };
	memcpy(szQuestBlock1, pQuest->m_csFlag.m_btQuestBlock1, AGSDQUEST_MAX_FLAG);
	memcpy(csQuestFlag.m_szQuestBlock1, szQuestBlock1, AGSDQUEST_MAX_FLAG);
	//csQuestFlag.m_szQuestBlock1[AGSDQUEST_MAX_FLAG] = '\0';

	BYTE szQuestBlock2[AGSDQUEST_MAX_FLAG + 1] = { 0, };
	memcpy(szQuestBlock2, pQuest->m_csFlag.m_btQuestBlock2, AGSDQUEST_MAX_FLAG);
	memcpy(csQuestFlag.m_szQuestBlock2, szQuestBlock2, AGSDQUEST_MAX_FLAG);
	//csQuestFlag.m_szQuestBlock2[AGSDQUEST_MAX_FLAG] = '\0';

	BYTE szQuestBlock3[AGSDQUEST_MAX_FLAG + 1] = { 0, };
	memcpy(szQuestBlock3, pQuest->m_csFlag.m_btQuestBlock3, AGSDQUEST_MAX_FLAG);
	memcpy(csQuestFlag.m_szQuestBlock3, szQuestBlock3, AGSDQUEST_MAX_FLAG);
...
BOOL AgsdRelay2Quest::SetParamExecute(AuStatement* pStatement)
{
	INT16 i = 0;
	pStatement->SetParam(i++, &m_bRegisterJournal);
	pStatement->SetParam(i++, m_szCharID, sizeof(m_szCharID));
	pStatement->SetParam(i++, &m_lQuestTID);
...
BOOL AgsdRelay2QuestFlag::SetParamUpdate(AuStatement* pStatement)
	{
	pStatement->SetParam(0, (CHAR*)m_szQuestBlock1, AGSDQUEST_MAX_FLAG);
	pStatement->SetParam(1, (CHAR*)m_szQuestBlock2, AGSDQUEST_MAX_FLAG);
	pStatement->SetParam(2, (CHAR*)m_szQuestBlock3, AGSDQUEST_MAX_FLAG);
