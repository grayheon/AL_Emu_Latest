...
BOOL AgsmRelay2::SendUseEscapeTime(AgpdCharacter *pAgpdCharacter)
{
	if (!pAgpdCharacter || !m_pAgsmServerManager->GetRelayServer())
		return FALSE;
	...
	PVOID	pvPacketCharacter = m_csPacketCharacter.MakePacket(FALSE, &nPacketLength, 0,
		...
		,&pAgpdCharacter->m_ulLastUseEscapeDate
		);
	if (!pvPacketCharacter)
		return FALSE;

	BOOL bResult = MakeAndSendRelayPacket(pvPacketCharacter, AGSMRELAY_PARAM_USE_ESCAPE_UPDATE);
	m_csPacketCharacter.FreePacket(pvPacketCharacter);
...
AgsdRelay2Escape::AgsdRelay2Escape(UINT32 ulNID, INT16 Operation)
{
	m_ulNID = ulNID;
	m_nOperation = Operation;
	m_eOperation = AGSMDATABASE_OPERATION_UPDATE;

	ZeroMemory(m_szCharID, sizeof(m_szCharID));
	ZeroMemory(m_szUseEscapeDate, sizeof(m_szUseEscapeDate));
	
	m_lUseEscapeDate = 0;
}

BOOL AgsdRelay2Escape::SetParamUpdate(AuStatement *pStatement)
{
	if (NULL == pStatement)
		return FALSE;
	...
	AuTimeStamp::ConvertTimeStampToOracleTime(m_lUseEscapeDate, m_szUseEscapeDate, sizeof(m_szUseEscapeDate));

	pStatement->SetParam(i++, m_szUseEscapeDate, sizeof(m_szUseEscapeDate));
	pStatement->SetParam(i++, m_szCharID, sizeof(m_szCharID));

	return TRUE;
}

BOOL AgsmRelay2::OnParamUseEscapeUpdate(INT16 nParam, PVOID pvPacket, UINT32 ulNID)
{
	if (NULL == pvPacket)
		return FALSE;

	AgsdRelay2Escape *pAgsdRelay2 = new AgsdRelay2Escape(ulNID, nParam);
	CHAR*	pszName = NULL;
	...
	strncpy(pAgsdRelay2->m_szCharID, pszName, _MAX_CHARNAME_LENGTH);
