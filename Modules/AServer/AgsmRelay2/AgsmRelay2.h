...
enum eRelayQueryIndex
{
	...
	AGSMRELAY_QUERY_REQUEST_COUPON_USE			= 1024,
	AGSMRELAY_QUERY_SERVERMOVE					= 1025,//JK_특성화서버
	AGSMRELAY_QUERY_PVP_UPDATE					= 1026,
	AGSMRELAY_QUERY_PVP_LOAD					= 1027,
};
...
class AgsmRelay2 : public AgsModule
	...
	protected:
		...
		BOOL	OnParamPvP(PACKET_HEADER* pvPacket, UINT32 ulNID);
		BOOL	OnParamUseEscapeUpdate(INT16 nParam, PVOID pvPacket, UINT32 ulNID);
	...
	public:
		...
		BOOL	SetCallbackUseEscapeDateUpdate(ApModuleDefaultCallBack pfCallback, PVOID pClass);
		...
		static BOOL CBUseEscapeDateUpdate(PVOID pData, PVOID pClass, PVOID pCustData);
		...
		static BOOL CBOperationResultCouponUse(PVOID pData, PVOID pClass, PVOID pCustData);

		static BOOL CBOperationResultServerMove(PVOID pData, PVOID pClass, PVOID pCustData);//JK_특성화서버

		static BOOL CBOperationResultLoadPvPResultSuccess(PVOID pData, PVOID pClass, PVOID pCustData);
		static BOOL CBOperationResultLoadPvPResultFail(PVOID pData, PVOID pClass, PVOID pCustData);
		...
		BOOL	OnParamCoupon(PACKET_HEADER* pvPacket, UINT32 ulNID);
		BOOL	OnParamServerMove(PACKET_HEADER* pvPacket, UINT32 ulNID);//JK_특성화서버

		BOOL	SendUseEscapeTime(AgpdCharacter *pAgpdCharacter);
	protected:
