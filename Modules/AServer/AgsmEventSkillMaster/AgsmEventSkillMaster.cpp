...
BOOL AgsmEventSkillMaster::OnAddModule()
{
	...
	if (!m_pcsAgsmItem->SetCallbackUseItemSkillRollbackScroll(CBUseItemSkillRollbackScroll, this))
		return FALSE;
	if (!m_pcsAgsmItem->SetCallbackUseItemJanusTalisman(CBUseItemJanusTalisman, this))
		return FALSE;
...
BOOL AgsmEventSkillMaster::CBLearnSkill(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	// 악당 레벨에 따른 제약이 있는지 확인 한다.
	if( pThis->m_pcsAgpmCharacter->HasPenalty( pcsCharacter, AGPMCHAR_PENALTY_SKILL_LEARN))
		return FALSE;
...
BOOL AgsmEventSkillMaster::CBUseItemJanusTalisman( PVOID pData, PVOID pClass, PVOID pCustData )
{
	if (!pData || !pClass || !pCustData)
		return FALSE;
	...
	return pThis->m_pcsAgsmSkill->CastSkill((ApBase *) pcsItem->m_pcsCharacter, ((AgpdItemTemplateUsableJanusTalisman *) pcsItem->m_pcsItemTemplate)->m_lSkillTID, ((AgpdItemTemplateUsableJanusTalisman *) pcsItem->m_pcsItemTemplate)->m_lSkillLevel, (ApBase *) pcsTargetCharacter, FALSE, FALSE, pcsItem->m_lID);
}
