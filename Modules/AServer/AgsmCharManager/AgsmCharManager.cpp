...
BOOL AgsmCharManager::OnInit()
{
	...
	if(!m_pagsmBillInfo || !m_pagpmUIStatus)
		return FALSE;
...
//				TRACEFILE(szBuffer);
			...
			//Account Manager에서 Coupon정보를 물어본다.
...
INT16 AgsmCharManager::EnterGameWorld(AgpdCharacter *pcsCharacter, BOOL bIsCheckValidate)
{
	...
	if (m_pagpmCharacter->IsArchlord(pcsCharacter))
	{
		EnumCallback(AGSMCHARMM_CB_SET_CHARACTER_GAME_DATA, pcsCharacter, NULL);
		SendCharacterWholeQuickBeltInfo(pcsCharacter, pcsAgsdCharacter->m_dpnidCharacter);
	}
...
BOOL AgsmCharManager::WriteLogoutLog(AgpdCharacter *pAgpdCharacter)
{
	...
	//{
		...
		m_pagpmLog->WriteLogInOut(	pAgsdCharacter->m_szAccountID, 
									...
									pAgsdCharacter->m_ulConnectedTimeStamp, //login시간.
									AuTimeStamp::GetCurrentTimeStamp(), //logout시간.
									m_pagsmCharacter->GetRealCharName(pAgpdCharacter->m_szID),
									lCharTID,
									lLevel,
...
BOOL AgsmCharManager::SendCharacterWholeQuickBeltInfo( AgpdCharacter *pcsCharacter, UINT32 ulNID )
{
	if (!pcsCharacter || ulNID == 0)
		return FALSE;

	INT16	nPacketLength	= 0;
	PVOID	pvPacket		= m_pagpmUIStatus->MakePacketWholeQuickBeltInfo(pcsCharacter, &nPacketLength);
	...
	m_pagpmUIStatus->m_csPacket.FreePacket(pvPacket);
