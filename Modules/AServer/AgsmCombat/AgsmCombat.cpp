...
INT32 AgsmCombat::GetCriticalDefenceRate( AgpdCharacter* pcsAttacker, AgpdCharacter* pcsTarget )
{
	...
	INT32 lRateOfSkill	= pcsTargetADSkill->m_stBuffedSkillCombatEffectArg.lDefenceCriticalAttack;		// 스킬에 의한 값
	INT32 lRateOfItem	= pAgsdTarget->m_stOptionSkillData.m_lCriticalDefenseRate;						// 아이템에 의한 값

	// 일단 더해주고
	lDefenceRate = lRateOfSkill + lRateOfItem;

	// 공격자의 크리티컬저항력 무시값을 빼준다
