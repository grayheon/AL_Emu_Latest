...
BOOL AgsmDropItem2::DropItem( AgpdCharacter *pcsAgpdCharacter, ApBase *pcsFirstLooter )
{
	...
	// 야누스 타입 체크
	if ( pcsLooter 
		&& ( pcsLooter->m_eJanusType == AGPDCHAR_JANUS_TALISMAN_NONE || pcsLooter->m_eJanusType == AGPDCHAR_JANUS_TALISMAN_DROP2 ) )
	{
	...
	else
	{
		...
		// 야누스 타입 체크
		if ( pcsLooter
			&& ( pcsLooter->m_eJanusType != AGPDCHAR_JANUS_TALISMAN_NONE && pcsLooter->m_eJanusType != AGPDCHAR_JANUS_TALISMAN_GHELD ) )
		{
			lMinGhelld = 1;
		}
