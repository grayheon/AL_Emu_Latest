...
INT32	AgsmEventGacha::ProcessGacha( AgpdCharacter * pcsCharacter , INT32 nGachaType )
{
	...
	// 가챠에서 이상한거 나오면 안댐!!!!!!!!!!!!!!!....ㅠㅜ 급한대로 하드코딩으로 뙇!!!
	if (nTID == 51301
		|| nTID == 51302
		|| nTID == 51303
		|| nTID == 51304)
	...
	if( pThis && nTID && pcsCharacter )
	{
			...
			iter++)
		{
			...
			case AgsdCharacter::GachaDelayItemUpdateData::STACKABLE:
				...
				break;
			case AgsdCharacter::GachaDelayItemUpdateData::CHARACTER_POINT:
				pThis->m_pcsAgsmItem->AddItemToCharacterPointForGacha( pcsCharacter, data.pcsItem );
				break;
			}
		}
