...
BOOL AgsmItemManager::GiveDefaultItem(AgpdCharacter *pcsCharacter, INT32 lSetIndex, BOOL bJumpingEvent/* = FALSE*/)
{
	...
	if(NULL == pcsItemADChar || NULL == pcsADCharTemplate)
		return FALSE;

	AgpdItemADCharTemplate::EquipItemSetInfo stEquipItemSetInfo;

	vector<AgpdItemADCharTemplate::EquipItemSetInfo>::iterator iterEquip = pcsADCharTemplate->m_vtEquipItemSetInfo.begin();
	for(iterEquip; iterEquip < pcsADCharTemplate->m_vtEquipItemSetInfo.end(); iterEquip++)
	{
		if(lSetIndex == iterEquip->m_nSetNumber && bJumpingEvent == iterEquip->m_bIsJumpingEventEquip)
		{
			stEquipItemSetInfo = *iterEquip;
			break;
		}
	}
	...
	{
		if (!stEquipItemSetInfo.m_lDefaultEquipITID[i])
			continue;

		AgpdItem *pcsItem = CreateItem(stEquipItemSetInfo.m_lDefaultEquipITID[i], pcsCharacter);
		if (!pcsItem)
			continue;

		if (!m_pagpmItem->EquipItem(pcsCharacter, pcsItem, TRUE, TRUE))
		{
			m_pagpmItem->RemoveItem(pcsItem->m_lID);

			continue;
		}
	}

	AgpdItemADCharTemplate::InvenItemSetInfo stInvenItemSetInfo;

	vector<AgpdItemADCharTemplate::InvenItemSetInfo>::iterator iterInven = pcsADCharTemplate->m_vtInvenItemSetInfo.begin();
	for(iterInven; iterInven < pcsADCharTemplate->m_vtInvenItemSetInfo.end(); iterInven++)
	{
		if(bJumpingEvent == iterInven->m_bIsJumpingEventItem)
		{
			stInvenItemSetInfo = *iterInven;
			break;
		}
	}
	...
	{
		if (stInvenItemSetInfo.m_aalDefaultInvenInfo[i][AGPDITEM_DEFAULT_INVEN_INDEX_TID] == AP_INVALID_IID)
			break;

		INT32	lCreateItemNum	= 1;
		AgpdItemTemplate	*pcsItemTemplate	= m_pagpmItem->GetItemTemplate(stInvenItemSetInfo.m_aalDefaultInvenInfo[i][AGPDITEM_DEFAULT_INVEN_INDEX_TID]);
		if (pcsItemTemplate && pcsItemTemplate->m_bStackable)
		{
			lCreateItemNum	= stInvenItemSetInfo.m_aalDefaultInvenInfo[i][AGPDITEM_DEFAULT_INVEN_INDEX_STACK_COUNT] / pcsItemTemplate->m_lMaxStackableCount;

			if (stInvenItemSetInfo.m_aalDefaultInvenInfo[i][AGPDITEM_DEFAULT_INVEN_INDEX_STACK_COUNT] % pcsItemTemplate->m_lMaxStackableCount)
				++lCreateItemNum;
		}

		INT32	lStackCount	= stInvenItemSetInfo.m_aalDefaultInvenInfo[i][AGPDITEM_DEFAULT_INVEN_INDEX_STACK_COUNT];
		for (int j = 0; j < lCreateItemNum; ++j)
		{
			AgpdItem *pcsItem = CreateItem(stInvenItemSetInfo.m_aalDefaultInvenInfo[i][AGPDITEM_DEFAULT_INVEN_INDEX_TID], NULL, lStackCount);
			if (!pcsItem)
				continue;
...
// 	if( pcsADCharTemplate->m_vtEquipItemSetInfo.size() )
// 	{
// 		for( INT nPart = AGPMITEM_PART_BODY ; nPart <= AGPMITEM_PART_HAND_RIGHT ; ++nPart )
// 		{
// 			if( pcsADCharTemplate->m_vtEquipItemSetInfo[0].m_lDefaultEquipITID[ nPart ] )
// 			{
// 				AgpdItem*	pcsItem	=	pThis->CreateItem( pcsADCharTemplate->m_vtEquipItemSetInfo[0].m_lDefaultEquipITID[nPart] , pcsCharacter );
// 				if( !pcsItem )
// 					continue;
// 
...
BOOL AgsmItemManager::CBGetItemExtraData( PVOID pData, PVOID pClass, PVOID pCustData )
{
	...
	{
		AgpdItemADChar *pcsItemADChar = pThis->m_pagpmItem->GetADCharacter(pcsItem->m_pcsCharacter);
		if (pcsItemADChar)
		{
			pPacketExtra->Type = 2;
			pPacketExtra->ExtraData.BagData.bUsing	= pcsItemADChar->m_bUseChagingInventory;
			pPacketExtra->ExtraData.BagData.unBagType = pcsItemADChar->m_eChargingBagType;
		}
	}
	else if( ((AgpdItemTemplateUsable*)pcsItem->m_pcsItemTemplate)->m_nUsableItemType == AGPMITEM_USABLE_TYPE_CERARIUMORB )
	{
		AgsdItem* pcsAgsdItem = pThis->m_pagsmItem->GetADItem(pcsItem);
		if(!pcsAgsdItem)
			return FALSE;
