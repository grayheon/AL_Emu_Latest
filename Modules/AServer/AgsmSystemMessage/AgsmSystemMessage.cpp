...
BOOL AgsmSystemMessage::OnInit()
{
	...
	if(!m_pcsAgpmCashMall || !m_pcsAgpmQuest)
		return FALSE;

	if(!m_pcsAgpmQuest->SetCallbackDisableForaWhile(CBSendMessageDisableForaWhile, this))
		return FALSE;

	if(!m_pcsAgpmCashMall->SetCallbackDisableForaWhile(CBSendMessageDisableForaWhile, this))
		return FALSE;
...
BOOL AgsmSystemMessage::CBSendMessageDisableForaWhile(PVOID pData, PVOID pClass, PVOID pCustData)
{
	if (!pClass || !pData)
		return FALSE;

	AgsmSystemMessage	*pThis			= (AgsmSystemMessage *)  pClass;
	AgpdCharacter		*pcsCharacter	= (AgpdCharacter *) pData;

	pThis->SendSystemMessage(pcsCharacter, AGPMSYSTEMMESSAGE_CODE_CANNOT_USE_BY_STATUS);
