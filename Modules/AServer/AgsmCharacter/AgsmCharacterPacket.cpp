...
PVOID AgsmCharacter::MakePacketAddCharacter(AgpdCharacter *pcsCharacter, INT16 *pnPacketLength, PVOID pvPacketFactor)
{
	...
	UINT32	lLastUsedEscapeTime = m_pcsAgpmCharacter->GetLastUseEscapeTime(pcsCharacter);
...
BOOL AgsmCharacter::SendPacketCharPosition(AgpdCharacter *pcsCharacter , BOOL bSendSelf )
{
	if (!pcsCharacter)
		return FALSE;
	...
	return SendPacketCharPosition(pcsCharacter, pcsAgsdCharacter->m_dpnidCharacter, FALSE, bSendSelf);
}
BOOL AgsmCharacter::SendPacketSyncCharPos(AgpdCharacter *pcsCharacter)
{
	if (!pcsCharacter)
		return FALSE;
...
BOOL AgsmCharacter::SendPacketCharPosition(AgpdCharacter *pcsCharacter, DPNID dpnid, BOOL bGroupNID , BOOL bSendSelf)
{
	if (!pcsCharacter)
		return FALSE;
...
BOOL AgsmCharacter::SendCouponInfo(AgpdCharacter *pcsCharacter, UINT32 ulNID)
{
	if(ulNID == 0 || NULL == pcsCharacter)
		return FALSE;

	CHAR *pszAccountID = GetAccountID(pcsCharacter);
	...
	CouponInfo *arrCouponList = new CouponInfo[lCouponCount];
	if(NULL == arrCouponList)
		return FALSE;

	INT32 lBuffSize = sizeof(CouponInfo) * lCouponCount;

	for(int i = 0; i < lCouponCount; ++i)
	{
		memcpy_s(&arrCouponList[i], sizeof(CouponInfo), &pcsAccount->m_csCoupon.m_pcsCouponInfo[i], sizeof(CouponInfo));
	}

	INT8 cOperation = AGSMCHAR_PACKET_OPERATION_COUPON_INFO;
	INT16 nPacketLength;
	PVOID pvCouponInfo = m_csPacketCoupon.MakePacket( FALSE, &nPacketLength, 0,
		pszAccountID,
		arrCouponList,
		&lBuffSize
		);
	...
	PVOID pvPacket = m_csPacket.MakePacket(TRUE, &nPacketLength, AGSMCHARACTER_PACKET_TYPE,
		...
		&pcsCharacter->m_lID,				// EncryptCode
		NULL,				// AccountID
		NULL,				// AccountID Len
		NULL,				// AccountPassword
		pvCouponInfo,				// AccountPassword Len
		NULL,				// lCID
		NULL				// pvServerGroupName;각종 서버의 주소를 넣어둔 패킷
		);

	m_csPacketCoupon.FreePacket(pvCouponInfo);	

	BOOL bResult = FALSE;
	if (NULL != pvPacket)
	{
		_tprintf(_T("Send Coupon Info\n"));
		bResult = SendPacket(pvPacket, nPacketLength, ulNID );
		m_csPacket.FreePacket(pvPacket);
	}

	if(arrCouponList)
		delete [] arrCouponList;

	return bResult;

}

BOOL AgsmCharacter::CalcCharacterIgnoreStatus( AgpdCharacter *pcsCharacter, AgpdCharacterIgnoreStatus *pcsIgnoreStatus, BOOL bSendPacket /*= TRUE*/ )
{
	if ( !pcsCharacter || !pcsCharacter->m_pcsCharacterTemplate || !pcsIgnoreStatus )
		return FALSE;

	pcsCharacter->m_stStatusIgnore = pcsCharacter->m_stStatusIgnore + *pcsIgnoreStatus;

	//pcsCharacter->m_stStatusIgnore.RegulateData();
	...
	if (bSendPacket)
	{
		AgpdCharacterIgnoreStatus stIgnoreStatus = pcsCharacter->m_stStatusIgnore;
		stIgnoreStatus.RegulateData();

		PACKET_CHARACTER_IGNORE_STATUS_UPDATE	pPacket;
		pPacket.m_lIgnorePhysicalAttackResist	= stIgnoreStatus.m_lIgnorePhysicalAttackResist;
		pPacket.m_lIgnoreBlockRate				= stIgnoreStatus.m_lIgnoreBlockRate;
		pPacket.m_lIgnoreSkillBlockRate			= stIgnoreStatus.m_lIgnoreSkillBlockRate;
		pPacket.m_lIgnoreEvadeRate				= stIgnoreStatus.m_lIgnoreEvadeRate;
		pPacket.m_lIgnoreDodgeRate				= stIgnoreStatus.m_lIgnoreDodgeRate;
		pPacket.m_lIgnoreCriticalDefenseRate	= stIgnoreStatus.m_lIgnoreCriticalDefenseRate;
		pPacket.m_lIgnoreStunDefenseRate		= stIgnoreStatus.m_lIgnoreStunDefenseRate;

		bSendResult = AgsEngine::GetInstance()->SendPacket( pPacket, GetCharDPNID(pcsCharacter) );
