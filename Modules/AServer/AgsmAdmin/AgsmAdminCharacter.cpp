...
BOOL AgsmAdmin::SearchCharacter(stAgpdAdminSearch* pstSearch, AgpdCharacter* pcsAgpdCharacter)
{
	...
	if(m_pagpmAdmin->GetAdminLevel(pcsAgpdCharacter) < AGPMADMIN_LEVEL_ADMIN)
		return FALSE;
...
BOOL AgsmAdmin::SendCharAllItemUseCustom(AgpdCharacter* pcsCharacter, INT32 lCID, UINT32 ulNID)
{
	...
	SendCharChargingInventoryUseCustom(pcsCharacter, lCID, ulNID);

	AgpdCharacter* pcsAdminCharacter = m_pagpmCharacter->GetCharacter(lCID);
	if(pcsAdminCharacter && m_pagpmAdmin->GetAdminLevel(pcsAdminCharacter) >= AGPMADMIN_LEVEL_ADMIN)
...
BOOL AgsmAdmin::SendCharChargingInventoryUseCustom( AgpdCharacter* pcsCharacter, INT32 lCID, UINT32 ulNID )
{
	if(!m_pagpmItem)
		return FALSE;
	...
	AgpdGrid* pcsGrid = m_pagpmItem->GetChargingInventory(pcsCharacter);
	if(!pcsGrid)
		return FALSE;
