...
#define AGSMADMIN_TIMEINTERVAL_CURRENT_USERCOUNT				3000000	// ms 단위 - 즉, 5분임
#define AGSMADMIN_TIMEINTERVAL_CURRENT_USERCOUNT_FOR_EXCEL		300000	// ms 단위 - 30 초마다 한번씩 체크해본다. 남기는 건 30분 단위
#define AGSMADMIN_TIMEINTERVAL_PING								10000	// ms 단위. 다른 서버와의 Ping 주기
#define AGSMADMIN_DISCONNECT_LIMIT_TIME							50 * 60	// s  단위. 5분 동안 Ping 이 없으면 접속을 끊어버린다.
...
enum _eAgsmAdminCommands
{
	...
	ACOMMAND_cashmall,
	ACOMMAND_WinGuild,
	ACOMMAND_makevolunteer,
	ACOMMAND_removevolunteer,
	ACOMMAND_quest,
	ACOMMAND_MAX
};
...
struct AgsdVolunteerBenefit
{
	INT32 m_lItemTID;
	INT32 m_lCount;

	AgsdVolunteerBenefit() : m_lItemTID(0), m_lCount(0) {}
};
...
class AgpmTitle;
class AgsmTitle;
class AgsmShrineBattle;//JK_쉬라인머지 
...
private:
	...
	AgsmShrineBattle *m_pagsmShrineBattle;	//JK_쉬라인머지
	...
	// Volunteer Benefit
	typedef std::vector<AgsdVolunteerBenefit>	VolunteerBenefit;
	typedef VolunteerBenefit::iterator			VolunteerIter;

	VolunteerBenefit	m_vtVolunteerBenefit;
	...
	// AdminTypeSpecial
	typedef std::map<std::string, stAgpdAdminTypeSpecialGroup> AdminTypeSpecialMap; // map<AdminName, AdminCommand>
	AdminTypeSpecialMap m_AdminTypeSpecial;
...
//	BOOL ProcessCommandQCreate(AgpdCharacter* pcsAgpdCharacter, CHAR* szMessage, INT32 lMessageLength);
	...
	BOOL ProcessCommandClearInventory(AgpdCharacter* pcsAgpdCharacter, INT16 lInvenLayerNum = 0);
	...
	BOOL ProcessCommandRecast(AgpdCharacter* pcsAgpdCharacter);
	BOOL ProcessCommandEscape(AgpdCharacter* pcsAgpdCharacter);
	BOOL ProcessCommandSyncPos(AgpdCharacter* pcsAgpdCharacter);
	...
	BOOL SendCharChargingInventoryUseCustom(AgpdCharacter* pcsCharacter, INT32 lCID, UINT32 ulNID);
	...
	// Stream Read
	BOOL	StreamReadAdminTypeSpecial(CHAR *szFile);

	BOOL	CheckAdminTypeSpecial(AgpdCharacter* pAgpdCharacter, CHAR* szCommand, INT32 lCommandSize );
};
