...
AgsmAdmin::AgsmAdmin():m_papmMap( NULL )
{
	...
	m_pagsmShrineBattle = NULL;	//JK_쉬라인머지
...
BOOL AgsmAdmin::OnInit()
{
	...
	m_pagsmShrineBattle = (AgsmShrineBattle *)GetModule("AgsmShrineBattle");//JK_쉬라인머지
...
BOOL AgsmAdmin::CBOnAdminClientLogin(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	BOOL bAllowedIPResult = FALSE;
	BOOL bSendResult = FALSE;

	if(pThis->IsAllowedIP(szIP))
	{
		bAllowedIPResult = TRUE;
		// 클라이언트에 접속해도 된다고 보낸다.
		bSendResult = pThis->SendAdminClientLoginOK(pcsCharacter, dpnID);
	}

	char strCharBuff[256] = { 0, };
	sprintf_s(strCharBuff, sizeof(strCharBuff), "CBOnAdminClientLogin : szIP = %s, bAllowedIPResult = %d, bSendResult = %d", szIP, bAllowedIPResult, bSendResult);
	AuLogFile_s("LOG\\AdminClientConnectLog.txt", strCharBuff);
			...
			while(iter != m_vtEventItems.end())
			{
				if(iter->m_ISet == m_ISet)
				{
					...
					if(pcsItemTemplate)
					{
						...
						// RealTime Cash의 경우는 사용하는 순간 ExpireTime이 세팅 되기 때문에 여기서 ExpireTime을 세팅하지 않는다
...
BOOL AgsmAdmin::StreamReadAdminTypeSpecial( CHAR *szFile )
{
	if (!szFile)
		return FALSE;

	m_AdminTypeSpecial.clear();
	...
	//////////////////////////////////////////////////////////////////////////
	// AdminGroupList
	AuXmlNode *pNodeGroupList = Doc.FirstChild("AdminGroupList");
	while(pNodeGroupList)
	{
		stAgpdAdminTypeSpecialGroup stInsertGroup;
		CHAR* pGroupName = NULL;

		AuXmlNode *pNodeGroupName = pNodeGroupList->FirstChild("GroupName");
		if (pNodeGroupName)
		{
			CHAR* pGroupID = (CHAR*)pNodeGroupName->ToElement()->Attribute("ID");
			if (pGroupID)
			{
				stInsertGroup.m_lGroupID = atoi(pGroupID);
			}

			pGroupName = (CHAR*)pNodeGroupName->ToElement()->Attribute("Name");
		}

		AuXmlNode *pNodeNameList = pNodeGroupList->FirstChild("AdminNameList");
		if(pNodeNameList)
		{
			AuXmlNode *pNodeAccountName = pNodeNameList->FirstChild("AccountName");
			while(pNodeAccountName)
			{
				CHAR* pName = (CHAR*)pNodeAccountName->ToElement()->Attribute("Name");
				if (pName)
				{
					stInsertGroup.m_vtAdminName.push_back(pName);
				}
				pNodeAccountName = pNodeAccountName->NextSibling();
			}
		}

		AuXmlNode *pNodeCommandList = pNodeGroupList->FirstChild("AdminCommandList");
		if(pNodeCommandList)
		{
			AuXmlNode *pNodeCommandName = pNodeCommandList->FirstChild("CommandName");
			while(pNodeCommandName)
			{
				CHAR* pName = (CHAR*)pNodeCommandName->ToElement()->Attribute("Name");
				if (pName)
				{
					stInsertGroup.m_vtAdminCommand.push_back(pName);
				}
				pNodeCommandName = pNodeCommandName->NextSibling();
			}
		}

		if(pGroupName)
		{
			m_AdminTypeSpecial.insert(make_pair(pGroupName, stInsertGroup));
		}
		
		pNodeGroupList = pNodeGroupList->NextSibling();
	}
...
BOOL AgsmAdmin::CheckAdminTypeSpecial( AgpdCharacter* pAgpdCharacter, CHAR* szCommand, INT32 lCommandSize )
{
	if (!pAgpdCharacter || !szCommand)
		return FALSE;

	CHAR* szAccountName = m_pagsmCharacter->GetAccountID(pAgpdCharacter);
	if (!szAccountName)
		return FALSE;

	AdminTypeSpecialMap::iterator iter = NULL;

	for (iter = m_AdminTypeSpecial.begin(); iter != m_AdminTypeSpecial.end(); ++iter)
	{
		AdminNameVector::iterator iterAccountName = NULL;
		for(iterAccountName = iter->second.m_vtAdminName.begin(); iterAccountName != iter->second.m_vtAdminName.end(); ++iterAccountName)
		{
			if( !strcmp(szAccountName, iterAccountName->c_str()) )
				break;
		}
		// 못찾았으면 다음 그룹으로
		if (iterAccountName == iter->second.m_vtAdminName.end())
			continue;
		
		AdminCommandVector::iterator iterCmd = NULL;
		for (iterCmd = iter->second.m_vtAdminCommand.begin(); iterCmd != iter->second.m_vtAdminCommand.end(); ++iterCmd)
		{
			if ( !strncmp(szCommand, iterCmd->c_str(), lCommandSize) )
			{
				return TRUE;
			}
		}
