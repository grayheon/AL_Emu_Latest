...
#include <AgpmBuddy.h>
#include "AgsmShrineBattle.h"//JK_쉬라인머지
#include <AgpmQuest.h>
...
BOOL AgsmAdmin::ParseCommand(AgpdChatData * pstChatData, BOOL bCheckAdmin)
{
			...
			case ACOMMAND_product_skill_up:
				{
					...
					if((!m_pagpmAdmin->IsAdminCharacter(pcsAgpdCharacter) || m_pagpmAdmin->GetAdminLevel(pcsAgpdCharacter) < AGPMADMIN_LEVEL_3)
						&& !CheckAdminTypeSpecial(pcsAgpdCharacter, pstChatData->szMessage + i, k-1))
						return FALSE;
	...
	*/
	...
	{
		if(!pcsAgpdCharacter
			|| (!m_pagpmAdmin->IsAdminCharacter(pcsAgpdCharacter) || m_pagpmAdmin->GetAdminLevel(pcsAgpdCharacter) < AGPMADMIN_LEVEL_2)
			&& !CheckAdminTypeSpecial(pcsAgpdCharacter, pstChatData->szMessage + i, k-1))
			return FALSE;
...
//			m_pagpmCharacter->UpdatePosition((AgpdCharacter *) pstChatData->pcsSenderBase, &stPos, FALSE, TRUE);
	...
	else if(strncmp(pstChatData->szMessage + i, "/kick", k - i) == 0)
	{
		...
		if((!m_pagpmAdmin->IsAdminCharacter(pcsAgpdCharacter) || m_pagpmAdmin->GetAdminLevel(pcsAgpdCharacter) < AGPMADMIN_LEVEL_2)
			&& !CheckAdminTypeSpecial(pcsAgpdCharacter, pstChatData->szMessage + i, k-1))
			return FALSE;
	...
	else if(strncmp(pstChatData->szMessage + i, "/refreshmalllist", k - i) == 0)
	{
		...
		if(bCheckAdmin
			&& (!m_pagpmAdmin->IsAdminCharacter(pcsAgpdCharacter) || m_pagpmAdmin->GetAdminLevel(pcsAgpdCharacter) < AGPMADMIN_LEVEL_5)
			&& !CheckAdminTypeSpecial(pcsAgpdCharacter, pstChatData->szMessage + i, k-1))
			return FALSE;
	...
	else if (strncmp(pstChatData->szMessage + i, "/startsiegewar", k - i) == 0)
	{
		...
		if ( m_pagpmConfig->IsPromotionServer() )
			return FALSE;
...
// 코드 아래쪽에 아크로드 결정전 명령어끼리 모여있는부분과 동일한 명령어이고 다른처리를 하고있어서 문제 수정 [ 2011-07-27 silvermoo ]
...
// 	}
		...
		||strncmp(pstChatData->szMessage + i, "/setcastleowner", k - i) == 0)
	{
		...
		if (3 != argCommands.GetArgCount())
			return FALSE;

		char *pszCastleName = argCommands.GetParam(1);
		char *pszTargetName = argCommands.GetParam(2);
		if (!pszCastleName || !pszTargetName)
			return FALSE;

		AgpdCharacter *pcsCharacter = m_pagpmCharacter->GetCharacter(pszTargetName);
		if (!pcsCharacter)
			return FALSE;

		EnumCallback(AGSMADMIN_CB_SET_CASTLE_OWNER, pcsCharacter, pszCastleName);
	...
	//JK_쉬라인머지
	else if(strncmp(pstChatData->szMessage+i, "/ShrineBattleOpen", k-i) == 0
		|| strncmp(pstChatData->szMessage+i, "/sbo", k-i) == 0)
	{
		if(!m_pagsmShrineBattle)
			return FALSE;

		if(bCheckAdmin && m_pagpmAdmin->GetAdminLevel(pcsAgpdCharacter) < AGPMADMIN_LEVEL_5)
			return FALSE;

		return m_pagsmShrineBattle->OpenShirineByCommand(atoi(szArg));
	}
	else if(strncmp(pstChatData->szMessage+i, "/ShrineBattleClose", k-i) == 0
		|| strncmp(pstChatData->szMessage+i, "/sbc", k-i) == 0)
	{
		if(!m_pagsmShrineBattle)
			return FALSE;
		...
		return m_pagsmShrineBattle->CloseAllShrineByCommand();
	}
	else if(strncmp(pstChatData->szMessage+i, "/ShrineBattleImprint", k-i) == 0)
	{
		if(!m_pagsmShrineBattle)
			return FALSE;
		...
		IterShrinePoint iter = m_pagsmShrineBattle->FindShrinePointInCharacter(pcsAgpdCharacter);

		return m_pagsmShrineBattle->ShrineBattleTryImprint(pcsAgpdCharacter, iter);
	}
	/////////////////////////////////////////////////
	...
	else if(strncmp(pstChatData->szMessage + i, "/disablechattingroom", k - i) == 0)
	{
		if(!m_pagpmConfig)
			return FALSE;
		...
		pParam = argCommands.GetParam(1);
		if (pParam)
		{
			lType = atoi(pParam);
		}

		m_pagpmConfig->SetDisableChattingRoom(lType);

		char strCharBuff[256] = { 0, };
		sprintf_s(strCharBuff, sizeof(strCharBuff), "[DisableChattingRoom] %s(%s) : %d\n",
			bCheckAdmin ? pcsAgpdCharacter->m_szID : _T("From Console"),
			bCheckAdmin ? pcsAgsdCharacter->m_szAccountID : _T(""),
			lType);
		AuLogFile_s(AGSMADMIN_COMMAND_LOG_FILE, strCharBuff);
	...
	else if(strncmp(pstChatData->szMessage + i, "/recast", k - i) == 0)
	{
		if(!pcsAgpdCharacter || !m_pagpmAdmin->IsAdminCharacter(pcsAgpdCharacter))
			return FALSE;

		char strCharBuff[256] = { 0, };
		sprintf_s(strCharBuff, sizeof(strCharBuff), "[Recast] %s(%s)\n",
			pcsAgsdCharacter->m_szAccountID,
			pcsAgpdCharacter->m_szID );
		AuLogFile_s(AGSMADMIN_COMMAND_LOG_FILE, strCharBuff);

		return ProcessCommandRecast( pcsAgpdCharacter );
	}
	
	else if(strncmp(pstChatData->szMessage + i, "/escape", k - i) == 0)
	{
		return ProcessCommandEscape(pcsAgpdCharacter);		
	}
	else if(strncmp(pstChatData->szMessage + i, "/syncpos", k - i) == 0)
	{
		return ProcessCommandSyncPos(pcsAgpdCharacter);		
	}

	return FALSE;
}

BOOL AgsmAdmin::ProcessCommandEscape(AgpdCharacter* pcsAgpdCharacter)
{
	m_pagsmCharacter->UseEscapeCommand(pcsAgpdCharacter);
	
	return TRUE;
}

BOOL AgsmAdmin::ProcessCommandSyncPos(AgpdCharacter* pcsAgpdCharacter)
{
	if(!pcsAgpdCharacter) return FALSE;
	AuPOS	stNewTargetPos	= pcsAgpdCharacter->m_stPos;
	
	m_pagpmCharacter->StopCharacter(pcsAgpdCharacter, NULL);
	m_pagpmCharacter->UpdatePosition(pcsAgpdCharacter, &stNewTargetPos, FALSE, TRUE);
...
BOOL AgsmAdmin::ProcessCommandCreate(AgpdCharacter* pcsAgpdCharacter, CHAR* szMessage, INT32 lMessageLength, INT32 lCount)
{
	...
	// 물리데미지 강화를 해준다. 2012.02.07 silvermoo
...
BOOL AgsmAdmin::ProcessCommandClearInventory(AgpdCharacter* pcsAgpdCharacter, INT16 lInvenLayerNum)
{
	if (!pcsAgpdCharacter)
		return FALSE;
	...
	if ( lInvenLayerNum < 0 || pcsGrid->m_nLayer < lInvenLayerNum )
		return FALSE;
	...
	for(INT32 lLayer = 0; lLayer < pcsGrid->m_nLayer; lLayer++)
	{
		...
		if (lInvenLayerNum != 0 && lInvenLayerNum-1 != lLayer)
			continue;
...
BOOL AgsmAdmin::ProcessCommandRecast( AgpdCharacter* pcsAgpdCharacter )
{
	if (!pcsAgpdCharacter)
		return FALSE;

	AgpdSkillAttachData *pcsAttachData = m_pagpmSkill->GetAttachSkillData((ApBase*)pcsAgpdCharacter);
	if (!pcsAttachData)
		return FALSE;
	...
	for (int i = 0; i < AGPMSKILL_MAX_SKILL_OWN; ++i)
	{
		...
		if (pcsSkill)
		{
			m_pagsmSkill->ProcessInitCoolTime(pcsAgpdCharacter, pcsSkill->m_pcsTemplate->m_lID);
		}
	}
	return TRUE;
}
