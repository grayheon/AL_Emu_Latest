...
//	2005.09.05. By SungHoon
	...
	if(!m_pagpmGuild->SetCallback(AGPMGUILD_CB_CHECK_LEAVE_POSSIBLE, CBCheckLeaveArea, this))
		return FALSE;
...
BOOL AgsmGuild::CheckPassword(AgpdGuild* pcsGuild, CHAR* szPassword)
{
	...
	// 테스트 서버에서는 길드해체 비밀번호체크를 하지 않는다. 2011-08-31 silvermoo
...
BOOL AgsmGuild::EnterGameWorld(AgpdCharacter* pcsCharacter)
{
	...
	if( pcsMember )
	{
		...
		//길마일 경우 길마 최종 로그아웃 시간 갱신
		if(strcmp(pcsMember->m_szID, pcsGuild->m_szMasterID) == 0)
		{
			pcsGuild->m_lMasterLastVisitDate = AuTimeStamp::GetCurrentTimeStamp();
		}
...
BOOL AgsmGuild::GuildLoadFromDB(AgsdRelay2GuildMaster* pAgsdRelay)
{
	...
	if(pcsGuild)
	{
		...
		pcsGuild->m_lMasterLastVisitDate = pAgsdRelay->m_lMasterLastVisitDate;
...
BOOL AgsmGuild::MemberLoadFromDB(CHAR* szMemberID, CHAR* szGuildID, INT32 lRank, INT32 lJoinDate, INT32 lLevel, INT32 lTID)
{
	...
	if(strcmp(szMemberID, AGSMGUILD_LOAD_COMPLETE_STRING) == 0)
	{
		...
		{
			m_pagpmGuild->m_csGuildAdmin.SortGuildMemberDesc( TRUE );
...
BOOL AgsmGuild::OnOperationCSSuccessionAnswer(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	if(!pPacket->Answer)
	{
		...
		if(pElem)
		{
			...
			sprintf_s(strCharBuff, sizeof(strCharBuff), "Guild Succession Failed : GuildName[ %s ], PrevGuildMaster[ %s ] to NewGuildMaster[ %s ]", 
				pcsGuild1->m_szID, pcsMaster->m_szID, pcsSuccessor->m_szID);
			AuLogFile_s("LOG\\GuildSuccession.log", strCharBuff);
		}
	...
	char strCharBuff[256] = { 0, };
	sprintf_s(strCharBuff, sizeof(strCharBuff), "Guild Succession Complete : GuildName[ %s ], PrevGuildMaster[ %s ] to NewGuildMaster[ %s ]", 
		pcsGuild1->m_szID, pcsMaster->m_szID, pcsSuccessor->m_szID);
	AuLogFile_s("LOG\\GuildSuccession.log", strCharBuff);
...
BOOL AgsmGuild::CBCheckLeaveArea(PVOID pData, PVOID pClass, PVOID pCustData)
{
	if(!pData || !pClass || !pCustData) return FALSE;
	...
	AgpdGuild* pcsGuild = pThis->m_pagpmGuild->GetGuild(szGuildId);
	//공성지역에 있는 케릭터는 탈퇴 시킬 수 없다.
	if(!pcsGuild) return FALSE;

	AgpdCharacter* pcsCharacter = pThis->m_pagpmCharacter->GetCharacter(szCharID);
	
	
	if(pcsCharacter)
	{
		if(pThis->m_pagpmSiegeWar->IsSiegeWarRegion(pcsCharacter))
		{
			AgpdCharacter* pcsGuildMasterChar = pThis->m_pagpmCharacter->GetCharacter(pcsGuild->m_szMasterID);
			if(pcsGuildMasterChar)
			{
				AgsdCharacter* pcsAgsdCharacter = pThis->m_pagsmCharacter->GetADCharacter(pcsGuildMasterChar);
				if (!pcsAgsdCharacter) return FALSE;

				pThis->SendSystemMessage(AGPMGUILD_SYSTEM_CODE_REJECTED_GUILD_LEAVE_IN_SIEGEWAR_AREA, pcsAgsdCharacter->m_dpnidCharacter);
			}
			return FALSE;
		}
	}
