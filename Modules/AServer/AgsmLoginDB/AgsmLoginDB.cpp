...
#define LOGIN_ERROR_FILE_NAME		_T("LOG\\LoginError.log")
#define LOGIN_AUTH_FILE_NAME		_T("LOG\\Auth.log")
#define LOGIN_DB_DISCONNECT			_T("LOG\\LoginDBDisconnect.log")

#define LOGIN_REMOVE_CHARACTER_CHECK_PW_FILE_NAME		_T("LOG\\RemoveCharacterCheckPW.log")
...
BOOL AgsmLoginGameDBGroup::Connect(AuDatabaseManager *pManager, TCHAR *pszUser, TCHAR *pszPwd, TCHAR *pszName, INT16 nConnection)
{
	...
	{
		if ( FALSE == m_pDatabases[i].Initialize(m_pManager) || FALSE == m_pDatabases[i].Connect(pszUser, pszPwd, pszName) )
		{
			for (INT j = 0; j < i; j++)
...
AuDatabase2* AgsmLoginGameDBGroup::GetIdle()
{
	...
	DWORD dwWait = m_Semaphores.Wait(FALSE, 10*1000);
	...
	{
		CHAR szLog[256] = { 0, };
		sprintf_s(szLog,sizeof(szLog), "%s[%d] return NULL", __FUNCTION__, __LINE__);
...
BOOL AgsmLoginDB::OnAddModule()
{
	...
	if( !m_pAgpmGrid || !m_pAgpmFactors || !m_pAgpmCharacter || !m_pAgpmItem || !m_pAgsmServerManager ||
		...
		!m_pAgpmConfig || !m_pAgsmReturnToLogin)
	{
			return FALSE;
	}
...
AuDatabase2* AgsmLoginDB::GetIdleMasterDatabase()
	...
	DWORD dwWait = m_MasterSema.Wait(FALSE, 10*1000);
...
AuDatabase2* AgsmLoginDB::GetIdleCompenDatabase()
	...
	DWORD dwWait = m_CompenSema.Wait(FALSE, 10*1000);
...
BOOL AgsmLoginDB::ProcessOperation(AgsmLoginQueueInfo *pQueryInfo, ApHttp *pHttp /*= NULL*/)
	...
	switch (pQueryInfo->m_nOperation)
		...
		case AGSMLOGIN_OPERATION_CREATE_JUMPING_CHARACTER_CHECK :
			bResult = ProcessCreateJumpingCharacterCheck(pQueryInfo);
			break;
		...
		case AGSMLOGIN_OPERATION_REMOVE_CHARACTER_CHECK_PW :
			bResult = ProcessRemoveCharacterCheckPW(pQueryInfo);
			break;
...
BOOL AgsmLoginDB::ProcessCheckAccount1(AgsmLoginQueueInfo *pQueryInfo, ApHttp* pHttp)
	...
	TCHAR	szOriginalAccountID[AGPACHARACTER_MAX_ID_STRING + 1] = { 0, };
	TCHAR	szOriginalIPAddress[AGPMLOGIN_IPADDRSIZE + 1] = { 0, };

	int		retval = 0;
	sprintf_s(szOriginalAccountID, sizeof(szOriginalAccountID), pQueryInfo->m_szAccountID);
	if( -1 == retval )
		return FALSE;

	sprintf_s(szOriginalIPAddress, sizeof(szOriginalIPAddress), pQueryInfo->m_szIPAddress);
	if( -1 == retval )
		return FALSE;

	AuLogConsole_s("[%s] %s - 1", pQueryInfo->m_szAccountID, __FUNCTION__);
	...
	AuLogConsole_s("[%s] %s - 2", pQueryInfo->m_szAccountID, __FUNCTION__);
	//
	if (m_pAgpmConfig->IsIgnoreAccountAuth())
	{
		AuLogConsole_s("[%s] %s - 2_1", pQueryInfo->m_szAccountID, __FUNCTION__);
	...
	else
	{
...
#if defined(_AREA_KOREA_) || defined(_AREA_JAPAN_)
		AuLogConsole_s("[%s] %s - 3", pQueryInfo->m_szAccountID, __FUNCTION__);
		if ( false == GetGameEnv().IsDebugTest() )
		{
			AuLogConsole_s("[%s] %s - 3_1", pQueryInfo->m_szAccountID, __FUNCTION__);
			int iAge = 0;
		...
		{
			AuLogConsole_s("[%s] %s - 3_2", pQueryInfo->m_szAccountID, __FUNCTION__);
			if (m_pAgpmConfig->IsNoAccountAuth())
			{
				AuLogConsole_s("[%s] %s - 3_2_1", pQueryInfo->m_szAccountID, __FUNCTION__);
				lResult	= AGSMLOGINDB_AUTH_RESULT_SUCCESS;
			}
			else if (m_pAgpmConfig->IsDBAccountAuth())
			{
				AuLogConsole_s("[%s] %s - 3_2_2", pQueryInfo->m_szAccountID, __FUNCTION__);
				lResult = ProcessCheckAccountFromDB(pQueryInfo);
			}
			else
			{
				AuLogConsole_s("[%s] %s - 3_2_3", pQueryInfo->m_szAccountID, __FUNCTION__);
		...
		/*if ( GetGameEnv().IsDebugTest() )//debugtest 모드일때
		{
			//std::cout << "Global Authentication : DebugTest mode Authentication Skip!!\n" << std::endl;

			char szLog[256]={0,};
			
			sprintf_s(szLog,sizeof(szLog),"AccoutID [%s] skip Archlord Authentication",pQueryInfo->m_szAccountID);

			AuLogFile_s(LOGIN_AUTH_FILE_NAME, szLog);
            AuLogConsole_s(szLog);
			lResult = AGSMLOGINDB_AUTH_RESULT_SUCCESS;
		}
		else
		{
			...
			{
				  AuLogConsole_s("IsCertificatedNID = true");
			...
			else
			{
                ...
                AuLogConsole_s("Global Authentication : ID = [%s] AuthString = [%s]",pQueryInfo->m_szAccountID,pQueryInfo->m_szGBAuthString);
				int iQueryResult = GlobalAuthFromDB( pQueryInfo );
	...
	AuLogConsole_s("[%s] %s - 4 (%d)", pQueryInfo->m_szAccountID, __FUNCTION__, lResult);
	//AuLogFile(LOGIN_ERROR_FILE_NAME, "AgsmLoginDB::ProcessCheckAccount1 - 1 : Result2:%d\n", lResult2);

	if( strcmp(szOriginalAccountID, pQueryInfo->m_szAccountID) || strcmp(szOriginalIPAddress, pQueryInfo->m_szIPAddress) )
	{
		char strCharBuff[256] = { 0, };
		sprintf_s(strCharBuff, sizeof(strCharBuff), "Check OriginalAccountID Failed in Auth : OriginalAccountID[ %s ], CheckedAccountID[ %s ], OriginalIP[ %s ], CheckedIP[ %s ]", 
			szOriginalAccountID, pQueryInfo->m_szAccountID, szOriginalIPAddress, pQueryInfo->m_szIPAddress);
		AuLogFile_s("LOG\\LoginProcessERR.log", strCharBuff);
		return FALSE;
	}
	...
	switch (lResult)
		...
		case AGSMLOGINDB_AUTH_RESULT_SUCCESS :		// success
			{
			// check archlord db.
			bResult = ProcessCheckAccount2(pQueryInfo);
			}
			break;
	...
	{
		char strCharBuff[256] = { 0, };
		sprintf_s(strCharBuff, sizeof(strCharBuff), "Check OriginalAccountID Failed in DB : OriginalAccountID[ %s ], CheckedAccountID[ %s ], OriginalIP[ %s ], CheckedIP[ %s ]", 
			szOriginalAccountID, pQueryInfo->m_szAccountID, szOriginalIPAddress, pQueryInfo->m_szIPAddress);
		AuLogFile_s("LOG\\LoginProcessERR.log", strCharBuff);
		return FALSE;
	}

	AuLogConsole_s("[%s] %s - 5", pQueryInfo->m_szAccountID, __FUNCTION__);
	return bResult;
	}
...
BOOL AgsmLoginDB::ProcessCheckAccount2(AgsmLoginQueueInfo *pQueryInfo)
	...
	AuLogConsole_s("ProcessCheckAccount2 - 1");
	...
	AuLogConsole_s("ProcessCheckAccount2 - 2");

	if (m_pAgpmConfig->IsNoAccountAuth())
	{
		pszQuery = GetQuery(_T("ACCOUNT_NOAUTH_SELECT"), pDatabase->GetVender());
		AuLogConsole_s("ProcessCheckAccount2 - 3");
	}
	else
	{
		pszQuery = GetQuery(_T("ACCOUNT_GENERAL_SELECT"), pDatabase->GetVender());
		AuLogConsole_s("ProcessCheckAccount2 - 4");
	}

	AuLogConsole_s("ProcessCheckAccount2 - 4.1");

	ASSERT(NULL != pszQuery);
	if (NULL == pszQuery)
		return FALSE;
	
	AuLogConsole_s("ProcessCheckAccount2 - 5");
	...
	if (!pDatabase->SetQueryText(pszQuery) ||
		...
		{
		AuLogConsole_s("ProcessCheckAccount2 - 6");
		pDatabase->EndQuery();
	...
	AuLogConsole_s("ProcessCheckAccount2 - 7");
	...
	if(GetGameEnv().IsDebugTest())
	{
		...
		//{
		//	AuLogConsole_s("ProcessCheckAccount2 - 8");
		//	pDatabase->EndQuery();
		//	pDatabase->AbortTransaction();
	...
	else
	{
		...
		if (AUDATABASE2_QR_SUCCESS != pDatabase->ExecuteQuery()&& !m_pAgpmConfig->IsNoAccountAuth())
			{
			AuLogConsole_s("ProcessCheckAccount2 - 9");
			pDatabase->EndQuery();
			pDatabase->AbortTransaction();
	...
	AuLogConsole_s("ProcessCheckAccount2 - 10");

	// 한국에서만 하자 -ㅂ-;
	if (AP_SERVICE_AREA_KOREA == g_eServiceArea && false == GetGameEnv().IsDebugTest() )
	{
		// 2007.11.20. steeple
		// 나이체크가 한게임 모듈로 변경.
	...
	AuLogConsole_s("ProcessCheckAccount2 - 14");
	// block
	if (!bPenaltyFinished && (0 == _tcsnicmp(szPenalty, _T("AC"), 2)))
		...
		AuLogConsole_s("ProcessCheckAccount2 - 15");
		pDatabase->EndQuery();
		pDatabase->AbortTransaction();
	...
	AuLogConsole_s("ProcessCheckAccount2 - 16");
	...
	if (!ProcessCheckBillInfo(pQueryInfo))
		{
		AuLogConsole_s("ProcessCheckAccount2 - 17");
		// 결제 확인이 실패했다. 접속을 끊는다.
		DestroyClient(ulClientNID);
	...
	AuLogConsole_s(_T("ProcessCheckAccount2 - 18\n"));

#ifdef _AREA_GLOBAL_
	//////////////////////////////////////////////////////////////////////////
	// 글로벌 드래곤 시온 베타키 이벤트 유저 체크 - arycoat 2010.11.09
	AgsdAccount* pAccount = m_pAgsmAccountManager->GetAccount(pQueryInfo->m_szAccountID);
	if( pAccount )
	{
		pAccount->m_dwAccountType = pQueryInfo->m_dwAccountType;
	}
#endif
...
// 		if ( m_pAgsmLoginClient->CheckProgressLoginInfo(pszAccountID, ulNID) )
// 		{
// 			m_pAgsmLoginClient->SetLoginStep(ulNID, AGPMLOGIN_STEP_PASSWORD_CHECK_OK);
// 			m_pAgsmLoginClient->SendSignOnSuccess(ulNID, pszAccountID, isLimited, isProtected);
// 		}
...
// 		{
// 			// 타계정 로그인
// 			//m_pAgsmLoginClient->SendLoginResult(AGPMLOGIN_RESULT_INVALID_ACCOUNT, NULL, ulNID);
// 
// 			char strCharBuff[256] = { 0, };
// 			sprintf_s(strCharBuff, sizeof(strCharBuff), "Other Account Login : NowProgressInfo[ %s ][ %d ]", 
// 				pszAccountID,
// 				ulNID
// 				);
// 			AuLogFile_s(LOGIN_OTHER_ACCOUNT_LOGIN_FILE_NAME, strCharBuff);
// 		}
	}
...
BOOL AgsmLoginDB::ProcessSelectCharacter(AgsmLoginQueueInfo *pQueryInfo)
	...
	//AuLogConsole_s("[%s] %s %s", pQueryInfo->m_szServerName, pQueryInfo->m_szAccountID);
...
//=============================================
//
BOOL AgsmLoginDB::ProcessCreateJumpingCharacterCheck(AgsmLoginQueueInfo *pQueryInfo)
{
	if (!pQueryInfo)
		return FALSE;
	...
	//////////////////////////////////////////////////////////////////////////
	// 점핑이벤트 플래그 체크 From DB

	TCHAR *pszQuery	= GetQuery(_T("EVENT_CHECK_JUMPING_CHARACTER"), pMaster->GetVender());
	ASSERT(NULL != pszQuery);
	if (NULL == pszQuery)
	{
		m_pAgsmLoginClient->SetLoginStep(ulClientNID, AGPMLOGIN_STEP_GET_EXIST_CHARACTER);
		...
		|| AUDATABASE2_QR_FAIL == pMaster->ExecuteQuery())
	{
		...
		m_pAgsmLoginClient->SendLoginResult(AGPMLOGIN_RESULT_CHECK_CREATE_JUMPING_CHARACTER_FAIL, NULL, ulClientNID);
	}
	else
	{
		// 쿼리 성공

		CHAR* csQueryResult = pMaster->GetQueryResult(0);
		if (!csQueryResult)
		{
			// 쿼리 결과가 NULL 값이라면 DB 필드에 데이터가 아얘 없는 것이므로 이벤트 플래그가 설정되어있지 않다고 보면 된다
			lQueryResult = 0;
		}
		else
		{
			lQueryResult = atoi(csQueryResult);
		}

		switch( lQueryResult )
		{
		case 0 : // 이벤트 플래그가 설정 안되어있다 -> 점핑 캐릭터를 만들 수 있다
			m_pAgsmLoginClient->SendLoginResult(AGPMLOGIN_RESULT_CHECK_CREATE_JUMPING_CHARACTER_SUCCESS, NULL, ulClientNID);
			break;

		case 1 : // 이벤트 플래그가 설정되어있다 -> 점핑 캐릭터를 만들 수 없다
			m_pAgsmLoginClient->SendLoginResult(AGPMLOGIN_RESULT_CHECK_CREATE_JUMPING_CHARACTER_FAIL, NULL, ulClientNID);
			break;

		default: // 이런거 오면 안대는데..
			TRACE("Jumping Event Flag Check ERROR");
			break;
		}
...
BOOL AgsmLoginDB::ProcessCreateCharacter(AgsmLoginQueueInfo *pQueryInfo)
	...
	INT8			cSlotNext = 0;
	UINT32			ulClientNID = pQueryInfo->m_ulCharNID;
	INT32			lIsJumpingCharacter = pQueryInfo->m_lIsJumpingCharacter;
	AgpdCharacter	*pAgpdCharacter = m_pAgpmCharacter->GetCharacter(pQueryInfo->m_lCID);
	AgsdCharacter	*pAgsdCharacter = m_pAgsmCharacter->GetADCharacter(pAgpdCharacter);
	...
	//////////////////////////////////////////////////////////////////////////
	// 글로벌 드래곤 시온 베타키 이벤트 유저 체크 - arycoat 2010.11.09
	// 이벤트 종료 - arycoat 2010.12.16
	/*if(pAgpdCharacter->m_pcsCharacterTemplate->m_lID == 1724)
	{
		AgsdAccount* pAccount = m_pAgsmAccountManager->GetAccount(pQueryInfo->m_szAccountID);
		printf("create TID = %d, AccountType = %d\n", pAgpdCharacter->m_pcsCharacterTemplate->m_lID, pAccount->m_dwAccountType);
		if(pAccount && pAccount->m_dwAccountType != 18)
		{
			m_pAgsmLoginClient->SendLoginResult(AGPMLOGIN_RESULT_NOT_BETAKEY, m_pAgsmCharacter->GetRealCharName(pAgpdCharacter->m_szID), ulClientNID);
			goto pc_fail;
		}
	}*/
#endif
	...
	//////////////////////////////////////////////////////////////////////////
	// Jumping Character Flag Update
	/*pszQuery = GetQuery(_T("JUMPING_CHARACTER_CREATE"), pMaster->GetVender());
	ASSERT(NULL != pszQuery);
	if (NULL == pszQuery)
	{
		goto pc_fail;
	...
	if (!pMaster->SetQueryText(pszQuery)
		|| !pMaster->SetParam(0, pQueryInfo->m_szAccountID, sizeof(pQueryInfo->m_szAccountID))
		|| !pMaster->SetParam(1, &lIsJumpingCharacter)
		|| !pMaster->SetParam(2, &lQueryResult, TRUE)
		|| AUDATABASE2_QR_FAIL == pMaster->ExecuteQuery() )
	{
		goto pc_fail;
	}
	else
	{
		...
		// 아래쪽에서 goto문 타고 넘어가면 로그 안남으니까 로그는 여기서 작성
		...
		else
		{
			...
			if( lIsJumpingCharacter )
			{
				...
				sprintf_s(strBuffer, sizeof(strBuffer), "AccountID = %s, CreateJumpingCharacter = %d, Result = %d",
					pQueryInfo->m_szAccountID, lIsJumpingCharacter, lQueryResult );
				AuLogFile_s("LOG\\JumpingCharacterCreate.log", strBuffer);
			}

			// 점핑캐릭터 신청하지 않았는데 노멀캐릭터 생성되지 않음
			if ( lIsJumpingCharacter == 0 && lQueryResult != 2 )
			{
				goto pc_fail;
			}
			// 점핑캐릭터 신청 -> 이미 점핑캐릭터를 만들었음
			else if ( lIsJumpingCharacter == 1 && lQueryResult == 0 )
			{
				goto pc_fail;
			}
		}
	}*/ //CMBDev: Disable Jumping Character until new query is made.
...
BOOL AgsmLoginDB::ProcessRemoveCharacterCheckPW( AgsmLoginQueueInfo *pQueryInfo )
{
	if (!pQueryInfo)
		return FALSE;
	...
	if ( GetGameEnv().IsDebugTest() )//debugtest 모드일때
	{
		...
		iQueryResult = AGSMLOGINDB_AUTH_RESULT_SUCCESS;
	}
	else
	{
		// iQueryResult = ProcessCheckAccountFromDB( pQueryInfo );
		iQueryResult = GlobalAuthFromDB( pQueryInfo );
	}
	...
	if( 1 == iQueryResult )
	{
		...
		pAgsmLoginQueueInfo->m_ulCharNID = pQueryInfo->m_ulCharNID;
		_tcsncpy(pAgsmLoginQueueInfo->m_szAccountID, pQueryInfo->m_szAccountID, AGPACHARACTER_MAX_ID_STRING);
		_tcsncpy(pAgsmLoginQueueInfo->m_szCharacterID, pQueryInfo->m_szCharacterID, AGPACHARACTER_MAX_ID_STRING);
		_tcsncpy(pAgsmLoginQueueInfo->m_szServerName, pQueryInfo->m_szServerName, AGPMLOGIN_SERVERGROUPNAMESIZE);
		_tcsncpy(pAgsmLoginQueueInfo->m_szDBName, pQueryInfo->m_szDBName, AGPMLOGIN_SERVERGROUPNAMESIZE);

		m_pAgsmLoginServer->CheckAndPushToQueue((PVOID) pAgsmLoginQueueInfo, __FILE__, __LINE__);

		bResult = TRUE;
	}
	else if ( -1 == iQueryResult )
	{
		// pDatabase->SetParam() 실패	
		bResult = FALSE;
	}
	else if ( -2 == iQueryResult )
	{
		// 쿼리 실행 실패
		bResult = FALSE;
	}
	...
	m_pAgsmLoginClient->SetLoginStep( pQueryInfo->m_ulCharNID, AGPMLOGIN_STEP_GET_EXIST_CHARACTER );

	char szLog[256]={0,};
	sprintf_s(szLog, sizeof(szLog), "RemoveCharacter CheckPW : AccountID[ %s ], CharacterID[ %s ], Result[ %d ]",
		pQueryInfo->m_szAccountID, pQueryInfo->m_szCharacterID, iQueryResult);
	AuLogFile_s(LOGIN_REMOVE_CHARACTER_CHECK_PW_FILE_NAME, szLog);
...
BOOL AgsmLoginDB::_SelectNonEquipItems(AuDatabase2 *pDatabase, AgpdCharacter *pAgpdCharacter)
	...
	do	{
		...
		if (pAgpdItem)
			...
			else if (AGPDITEM_STATUS_CHARGING_INVENTORY == pAgpdItem->m_eStatus)
			{
				pAgpdItem->m_eStatus	= AGPDITEM_STATUS_NOTSETTING;
				AgpdGrid *pAgpdGrid = m_pAgpmItem->GetChargingInventory(pAgpdCharacter);
				if (pAgpdGrid)
				{
					if (!m_pAgpmItem->AddItemToChargingInventory(pAgpdCharacter, pAgpdItem, pAgpdItem->m_anGridPos[0], pAgpdItem->m_anGridPos[1], pAgpdItem->m_anGridPos[2], FALSE))
					{
						if (!m_pAgpmItem->AddItemToInventory(pAgpdCharacter, pAgpdItem, FALSE))
							m_pAgpmItem->RemoveItem(pAgpdItem->m_lID);
					}
...
BOOL AgsmLoginDB::_IsArchlordGuard(AuDatabase2 *pDatabase, CHAR *pszChar)
	...
	if (!pDatabase->SetQueryText(pszQuery)
		|| !pDatabase->SetParam(0, pszChar, (INT32)((_tcslen(pszChar) + 1) * sizeof(TCHAR)))
		|| AUDATABASE2_QR_FAIL == pDatabase->ExecuteQuery()
		)
		goto isg_fail;
...
#ifdef _AREA_GLOBAL_
int AgsmLoginDB::GlobalAuthFromDB(AgsmLoginQueueInfo *pQueryInfo)
{
	AuDatabase2		*pDatabase = NULL;
	TCHAR *pszQuery	= NULL;
	...
	{
		char strCharBuff[256] = { 0, };
		sprintf_s(strCharBuff, sizeof(strCharBuff), "AgsmLoginDB::GlobalAuthFromDB - 1\n");
		AuLogFile_s(LOGIN_ERROR_FILE_NAME, strCharBuff);
	...
	// 이놈이 살아있는지를 점검
	if(FALSE == pDatabase->IsConnect())
	{
		CHAR szLog[256] = { 0, };
		sprintf_s(szLog,sizeof(szLog), "LoginDB_Disconnect");
		AuLogFile_s(LOGIN_DB_DISCONNECT, szLog);

		m_pAgsmLoginServer->SetServerStatus(GF_SERVER_DB_DISCONNECTED);

		DisconnectDatabase();
		BOOL bIsConnect = ConnectDatabase();
		if(bIsConnect == TRUE)
		{
			m_pAgsmLoginServer->SetServerStatus(GF_SERVER_START);
		}
	...
	pszQuery = GetQuery(_T("GLOBAL_AUTH"), pDatabase->GetVender());
	ASSERT(NULL != pszQuery);
	if (NULL == pszQuery)
		return FALSE;
	...
	ParseGlobalGamestring(pQueryInfo->m_szGBAuthString, szGameId, szAuthKey, szMacAddress);
	
	char* gameId = const_cast<char*>(szGameId.c_str());
	char* authKey = const_cast<char*>(szAuthKey.c_str());
	char* ipadress = GetPlayerIPAddress(pQueryInfo->m_ulCharNID);
	char* macaddress = const_cast<char*>(szMacAddress.c_str());

	CHAR arrGameID[32];
	CHAR arrAuthKey[48];
	CHAR arrIPAddress[32];
	CHAR arrMacAddress[32];

	ZeroMemory(arrGameID, sizeof(arrGameID));
	ZeroMemory(arrAuthKey, sizeof(arrAuthKey));
	ZeroMemory(arrIPAddress, sizeof(arrIPAddress));
	ZeroMemory(arrMacAddress, sizeof(arrMacAddress));

	strncpy_s(arrGameID, sizeof(arrGameID), gameId, _TRUNCATE);
	strncpy_s(arrAuthKey, sizeof(arrAuthKey), authKey, _TRUNCATE);
	strncpy_s(arrIPAddress, sizeof(arrIPAddress), ipadress, _TRUNCATE);
	strncpy_s(arrMacAddress, sizeof(arrMacAddress), macaddress, _TRUNCATE);


	if( GetGameEnv().IsDebugTest() )
		gameId = pQueryInfo->m_szAccountID;

	//cout << "[Global Auth - Call Function] GlobalAuthFromDB()" << endl;
	//cout << "[Global Auth - Input Parameter] SetParam(1)" << "GameID = " << gameId << endl;
	...
	//cout << "[Global Auth - Input Parameter] SetParam(5)" << "MacAddress = " << macaddress << endl;
	AuLogConsole_s("Exec queryInfo:%s", pszQuery, __FUNCTION__);
	if (!pDatabase->SetQueryText(pszQuery) ||
		!pDatabase->SetParam(0, arrGameID, sizeof(arrGameID)) ||
		!pDatabase->SetParam(1, &gameCode) || //Gamecode
		!pDatabase->SetParam(2, arrAuthKey, sizeof(arrAuthKey)) ||
		!pDatabase->SetParam(3, arrIPAddress, sizeof(arrIPAddress) ) ||
		!pDatabase->SetParam(4, arrMacAddress, sizeof(arrMacAddress)) ||
		!pDatabase->SetParam(5, &re1, TRUE) ||
		!pDatabase->SetParam(6, &re2, TRUE) ||
		!pDatabase->SetParam(7, &re3, TRUE) ||
		!pDatabase->SetParam(8, &re4, TRUE)
		)
	{
		pDatabase->EndQuery();
		
		char strCharBuff[256] = { 0, };
		sprintf_s(strCharBuff, sizeof(strCharBuff), "GlobalAuthFromDB - SetParm Error ( %s, %d, %s, %s, %s)\n" ,gameId, gameCode, authKey, ipadress, macaddress);
	...
	if (AUDATABASE2_QR_FAIL == pDatabase->ExecuteQuery())
	{
		...
		char strCharBuff[256] = { 0, };
		sprintf_s(strCharBuff, sizeof(strCharBuff), "GlobalAuthFromDB - ExecuteQuery Error\n");
		AuLogFile_s(LOGIN_AUTH_FILE_NAME, strCharBuff);
	...
	//cout << "[Global Auth - Call Function] GlobalAuthFromDB()" << endl;
	//cout << "[Global Auth - Output Parameter] Result(1)" << "AccountGuid = " << re1 << endl;
	cout << "[Global Auth - Output Parameter] Result(2)" << "AccountType = " << re2 << endl;
	//cout << "[Global Auth - Output Parameter] Result(3)" << "BlockState = " << re3 << endl;
	//cout << "[Global Auth - Output Parameter] Result(4)" << "Result = " << re4 << endl;

	//result re4 값 여부에 따른 에러 처리 코드 작업 해야함.

	if(re2 > 0)
		pQueryInfo->m_dwAccountType = re2;
