...
//	Finish 2005.06.02 By SungHoon
	...
	m_pstUDUseNewSlot = m_pcsAgcmUIManager2->AddUserData("OP_UsableNewSlot", &m_bUseNewSlot, sizeof(BOOL), 1, AGCDUI_USERDATA_TYPE_BOOL);
	if (m_pstUDUseNewSlot < 0)		return FALSE;
	if (!m_pcsAgcmUIManager2->AddFunction(this, "OP_UseNewSlot", CBUseNewSlot, 0))			return FALSE;
...
void	AgcmUIOption::UpdateImpact(BOOL	bRefresh)
{
	if (m_pcsAgcmRender)
	{
		if (m_eImpact == AGCD_OPTION_TOGGLE_OFF)
		{
			...
			if(m_eBloom == AGCD_OPTION_EFFECT2_OFF && m_eAutoExposure == AGCD_OPTION_TOGGLE_OFF && m_pcsAgcmPostFX && m_pcsAgcmPostFX->isReady())
			{
...
void AgcmUIOption::UpdateUseNewSlot( BOOL bRefresh )
{
	if(bRefresh)
		m_pcsAgcmUIManager2->SetUserDataRefresh( m_pstUDUseNewSlot );

	if(m_bSave)
		SaveToINI(PATCHCLIENT_OPTION_FILENAME);

	EnumCallback(AGCMUIOPTION_CB_ID_USE_NEW_SLOT, (PVOID)m_bUseNewSlot, NULL);
}
...
//#endif
		...
		else if(!strcmp(szSectionName,"ETC_Option"))
		{
			while(csStream.ReadNextValue())
			{
				...
				else if(!strcmp(szValueName, "VS_USENEWSLOT"))
				{
					csStream.GetValue(&m_bUseNewSlot);
				}
...
// #endif
	...
	if(!csStream.WriteValue("VS_USENEWSLOT", m_bUseNewSlot))		return FALSE;
...
BOOL AgcmUIOption::CBUseNewSlot( PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl )
{
	AgcmUIOption *	pThis = (AgcmUIOption *) pClass;

	if (pcsSourceControl && pcsSourceControl->m_lType == AcUIBase::TYPE_BUTTON)
		pThis->m_bUseNewSlot = (((AcUIButton *) pcsSourceControl->m_pcsBase)->GetButtonMode() == ACUIBUTTON_MODE_CLICK);
	pThis->UpdateUseNewSlot(FALSE);
...
void AgcmUIOption::AddSystemMessageForSkillEffect( BOOL bIsOFF )
{
	...
	if( szMessage )
	{
		...
		AgcChatManager::OnAddSystemMessage(szMessage, dwColor);
	}
}
...
BOOL AgcmUIOption::SetCallbackChangeOptionUseNewSlot( ApModuleDefaultCallBack pfCallback, PVOID pClass )
{
	return SetCallback(AGCMUIOPTION_CB_ID_USE_NEW_SLOT, pfCallback, pClass);
}
