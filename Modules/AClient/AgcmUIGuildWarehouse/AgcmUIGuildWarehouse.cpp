...
BOOL AgcmUIGuildWarehouse::OnItemMovedIn(AgpdItem *pAgpdItem, INT16 nLayer, INT16 nRow, INT16 nColumn)
	...
	if (NULL == m_pWarehouseGrid
		|| ((AGPDITEM_STATUS_INVENTORY != pAgpdItem->m_eStatus) && (AGPDITEM_STATUS_SUB_INVENTORY != pAgpdItem->m_eStatus) && (AGPDITEM_STATUS_CHARGING_INVENTORY != pAgpdItem->m_eStatus)))
		return FALSE;
#if defined(_AREA_GLOBAL_)
	if(pAgpdItem->m_lExpireTime > 0)
#elif defined( _AREA_TAIWAN_ )
	if( pAgpdItem->m_lExpireTime == 0xFFFFFFFF )
#else
	if(pAgpdItem->m_lExpireTime == 0 )//|| pAgpdItem->m_lExpireTime == 0xFFFFFFFF)
#endif
		return FALSE;
...
BOOL AgcmUIGuildWarehouse::OnItemMovedOut(AgpdItem *pAgpdItem, INT16 nStatus, INT16 nLayer, INT16 nRow, INT16 nColumn)
	...
	else if(nStatus == AGPDITEM_STATUS_CHARGING_INVENTORY)
	{
		// charging inventory full check
		// is empty grid position
		if(m_pAgpmGrid->IsFullGrid(&pAgpdItemADChar->m_csChargingInventoryGrid) ||
			!m_pAgpmGrid->IsEmptyGrid(&pAgpdItemADChar->m_csChargingInventoryGrid, nLayer, nRow, nColumn, 1, 1))
			return FALSE;
	}
	else
		return FALSE;
...
BOOL AgcmUIGuildWarehouse::CBOpenResult(PVOID pData, PVOID pClass, PVOID pCustData)
	{
// #ifdef _AREA_GLOBAL_
// 		return FALSE;	// 복사이슈로 인한 길드창고 임시 블럭
// #else
		if (!pData || !pClass || !pCustData)
			return FALSE;
