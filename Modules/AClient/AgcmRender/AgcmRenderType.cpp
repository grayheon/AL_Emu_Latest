...
AgcdClumpRenderTypeData::AgcdClumpRenderTypeData( void )
{
}
...
void AgcdClumpRenderTypeData::Alloc( int nSize )
{
	m_vecRenderType.resize( nSize );
	m_vecCustData.resize( nSize );
}

void AgcdClumpRenderTypeData::MemsetRenderType( INT32 nValue )
{
	int nRenderTypeCount = ( int )m_vecRenderType.size();
// 	vector< INT32 >::iterator iter = m_vecRenderType.begin();

	for( int nCount = 0 ; nCount < nRenderTypeCount ; nCount++ )
	{
		m_vecRenderType[ nCount ] = nValue;
	}
}

void AgcdClumpRenderTypeData::MemsetCustData( INT32 nValue )
{
	int nCustDataCount = ( int )m_vecCustData.size();
// 	vector< INT32 >::iterator iter = m_vecCustData.begin();

	for( int nCount = 0 ; nCount < nCustDataCount ; nCount++ )
	{
		m_vecCustData[ nCount ] = nValue;
	}
}

void AgcdClumpRenderTypeData::Clear()
{
	int nRenderTypeCount = ( int )m_vecRenderType.size();
	if( nRenderTypeCount > 0 )
	{
