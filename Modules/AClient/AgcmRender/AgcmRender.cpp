...
{
	m_mapAtomics.insert( make_pair( BlendAtomic.m_fSortFactor , BlendAtomic ) );
}
...
void stBlendAtomicEntry::Add( enumRenderBMode eRenderMode, stBlendAtomic BlendAtomic )
{
	...
	{
		stBlendAtomicInfo NewBlendType;
...
//Error log 용 디버그 변수
int g_DebugHashKey	= -1;
DWORD g_DebugHashTableAtomic[R_HASH_SIZE];
DWORD g_DebugHashTableTexaddr[R_HASH_SIZE];
DWORD g_DebugHashTableSibling[R_HASH_SIZE];
DWORD g_DebugHashTableNext[R_HASH_SIZE];
//
...
void AgcmRender::RenderAtomic(RpAtomic*	atomic,BOOL	 bBill)
{
	...
	if( m_pAmbientLight && m_pDirectLight && pClump )
	{
		...
		case LIGHT_QUEST_POISON:
			...
			break;
		case LIGHT_PICKING:
			pAmbientColor = &m_lightColorPicking;
			bNeedLightRestore = TRUE;
			break;
		}
...
//#endif
	...
	ZeroMemory(new_node, sizeof(SkinningTSortAtomic));
	...
	for(int i=0;i<R_HASH_SIZE;++i)
	{
		if(m_pThisAgcmRender->m_listSkinning.hash_table[i])
		{
			g_DebugHashTableAtomic[i] = (DWORD)m_pThisAgcmRender->m_listSkinning.hash_table[i]->atomic;
			g_DebugHashTableSibling[i] = (DWORD)m_pThisAgcmRender->m_listSkinning.hash_table[i]->sibling;
			g_DebugHashTableTexaddr[i] = (DWORD)m_pThisAgcmRender->m_listSkinning.hash_table[i]->texaddr;
			g_DebugHashTableNext[i] = (DWORD)m_pThisAgcmRender->m_listSkinning.hash_table[i]->next;
		}
		else
		{
			g_DebugHashTableAtomic[i] = 0;
			g_DebugHashTableSibling[i] = 0;
			g_DebugHashTableTexaddr[i] = 0;
			g_DebugHashTableNext[i] = 0;
		}
	}
...
FLOAT	AgcmRender::_GetSortFactor( RpAtomic* pAtomic )
{
	...
	if( g_pEngine && g_pEngine->m_pCamera )
	{
		RwMatrix * viewMat = RwCameraGetViewMatrix( g_pEngine->m_pCamera );

		RwMatrix m;

		RwMatrixMultiply( &m, pMatrix, viewMat );
...
void	AgcmRender::OcTreeModeRender(RwCamera*	pCheckCamera,AgcmRenderTestRenderTerrain	pTerrainFunc,BOOL bWorldAdded)
{
	...
	RwMatrixSetIdentity(&mat); // 2010.11.23 kdi 추가 (없어도되나.)
...
void AgcmRender::ComputeLightColors()		// Compute light colors in shadow, at login selection, of quest objects.
{
	...
	m_lightColorPicking.red		= m_pAmbientLight->color.red * 2.0f;
	m_lightColorPicking.green	= m_pAmbientLight->color.green * 2.0f;
	m_lightColorPicking.blue	= m_pAmbientLight->color.blue * 2.0f;
...
bool AgcmRender::_RenderBlendAtomic_BillBoard( void )
{
	...
	mmapBlendAtomicIter		Iter		=	pBlendAtomic->m_mapAtomics.begin();
	for( ; Iter != pBlendAtomic->m_mapAtomics.end() ; ++Iter )
	{
		RwD3D9SetRenderState( D3DRS_BLENDOP , D3DBLENDOP_ADD );
...
{
	mmapBlendAtomicIter		Iter		=	m_BlendAtomics.m_LastBlendAtomic.m_mapAtomics.begin();
	for( ; Iter != m_BlendAtomics.m_LastBlendAtomic.m_mapAtomics.end() ; ++Iter )
	{
		stBlendAtomic* pAtomic = &Iter->second;
		if( pAtomic )
		{
