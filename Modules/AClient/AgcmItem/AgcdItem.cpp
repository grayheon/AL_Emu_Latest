...
ItemClumpInfo::Dic ItemClumpInfo::dic_;

void AgcdItemTemplate::SetFileNameTexture( char* pFileName )
{
	if( !pFileName || strlen( pFileName ) <= 0 ) return;

	memset( m_strFileNameTexture, 0, sizeof( char ) * 256 );
	strcpy_s( m_strFileNameTexture, sizeof( char ) * 256, pFileName );
}
