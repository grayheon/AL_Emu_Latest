...
BOOL AgcmItem::ItemAddCB(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	if(((AgpdItemTemplateUsable *)pcsAgpdItem->m_pcsItemTemplate)->m_nUsableItemType == AGPMITEM_USABLE_TYPE_CHARGING_BAG)
	{
		pThis->UseChargingInventory(pcsAgpdItem);
	}
...
#ifdef _BIN_EXEC_
	ItemClumpInfo & textureInfo = ItemClumpInfo::Get( pcsItemTemplate->GetID() );

	CHAR	pszBaseClump[32]		= {'\0',};
	
	strcpy_s(pszBaseClump, 32, textureInfo.baseDff());

	_strlwr_s(pszBaseClump, 32);

	if( strcmp( pszBaseClump, "item_food.rws") == 0 )
	{
		AgcmUIManager2* pcsAgcmUIManager = (AgcmUIManager2*)g_pEngine->GetModule("AgcmUIManager2");

		if( pcsAgcmUIManager )
		{
			pcsAgcmUIManager->ThrowEvent( pcsAgcmUIManager->m_nEventEatSound );
		}
	}
#endif
...
BOOL AgcmItem::SendChargingInventoryInfo( INT32 lIID, INT32 lCID, INT16 nInvIdx /*= -1*/, INT16 nInvRow /*= -1*/, INT16 nInvCol /*= -1*/ )
{
	//-----------------------------------------------------------------------
	// 2010.01.06 kdi
	// 드래곤 시온의 장착해제 불가 아이템을 비정상적으로 해제하여 스텟을 어뷰징하는 버그를 클라이언트에서 차단합니다.
	...
	INT16		nSize;
	INT8		cOperation = AGPMITEM_PACKET_OPERATION_UPDATE;
	INT8        cStatus    = AGPDITEM_STATUS_CHARGING_INVENTORY;
...
BOOL AgcmItem::SendTooltipRegisterItemRequest( INT32 llID , INT lTID , INT32 lCID, eAgpmItemTooltipType type )
{
	INT16	nPacketLength	=	0;
	PVOID	pvPacket		=	m_pcsAgpmItem->MakePacketItemInforMation( llID , lTID , lCID , type, &nPacketLength );
...
BOOL AgcmItem::LoadTemplateClump(AgpdItemTemplate *pcsAgpdItemTemplate, AgcdItemTemplate *pstAgcdItemTemplate, BOOL bArmour)
{
	...
	else
	{
		...
		ItemClumpInfo & textureInfo = ItemClumpInfo::Get( pcsAgpdItemTemplate->GetID() );

		pszBaseClump = textureInfo.baseDff();
		pszSecondClump = textureInfo.secondDff();
		pszFieldClump = textureInfo.fieldDff();
		pszPickingClump = textureInfo.pickDff();
...
BOOL AgcmItem::LoadTemplateTexture(AgpdItemTemplate *pcsAgpdItemTemplate, AgcdItemTemplate *pstAgcdItemTemplate)
{
	...
	else
	{
		...
		pszTexture = textureInfo.textureName();
		pszSmallTexture = textureInfo.smallTextureName();
		pszDurZeroTexture = textureInfo.durZeroTextureName();
		pszDur5UnderTexture = textureInfo.dur5UnderTextureName();
...
BOOL AgcmItem::UseItem(AgpdItem *pcsItem, AgpdCharacter *pcsCharacter)
{
	...
	if( !pcmUIItem->OnCheckUseEnable( pcsItem, pcsCharacter ) ) return FALSE;
...
BOOL	AgcmItem::SetCallbackUseChargingInventory		(ApModuleDefaultCallBack pfCallback, PVOID pClass)
{
	return SetCallback(AGCMITEM_CB_ID_USE_MAP_ITEM, pfCallback, pClass);
}
	...
	if (!pcsAgcdItemTemplate->m_pTexture)
	{
		...
		if (!pcsAgcdItemTemplate->m_pTexture)
		{
			...
			else
			{
				...
				ItemClumpInfo & textureInfo = ItemClumpInfo::Get( pcsAgpdItemTemplate->GetID() );
				pcsAgcdItemTemplate->m_pTexture	= GetTexture( textureInfo.textureName() );
...
BOOL AgcmItem::ReceiveUnuseItem( AgpdItem *pcsItem )
{
	...
	{
		AgpdItemADChar *pcsItemADChar = m_pcsAgpmItem->GetADCharacter(pcsItem->m_pcsCharacter);
		if (!pcsItemADChar)
			return FALSE;

		pcsItemADChar->m_eChargingBagType = AGPMITEM_USABLE_CHARGING_BAG_TYPE_NONE;
		pcsItemADChar->m_bUseChagingInventory = FALSE;
	}
...
BOOL AgcmItem::UseChargingInventory( AgpdItem *pcsItem )
{
	...
	AgpdItemADChar *pAttachedData = m_pcsAgpmItem->GetADCharacter(pcsItem->m_pcsCharacter);
	if( !pAttachedData )
		return FALSE;
	...
	AgcmUIItem* pcsUIItem = (AgcmUIItem*)GetModule("AgcmUIItem");
	if(!pcsUIItem)
		return FALSE;
	...
	case AGPMITEM_USABLE_CHARGING_BAG_TYPE_A:
		{
			pcsUIItem->SetChargingBagType(AGPMITEM_USABLE_CHARGING_BAG_TYPE_A);
			pAttachedData->m_eChargingBagType = AGPMITEM_USABLE_CHARGING_BAG_TYPE_A;
		} break;

	case AGPMITEM_USABLE_CHARGING_BAG_TYPE_B:
		{
			pcsUIItem->SetChargingBagType(AGPMITEM_USABLE_CHARGING_BAG_TYPE_B);
