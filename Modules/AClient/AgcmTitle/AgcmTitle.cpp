...
BOOL AgcmTitle::_LoadAllTitleCategoryInfo( void )
{
	...
	{
		AgpdTitleCategory TitleCategory = *Iter;
		{
			stTitleCategory NewCategory;

			strcpy_s( NewCategory.m_strName, sizeof( char ) * STRING_LENGTH_TITLE_NAME, TitleCategory.m_szTitleClass );
			NewCategory.m_nCategoryID = TitleCategory.m_nFirstCategory;

			vectorTitleStringTemplate vecTitle = ppmTitle->m_stAgpaTitleStringTemplate.FindByCategoryID( TitleCategory.m_nFirstCategory );

			vectorTitleStringTemplate::iterator IterTitle = vecTitle.begin();
			while( IterTitle != vecTitle.end() )
			{
				AgpdTitleStringTemplate pTitleString = *IterTitle;
				{
		...
		if( pcmUIManager /*&& pcmChatting*/ )
		{
			...
			if( pMessageFormat && strlen( pMessageFormat ) > 0 )
			{
				...
				if( pTitleName && strlen( pTitleName ) > 0 )
				{
					...
					AgcChatManager::OnAddSystemMessage( strMessage );
				}
			}
		}
	}
