...
BOOL AgcmUIStatus::CBReceiveQuickBeltInfo(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	for (int nLayer	= 0; nLayer < AGPMUISTATUS_MAX_QUICKBELT_LAYER; ++nLayer)
	{
		...
		for (int i = nLayer * AGPMUISTATUS_MAX_QUICKBELT_COLUMN; i < nLayer * AGPMUISTATUS_MAX_QUICKBELT_COLUMN + AGPMUISTATUS_HALF_QUICKBELT_COLUMN; ++i)
		{
				...
				pcsAttachData->m_acsQBeltItem[i].m_lID != 0)
			{
				...
				if (pcsBase)
				{
					...
					case APBASE_TYPE_ITEM:
						{
							if(pThis->m_pcsAgcmUIOption->m_bUseNewSlot)
								bIsAdded	= pThis->m_pcsAgcmUIMain->AddShortcut(&pThis->m_pcsAgcmUIMain->m_astQuickBeltGrid[nLayer*2], ((AgpdItem *) pcsBase)->m_pcsGridItem, 0, 0, nColumn, FALSE, nLayer);
							else
								bIsAdded	= pThis->m_pcsAgcmUIMain->AddShortcut(&pThis->m_pcsAgcmUIMain->m_astQuickBeltGrid[0], ((AgpdItem *) pcsBase)->m_pcsGridItem, 0, 0, nColumn, FALSE, nLayer);
						}
						break;
					...
					case APBASE_TYPE_SKILL:
						{
							if(pThis->m_pcsAgcmUIOption->m_bUseNewSlot)
								bIsAdded	= pThis->m_pcsAgcmUIMain->AddShortcut(&pThis->m_pcsAgcmUIMain->m_astQuickBeltGrid[nLayer*2], ((AgpdSkill *) pcsBase)->m_pcsGridItem, 0, 0, nColumn, FALSE, nLayer);
							else
								bIsAdded	= pThis->m_pcsAgcmUIMain->AddShortcut(&pThis->m_pcsAgcmUIMain->m_astQuickBeltGrid[0], ((AgpdSkill *) pcsBase)->m_pcsGridItem, 0, 0, nColumn, FALSE, nLayer);
						}
						break;
					}
				}
			...
			{
				if(pThis->m_pcsAgcmUIOption->m_bUseNewSlot)
					pThis->m_pcsAgcmUIMain->RemoveShortcut(&pThis->m_pcsAgcmUIMain->m_astQuickBeltGrid[nLayer*2], 0, 0, nColumn, TRUE, nLayer);
				else
					pThis->m_pcsAgcmUIMain->RemoveShortcut(&pThis->m_pcsAgcmUIMain->m_astQuickBeltGrid[0], nLayer, 0, nColumn, TRUE, 0);
			}
		...
		for (int i = nLayer * AGPMUISTATUS_MAX_QUICKBELT_COLUMN + AGPMUISTATUS_HALF_QUICKBELT_COLUMN; i < (nLayer + 1) * AGPMUISTATUS_MAX_QUICKBELT_COLUMN; ++i)
		{
				...
				pcsAttachData->m_acsQBeltItem[i].m_lID != 0)
			{
				...
				if (pcsBase)
				{
					...
					case APBASE_TYPE_ITEM:
						{
							if(pThis->m_pcsAgcmUIOption->m_bUseNewSlot)
								bIsAdded	= pThis->m_pcsAgcmUIMain->AddShortcut(&pThis->m_pcsAgcmUIMain->m_astQuickBeltGrid[nLayer*2+1], ((AgpdItem *) pcsBase)->m_pcsGridItem, 0, 0, nColumn, FALSE, nLayer);
							else
								bIsAdded	= pThis->m_pcsAgcmUIMain->AddShortcut(&pThis->m_pcsAgcmUIMain->m_astQuickBeltGrid[1], ((AgpdItem *) pcsBase)->m_pcsGridItem, 0, 0, nColumn, FALSE, nLayer);
						}
						break;
					...
					case APBASE_TYPE_SKILL:
						{
							if(pThis->m_pcsAgcmUIOption->m_bUseNewSlot)
								bIsAdded	= pThis->m_pcsAgcmUIMain->AddShortcut(&pThis->m_pcsAgcmUIMain->m_astQuickBeltGrid[nLayer*2+1], ((AgpdSkill *) pcsBase)->m_pcsGridItem, 0, 0, nColumn, FALSE, nLayer);
							else
								bIsAdded	= pThis->m_pcsAgcmUIMain->AddShortcut(&pThis->m_pcsAgcmUIMain->m_astQuickBeltGrid[1], ((AgpdSkill *) pcsBase)->m_pcsGridItem, 0, 0, nColumn, FALSE, nLayer);
						}
						break;
					}
				}
			...
			{
				if(pThis->m_pcsAgcmUIOption->m_bUseNewSlot)
					pThis->m_pcsAgcmUIMain->RemoveShortcut(&pThis->m_pcsAgcmUIMain->m_astQuickBeltGrid[nLayer*2+1], 0, 0, nColumn, TRUE, nLayer);
				else
					pThis->m_pcsAgcmUIMain->RemoveShortcut(&pThis->m_pcsAgcmUIMain->m_astQuickBeltGrid[1], nLayer, 0, nColumn, TRUE, 0);
			}
...
BOOL AgcmUIStatus::CBAddShortcut(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	if (&pThis->m_pcsAgcmUIMain->m_astQuickBeltGrid[lLayer2] == pcsGrid)
	{
...
BOOL AgcmUIStatus::CBRemoveShortcut(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	if (bIsCleared)
	{
		...
		if (&pThis->m_pcsAgcmUIMain->m_astQuickBeltGrid[lLayer*2] == pcsGrid)
		{
