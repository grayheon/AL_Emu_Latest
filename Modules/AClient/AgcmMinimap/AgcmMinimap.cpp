...
#define		MINIMAP_X_INTERVAL		500
#define		MINIMAP_X_DURATION_TIME	3000


#define		MAX_REGISTER_POS_COUNT	10
...
{
	return		( INT32 ) ( fPos / ( MAP_SECTOR_WIDTH * MAP_DEFAULT_DEPTH ) * ( ACUIRAIDER_RANGE_SIZE << 1 ) );
	return 0;
}
...
#define LOADUITEXTUREMACRO( filename )			__LoadTextureMacro( this , AGCMMAP_MINIMAP_UI_FOLDER , filename )
...
BOOL	AgcmMinimap::OnAddModule	()
{
	...
	if (m_pcsAgcmUIManager2)
	{
		...
		m_pcsAgcmUIManager2->AddDisplay( this , "Minimap_Region_Name" , 0 , CBDisplayRegionName , AGCDUI_USERDATA_TYPE_INT32 );

		m_pcsAgcmUIManager2->AddFunction( this	, "Minimap_Plus_Click"	, CBClickPlusButton , 0 );
		m_pcsAgcmUIManager2->AddFunction( this	, "Minimap_Minus_Click" , CBClickMinusButton , 0 );

		m_pcsAgcmUIManager2->AddFunction( this , "Minimap_CameraView_Click" , CBClickCameraView , 0 );
		m_pcsAgcmUIManager2->AddFunction( this , "Minimap_Map_Toggle"		, CBClickMapToggle , 0 );

		m_pstUDPosition = m_pcsAgcmUIManager2->AddUserData( "Raider_Pos" , &m_stSelfCharacterPos , sizeof(AuPOS) , 1 , AGCDUI_USERDATA_TYPE_POS );
		if( !m_pstUDPosition )
			return FALSE;

		m_pstMMRegionNameUD	=	m_pcsAgcmUIManager2->AddUserData( "Minimap_Region_Name_UD" , &m_lDummy , sizeof(INT32) , 1 , AGCDUI_USERDATA_TYPE_INT32 );
	...
	if( m_pcsAgcmUIManager2 )
	{
		...
		m_lEventMinimapON	=	m_pcsAgcmUIManager2->AddEvent( "Minimap_Map_ON" );
		m_lEventMinimapOFF	=	m_pcsAgcmUIManager2->AddEvent( "Minimap_Map_OFF" );
	...
	// 월드맵 설정 읽어들임.
	{
		...
		bIsEncrypted = FALSE;
#endif // BIN_EXEC
		if( pcsApmMap->LoadWorldMap( "Ini\\" WORLDMAPTEMPLATE , bIsEncrypted ) )
		#endif // USE_MFC
		{
			// 지도 정보 추가.
...
BOOL	AgcmMinimap::OnDestroy		()
{
	...
	if( m_pRegisterPosTexture )
	{
		RwTextureDestroy( m_pRegisterPosTexture );
		m_pRegisterPosTexture	=	NULL;
	}
...
BOOL AgcmMinimap::CBBindingRegionChange(PVOID	pData,PVOID	pClass,PVOID	pCustData)
{
	...
	pThis->m_pcsAgcmUIManager2->SetUserDataRefresh( pThis->m_pstMMRegionNameUD );
...
{
	if( NULL == this->m_pcsAgcmResourceLoader ) 
		return NULL;

	if( m_pDirectionCursor ) 
		return m_pDirectionCursor;

	m_pDirectionCursor	=	LOADUITEXTUREMACRO( AGCMMAP_MINIMAP_FILE_CURSOR );
...
RwTexture	*	AgcmMinimap::GetRegisterPosTexture()
{
	if( NULL == this->m_pcsAgcmResourceLoader ) 
		return NULL;
	...
	if( !m_pRegisterPosTexture )
	{
		m_pRegisterPosTexture	= LOADTEXTUREMACRO( AGCMMAP_REGISTER_POSITION );

		// RwTextureSetFilterMode( m_pMemberPos , rwFILTERNEAREST );
		if( m_pRegisterPosTexture )
			RwTextureSetAddressing ( m_pRegisterPosTexture , rwTEXTUREADDRESSCLAMP );
	}

	return m_pRegisterPosTexture;
}
...
INT32				AgcmMinimap::AddPoint		( INT32 nType , char * strText , AuPOS * pPos , INT32 nControlGroupID )
{
	...
	*/
	...
	//월드맵에도 추가
	AgcmUIWorldmap* pWorldMap = (AgcmUIWorldmap*)GetModule("AgcmUIWorldmap");
	if( pWorldMap )
	{
		RwV2d vPos;
		vPos.x = (RwReal)newInfo.pos.x;
		vPos.y = (RwReal)newInfo.pos.z;

		AGUIDrawMark::E_MARKINFO_TYPE eType;
		switch( (MPInfo::TYPE)nType )
		{
		case MPInfo::MPI_NPC: eType = AGUIDrawMark::E_TYPE_NPC; break;
		case MPInfo::MPI_QUESTNEW: eType = AGUIDrawMark::E_TYPE_NPC_QUEST_NEW; break;
		case MPInfo::MPI_QUESTINCOMPLETE: eType = AGUIDrawMark::E_TYPE_NPC_QUEST_COMPLETE; break;
		case MPInfo::MPI_SIEGENORMAL: eType = AGUIDrawMark::E_TYPE_SIEGE; break;
		case MPInfo::MPI_SIEGEDESTROIED: eType = AGUIDrawMark::E_TYPE_SIEGE_DESTROY; break;
		}

		pWorldMap->AddPoint( strText, vPos , eType );
	}
...
BOOL				AgcmMinimap::RemovePoint		( INT32 nIndex )
{
	...
	{
		RwV2d vPos;
		vPos.x = (RwReal)pInfo->pos.x; vPos.y = (RwReal)pInfo->pos.z;
		pWorldMap->RemovePoint( pInfo->strText, vPos);
	}
...
INT32				AgcmMinimap::RemovePointGroup		( INT32 nControlGroupID )
{
	...
	for ( int i = 0 ; i < GetMPCount() ; i ++ )
	{
		if( pArray[ i ].nControlGroupID == nControlGroupID )
		{
			...
			{
				RwV2d vPos;
				vPos.x = (RwReal)pArray[ i ].pos.x; vPos.y = (RwReal)pArray[ i ].pos.z;
				pWorldMap->RemovePoint( pArray[ i ].strText, vPos);
		}
	}
	}
...
BOOL	AgcmMinimap::CBClickPlusButton(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl)
{
	AgcmMinimap*	pThis	=	static_cast< AgcmMinimap* >(pClass);
	if( !pThis )
		return FALSE;

	pThis->m_pclMinimapWindow1->MapEnlargement();

	return TRUE;
}

BOOL	AgcmMinimap::CBClickMinusButton(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl)
{
	AgcmMinimap*	pThis	=	static_cast< AgcmMinimap* >(pClass);
	if( !pThis )
		return FALSE;

	pThis->m_pclMinimapWindow1->MapReduction();

	return TRUE;
}

BOOL	AgcmMinimap::CBClickCameraView(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl)
{
	AgcmMinimap*	pThis	=	static_cast< AgcmMinimap* >(pClass);
	if( !pThis )
		return FALSE;

	pThis->ToggleCameraView();

	return TRUE;
}

BOOL	AgcmMinimap::CBClickMapToggle(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl)
{
	AgcmMinimap*	pThis	=	static_cast< AgcmMinimap* >(pClass);
	if( !pThis )
		return FALSE;

	pThis->ToggleMinimapSize();

	return TRUE;
}

BOOL	AgcmMinimap::CBDisplayRegionName( PVOID pClass, PVOID pData, AgcdUIDataType eType, INT32 lID, CHAR *szDisplay, INT32 *plValue, AgcdUIControl* pcsSourceControl )
{
	AgcmMinimap*	pThis	=	static_cast< AgcmMinimap* >(pClass);
	if( !pThis )
		return FALSE;
	...
	ApmMap::RegionTemplate* pTemplate	= pThis->m_pcsApmMap->GetTemplate( pcsSelfCharacter->m_nBindingRegionIndex );
	if( !pTemplate )
		return FALSE;

	string					strRegionName	= RegionLocalName().GetStr( pTemplate->pStrName );

	if( strRegionName.length() )
		sprintf( szDisplay , "%s" , strRegionName.c_str() );
	else
...
BOOL	AgcmMinimap::SetCallbackRegisterMapPos( ApModuleDefaultCallBack pfCallback , PVOID pClass )
{
	return SetCallback( AGCMMINIMAP_CB_MAP_POS_REGISTER , pfCallback , pClass );
}

BOOL	AgcmMinimap::OpenDlgAndAddPos( RwV2d* pvPosition )
{
	if( !pvPosition )
		return FALSE;
	...
	CHAR*	pszMessage	=	m_pcsAgcmUIManager2->GetUIMessage( "Position_NameInput_Message" );
	if( m_pcsAgcmUIManager2->ActionMessageEditOKCancelDialog( pszMessage , szName  ) )
	{
		INT	nID	=	(INT)(pvPosition->x + pvPosition->y);

		nID		+=	strlen( szName );
		nID		=	abs( nID );

		nResult	=	AddMapPosition(  szName , nID , pvPosition );
		if( nResult )
		{
			m_pcsAgcmUIChatting2->OnReadyMapPosRegister( szName , nID , (INT)pvPosition->x , (INT)pvPosition->y );
		}
	}
...
BOOL	AgcmMinimap::AddMapPosition( CONST string&	strName , INT nID , RwV2d* pvPosition , INT iRegionIndex, BOOL bTempShow )
{
	AgcmUIWorldmap* pWorldMap = (AgcmUIWorldmap*)GetModule("AgcmUIWorldmap");
	if( pWorldMap )
	{
		pWorldMap->AddPoint( strName.c_str(), *pvPosition , AGUIDrawMark::E_TYPE_X_MARK, iRegionIndex, bTempShow );
	}

	stMapRegisterInfo	stMap;

	stMap.m_nID				=	nID;
	stMap.m_strName			=	strName;
	stMap.m_vPosition		=	(*pvPosition);
	stMap.m_dwInterval		=	MINIMAP_X_INTERVAL;
	stMap.m_dwDurationTime	=	MINIMAP_X_DURATION_TIME;
	stMap.m_dwStartTime		=	GetClockCount();


	if( GetMapPosition( nID ) )
		return TRUE;

	// 리스트에 추가하고
	m_listMapRegisterInfo.push_back( stMap );

	if( m_listMapRegisterInfo.size() > MAX_REGISTER_POS_COUNT )
	{
		m_listMapRegisterInfo.erase( m_listMapRegisterInfo.begin() );
	}

	// 처음 들어올때 이벤트 필요한데서 불러서 쓴다
	EnumCallback( AGCMMINIMAP_CB_MAP_POS_REGISTER , &stMap , NULL );

	return TRUE;
}

BOOL	AgcmMinimap::AddMapPosition( CONST string& strName , INT nID , INT nX , INT nY , INT iRegionIndex , BOOL bTempShow)
{
	RwV2d	vPosition;

	vPosition.x	=	(RwReal)nX;
	vPosition.y	=	(RwReal)nY;

	return AddMapPosition( strName , nID , &vPosition , iRegionIndex, bTempShow );
}

BOOL	AgcmMinimap::DestroyMapPosition( CONST string& strName )
{
	ListMapRegisterInfoIter	Iter	=	m_listMapRegisterInfo.begin();
	AgcmUIWorldmap* pWorldMap = (AgcmUIWorldmap*)GetModule("AgcmUIWorldmap");
	for( ; Iter != m_listMapRegisterInfo.end() ; ++Iter )
	{
		if( Iter->m_strName	== strName )
		{
			...
			if( pWorldMap )
			{
				pWorldMap->RemovePoint( Iter->m_nID );
			}

			m_listMapRegisterInfo.erase( Iter );
			return TRUE;
		}
	}
...
BOOL	AgcmMinimap::DestroyMapPosition( INT nID )
{
	ListMapRegisterInfoIter	Iter	=	m_listMapRegisterInfo.begin();
	AgcmUIWorldmap* pWorldMap = (AgcmUIWorldmap*)GetModule("AgcmUIWorldmap");
	for( ; Iter != m_listMapRegisterInfo.end() ; ++Iter )
...
stMapRegisterInfo*	AgcmMinimap::GetMapPositionIndex( INT nIndex )
{
	ListMapRegisterInfoIter	Iter	=	m_listMapRegisterInfo.begin();
	for( INT i = 0; Iter != m_listMapRegisterInfo.end() ; ++Iter , ++i )
	{
		if( i == nIndex )
		{
			return &(*Iter);
...
stMapRegisterInfo*	AgcmMinimap::GetMapPosition( CONST string& strName )
{
	ListMapRegisterInfoIter	Iter	=	m_listMapRegisterInfo.begin();
	for( ; Iter != m_listMapRegisterInfo.end() ; ++Iter )
	{
...
stMapRegisterInfo*	AgcmMinimap::GetMapPosition( INT nID )
{
	ListMapRegisterInfoIter	Iter	=	m_listMapRegisterInfo.begin();
	for( ; Iter != m_listMapRegisterInfo.end() ; ++Iter )
	{
...
VOID	AgcmMinimap::ClearMapPosition( VOID )
{
	m_listMapRegisterInfo.clear();
}

VOID	AgcmMinimap::ToggleCameraView( VOID )
{
	AgcmCamera2*	pAgcmCamera2	=	static_cast< AgcmCamera2* >( GetModule( "AgcmCamera2" ) );
	if( !pAgcmCamera2 )
		return;

	AgcuCamMode::eMode		eCameraMode	=	AgcuCamMode::bGetInst().bGetCurrMode();
	eCameraMode	=	(AgcuCamMode::eMode)(eCameraMode + 1);

	if( eCameraMode >= AgcuCamMode::eMode_1stPerson )
		eCameraMode	=	(AgcuCamMode::eMode)0;


	AgcuCamMode::bGetInst().bChageMode( *pAgcmCamera2, eCameraMode );
}

VOID	AgcmMinimap::ToggleMinimapSize( VOID )
{
	m_bMinimapON	=	!m_bMinimapON;

	AgcdUI*	pUI	=	m_pcsAgcmUIManager2->GetUI( "MiniMap" );
	if( !pUI )
		return;

	if( m_bMinimapON )
	{
		m_pcsAgcmUIManager2->ThrowEvent( m_lEventMinimapON );
		pUI->m_pcsUIWindow->SetRednerTexture( 1 );
	}

	else
	{
		m_pcsAgcmUIManager2->ThrowEvent( m_lEventMinimapOFF );
		pUI->m_pcsUIWindow->SetRednerTexture( 2 );
	}
