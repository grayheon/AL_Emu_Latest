        ...
        case WM_CREATE:
			{
				...
				case AP_SERVICE_AREA_GLOBAL:
					// 0x040C는 French lang ID xiaomi support chinese
					AuInputComposer::GetInstance().OnInputLanguageChange(window, 0, 0x0804);
				...
				case AP_SERVICE_AREA_TAIWAN:
					// 0x0404 is Taiwan language ID
					AuInputComposer::GetInstance().OnInputLanguageChange(window, 0, 0x0404);
					break;

// 				case AP_SERVICE_AREA_RUSSIAN:
// 					// 0x0419 is Russian language ID
// 					AuInputComposer::GetInstance().OnInputLanguageChange(window, 0, 0x0419);
// 					break;
        ...
        int cmdShow)
{
	...
	*/
	...
	int nChannelingRetVal = AgcmUILogin::ParseChannelingCodeString(argc >= 3 ? argv[ 2 ] : NULL);
	switch(nChannelingRetVal)
	{
	case AP_SERVICE_CHANNELING_MAX:
		//ArchlordTest.ini파일이 있음으로 인한 테스트모드
		g_eChannelingService = AP_SERVICE_CHANNELING_ORIGINAL_PATH;
		break;
	...
	case AP_SERVICE_CHANNELING_TAHADI_PATH:
		//TAHADI에서 접속함
		g_eChannelingService = AP_SERVICE_CHANNELING_TAHADI_PATH;
		break;
	}
#endif
    ...
    while( !RsGlobal.quit )
    {
        ...
        //while( PeekMessage(&g_msgLastReceived, NULL, 0U, 0U, PM_REMOVE) )
		...
		else if ( RwInitialized )
		{
			RsEventHandler( rsIDLE, NULL );

			AgcmFPSManager::AdjustFPS( ForegroundApp ? true : false );
		}
		/*
