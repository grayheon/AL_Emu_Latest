...
BOOL AgcmUISkill2::_IsUsableSkillForMyRaceAndClass( void* pCharacter, void* pSkillTemplate )
{
	...
	if(m_pcsAgpmCharacter->IsArchlord(ppdCharacter))
	{
		ArchlordSkillList vecArchlordSkillList = m_pcsAgpmEventSkillMaster->GetArchlordSkillList();
		if(vecArchlordSkillList.find(ppdSkillTemplate->m_lID) != vecArchlordSkillList.end())
			return TRUE;
	}
