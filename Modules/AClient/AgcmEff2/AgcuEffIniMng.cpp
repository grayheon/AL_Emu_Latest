...
BOOL AgcuEffIniMng::Read_txt( RwUInt32 dwEffSetID, AgcdEffSet* pOut, BOOL bFullRead )
{
	...
	RwChar	szFullName[MAX_PATH]	= {0,};
	sprintf( szFullName, "%s%d_eff.ini",AgcuEffPath::GetPath_Ini(),dwEffSetID);
...
BOOL AgcuEffIniMng::_Read_EffSet_txt( AgcdEffSet* pOut, BOOL bFullRead )
{
	...
	{//이펙트셋 reference counter 가 0 일때.
		...
		{
			sprintf_s( szEffTex , "%s:%d" , EFF_KEY[ eEffKey_set_efftex ][e_key] , i  );
			sprintf_s( szEffMask , "%s:%d" , EFF_KEY[ eEffKey_set_effmask ][e_key] , i  );
		...
		for( i=0; i<pOut->bGetVarSizeInfo().m_nNumOfBaseDependancy; ++i, ++nCurrKey )
		{
			...
			sprintf_s( szBaseDpnd , "%s:%d" , EFF_KEY[ eEffKey_set_depnd ][e_key] , i );
										  ...
										  , RwInt32 nIndex )
{
	...
	sprintf_s( szTypeName , "%s:%d" , EFF_KEY[ eEffKey_effAnim_type ][e_key] , nIndex );
	sprintf_s( szFlagName , "%s:%d" , EFF_KEY[ eEffKey_effAnim_flag ][e_key] , nIndex );
	sprintf_s( szLifeName , "%s:%d" , EFF_KEY[ eEffKey_effAnim_life ][e_key] , nIndex );
	sprintf_s( szLoopName , "%s:%d" , EFF_KEY[ eEffKey_effAnim_loopOpt ][e_key] , nIndex );
										   ...
										   , const stAnimBaseInfo& stAnimBaseInfo )
{
	...
	for( i=0; i<nSize; ++i )
	{	
		sprintf_s( szAnimColr , "%s:%d" , EFF_KEY[ eEffKey_effAnimColr ][e_key] , i );
										   ...
										   , const stAnimBaseInfo& stAnimBaseInfo )
{
	...
	for( i=0; i<nSize; ++i )
	{
		...
		sprintf_s( szAnimTuTvRect , "%s:%d" , EFF_KEY[ eEffKey_effAnimTuTvRect ][e_key] , i );
										   ...
										   , const stAnimBaseInfo& stAnimBaseInfo )
{
	...
	RwReal	fAccel	=	(RwReal)m_cIniManager.GetValueF(szSectionName , (CHAR*)EFF_KEY[eEffKey_effAnimMissile_accel][e_key]			);
	RwReal	fSpeed	=	(RwReal)m_cIniManager.GetValueF(szSectionName , (CHAR*)EFF_KEY[eEffKey_effAnimMissile_initSped][e_key]		);
	RwReal	fRotate =	(RwReal)m_cIniManager.GetValueF(szSectionName , (CHAR*)EFF_KEY[eEffKey_effAnimMissile_rotate][e_key]		);
	RwReal	fRadius =	(RwReal)m_cIniManager.GetValueF(szSectionName , (CHAR*)EFF_KEY[eEffKey_effAnimMissile_radius][e_key]		);
	RwReal	fZigzag	=	(RwReal)m_cIniManager.GetValueF(szSectionName , (CHAR*)EFF_KEY[eEffKey_effAnimMissile_zigzaglength][e_key]	);
	RwReal	fMinSpd	=	(RwReal)m_cIniManager.GetValueF(szSectionName , (CHAR*)EFF_KEY[eEffKey_effAnimMissile_minspeed][e_key]		);
	RwReal	fMaxSpd	=	(RwReal)m_cIniManager.GetValueF(szSectionName , (CHAR*)EFF_KEY[eEffKey_effAnimMissile_maxspeed][e_key]		);
										   ...
										   , const stAnimBaseInfo& stAnimBaseInfo )
{
	...
	for( i=0; i<nSize; ++i )
	{
		sprintf_s( szLinear_Pos , "%s:%d" , EFF_KEY[eEffKey_effAnimLinear_pos][e_key] , i );
										 ...
										 , const stAnimBaseInfo& stAnimBaseInfo )
{
	...
	for( i=0; i<nSize; ++i )
	{
		...
		sprintf_s( szAnimRecv , "%s:%d" , EFF_KEY[ eEffKey_effAnimRev_rev ][e_key] , i );
										 ...
										 , const stAnimBaseInfo& stAnimBaseInfo )
{
	...
	for( i=0; i<nSize; ++i )
	{
		...
		sprintf_s( szAnimRot , "%s:%d" , EFF_KEY[ eEffKey_effAnimRot_rot ][e_key]  , i );
										   ...
										   , const stAnimBaseInfo& stAnimBaseInfo )
{
	...
	for( i=0; i<nSize; ++i )
	{
		sprintf_s( szAnimScale , "%s:%d" , EFF_KEY[ eEffKey_effAnimScale_scale ][e_key] , i );
