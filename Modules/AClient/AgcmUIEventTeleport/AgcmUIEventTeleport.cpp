...
BOOL AgcmUIEventTeleport::CBSelectTeleportPoint(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	// 악당포인트 3단계 이상이면 포탈을 사용할 수 없다.
	if( !pThis->CheckIsUsablePortalByMurdererPoint() )
	{
		AgcmUIManager2* pcmUIManager = ( AgcmUIManager2* )g_pEngine->GetModule( "AgcmUIManager2" );
		if( !pcmUIManager ) return FALSE;

		char* pMsg = pcmUIManager->GetUIMessage( "CharacterPenalty_CantUsePortal" );
...
BOOL AgcmUIEventTeleport::CBCannotUseForTransforming(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	AgcChatManager::OnAddSystemMessage(pThis->m_pcsAgcmUIManager2->GetUIMessage(UI_MESSAGE_ID_TELEPORT_TRANSFORM_CHAR));
...
BOOL AgcmUIEventTeleport::CBCannotUseForCombatMode(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	AgcChatManager::OnAddSystemMessage(pThis->m_pcsAgcmUIManager2->GetUIMessage(UI_MESSAGE_ID_TELEPORT_COMBAT_MODE));
...
BOOL AgcmUIEventTeleport::CheckIsUsablePortalByMurdererPoint( void )
{
	...
	int nMurdererLeLvel = ppmCharacter->GetMurdererLevel( ppdSelfCharacter );
	switch( nMurdererLeLvel )
	{
	case AGPMCHAR_MURDERER_LEVEL1_POINT :		return TRUE;		break;
	case AGPMCHAR_MURDERER_LEVEL2_POINT :		return TRUE;		break;
	case AGPMCHAR_MURDERER_LEVEL3_POINT :		return FALSE;		break;
	}
