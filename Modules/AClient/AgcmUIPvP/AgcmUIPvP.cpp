...
#ifdef _AREA_GLOBAL_
				pThis->m_pcsPvPFreeGridItem->SetTooltip(ClientStr().GetStr(STI_ALARM_FREEAREA));
#else
...
#ifdef _AREA_GLOBAL_
				pThis->m_pcsPvPDangerGridItem->SetTooltip(ClientStr().GetStr(STI_ALARM_DANGERAREA));
#else
...
BOOL AgcmUIPvP::CBSystemMessage(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	// 타겟이 무적인 상태라는 메시지는 한번만 출력
	if(pstSystemMessage->m_lCode == AGPMPVP_SYSTEM_CODE_TARGET_INVINCIBLE
		&& pThis->m_lPrevCode == AGPMPVP_SYSTEM_CODE_TARGET_INVINCIBLE)
		return FALSE;

	// 이전 값 저장
	m_lPrevCode = pstSystemMessage->m_lCode;
	...
	case AGPMPVP_SYSTEM_CODE_CHARISMA_UP:
		{
			...
			if (pszFormat)
			{
				...
				SystemMessage.ProcessSystemMessage(szBuffer, 0xffffcc00);
			}
		}
		break;
	...
	case AGPMPVP_SYSTEM_CODE_TARGET_INVINCIBLE:
		{
			...
			AgpmShrineBattle* ppmShrineBattle = ( AgpmShrineBattle* )g_pEngine->GetModule( "AgpmShrineBattle" );
			...
			if( !ppmBattleGround || !ppmShrineBattle || !pcmCharacter || !ppmCharacter ) return FALSE;
			...
			memset( szMessage, 0, sizeof( char ) * 255 );
			sprintf_s( szMessage, sizeof( char ) * 255, pFormat, (ppmBattleGround->IsInBattleGround( ppdTarget )||ppmShrineBattle->IsInShrineRegionByIndex(ppdTarget)) ? "****" : ppdTarget->m_szID );
	...
	case AGPMPVP_SYSTEM_CODE_MURDER_PENALTY_CHARISMA_POINT:
		INT32 lPoint = pstSystemMessage->m_alData[0];
		INT32 lMurdererLevel = pstSystemMessage->m_alData[1];
		...
		CHAR szBuffer[256];
		CHAR *pszFormat = pThis->m_pcsAgcmUIManager2->GetUIMessage( UI_MESSAGE_ID_CHARISMA_LOSE_POINT );
		if (pszFormat)
		{
			sprintf(szBuffer, pszFormat, lMurdererLevel, lPoint);
			SystemMessage.ProcessSystemMessage(szBuffer, 0xffffcc00);
		}
		break;
	}
