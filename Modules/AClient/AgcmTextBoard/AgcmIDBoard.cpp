...
: AgcmBaseBoard( eType )
{
	...
	m_nTimeLastTick = 0;
	m_nTimeTickDuration = 1000;		// 1초에 한번씩
	m_bIsDrawName = TRUE;
}
...
void AgcmIDBoard::_DrawName( RwReal fPosX, RwReal fPosY, INT nAlpha , BOOL bDraw )
{
	...
	if( pSelfCharacter != pCharacter )
	{
		...
		else if( ms_pMng->GetAgpmShrineBattle()->IsInShrineRegionByIndex( pSelfCharacter ) && ms_pMng->GetAgpmCharacter()->IsPC(pCharacter) )
		{
			INT32	nType		= ms_pMng->GetAgpmFactors()->GetRace( &pCharacter->m_csFactor );
			INT32	nSelfType	= ms_pMng->GetAgpmFactors()->GetRace( &pSelfCharacter->m_csFactor );

			// 쉬라인 지역에 있으면 다른 종족의 이름을 표시하지 않는다.
			// 그리고 색깔을 부여한다.
			if(nType != nSelfType)
			{
				switch(nType)
				...
				case AURACE_TYPE_HUMAN:
					dwColor = ms_pMng->GetSHNameColor(SHRINE_HUMAN_NAME_COLOR);
					break;
				case AURACE_TYPE_ORC:
					dwColor = ms_pMng->GetSHNameColor(SHRINE_ORC_NAME_COLOR);
					break;
				case AURACE_TYPE_MOONELF:
					dwColor = ms_pMng->GetSHNameColor(SHRINE_MOONELF_NAME_COLOR);
					break;
				case AURACE_TYPE_DRAGONSCION:
					dwColor = ms_pMng->GetSHNameColor(SHRINE_DRAGONSCION_NAME_COLOR);
					break;
				default:
					break;
				}
	...
	if(FALSE == ms_pMng->GetAgpmShrineBattle()->IsInShrineRegionByIndex( pSelfCharacter ))
	{
...
void AgcmIDBoard::_DrawTitleName( RwReal fPosX, RwReal fPosY, INT nAlpha )
{
	...
	if( pSelfCharacter != pCharacter )
	{
		// 배틀 그라운드에서 종족별로 이름 색깔을 바꿔준다
		if( ( ms_pMng->GetAgpmBattleGround()->IsInBattleGround( pSelfCharacter ) && ms_pMng->GetAgpmCharacter()->IsPC( pCharacter )) ||
			( ms_pMng->GetAgpmShrineBattle()->IsInShrineRegionByIndex( pSelfCharacter ) && ms_pMng->GetAgpmCharacter()->IsPC(pCharacter) ) )
		{
			return;
		}
	}
...
void AgcmIDBoard::_DrawNameEx( RwReal fPosX, RwReal fPosY, INT nAlpha )
{
	...
	if( ms_pMng->GetAgpmBattleGround()->IsInBattleGround( pcsSelfCharacter ) || ms_pMng->GetAgpmShrineBattle()->IsInShrineRegionByIndex( pcsSelfCharacter ))
		return;
...
VOID AgcmIDBoard::_DrawGuildCompetition( VOID )
{
	...
	if( ppmBattleGround->IsInBattleGround( ( AgpdCharacter* )GetApBase() ) || ms_pMng->GetAgpmShrineBattle()->IsInShrineRegionByIndex( (AgpdCharacter*)GetApBase() ) ) return;
...
void AgcmIDBoard::_DrawGuild( RwReal fPosX, RwReal fPosY, INT nAlpha )
{
	...
	if( ms_pMng->GetAgpmBattleGround()->IsInBattleGround( pcsSelfCharacter ) || ms_pMng->GetAgpmShrineBattle()->IsInShrineRegionByIndex( pcsSelfCharacter ) )	return;
