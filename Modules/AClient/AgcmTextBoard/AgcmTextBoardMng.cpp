...
//---------------------------- CPostFX -----------------------------
CPostFX::CPostFX() : m_pcsAgcmPostFX(NULL),m_pTechnique(NULL)
{
	m_fStart		= 0;
	m_bCircleFX		= false;
	m_bPostFXOn		= FALSE;
...
void	CPostFX::Update()
{
	...
	//{
	//	m_pcsAgcmPostFX->SetRadius( m_pTechnique , fElapsed * 1.5f );
	//}
	...
	//		if( m_pTechnique )
	//		{
	//			m_pcsAgcmPostFX->DeleteFX( m_pTechnique );
	//			m_pTechnique = NULL;
	//			m_pcsAgcmPostFX->PostFX_ON();
	//		}
	//	}
	//	else
	//		m_pcsAgcmPostFX->PostFX_OFF(true);
...
{
	//m_pTechnique	=	m_pcsAgcmPostFX->CreateFX( POSTFX_NAME_GENERATE , E_PIPELINE_WAVE );
	//m_pcsAgcmPostFX->PostFX_ON();
	...
	//	m_pTechnique->SetStartTime( FLOAT( clock() ) );
	//	m_pTechnique->SetSpeedScale( 1.5f );
	//	m_pTechnique->SetPlayTime( 1000 );
	//	m_pTechnique->SetUpdate( TRUE );
}

VOID	CPostFX::SetCenter( FLOAT fX , FLOAT fY )
{
	//if( m_pTechnique )
	//{
	//	m_pcsAgcmPostFX->SetCenter( m_pTechnique , fX , fY );
	//}
}

AgcmTextBoardMng::AgcmTextBoardMng(void)
: m_pcsAgcmUIManager2(NULL)
, m_pcsAgpmShrineBattle(NULL)
...
sBoardDataPtr AgcmTextBoardMng::CreateBoard( CHAR* szGameID , sBoardAttrPtr pAttr )
{
	if( !pAttr )
		return NULL;

	AgpdCharacter*	pdCharacter	=	m_pcCmCharacter->GetAgpmCharacter()->GetCharacter( szGameID );
...
BOOL AgcmTextBoardMng::OnInit()
{
	...
	AgcmResourceLoader * rl = (AgcmResourceLoader *)GetModule( "AgcmResourceLoader" );

	if( rl )
		rl->SetTexturePath( "Texture\\UI\\Base\\" );
	...
	m_ShrineNameColor[ SHRINE_HUMAN_NAME_COLOR ] = ARGB_TO_ABGR(pLoader->GetColor("SH_HUMAN_NAME"));
	m_ShrineNameColor[ SHRINE_ORC_NAME_COLOR ] = ARGB_TO_ABGR(pLoader->GetColor("SH_ORC_NAME"));
	m_ShrineNameColor[ SHRINE_MOONELF_NAME_COLOR ] = ARGB_TO_ABGR(pLoader->GetColor("SH_MOONELF_NAME"));
	m_ShrineNameColor[ SHRINE_DRAGONSCION_NAME_COLOR ] = ARGB_TO_ABGR(pLoader->GetColor("SH_DRAGONSCION_NAME"));

	m_pcsAgpmShrineBattle = static_cast<AgpmShrineBattle*>(GetModule("AgpmShrineBattle"));
	m_pcsAgcmResourceLoader = static_cast<AgcmResourceLoader*>(GetModule("AgcmResourceLoader"));
...
BOOL AgcmTextBoardMng::OnDestroy()
{
	...
	for( int i = 0 ; i < TB_IMAGE_COUNT      ; ++i )
		RW_TEXTURE_DESTROY( m_ImgBalloon[i] );

	for( int i = 0 ; i < TB_IMAGE_TAIL_COUNT ; ++i )
		RW_TEXTURE_DESTROY( m_ImgBalloonTail[i] );

	for( int i = 0 ; i < TB_MAX_TAG          ; ++i )
		RW_TEXTURE_DESTROY( m_ImgIDTag[i] );

	for( int i = 0 ; i < IMOTICON_COUNT-530  ; ++i )
		RW_TEXTURE_DESTROY( m_ImgImoticon[i] );

	RW_TEXTURE_DESTROY( m_ImgDamage );
	RW_TEXTURE_DESTROY( m_ImgGOMark );
	RW_TEXTURE_DESTROY( m_ImgVolunteer );
...
sBoardDataPtr AgcmTextBoardMng::EnableGuild( ApBase* pBase , const char* szString , DWORD dwGuildColor )
{
	...
	if( pBoardData && pBoardData->pHPBar && pBoardData->pHPBar->GetBoardType() == AGCM_BOARD_HPMPBAR )
	{
		...
		ZeroMemory( TmpString , len+10 );
		sprintf_s( TmpString , "[%s]" , szString );
...
// VOID AgcmTextBoardMng::DrawGuildImage( AgcmIDBoard* pIDBoard , RwReal fPosX, RwReal fPosY, RwTexture* pTexture, DWORD dwColor /*= 0xffffffff*/, float fScale /*= 1.f*/, int nJoinMax /*= 0*/, int nJoinIndex /*= 0*/, BOOL bIsJoinSiegeWar /*= FALSE */ )
// {
// 	if( !pIDBoard|| !pTexture )	return;
// 
// 	RwV2d	vOffset;
...
// 
// 	//vOffset.y = pIDBoard->GetOffset().y - ( vImageSize.y * .5f )/* - nCharismaYOffset*/;
// 	vOffset.y = fPosY - ( vImageSize.y * .5f );
// 	if( nJoinMax )		CalculateJointGuildMarkPositionOffset( &vOffset, vImageSize.x * fScale, nJoinMax, nJoinIndex );
// 
// 	g_pEngine->DrawIm2DInWorld( pTexture, &pIDBoard->GetCameraPos(), &pIDBoard->GetScreenPos(),pIDBoard->GetRecipz() , &vOffset, vImageSize.x * fScale, vImageSize.y * fScale, 0.0f, 0.0f, 1.0f, 1.0f, dwColor );
// }
...
BOOL	AgcmTextBoardMng::SetTexture( INT32 nType, INT32 nIndex, RwTexture* pTexture )
{
	...
	case 9:										m_ImgVolunteer	=	pTexture;			break;
	}
...
{
	if( uiState & AGPDCHAR_SPECIAL_STATUS_VOLUNTEER )
		return	GetVolunteerTexture();
	else if( uiState & AGPDCHAR_SPECIAL_STATUS_HUMAN_CASTLE_OWNER )
