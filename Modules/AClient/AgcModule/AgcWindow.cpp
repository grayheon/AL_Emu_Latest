...
AgcWindow::AgcWindow()
{
	...
	m_pfnPreDocking		=	NULL;
	m_pPreClass			=	NULL;
	m_pPreData1			=	NULL;
	m_pPreData2			=	NULL;

	ZeroMemory( &m_rectClipping , sizeof(RECT) );

	//m_bPropertyUpdate		= FALSE;
	//m_bReSize				= FALSE;
	//m_eMouseState			= MS_None;
	//m_v2dReSizeStartPoint.x = 0;
	//m_v2dReSizeStartPoint.y = 0;

	//m_lInitW	= w;
	//m_lInitH	= h;

	//m_bSaveClipping = FALSE;
	//m_bSaveSizeEx = FALSE;
	m_bSmoothOpening = FALSE;
	m_listMessage.clear();
}
...
void	AgcWindow::MessagePush( INT32 id, PVOID param1, PVOID param2 )
{
...
void	AgcWindow::MessageUpdate()
{
	if( m_listMessage.size() <= 0 ) return;

	for( MessageListItr Itr = m_listMessage.begin(); Itr != m_listMessage.end(); ++Itr )
		WindowControlMessageProc( (*Itr).message, (*Itr).pParam1, (*Itr).pParam2 );
...
RsEventStatus	AgcWindow::UserInputMessageProc		( RsEvent event , PVOID param ) // 마우스 메시지 받음;
{
	...
	case rsLEFTBUTTONUP:
		{			
			...
			{
				//4747g_pEngine->m_pFocusedWindow	=	this	;
				return rsEVENTPROCESSED		;
			}
			else
				return rsEVENTNOTPROCESSED	;
...
BOOL AgcWindow::AllDeleteChild( BOOL bClose /* = TRUE  */, BOOL bDeleteMemory /* = FALSE */ )
{
	AgcWindowNode*	cur_node = m_listChild.head;
	while( cur_node )
	{
		if( cur_node->pWindow )
		{
			g_pEngine->PopMonopolyWindow(cur_node->pWindow);

			if( cur_node->pWindow->m_bOpened && bClose )
			{
				cur_node->pWindow->WindowControlMessageProc( AgcWindow::MESSAGE_CLOSE );
			}
			else
			{
			...
			cur_node->pWindow->m_bDeleteThis	= bDeleteMemory	;
			cur_node->pWindow->m_bOpened		= FALSE	;
		}
...
void AgcWindow::MoveWindow		( INT32 x , INT32 y , INT32 w , INT32 h )
{
	...
	// Docking전 이벤트
	if( m_pfnPreDocking )
		m_pfnPreDocking( m_pPreClass , m_pPreData1 , m_pPreData2 );
...
void	AgcWindow::WindowListUpdate()
{
	...
	while(cur_node)
	{
		...
		if(cur_node->pWindow && cur_node->pWindow->m_Property.bVisible && cur_node->pWindow->m_bOpened )
...
BOOL AgcWindow::HasChild( AgcWindow* pChild )
{
	AgcWindowNode*	pNode	=	m_listChild.head;
	while( pNode )
	{
		if( pNode->pWindow == pChild )
			return TRUE;
...
AgcWindow *	AgcWindow::WindowFromPoint		( INT32 x , INT32 y, BOOL bCheckUseInput )
{
	...
	while(cur_node)
	{
		if( cur_node->pWindow && cur_node->pWindow->HitTest( x , y ) )
		{
			// Reculsive Call ..
...
VOID	AgcWindow::SetDockingCallback( PVOID pClass , PVOID pData1 , PVOID pData2 , fnDockingCallback pCallback )
{
	m_pClass		=	pClass;
	m_pData1		=	pData1;
	m_pData2		=	pData2;

	m_pfnDocking	=	pCallback;
}

VOID	AgcWindow::SetDockingPreCallback( PVOID pClass , PVOID pData1 , PVOID pData2 , fnDockingCallback pCallback )
{
	m_pPreClass		=	pClass;
	m_pPreData1		=	pData1;
	m_pPreData2		=	pData2;

	m_pfnPreDocking	=	pCallback;
}


// 최상위 부모부터 클리핑 영역을 만드는 함수
VOID AgcWindow::UpdateClippingRectParent( VOID )
{
	if( pParent )
		pParent->UpdateClippingRectParent();

	UpdateClippingRect();

}

// 이거 잘써서 이미지 클리핑 잘 해보자 ㅠ,.ㅠ
VOID AgcWindow::UpdateClippingRect( VOID )
{
	...
	m_rectClipping.left		=	nClipLeft	=	nX;
	m_rectClipping.right	=	nClipRight	=	nX+w;
	m_rectClipping.top		=	nClipTop	=	nY;
	m_rectClipping.bottom	=	nClipBottom	=	nY+h;

	// 최상위다
	if( !pParent )
		return;

	RECT&	rParentRect	=	pParent->GetClippingRect();

	// left가 벗어났다
	if( rParentRect.left > nClipLeft )
	{
		m_rectClipping.left	=	nClipLeft + (rParentRect.left-nClipLeft);
	}

	// right가 벗어났다
	if( rParentRect.right < nClipRight )
	{
		m_rectClipping.right	=	nClipRight - (nClipRight-rParentRect.right);
	}

	// top 벗어났다
	if( rParentRect.top > nClipTop )
	{
		m_rectClipping.top	=	nClipTop + (rParentRect.top-nClipTop);
	}

	// bottom 벗어났다
	if( rParentRect.bottom < nClipBottom )
	{
		m_rectClipping.bottom	=	nClipBottom -(nClipBottom-rParentRect.bottom);
	}
...
INT32		AgcModalWindow::DoModal		( AgcWindow * pParentWindow	)
{
	...
	if( !m_lModalCount )
	{
		if( !pParentWindow->AddChild( this, 0, FALSE ) )
			return 0;
