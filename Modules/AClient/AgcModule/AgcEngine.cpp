...
AgcEngine::AgcEngine():m_uSlowIdleIndicator( 0 )
{
	...
	m_eRatioType			= RESOLUTION_RATIO_0403;
...
#ifndef _RW_FOR_MFC_	
	//멀티 샘플링 값을 6으로 고정 1.0버전에선 켜고(6)끄고(1)를 할 수 있었지만.. 1.5에선 무조건 켠다
	if( !RsRwInitialize( param, 6 ) )
	{
...
{
	if( !m_pCamera || !m_pCamera->frameBuffer || !m_pCamera->zBuffer ) return;
...

	{
		...
		RwCameraClear(m_pCamera, &m_rgbBackgroundColor, rwCAMERACLEARZ);
...
//	}
	...
	//render fade -- fade 처리는 맨 마지막에 한다 by psl
	ApModule* pModule = GetModule( "AgcmUIManager2" );
	if( pModule )
		((AgcmUIManager2*)pModule)->OnRender();


	// Display Camera Raster
	{
		RsCameraShowRaster( m_pCamera );	// Flip the Raster! - -+
		//OnDrawScreen( m_pCamera );
...
{
#ifndef USE_MFC
	if( !g_pEngine || !m_clCamera || !m_pCamera || !pRect )
		return TRUE;
#endif
	LockFrame();
...
RwCamera *AgcEngine::CreateCamera			( RpWorld *world )
{
	...
	for( int i=0; i<4; ++i )
	{
		...
		RwIm2DVertexSetScreenZ( &m_vBoxFan2[i] , NearZ );
		RwIm2DVertexSetRecipCameraZ( &m_vBoxFan2[i] , recipCameraZ );
	}
...
RsEventStatus	AgcEngine::KeyboardHandler	( RsEvent event , void *param )
{
	...
	*/
	...
	ApModuleList *		pList = g_pEngine->m_listModule;
	while(pList)
	{
		if( pList->pModule->GetModuleType() == APMODULE_TYPE_CLIENT )
			((AgcModule*)(pList->pModule))->UserInputMessageProc(event , param);
...
INT32	AgcEngine::EditMessageDialog( AgcWindow* pWindow, char* pszMessage, CHAR* pEditString , BOOL bOnlyNumber )
{
	if ( NULL == pWindow ) 
	{
		if ( NULL == m_pCurrentFullUIModule ) return -1;
	...
	if( NULL != m_pEditOkDialog )
	{
		...
		pEditOKDialog->m_clEdit.m_bNumberOnly	=	bOnlyNumber;
...
INT32	AgcEngine::EditOKCancelMessageDialog( AgcWindow* pWindow , CHAR* pszMessage , CHAR* pEditString , BOOL bOnlyNumber /* = FALSE */ )
{
	if ( NULL == pWindow ) 
	{
		if ( NULL == m_pCurrentFullUIModule ) return -1;
	...
	if( NULL != m_pEditOKCancelDialog )
	{
		INT32 lReturn = -1;
		m_pEditOKCancelDialog->SetMessage( pszMessage );
		m_pEditOKCancelDialog->ShowWindow( TRUE );

		AcUIEditOKDialog* pEditOKCancelDialog = ( AcUIEditOKDialog* )m_pEditOKCancelDialog;
		pEditOKCancelDialog->m_clEdit.ShowWindow( TRUE );

		pEditOKCancelDialog->m_clEdit.m_bNumberOnly	=	bOnlyNumber;
		pEditOKCancelDialog->m_clEdit.m_ulTextMaxLength	=	32;

		lReturn = pEditOKCancelDialog->DoModal( pWindow );
		const CHAR* pEditBoxText = pEditOKCancelDialog->m_clEdit.GetText();
		if( pEditBoxText && strlen( pEditBoxText ) > 0 )
		{
			strcpy_s( pEditString, sizeof( char ) * 256, pEditOKCancelDialog->m_clEdit.GetText() );
			pEditOKCancelDialog->m_clEdit.ClearText();
		}
...
INT32	AgcEngine::EditMessageDialog2( AgcWindow* pWindow, char* pszMessage, CHAR* pEditString , BOOL bOnlyNumber )
{
	if ( NULL == pWindow ) 
	{
		if ( NULL == m_pCurrentFullUIModule ) return -1;
...
VOID	AgcEngine::FindTextureDirectory( CONST CHAR*	szFindDirectory , string& strTexturePath )
{
	...
	sprintf_s( szDirectory , MAX_PATH , "%s\\*.*" , szFindDirectory );

	FileFind.FindFile( szDirectory , FALSE );
	...
	{	
		...
		if( FileFind.IsDirectory() && !FileFind.IsHidden() )
		{
			...
			FileFind.GetFileName( szPath );

			if( szPath[0] == '.' )
				continue;

			sprintf_s( szCurrentDir , MAX_PATH , "%s\\%s" , szFindDirectory , szPath );
			strTexturePath	=	strTexturePath	+ ";" + szCurrentDir + "\\";

			FindTextureDirectory( szCurrentDir , strTexturePath );
		}
...
//by psl
BOOL AgcEngine::Draw2DPanel( INT32 nX, INT32 nY, INT32 nWidth, INT32 nHeight, DWORD dwColor , LPDIRECT3DTEXTURE9 pTex)
{
	/*static My2DVertex pVertices[] = 
	{
		{ (float)nX - 0.5f,					(float)nY - 0.5f,					0.1f, 0.f, 0.f, dwColor },
		{ (float)(nX + nWidth) - 0.5f,		(float)nY - 0.5f,					0.1f, 0.f, 0.f, dwColor  },
		{ (float)(nX + nWidth) - 0.5f,		(float)(nY + nHeight) - 0.5f,  0.1f, 0.f, 0.f, dwColor  },
		{ (float)nX - 0.5f,					(float)(nY + nHeight) - 0.5f,  0.1f, 0.f, 0.f, dwColor  },
	};*/
...
bool	AgcEngine::DrawIm2D(RwTexture* pTex,float x,float y,float w,float h,
							...
							DWORD color,UINT8	alpha , RECT* pClipping )
{
	PROFILE("AgcEngine::DrawIm2D");

	DWORD	mycolor = ( color & 0x00ffffff ) | (alpha << 24);

	// 클리핑 영역이 들어왔다면 클리핑 해서 찍어주자 
	if( pClipping )
	{
		if( !pClipping->bottom && !pClipping->left && !pClipping->right && !pClipping->top )
			return false;

		INT	nTemp		=	0;
		INT	nTempLeft	=	(INT)x;
		INT	nTempRight	=	(INT)(x+w);
		INT	nTempTop	=	(INT)y;
		INT	nTempBottom	=	(INT)(y+h);

		if( pClipping->left	> nTempLeft )
		{
			nTemp		=	pClipping->left-nTempLeft;
			st_u		+=	( (FLOAT)nTemp / (FLOAT)w );
			nTempLeft	=	nTempLeft + nTemp;
		}

		if( pClipping->right < nTempRight )
		{
			nTemp		=	nTempRight - pClipping->right;
			off_u		-=	( (FLOAT)nTemp / (FLOAT)w );
			nTempRight	=	nTempRight - nTemp;
	
		}

		if( pClipping->top > nTempTop )
		{
			nTemp		=	pClipping->top - nTempTop;
			st_v		+=	( (FLOAT)nTemp / (FLOAT)h ) ;
			nTempTop	=	nTempTop + nTemp;
		}

		if( pClipping->bottom < nTempBottom )
		{
			nTemp		=	nTempBottom - pClipping->bottom;
			off_v		-=	( (FLOAT)nTemp / (FLOAT)h );
			nTempBottom	=	nTempBottom - nTemp;
		}

		x	=	(FLOAT)nTempLeft;
		y	=	(FLOAT)nTempTop;
		w	=	(FLOAT)(nTempRight	- nTempLeft);
		h	=	(FLOAT)(nTempBottom	- nTempTop);
								...
								DWORD color,UINT8	alpha)
{
	...
	recipZ = floor(recipZ * 10000)/10000;
...
BOOL	AgcEngine::SetProjection			( FLOAT fRate			)
{
	...
	#pragma region 예전_비율_계산에_문제가_있어서_수정함
	/*
...
#pragma endregion
	...
	ViewWindow.x = (RwReal)videoMode.width;
	ViewWindow.y = (RwReal)videoMode.height;

	RwRaster* pRas = RwCameraGetRaster(m_pCamera);

	if( pRas )
	{
		ViewWindow.x = ( FLOAT ) RwRasterGetWidth( pRas );
		ViewWindow.y = ( FLOAT ) RwRasterGetHeight( pRas );
	}

	ViewWindow.y = fRate * ViewWindow.y / ViewWindow.x;
	ViewWindow.x = fRate;
					...
					RECT* pClipping									)
{
	...
	if( pClipping )
	{
		...
		INT	nTemp		=	0;
		INT	nTempLeft	=	(INT)nX;
		INT	nTempRight	=	(INT)(nX+nWidth);
		INT	nTempTop	=	(INT)nY;
		INT	nTempBottom	=	(INT)(nY+nHeight);

		if( pClipping->left	> nTempLeft )
		{
			nTemp		=	pClipping->left-nTempLeft;
			fUStart		+=	( (FLOAT)nTemp / (FLOAT)nWidth );
			nTempLeft	=	nTempLeft + nTemp;
		}
		...
		{
			nTemp		=	nTempRight - pClipping->right;
			fUEnd		-=	( (FLOAT)nTemp / (FLOAT)nWidth );
			nTempRight	=	nTempRight - nTemp;
		...
		{
			nTemp		=	pClipping->top - nTempTop;
			fVStart		+=	( (FLOAT)nTemp / (FLOAT)nHeight ) ;
			nTempTop	=	nTempTop + nTemp;
		}
		...
		{
			nTemp		=	nTempBottom - pClipping->bottom;
			fVEnd		-=	( (FLOAT)nTemp / (FLOAT)nHeight );
			nTempBottom	=	nTempBottom - nTemp;
		}

		nX		=	nTempLeft;
		nY		=	nTempTop;
		nWidth	=	(nTempRight	- nTempLeft);
		nHeight	=	(nTempBottom	- nTempTop);
	...
	RwD3D9GetTextureStageState( 0 , D3DTSS_COLOROP		, &unColorOP	);
	RwD3D9GetTextureStageState( 0 , D3DTSS_COLORARG1	, &unColorArg1	);
	RwD3D9GetTextureStageState( 0 , D3DTSS_COLORARG2	, &unColorArg2	);

	RwD3D9GetTextureStageState( 0 , D3DTSS_ALPHAOP		, &unAlphaOP	);
	RwD3D9GetTextureStageState( 0 , D3DTSS_ALPHAARG1	, &unAlphaArg1	);
	RwD3D9GetTextureStageState( 0 , D3DTSS_ALPHAARG2	, &unAlphaArg2	);
	...
	RwD3D9SetTextureStageState( 0 , D3DTSS_COLOROP		, unColorOP		);
	RwD3D9SetTextureStageState( 0 , D3DTSS_COLORARG1	, unColorArg1	);
	RwD3D9SetTextureStageState( 0 , D3DTSS_COLORARG2	, unColorArg2	);

	RwD3D9SetTextureStageState( 0 , D3DTSS_ALPHAOP		, unAlphaOP		);
	RwD3D9SetTextureStageState( 0 , D3DTSS_ALPHAARG1	, unAlphaArg1	);
	RwD3D9SetTextureStageState( 0 , D3DTSS_ALPHAARG2	, unAlphaArg2	);
	...
	UINT8 uAlpha								)
{
	...
	INT32	nMaskImageWidth	=	RwRasterGetWidth( RwTextureGetRaster( pMask) );
	INT32	nMaskImageHeight=	RwRasterGetHeight( RwTextureGetRaster( pMask) );
	...
	///////////////////////////////////////////////////////////////---여기부터FLOAT	fUStart2= ( FLOAT ) nUStart;
	...
	DWORD	mycolor = ( uColor & 0x00ffffff ) | ( uAlpha << 24 );
	m_vBoxFan2[0].dwColor = mycolor;
	m_vBoxFan2[1].dwColor = mycolor;
	m_vBoxFan2[2].dwColor = mycolor;
	m_vBoxFan2[3].dwColor = mycolor;

#define fD3D_PIXEL_CORRECTION_OFFSET	0.5f

	m_vBoxFan2[0].x = ( float ) ( nX + nWidth - fD3D_PIXEL_CORRECTION_OFFSET	);
	m_vBoxFan2[0].y = ( float ) ( nY + nHeight - fD3D_PIXEL_CORRECTION_OFFSET ); 
	m_vBoxFan2[0].u1 = fUEnd;
	m_vBoxFan2[0].v1 = fVEnd;
	m_vBoxFan2[0].u2 = fMaskUEnd;
	m_vBoxFan2[0].v2 = fMaskVEnd;

	m_vBoxFan2[1].x = ( float ) ( nX + nWidth - fD3D_PIXEL_CORRECTION_OFFSET );
	m_vBoxFan2[1].y = ( float ) ( nY	- fD3D_PIXEL_CORRECTION_OFFSET );
	m_vBoxFan2[1].u1 = fUEnd;
	m_vBoxFan2[1].v1 = fVStart;	
	m_vBoxFan2[1].u2 = fMaskUEnd;
	m_vBoxFan2[1].v2 = fMaskVStart;

	m_vBoxFan2[2].x = ( float ) ( nX	- fD3D_PIXEL_CORRECTION_OFFSET );
	m_vBoxFan2[2].y = ( float ) ( nY + nHeight - fD3D_PIXEL_CORRECTION_OFFSET ); 
	m_vBoxFan2[2].u1 = fUStart;
	m_vBoxFan2[2].v1 = fVEnd;
	m_vBoxFan2[2].u2 = fMaskUStart;
	m_vBoxFan2[2].v2 = fMaskVEnd;

	m_vBoxFan2[3].x = ( float ) ( nX	- fD3D_PIXEL_CORRECTION_OFFSET );
	m_vBoxFan2[3].y = ( float ) ( nY - fD3D_PIXEL_CORRECTION_OFFSET );
	m_vBoxFan2[3].u1 = fUStart;
	m_vBoxFan2[3].v1 = fVStart;
	m_vBoxFan2[3].u2 = fMaskUStart;
	m_vBoxFan2[3].v2 = fMaskVStart;
	...
	RwD3D9SetFVF(VERTEX_FLAG_TEX2);
	RwD3D9SetRenderState( D3DRS_ALPHABLENDENABLE , TRUE );
	RwD3D9DrawPrimitiveUP( D3DPT_TRIANGLESTRIP , 2 , m_vBoxFan2 , SIZE_TEX2_VERTEX );
...
VOID	AgcEngine::InitRatio( VOID )
{

	if( (RsGlobal.maximumWidth / 16 * 10) == RsGlobal.maximumHeight )
	{
		m_eRatioType	=	RESOLUTION_RATIO_1610;
	}

	else if( (RsGlobal.maximumWidth / 16 * 9) == RsGlobal.maximumHeight )
	{
		m_eRatioType	=	RESOLUTION_RATIO_1609;
	}
