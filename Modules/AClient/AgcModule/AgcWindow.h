...
public:
	...
	BOOL	bVisibleTempForTool;

	BOOL	bClipping	;	// 윈도우를 벗어나는 부분을 그리지 않는다.
	BOOL	bResizable;

	AgcWindowProperty() : bUseInput(TRUE), bMovable(FALSE), bModal(FALSE), bTopmost( FALSE ), bVisible(TRUE), bClipping(FALSE), bResizable(FALSE), bVisibleTempForTool(TRUE)		{		}
};
...
typedef	VOID	(*fnDockingCallback)( PVOID pClass , PVOID pData1 , PVOID pData2 );
class AcUIBase;
...
public:
	...
	RECT			m_rectClipping;		//	Clipping 영역

	AgcWindowProperty	m_Property;	// 윈도우 속성 저장.

	BOOL				m_bIgnore;								//이벤트 무시1

	// Docking에서 쓰인다
	fnDockingCallback	m_pfnDocking;
	PVOID				m_pClass;
	PVOID				m_pData1;
	PVOID				m_pData2;

	// Docking 전 이벤트
	fnDockingCallback	m_pfnPreDocking;
	PVOID				m_pPreClass;
	PVOID				m_pPreData1;
	PVOID				m_pPreData2;
	...
	AgcWindowProperty	m_PropertyForUITool;	// UITool을 위한 속성 저장.

	BOOL												m_bSmoothOpening;
...
public:
	...
	BOOL			HasChild( AgcWindow* pChild );
	...
	VOID		UpdateClippingRect				( VOID );	//	상위 부모와의 클리핑 영역 체크
	VOID		UpdateClippingRectParent		( VOID );	//	모든 부모의 클리핑 영역 체크

	RECT&		GetClippingRect					( VOID )	{	return m_rectClipping;	}
	...
	BOOL		AllDeleteChild( BOOL bClose = TRUE , BOOL bDeleteMemory = FALSE );
	...
	void		MoveWindow( INT32 x , INT32 y , INT32 w = 0, INT32 h = 0 );
	BOOL		MovableOn( RsMouseStatus *ms );
	BOOL		MovableOff();

	VOID		SetDockingPreCallback	( PVOID pClass , PVOID pData1 , PVOID pData2 , fnDockingCallback pCallback );
	VOID		SetDockingCallback		( PVOID pClass , PVOID pData1 , PVOID pData2 , fnDockingCallback pCallback );
	...
	//minasang:
	void		InitWindowOpening(INT32 w , INT32 h);
