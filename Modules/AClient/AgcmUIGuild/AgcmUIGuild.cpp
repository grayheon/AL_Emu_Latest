...
BOOL AgcmUIGuild::OnAddModule()
{
	...
	if(!m_pcsAgpmGuild || !m_pcsAgpmPvP ||
		!m_pcsAgcmCharacter || !m_pcsAgcmGuild || !m_pcsAgcmUIManager2 || !m_pcsAgcmEventGuild || 
		!m_pcsAgcmFont
		)
		return FALSE;
...
BOOL AgcmUIGuild::CBUpdateNotice(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	if(pcsGuild->m_szNotice && strlen(pcsGuild->m_szNotice) > 0)
	{
		...
		sprintf(szMsg, "%s %s", pThis->m_pcsAgcmUIManager2->GetUIMessage("Chat_Notice_Header"), szTmp);
	}
	else
		sprintf(szMsg, "%s", pThis->m_pcsAgcmUIManager2->GetUIMessage("Chat_Notice_Header"));
	...
	AgcChatManager::OnAddChattingMessage( CHAT_MESSAGE_GUILD, &stChatData);
...
//	2005.10.20. By SungHoon 추가
		...
		case AGPMGUILD_SYSTEM_CODE_REJECTED_GUILD_LEAVE_IN_SIEGEWAR_AREA:
			szUIMessage = pThis->m_pcsAgcmUIManager2->GetUIMessage(AGCMUIGUILD_REJECTED_GUILD_LEAVE_IN_SIEGEWAR_AREA);
			break;
