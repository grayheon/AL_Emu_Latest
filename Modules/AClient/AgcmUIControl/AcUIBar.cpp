...
void AcUIBar::_CalculateNextRate( INT32 eBarType, AcUIBarAnimType eAnimType, INT32 lTotalPoint, INT32 lCurrentPoint )
{
	...
	if( fNextRate != m_fCurrRate )
	{
		...
		if( (eBarType & ACUIBAR_TYPE_SMOOTH) && !m_bInit )
		{
			...
			{
				// 만약 증가 애니메이션을 하고 있었다면 증가할 만큼 바로 채워버린다.
				if( m_eAnim  == ACUIBAR_ANIM_INCREASE )
				{
...
BOOL AcUIBar::_DrawTexture( RwTexture* pTexture, INT32 lPosX, INT32 lPosY, INT32 lWidth, INT32 lHeight, INT32 lUStart, INT32 lVStart, INT32 lUEnd, INT32 lVEnd, FLOAT fAlphaValue )
{
	...
	if( g_pEngine )
	{
		...
		if( lUStart > nImageWidth || lUEnd > nImageWidth )
		{
			lUStart	=	0;
			lUEnd	=	nImageWidth;
		}

		if( lVStart > nImageHeight || lVEnd > nImageHeight  )
		{
			lVStart	=	0;
			lVEnd	=	nImageHeight;
		}
		...
		//return g_pEngine->DrawIm2D( pTexture , lPosX , lPosY , lWidth , lHeight , lUStart , lVStart , lUEnd , lVEnd , uColor , ( UINT8 )( fAlphaValue * fAlphaRate ) , &m_rectClipping );
	}
...
BOOL AcUIBar::_DrawBody( RwTexture* pTexture, INT32 lAbsX, INT32 lAbsY )
{
	...
	case ACUIBAR_ANIM_NONE :
		{
			bResult	=	_DrawBodyNoAnim( pTexture, lAbsX, lAbsY );
		}
		break;

	default :
		{
			bResult	=	 _DrawBodySmooth( pTexture, lAbsX, lAbsY );
		}
		break;
	}

	m_ulLastTick = g_pEngine->GetClockCount();
