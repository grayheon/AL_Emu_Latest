...
void AcUIButton::AddButtonImage( char* filename, UINT8 cButtonMode )
{
	if( ACUIBUTTON_MODE_NORMAL <= cButtonMode && ACUIBUTTON_MODE_DISABLE >= cButtonMode )
	{
		if(!filename || strlen(filename) == 0)
			m_lButtonTextureID[ cButtonMode ] = ACUIBUTTON_INIT_INDEX;
		else
...
void AcUIButton::SetButtonSelectImage( char* pFileName )
{
	m_nButtonSelectTextureID = m_csTextureList.AddImage( pFileName );
	OnAddImage( m_csTextureList.GetImage_ID( m_nButtonSelectTextureID ) );
}
...
VOID AcUIButton::ClearImage( VOID )
{
...
void AcUIButton::_DrawSelectImage( void )
{
	RwTexture* pTextureSelect = m_csTextureList.GetImage_ID( m_nButtonSelectTextureID );
	if( !pTextureSelect || !pTextureSelect->raster ) 
		return;

	UINT8 nAlpha = ( UINT8 )( m_stRenderInfo.cRenderAlpha * ( m_pfAlpha ? *m_pfAlpha : 1 ) );

	g_pEngine->DrawIm2D( pTextureSelect, (float)m_lAbsolute_x, (float)m_lAbsolute_y, (float)w, (float)h, m_stRenderInfo.fStartU, m_stRenderInfo.fStartV,
						m_stRenderInfo.fEndU, m_stRenderInfo.fEndV,	m_stRenderInfo.lRenderColor, nAlpha , m_Property.bClipping ? &m_rectClipping : NULL );
}


VOID AcUIButton::_DrawBgColor( VOID )
{
	INT		nX	=	0;
	INT		nY	=	0;
	...
	g_pEngine->Draw2DPanel( nX+2 , nY+2 , this->w-2 , this->h-2 , m_dwBackgroundColor , NULL );
}