...
AgcmUIControl::AgcmUIControl( void )
{
	...
	m_pStateBattle					= NULL;
	m_pStateBattleEnd				= NULL;

	m_pStateBeWanted				= NULL;
...
#endif
	m_clEditOKCancelDialog.OnInit();
	m_clEditOKCancelDialog.ShowWindow( FALSE );
	g_pEngine->SetEditOKCancelDialog( (AgcModalWindow*)&m_clEditOKCancelDialog );
...
{
	AuINIPackingManager clIniManager;
	...
	_LoadTextureFromINI( (AuIniManagerA*)&clIniManager );
	_LoadSoundFromINI( (AuIniManagerA*)&clIniManager );
	return TRUE;
}
...
BOOL	AgcmUIControl::SetMessageDialogPosition(INT32 lPosX, INT32 lPosY)
{
	...
	m_clEditOKCancelDialog.MoveWindow( lPosX, lPosY, AGCMUICONTROL_MESSAGE_WIDTH, AGCMUICONTROL_MESSAGE_HEIGHT );
...
void AgcmUIControl::_LoadTextureFromINI( AuIniManagerA* pINIManager )
{
	...
	for( INT32 nKeyCount = 0 ; nKeyCount < lNumKey_Section0 ; ++nKeyCount )
	{
		...
		else if( !strcmp( szKeyName, AGCMUICONTROL_KEY_NAME_MESSAGE_BACK ) )
		{
			...
			m_clEditOKCancelDialog.AddImage( szKeyValue );
#ifdef _AREA_GLOBAL_
		...
		else if( !strcmp( szKeyName, AGCMUICONTROL_KEY_NAME_WAIT_BACK ) )
		{
			m_clWaitingDialog.AddImage( szKeyValue );
		}

		else if( !strcmp( szKeyName, AGCMUICONTROL_KEY_NAME_EDIT_CONTROL_IMAGE) )
		{

			m_clEditOKDialog.EditControlImage( szKeyValue );
			m_clEditOKCancelDialog.EditControlImage( szKeyValue );
		...
		else if( !strcmp( szKeyName, AGCMUICONTROL_KEY_NAME_MESSAGE_BUTTON0 ) )
		{
			...
			m_clEditOKCancelDialog.m_clOK.AddButtonImage( szKeyValue , ACUIBUTTON_MODE_NORMAL );
			m_clEditOKCancelDialog.m_clCancelButton.AddButtonImage( szKeyValue , ACUIBUTTON_MODE_NORMAL );
#ifdef _AREA_GLOBAL_
		...
		else if( !strcmp( szKeyName, AGCMUICONTROL_KEY_NAME_MESSAGE_BUTTON1 ) )
		{
			...
			m_clEditOKCancelDialog.m_clOK.AddButtonImage( szKeyValue , ACUIBUTTON_MODE_ONMOUSE );
			m_clEditOKCancelDialog.m_clCancelButton.AddButtonImage( szKeyValue , ACUIBUTTON_MODE_ONMOUSE );
#ifdef _AREA_GLOBAL_
		...
		else if( !strcmp( szKeyName, AGCMUICONTROL_KEY_NAME_MESSAGE_BUTTON2 ) )
		{
			...
			m_clEditOKCancelDialog.m_clOK.AddButtonImage( szKeyValue , ACUIBUTTON_MODE_CLICK );
			m_clEditOKCancelDialog.m_clCancelButton.AddButtonImage( szKeyValue , ACUIBUTTON_MODE_CLICK );
#ifdef _AREA_GLOBAL_
		...
		else if( !strcmp( szKeyName, AGCMUICONTROL_KEY_NAME_PVP_SAFE))
		{
			m_pPvPSafe = RwTextureRead( szKeyValue, NULL );
			ASSERT( NULL != m_pPvPSafe );
		}
		else if( !strcmp( szKeyName, AGCMUICONTROL_KEY_NAME_PVP_WAR))
		{
			m_pPvPWar = RwTextureRead( szKeyValue, NULL );
			ASSERT( NULL != m_pPvPWar );
		}
		else if( !strcmp( szKeyName, AGCMUICONTROL_KEY_NAME_STATE_BATTLE))
		{
			m_pStateBattle = RwTextureRead( szKeyValue, NULL );
			ASSERT( NULL != m_pStateBattle );
		}
		else if( !strcmp( szKeyName, AGCMUICONTROL_KEY_NAME_STATE_BATTLE_END))
		{
			m_pStateBattleEnd = RwTextureRead( szKeyValue, NULL );
			ASSERT( NULL != m_pStateBattleEnd );
		}
		else if( !strcmp( szKeyName, AGCMUICONTROL_KEY_NAME_STATE_BE_WANTED))
		{
			m_pStateBeWanted = RwTextureRead( szKeyValue, NULL );
			ASSERT( NULL != m_pStateBeWanted );
		}
