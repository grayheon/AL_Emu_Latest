...
INT32 AcuTextureList::AddImage( char * filename, BOOL bRead	)
{
...
#ifndef USE_MFC
	strTemp	=	_ApplyRatio(filename);
#endif

	stTextureListInfo TextureListInfo;

	sprintf_s( TextureListInfo.szName , 64 , "%s" , strTemp.c_str() );
	...
	if( bRead )
	{
		RwTexture* pTexture = LoadTexture( TextureListInfo.szName );
...
INT32 AcuTextureList::AddOnlyThisImage( char* filename, BOOL bLoad )
{
	...
	string strTemp = _ApplyRatio(filename);
	...
	if( bLoad )
	{
		RwTexture* pTexture = LoadTexture( const_cast<char*>(strTemp.c_str()) );
...
std::string AcuTextureList::_ApplyRatio( string fileName )
{
	string	strTemp	=	fileName;

	eRatioType	eType	=	g_pEngine->GetRatio();
	if( eType != RESOLUTION_RATIO_0403 )
	{
		INT	nPos	=	strTemp.find( "0403" );
		if(  nPos != string::npos )
		{
			switch( eType )
			{
			case RESOLUTION_RATIO_1609:
				strTemp.replace( nPos , 4 , "1609" , 4 );
				break;
			case RESOLUTION_RATIO_1610:
				strTemp.replace( nPos , 4 , "1610" , 4 );
				break;
			}
		}
	}
