...
AcUIEdit::AcUIEdit( BOOL bPassword, BOOL bNumber )
{
	...
	*/
	...
	m_bEnableParseString	=	FALSE;
	m_pfParseStringCallback	=	NULL;

	m_bChangeText = FALSE;

	m_bIsScrolledByMouse = TRUE;
	m_bCanSetScroll = FALSE;
}
		...
		case ACUIBASE_HALIGN_CENTER:
			{
				...
				else
				{
					...
					{
						lOffsetX += m_pstStringInfoCursor->lOffsetX + m_pAgcmFont->GetTextExtent( m_pstStringInfoCursor->lFontType, ( char* )m_pstStringInfoCursor->szString.c_str(), m_lCursorColumn );
					}
				}
			}
			break;
		...
		case ACUIBASE_HALIGN_RIGHT:
			{
				...
				if( m_pstStringInfoCursor/* && m_pstStringInfoCursor->szString*/ )
				{
					lOffsetX = lOffsetX - _GetStringWidth( m_lCursorRow ) + m_pstStringInfoCursor->lOffsetX + m_pAgcmFont->GetTextExtent( m_pstStringInfoCursor->lFontType, ( char* )m_pstStringInfoCursor->szString.c_str(), m_lCursorColumn );
				}
			}
			break;
...
BOOL AcUIEdit::OnLButtonDblClk( RsMouseStatus *ms )
{

	// ID를 더블클릭 하면 대화창에 귓말을 보낼 수 있도록 한다
	if( m_bReadOnly )
	{
		string		strParseString;
		_CurrentStringParse( ms , strParseString );

		if( m_pfParseStringCallback )
			m_pfParseStringCallback( m_pParseStringClass , NULL , NULL , NULL );
	}
...
void AcUIEdit::OnScrollTop( void )
{
	if( !m_pcsVScroll ) return;
	m_pcsVScroll->SetScrollValue( 0.0f );
}

void AcUIEdit::OnScrollBottom( void )
{
	if( !m_pcsVScroll ) return;
	m_pcsVScroll->SetScrollValue( 1.0f );
}
...
BOOL AcUIEdit::SetText(const CHAR* szText , BOOL bChatting , BOOL bCursorPointRefresh )
{
	if( !m_szText ) return FALSE;
	...
	{
		// 우선 임시로 저장한다.
		INT		lTempCursorPoint	=	m_lCursorPoint;

		ClearText();

		m_lTextLength =	strlen( szText );

		if( bCursorPointRefresh )
		{
		m_lCursorPoint = m_lTextLength;
		}

		// 전에 가지고 있던 커서 포인트 위치로 갱신한다.
		else
		{
			if( m_lTextLength < lTempCursorPoint )
				m_lCursorPoint	=	m_lTextLength;

			else
				m_lCursorPoint	=	lTempCursorPoint;
		}
	...
	if( m_lStringHeight )
	{
		...
		UINT32 nEditMaxLine = ( UINT32 )h / m_lStringHeight;
...
AgcdUIEditStringInfo* AcUIEdit::AddStringItem( CHAR *szString, DWORD dwColor, INT32 lFontType, LineControl eNewLine, INT32 lStringLength, DWORD dwBGColor, BOOL bBold, BOOL bChatting )
{
	...
	if( pstNewStringInfo->lLineIndex < 0 )
		pstNewStringInfo->lLineIndex	=	0;
	...
	if( m_bAutoLF && pstNewStringInfo->lOffsetX + pstNewStringInfo->lWidth + ACUI_EDIT_LEFT_MARGIN * 2 > m_lBoxWidth )
	{
		...
		for( szNewLine = szString ; szNewLine - szString < lStringLength && szNewLine ; szNewLine = CharNextA( szNewLine ) )
		{
			...
			if( pstNewStringInfo->lOffsetX + m_pAgcmFont->GetTextExtent( lFontType, szString, szNewLine - szString + 1 ) + ACUI_EDIT_LEFT_MARGIN * 2 > m_lBoxWidth )
			{
				...
				pstNewStringInfo->lWidth = m_pAgcmFont->GetTextExtent( lFontType, ( char* )pstNewStringInfo->szString.c_str(), pstNewStringInfo->lStringLength );
...
VOID AcUIEdit::SetLookLine( INT32 nLine )
{
	m_lStartLine = nLine;
	m_lStartY =	-_GetStringStartPosY( m_lStartLine );
	_UpdateScrollBarVert();
}
...
{
	if( !m_bMultiLine || !m_bIsScrolledByMouse)	return TRUE;
...

{
	...
	while( pStringList )
	{
		...
		if( nStartY + _GetStringHeight( pStringList->lLineIndex ) >= pClippingBox->y && nStartY <= pClippingBox->y + pClippingBox->h )
		{
			...
			m_pAgcmFont->DrawTextIM2D( fPosX, fPosY, ( char* )pStringList->szString.c_str(), m_lFontType, nAlpha,
										...
										INT32 nFontType, INT32 nAbsX, INT32 nAbsY, INT32 nScrollMargin )
{
	...
	while( pStringList )
	{
		...
		{
			nStartY = nAbsY + pStringList->lOffsetY;
		}
...
BOOL AcUIEdit::_ProcessKeyText( char* pInputChar )
{
	if( !pInputChar ) return FALSE;
	//minasang:
	if( !m_bChangeText ) m_bChangeText = TRUE;
...
BOOL AcUIEdit::_ProcessKeyUp( void )
{
	...
	do
	{
		...
		if( lCursorX < pstIter->lOffsetX + pstIter->lWidth )
		{
			for( lIndex = 0 ; lIndex < pstIter->lStringLength ; ++lIndex )
			{
				...
				ulTemp = abs( pstIter->lOffsetX + m_pAgcmFont->GetTextExtent( pstIter->lFontType, ( char* )pstIter->szString.c_str(), lIndex ) - lCursorX );
	...
	if( bFound )
	{
		m_lCursorPoint = m_lCursorPointDisplay = ( char* )pstIter->szString.c_str() + lIndexOK - m_szTextDisplay;
	}
	else
	{
		m_lCursorPoint = m_lCursorPointDisplay = ( char* )pstCurrLine->szString.c_str() + pstCurrLine->lStringLength - m_szTextDisplay;
	}
...
{
	size_t displayLen = strlen(m_szTextDisplay);

	INT32 lLine = 0;
	while( pStringList )
	{
		if( displayLen + m_lCursorPointDisplay < pStringList->szString.length() )
		{
			m_lCursorRow = lLine - 1;
			m_lCursorColumn = displayLen + m_lCursorPointDisplay - pStringList->szString.length();
	...
	{
		m_lCursorRow = lLine - 1;
		m_lCursorColumn = displayLen + m_lCursorPointDisplay - (m_pstStringInfoCursor ? m_pstStringInfoCursor->szString.length() : 0);
	}
	...
	if( !m_bReadOnly )
	{
		...
		else
		{
			m_lCursorPosX = m_pstStringInfoCursor->lOffsetX + ACUI_EDIT_LEFT_MARGIN + 
				m_pAgcmFont->GetTextExtent( m_pstStringInfoCursor->lFontType, (char*)m_pstStringInfoCursor->szString.c_str(), m_lCursorColumn );
		}
...
BOOL AcUIEdit::_CurrentStringParse( RsMouseStatus* ms , OUT string& strOutString )
{
	INT		nAbsoluteX		=	0;
	INT		nAbsoluteY		=	0;
...
void AcUIEdit::SetLookEndLine()
{
	UINT32 nEditMaxLine = ( UINT32 )h / m_lStringHeight;
	m_lStartLine = ( m_ulLine - 1 ) - nEditMaxLine;
	if(m_lStartLine < 0) m_lStartLine = 0;
	m_lStartY =	-_GetStringStartPosY(m_lStartLine);
