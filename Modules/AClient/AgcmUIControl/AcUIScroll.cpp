...
AcUIScroll::AcUIScroll( void )
{
	...
	m_dwCenterLineColor	=	0x70000000;
	m_nCenterLineSize	=	2;

	m_pClass				=	NULL;
	m_pfnRefreshCallback	=	NULL;
	m_pData1				=	NULL;
	m_pData2				=	NULL;
	m_pData3				=	NULL;
...
BOOL AcUIScroll::SetScrollValue( float fScrollValue, BOOL bSendMessage )
{
	...
	if( m_pUseControl && bSendMessage )
	{
		m_pUseControl->SendMessage( MESSAGE_COMMAND, ( PVOID )UICM_SCROLL_CHANGE_SCROLLVALUE, &m_lControlID );
	}
...
BOOL AcUIScroll::OnCommand( INT32 nID , PVOID pParam )
{
	...
	if( m_pcsButtonScroll && *( INT32* )pParam == m_pcsButtonScroll->GetControlID() )
	{
		if( UICM_SCROLLBUTTON_MOVE == nID )
		{
			...
			if( m_pfnRefreshCallback )
				m_pfnRefreshCallback( m_pClass , m_pData1 , m_pData2 , m_pData3 );
	...
	else if( m_pcsButtonUp && *( INT32* )pParam == m_pcsButtonUp->GetControlID() )
	{
		if( UICM_BUTTON_MESSAGE_CLICK == nID )
		{
			...
			if( m_pUseControl )
			{
				m_pUseControl->SendMessage( MESSAGE_COMMAND, ( PVOID )UICM_SCROLL_UPBUTTON_DOWN, &m_lControlID );
			}
		}
		else if( UICM_BASE_MESSAGE_CLOSE == nID )
		{
	...
	else if( m_pcsButtonDown && *( INT32* )pParam == m_pcsButtonDown->GetControlID() )
	{
		if( UICM_BUTTON_MESSAGE_CLICK == nID )
		{
			...
			if( m_pUseControl )
			{
				m_pUseControl->SendMessage( MESSAGE_COMMAND, ( PVOID )UICM_SCROLL_DOWNBUTTON_DOWN, &m_lControlID );
			}
		}
		else if( UICM_BASE_MESSAGE_CLOSE == nID )
		{
	...
	if( FALSE != m_bMoveMovableWindow )
	{
		...
		if( m_bVScroll )	
		{
			...
			if( m_bMoveByUnit && m_fScrollUnit )
			{
				...
				lMovePosition = lMovePosition - m_lMinPosition + INT32( fUnitGap );
		...
		else
		{
			...
			if( m_bMoveByUnit && m_fScrollUnit )
			{
				lMovePosition /= (INT32)m_fScrollUnit;			
				lMovePosition *= (INT32)m_fScrollUnit;
			}
...
VOID	AcUIScroll::SetRefreshCallback( PVOID pClass , AcUIDefalutCallback pfnCallback , PVOID pData1 , PVOID pData2 , PVOID pData3 )
{
	m_pClass				=	pClass;
	m_pData1				=	pData1;
	m_pData2				=	pData2;
	m_pData3				=	pData3;
	m_pfnRefreshCallback	=	pfnCallback;
}

VOID	AcUIScroll::OnWindowRender( VOID )
{	
	if( m_bDrawCenterLine )
	{
		...
		DWORD	dwRenderAlpha	=	(DWORD)((m_dwCenterLineColor >> 24) * (m_pfAlpha ? *m_pfAlpha : 1));
		DWORD	dwRenderColor	=	(m_dwCenterLineColor & 0x00ffffff) | (dwRenderAlpha<<24);

		if( m_bVScroll )
		{
			INT		nPosX			=	nX + (w/2) - (m_nCenterLineSize/2);

			g_pEngine->Draw2DPanel( nPosX , nY , m_nCenterLineSize , h , dwRenderColor  );
		}

		else
		{
			INT	nPosY = nY + (h/2) - (m_nCenterLineSize/2);
			g_pEngine->Draw2DPanel( nX , nPosY , w , m_nCenterLineSize , m_dwCenterLineColor );
		}
	}
