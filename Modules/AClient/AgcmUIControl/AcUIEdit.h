...
public:
	...
	BOOL												m_bEnableParseString;

	PVOID												m_pParseStringClass;
	AcUIDefalutCallback									m_pfParseStringCallback;

	//@{ 2006/07/02 burumal
	static AcUIDefalutCallback							m_pCBHotkeyOverwriteCheck;
	static PVOID										m_pHotkeyCheckTargetInstacne;

	BOOL												m_bIsScrolledByMouse;
	BOOL												m_bCanSetScroll;
...
private :
	...
	BOOL						_CurrentStringParse		( RsMouseStatus*	ms , OUT string& strOutString );
...
public :
	...
	BOOL						SetText					( const CHAR *szText = NULL, BOOL bChatting = FALSE , BOOL bCursorPointRefresh = TRUE );
	VOID						ClearText				( void );
	...
	VOID						EnableDoubleClickStringParse( BOOL bEnable , AcUIDefalutCallback pfCallback = NULL );
	

	VOID						SetIsScrollByMouse		(BOOL bIsScrollByMouse){ m_bIsScrolledByMouse = bIsScrollByMouse;}
	VOID						SetCanScroll		(BOOL bCanSetScroll){ m_bCanSetScroll = bCanSetScroll;}

	VOID                        SetLookEndLine			(VOID);
	VOID						SetLookLine				( INT32 nLine = 0 );
