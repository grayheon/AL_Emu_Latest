...
BOOL AgcmLogin::OnReceive(UINT32 ulType, PVOID pvPacket, INT16 nSize, UINT32 ulNID, DispatchArg *pstCheckArg)
{
	...
	case AGPMLOGIN_SIGN_ON :	// id, pwd check result
		...
		// 다른 이름의 계정이 보내지는 경우가 있어서 이 부분은 제외시킴 - kdi 2010.12.06
		/*
...
INT16 AgcmLogin::ConnectLoginServer()
{
	...
	// LoginServer Select 기능을 이용하더라도 이건 하기는 해야 한다.
...
{
	/*
	// 국내와 글로벌은 ArchTest.ini 파일이 있으면 그걸 사용한다.
	...
	RegCloseKey(hRegKey); 
*/
	if( _IsExistLoginSelectFile() ) return TRUE; // ArchTest.ini 파일이 있으면 그걸 사용한다.
...
BOOL AgcmLogin::SendAccount(TCHAR *pszAccount, TCHAR *pszPassword, INT16 nNID)
{
	...
	*/
	...
	std::string md5PW = m_csMD5Encrypt.GetMD5Hash(password);
	std::transform(md5PW.begin(), md5PW.end(), md5PW.begin(), ::tolower);
	char* md5Password = (char*)md5PW.c_str();

	INT8 cAccountLen = (INT8)strlen( account );
	INT8 cPasswordLen = (INT8)strlen( password );
	INT8 cmd5PassLen = (INT)strlen(md5Password);

	if( !AuthManager().encrypt( m_szEncryptCode, account, (size_t)cAccountLen, md5Password, cmd5PassLen ) )
		return FALSE;
	...
	PVOID	pvPacket = m_pAgpmLogin->m_csPacket.MakePacket(TRUE, &nPacketLength, AGPMLOGIN_PACKET_TYPE,
															...
															md5Password,			// AccountPassword
															&cmd5PassLen,		// Password Length
															NULL,				// lCID
															NULL,				// World Name;
															NULL,				// pvCharDetailInfo; 캐릭생성시 상세데이터를 넣는 패킷
															NULL,				// pvDetailServerInfo; 서버군 정보.
