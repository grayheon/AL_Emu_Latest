...
public:
	...
	VOID			CharacterKeyDownMove( INT	nKeyCharCode );
	VOID			CharacterKeyUpMove	( INT	nKeyCharCode );
...
//	AgcdEventCharacterEffect*	GetEFfectData(AgpdCharacterTemplate *pstTemplate);
	...
	// Action 함수들
	BOOL				MoveSelfCharacter		( AuPOS * pstPos, MOVE_DIRECTION eMoveDirection, BOOL bPathFinding = FALSE, BOOL bMoveFast = FALSE, RwV3d *pstAt = NULL , BOOL bForce = FALSE );
...
//	static BOOL			CBRemoveAnimation		( PVOID pData, PVOID pClass, PVOID pCustData				);
	...
	static BOOL			CBUpdateIgnoreStatus	( PVOID pData, PVOID pClass, PVOID pCustData				);
...
//	BOOL SetCallbackActionSkill(ApModuleDefaultCallBack pfCallback, PVOID pClass);
	...
	BOOL				SendMoveCharacter	( AuPOS* posCurrent, AuPOS *posDest, MOVE_DIRECTION eMoveDirection, BOOL bPathFinding, BOOL bMoveFast);
...
//	BOOL				UpdataSpecificCharacter(UINT32	CID);
	...
	*/
	...
	static	BOOL					CBSelfCharacterKeyMove( PVOID pData , PVOID pClass , PVOID pCustData );
...
private :
	...
	BOOL	m_bKeyMoveInterpolation;

	RwV3d	m_vLastCameraAt;
