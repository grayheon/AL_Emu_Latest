...
BOOL AgcmEventEffect::OnInit()
{
	...
	{
		if( !m_pcsAgpmCharacter->SetCallbackEvolution(CBRestoreCommonEffectsByEvolution, this) )
			return FALSE;
		if( !m_pcsAgpmCharacter->SetCallbackRestoreEvolution(CBRestoreCommonEffectsByEvolution, this) )
			return FALSE;
	}
	...
	case APBASE_TYPE_CHARACTER:
		{
			...
			if (((AgpdCharacter *) ppObject)->m_ulSpecialStatus & AGPDCHAR_SPECIAL_STATUS_EVENT_GIFTBOX)
			{
				...
				if( pThis->m_pcsAgpmCharacter->IsTrap( ppObject ) )
					return FALSE;
...
BOOL AgcmEventEffect::SetCommonCharacterEffect(ApBase *pcsBase, eAgcmEventEffectDataCommonCharType eCommonCharType, INT32 lTargetCID)
{
	if(pcsBase)
	{
		return SetCommonCharEffect(pcsBase, eCommonCharType, lTargetCID);
	}
	return FALSE;
}
...
BOOL AgcmEventEffect::RemoveCommonCharacterEffect(ApBase *pcsBase, eAgcmEventEffectDataCommonCharType eCommonCharType)
{
	...
	if(pcsBase->m_eType == APBASE_TYPE_CHARACTER)
	{
		if(m_pcsAgpmCharacter->GetCharacterRace((AgpdCharacter*)pcsBase).detail.nRace == AURACE_TYPE_DRAGONSCION)
			RemoveCurrentEffectEvolution(eCommonCharType);
	}
...
eAcReturnType AgcmEventEffect::SetAttrDamageEffect(AgpdCharacter *pcsAgpdAttacker, AgpdFactor* pcsDamageFactor )
{
	if( !pcsDamageFactor )		
		return E_AC_RETURN_TYPE_SKIP;

	INT32 lDamage = 0;
	if( !m_pcsAgpmFactors->GetValue( pcsDamageFactor, &lDamage, AGPD_FACTORS_TYPE_CHAR_POINT, AGPD_FACTORS_CHARPOINT_TYPE_DMG_NORMAL ) )
	...
	if( lSpiritStoneType == -1 && m_pcsAgpmFactors->GetValue( pcsDamageFactor, &lDamage, AGPD_FACTORS_TYPE_CHAR_POINT, AGPD_FACTORS_CHARPOINT_TYPE_DMG_ATTR_MAGIC ) )
		if( lDamage < 0 )
			lSpiritStoneType = E_SPIRIT_STONE_TYPE_MAGIC;

	if(	lSpiritStoneType == -1 && m_pcsAgpmFactors->GetValue( pcsDamageFactor, &lDamage, AGPD_FACTORS_TYPE_CHAR_POINT, AGPD_FACTORS_CHARPOINT_TYPE_DMG_ATTR_WATER	) )
		if (lDamage < 0)
			lSpiritStoneType = E_SPIRIT_STONE_TYPE_WATER;

	if( lSpiritStoneType == -1 && m_pcsAgpmFactors->GetValue( pcsDamageFactor, &lDamage, AGPD_FACTORS_TYPE_CHAR_POINT, AGPD_FACTORS_CHARPOINT_TYPE_DMG_ATTR_FIRE ) )
		if (lDamage < 0)
			lSpiritStoneType = E_SPIRIT_STONE_TYPE_FIRE;

	if( lSpiritStoneType == -1 && m_pcsAgpmFactors->GetValue( pcsDamageFactor, &lDamage, AGPD_FACTORS_TYPE_CHAR_POINT, AGPD_FACTORS_CHARPOINT_TYPE_DMG_ATTR_EARTH ) )
		if (lDamage < 0)
			lSpiritStoneType = E_SPIRIT_STONE_TYPE_EARTH;

	if(	lSpiritStoneType == -1 && m_pcsAgpmFactors->GetValue( pcsDamageFactor, &lDamage, AGPD_FACTORS_TYPE_CHAR_POINT, AGPD_FACTORS_CHARPOINT_TYPE_DMG_ATTR_AIR ) )
		if (lDamage < 0)
...
eAcReturnType AgcmEventEffect::SetAttrDamageEffect(AgpdCharacter *pcsAgpdAttacker, AgpdCharacter *pcsAgpdTarget)
{
	...
	AgpdCharacter*		pTarget			=	pcsAgpdTarget;

	AgpdFactor*	pcsAgpdFactor	=	NULL;
	pcsAgpdFactor	= m_pcsAgcmCharacter->GetNextReceivedActionDamageFactor( pcsAgpdAttacker, pTarget, AGPDCHAR_ACTION_TYPE_ATTACK );
...
{
	RemoveCommonCharacterEffect(pcsBase, eType);
	...
	if (pcsBase->m_eType == APBASE_TYPE_CHARACTER)
	{
		...
		if(m_pcsAgpmCharacter->GetCharacterRace((AgpdCharacter*)pcsBase).detail.nRace == AURACE_TYPE_DRAGONSCION)
		{
			AgpmShrineBattle* pcsAgpmShrine = ( AgpmShrineBattle* )GetModule("AgpmShrineBattle");
			if(pcsAgpmShrine && pcsAgpmShrine->IsInShrineRegionByIndex((AgpdCharacter*)pcsBase))
				SaveCurrentEffectEvolution(eType);
		}
	...
	for (INT32 lCount = 0; lCount < AGCM_EVENT_EFFECT_DATA_MAX_EFFECT_DATA; ++lCount)
	{
		...
		if( !pcsEffectSetData || !pcsEffectSetData->m_ulEID )
			return TRUE;

		pcsEffectSetData->m_lTargetCID = lTargetCID;
		if( !SetBaseEffect( pcsBase, pcsEffectSetData ) )
			return FALSE;
...
BOOL AgcmEventEffect::CBRestoreCommonEffectsByEvolution( PVOID pData, PVOID pClass, PVOID pCustData )
{
	if(!pData || !pClass)
		return FALSE;
	...
	if(AURACE_TYPE_DRAGONSCION != pThis->m_pcsAgpmCharacter->GetCharacterRace(pcsCharacter).detail.nRace)
		return FALSE;

	AgpmShrineBattle* pcsShrineBattle = (AgpmShrineBattle*)pThis->GetModule("AgpmShrineBattle");
	if(!pcsShrineBattle || !pcsShrineBattle->IsInShrineRegionByIndex(pcsCharacter))
		return FALSE;

	for(std::set<eAgcmEventEffectDataCommonCharType>::iterator itr = pThis->m_listCommonEffectData.begin() ; itr != pThis->m_listCommonEffectData.end() ; ++itr)
		pThis->SetCommonCharacterEffect((ApBase*)pcsCharacter, *itr);

	return TRUE;
}

BOOL AgcmEventEffect::SaveCurrentEffectEvolution( eAgcmEventEffectDataCommonCharType eEffect )
{
	m_listCommonEffectData.insert(eEffect);
	return TRUE;
}

BOOL AgcmEventEffect::RemoveCurrentEffectEvolution( eAgcmEventEffectDataCommonCharType eEffect )
{
	m_listCommonEffectData.erase(eEffect);
	return TRUE;
}
