...
: m_pcsAgcmUIOption(NULL)
{
	SetModuleName("AgcmUIMain");
	SetDelayTick( 500 );

	ZeroMemory(m_apstQuickBelt, sizeof(AgcdUIUserData)*(AGCMUIMAIN_MAX_QUICKBELT*2));
...
BOOL			AgcmUIMain::OnAddModule()
{
	...
	if (m_pcsAgpmGrid)
	{
		for(int i=0 ; i<AGCMUIMAIN_MAX_QUICKBELT*2 ; ++i)
		{
			m_astQuickBeltGrid[i].m_lGridType = AGPDGRID_ITEM_TYPE_ITEM | AGPDGRID_ITEM_TYPE_SKILL | AGPDGRID_ITEM_TYPE_SOCIALACTION;

			if(i<2)
				m_pcsAgpmGrid->Init(&m_astQuickBeltGrid[i], AGCMUIMAIN_MAX_QUICKBELT, 1, AGPMUISTATUS_HALF_QUICKBELT_COLUMN);
			else
			m_pcsAgpmGrid->Init(&m_astQuickBeltGrid[i], 1, 1, AGPMUISTATUS_HALF_QUICKBELT_COLUMN);
		}
	...
	{
		for(int i=0 ; i<AGCMUIMAIN_MAX_QUICKBELT ; ++i)
		{
			char szTempL[32] = {0,};
			char szTempR[32] = {0,};
			...
			sprintf(szTempL, "Quick_Belt_%d(L)", i);
			sprintf(szTempR, "Quick_Belt_%d(R)", i);

			m_apstQuickBelt[nLeft]	= m_pcsAgcmUIManager2->AddUserData(szTempL, &m_astQuickBeltGrid[nLeft], sizeof(AgpdGrid), 1, AGCDUI_USERDATA_TYPE_GRID);
			if (!m_apstQuickBelt[nLeft])
			return FALSE;

			m_apstQuickBelt[nRight]	= m_pcsAgcmUIManager2->AddUserData(szTempR, &m_astQuickBeltGrid[nRight], sizeof(AgpdGrid), 1, AGCDUI_USERDATA_TYPE_GRID);
			if (!m_apstQuickBelt[nRight])
				return FALSE;
		}
		...
		m_apstQBLayer[0]	= m_pcsAgcmUIManager2->AddUserData("QB_Layer(L)", &m_nNumOfActiveQuickbelt, sizeof(INT32), 1, AGCDUI_USERDATA_TYPE_INT32);
		if (!m_apstQBLayer[0])
			return FALSE;

		m_apstQBLayer[1]	= m_pcsAgcmUIManager2->AddUserData("QB_Layer(R)", &m_nNumOfActiveQuickbelt, sizeof(INT32), 1, AGCDUI_USERDATA_TYPE_INT32);
		if (!m_apstQBLayer[1])
			return FALSE;
	...
	// Function을 등록한다.
	{
		if (!m_pcsAgcmUIManager2->AddFunction(this, "QB_Add", CBAddShortcut, 1, "Self Grid Control"))
			return FALSE;
		if (!m_pcsAgcmUIManager2->AddFunction(this, "QB_Remove", CBRemoveShortcut, 1, "Self Grid Control"))
			return FALSE;

		if (!m_pcsAgcmUIManager2->AddFunction(this, "HPPotion_Add", CBAddHPPotion, 1, "Self Grid Control"))
			return FALSE;
		if (!m_pcsAgcmUIManager2->AddFunction(this, "MPPotion_Add", CBAddMPPotion, 1, "Self Grid Control"))
			return FALSE;
		if (!m_pcsAgcmUIManager2->AddFunction(this, "QB_Potion_Remove", CBRemovePotion, 0))
			return FALSE;
	...
	// Event를 등록한다.
	{
		...
		*/
		...
		m_lEventShowAdditionalQuickBelt[0] = m_pcsAgcmUIManager2->AddEvent("Event_ShowAdditionalQuickBelt1");
		if (m_lEventShowAdditionalQuickBelt[0] < 0)
			return FALSE;
		m_lEventHideAdditionalQuickBelt[0] = m_pcsAgcmUIManager2->AddEvent("Event_HideAdditionalQuickBelt1");
		if (m_lEventHideAdditionalQuickBelt[0] < 0)
			return FALSE;

		m_lEventShowAdditionalQuickBelt[1] = m_pcsAgcmUIManager2->AddEvent("Event_ShowAdditionalQuickBelt2");
		if (m_lEventShowAdditionalQuickBelt[1] < 0)
			return FALSE;
		m_lEventHideAdditionalQuickBelt[1] = m_pcsAgcmUIManager2->AddEvent("Event_HideAdditionalQuickBelt2");
		if (m_lEventHideAdditionalQuickBelt[1] < 0)
			return FALSE;

		m_lEventShowAdditionalQuickBelt[2] = m_pcsAgcmUIManager2->AddEvent("Event_ShowAdditionalQuickBelt3");
		if (m_lEventShowAdditionalQuickBelt[2] < 0)
			return FALSE;
		m_lEventHideAdditionalQuickBelt[2] = m_pcsAgcmUIManager2->AddEvent("Event_HideAdditionalQuickBelt3");
		if (m_lEventHideAdditionalQuickBelt[2] < 0)
			return FALSE;
...
BOOL			AgcmUIMain::OnDestroy()
{
	if (m_pcsAgpmGrid)
	{
		for(int i=0 ; i<AGCMUIMAIN_MAX_QUICKBELT*2 ; ++i)
		{
			m_pcsAgpmGrid->Remove(&m_astQuickBeltGrid[i]);
		}
...
#endif
	m_pcsAgcmUIOption = static_cast<AgcmUIOption *>(GetModule("AgcmUIOption"));
	if(!m_pcsAgcmUIOption)
		return FALSE;

	if(!m_pcsAgcmUIOption->SetCallbackChangeOptionUseNewSlot(CBChangedUseNewSlot, this))
		return FALSE;
...
{
	m_nNumOfActiveQuickbelt = 0;
	OnAdjustHPMPBarPosition(m_nNumOfActiveQuickbelt);
	for (int i = 0; i < AGCMUIMAIN_MAX_QUICKBELT*2; ++i)
	{
		if (m_apstQuickBelt[i]->m_ppControls)
		{
			for (INT32 lIndex = 0; lIndex < m_apstQuickBelt[i]->m_lControl; ++lIndex)
			{
				...
				if (pcsUserDataControl)
				{
					...
					if (pcsUIGrid)
					{
						pcsUIGrid->SetNowLayer(m_nNumOfActiveQuickbelt);
...
BOOL			AgcmUIMain::OnIdle(UINT32 ulClockCount)
{
	...
	else
	{
		...
		if(m_pcsAgcmUIOption->m_bUseNewSlot)
		m_bQuickBeltSwitchButton[0] = FALSE;
		else
		...
		{
			m_nSavedCurrentQuickBeltLayer = m_nNumOfActiveQuickbelt;
	...
	while ( (!m_bQuickBeltSwitchButton[0] ) && 
		...
		(m_nNumOfActiveQuickbelt != m_nSavedCurrentQuickBeltLayer) )
	{
		CBQuickBeltLayerUp(this, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
		if ( m_nNumOfActiveQuickbelt == m_nSavedCurrentQuickBeltLayer )
			m_nSavedCurrentQuickBeltLayer = -1;
	}
	//@}
...
BOOL			AgcmUIMain::AddShortcut(AgpdGrid *pcsGrid, AgpdGridItem *pcsGridItem, INT32 lLayer, INT32 lRow, INT32 lColumn, BOOL bIsThrowEvent, INT32 lRealLayer)
{
...
//		for(int i=0 ; i<AGCMUIMAIN_MAX_QUICKBELT ; ++i)
		{
			int nLeft = lRealLayer*2;
		int nRight = nLeft+1;

		RemoveGridLayer(&m_astQuickBeltGrid[nLeft], pcsGridItem, lLayer);
		RemoveGridLayer(&m_astQuickBeltGrid[nRight], pcsGridItem, lLayer);

			// 구 슬롯 시스템과 동기화
			RemoveGridLayer(&m_astQuickBeltGrid[0], pcsGridItem, lRealLayer);
			RemoveGridLayer(&m_astQuickBeltGrid[1], pcsGridItem, lRealLayer);

		if (pcsGrid == &m_astQuickBeltGrid[nLeft])
		{
			m_pcsAgpmGrid->Clear(&m_astQuickBeltGrid[nLeft], lLayer, lRow, lColumn, 1, 1);
			m_pcsAgpmGrid->Add(&m_astQuickBeltGrid[nLeft], lLayer, lRow, lColumn, pcsGridItem, 1, 1);
				if(lRealLayer != 0)
				{
					m_pcsAgpmGrid->Clear(&m_astQuickBeltGrid[0], lRealLayer, lRow, lColumn, 1, 1);
					m_pcsAgpmGrid->Add(&m_astQuickBeltGrid[0], lRealLayer, lRow, lColumn, pcsGridItem, 1, 1);
		}
			}
		else if(pcsGrid == &m_astQuickBeltGrid[nRight])
		{
			m_pcsAgpmGrid->Clear(&m_astQuickBeltGrid[nRight], lLayer, lRow, lColumn, 1, 1);
			m_pcsAgpmGrid->Add(&m_astQuickBeltGrid[nRight], lLayer, lRow, lColumn, pcsGridItem, 1, 1);
				if(lRealLayer != 0)
				{
					m_pcsAgpmGrid->Clear(&m_astQuickBeltGrid[1], lRealLayer, lRow, lColumn, 1, 1);
					m_pcsAgpmGrid->Add(&m_astQuickBeltGrid[1], lRealLayer, lRow, lColumn, pcsGridItem, 1, 1);
		}
	}
		}
	}
	else
	{
		// 구 슬롯 시스템(변경형)
		if(bIsThrowEvent)
			lLayer^=lRealLayer^=lLayer^=lRealLayer;
	...
	RefreshQBeltGridItemStackCount(m_nNumOfActiveQuickbelt);
...
BOOL			AgcmUIMain::RemoveShortcut(AgpdGrid *pcsGrid, INT32 lLayer, INT32 lRow, INT32 lColumn, BOOL bIsThrowEvent, INT32 lRealLayer)
{
	if (!pcsGrid)
		return FALSE;
...
//		for(int i=0 ; i<AGCMUIMAIN_MAX_QUICKBELT ; ++i)
		...
		if (pcsGrid == &m_astQuickBeltGrid[nLeft])
			{
			bIsCleared |= m_pcsAgpmGrid->Clear(&m_astQuickBeltGrid[nLeft], lLayer, lRow, lColumn, 1, 1);
				if( lRealLayer != 0 )
				{
					//구 슬롯 시스템과 동기화
					bIsCleared |= m_pcsAgpmGrid->Clear(&m_astQuickBeltGrid[0], lRealLayer, lRow, lColumn, 1, 1);
				}
			}
		else if(pcsGrid == &m_astQuickBeltGrid[nRight])
			{
			bIsCleared |= m_pcsAgpmGrid->Clear(&m_astQuickBeltGrid[nRight], lLayer, lRow, lColumn, 1, 1);
				if( lRealLayer != 0 )
				{
					//구 슬롯 시스템과 동기화
					bIsCleared |= m_pcsAgpmGrid->Clear(&m_astQuickBeltGrid[1], lRealLayer, lRow, lColumn, 1, 1);
	}
			}
		}
	}
...
BOOL			AgcmUIMain::CBAddShortcut(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl)
{
	...
	INT32 nRealLayer = 0;
	std::string strUIName = pcsControl->m_szName;

	int nPos = strUIName.find("CONTROL_GRID_LEFT_");
	if(nPos == std::string::npos)
	{
		nPos = strUIName.find("CONTROL_GRID_RIGHT_");
		if(nPos == std::string::npos)
			return FALSE;
		else
		{
			sscanf(pcsControl->m_szName, "CONTROL_GRID_RIGHT_%d", &nRealLayer);
		}
	}
	else
	{
		sscanf(pcsControl->m_szName, "CONTROL_GRID_LEFT_%d", &nRealLayer);
	}
	...
	if (pstGrid)
	{
		pThis->AddShortcut(pstGrid, pstDragInfo->pGridItem, pcsUIGrid->m_lNowLayer, pstDragInfo->lGridRow, pstDragInfo->lGridColumn, TRUE, nRealLayer);
	}
...
BOOL			AgcmUIMain::CBRemoveShortcut(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl)
{
	...
	if (pstGrid)
	{
		pThis->RemoveShortcut(pstGrid, pcsUIGrid->m_lNowLayer, pstDragInfo->lGridRow, pstDragInfo->lGridColumn, TRUE, nRealLayer);
	}
...
BOOL	AgcmUIMain::CBDisplayCurrentLayer(PVOID pClass, PVOID pData, AgcdUIDataType eType, INT32 lID, CHAR *szDisplay, INT32 *plValue, AgcdUIControl *pcsSourceControl)
{
	...
	sprintf(szDisplay, "%d", pThis->m_nNumOfActiveQuickbelt + 1);
...
BOOL	AgcmUIMain::CBLClickDown(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl)
{
	...
	*/
	...
	for(int i=0 ; i<AGCMUIMAIN_MAX_QUICKBELT*2 ; ++i)
	{
		pThis->m_astQuickBeltGrid[i].m_pcsSelectedGridItem	= pcsClickInfo->pGridItem;
	}
...
BOOL	AgcmUIMain::CBQuickBeltUse_1(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl)
{
	...
	if(pThis->m_pcsAgcmUIOption->m_bUseNewSlot)
	{
	INT32	nKeyCode = pThis->GetKeyCode();

	//단축키(Shift/Ctrl/Alt)가 눌렸으면 현재 열려있는 퀵벨트 갯수를 확인해서 열려있어야만 진행함
	if(pThis->m_nNumOfActiveQuickbelt < nKeyCode)
		return TRUE;

	if (pThis->m_apstQuickBelt && pThis->m_apstQuickBelt[nKeyCode*2] && pThis->m_apstQuickBelt[nKeyCode*2]->m_stUserData.m_pvData)
	{
		AgpdGrid *pstQuickBeltGrid = (AgpdGrid *) pThis->m_apstQuickBelt[nKeyCode*2]->m_stUserData.m_pvData;

		AgpdGridItem	*pcsGridItem	= pThis->m_pcsAgpmGrid->GetItem(pstQuickBeltGrid, 0, 0, 0);
		if (pcsGridItem)
		{
			for(int i=0 ; i<AGCMUIMAIN_MAX_QUICKBELT*2 ; ++i)
				pThis->m_astQuickBeltGrid[i].m_pcsSelectedGridItem	= pcsGridItem;
	...
	else
	{
		if (pThis->m_apstQuickBelt && pThis->m_apstQuickBelt[0] && pThis->m_apstQuickBelt[0]->m_stUserData.m_pvData)
		{
			...
			AgpdGridItem	*pcsGridItem	= pThis->m_pcsAgpmGrid->GetItem(pstQuickBeltGrid, pThis->m_nNumOfActiveQuickbelt, 0, 0);
			if (pcsGridItem)
			{
...
BOOL	AgcmUIMain::CBQuickBeltUse_2(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl)
{
	...
	if(pThis->m_pcsAgcmUIOption->m_bUseNewSlot)
	{
		...
		if (pThis->m_apstQuickBelt && pThis->m_apstQuickBelt[nKeyCode*2] && pThis->m_apstQuickBelt[nKeyCode*2]->m_stUserData.m_pvData)
		{
			...
			AgpdGridItem	*pcsGridItem	= pThis->m_pcsAgpmGrid->GetItem(pstQuickBeltGrid, 0, 0, 1);
			if (pcsGridItem)
			{
				for(int i=0 ; i<AGCMUIMAIN_MAX_QUICKBELT*2 ; ++i)
					pThis->m_astQuickBeltGrid[i].m_pcsSelectedGridItem	= pcsGridItem;
	...
	else
	{
		if (pThis->m_apstQuickBelt && pThis->m_apstQuickBelt[0] && pThis->m_apstQuickBelt[0]->m_stUserData.m_pvData)
		{
			...
			AgpdGridItem	*pcsGridItem	= pThis->m_pcsAgpmGrid->GetItem(pstQuickBeltGrid, pThis->m_nNumOfActiveQuickbelt, 0, 1);
			if (pcsGridItem)
			{
				pThis->m_astQuickBeltGrid[0].m_pcsSelectedGridItem	= pcsGridItem;
				pThis->m_astQuickBeltGrid[1].m_pcsSelectedGridItem	= pcsGridItem;
...
BOOL	AgcmUIMain::CBQuickBeltUse_3(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl)
{
	...
	if(pThis->m_pcsAgcmUIOption->m_bUseNewSlot)
	{
		...
		if (pThis->m_apstQuickBelt && pThis->m_apstQuickBelt[nKeyCode*2] && pThis->m_apstQuickBelt[nKeyCode*2]->m_stUserData.m_pvData)
		{
			...
			AgpdGridItem	*pcsGridItem	= pThis->m_pcsAgpmGrid->GetItem(pstQuickBeltGrid, 0, 0, 2);
			if (pcsGridItem)
			{
				for(int i=0 ; i<AGCMUIMAIN_MAX_QUICKBELT*2 ; ++i)
					pThis->m_astQuickBeltGrid[i].m_pcsSelectedGridItem	= pcsGridItem;
	...
	else
	{
		if (pThis->m_apstQuickBelt && pThis->m_apstQuickBelt[0] && pThis->m_apstQuickBelt[0]->m_stUserData.m_pvData)
		{
			...
			AgpdGridItem	*pcsGridItem	= pThis->m_pcsAgpmGrid->GetItem(pstQuickBeltGrid, pThis->m_nNumOfActiveQuickbelt, 0, 2);
			if (pcsGridItem)
			{
				pThis->m_astQuickBeltGrid[0].m_pcsSelectedGridItem	= pcsGridItem;
				pThis->m_astQuickBeltGrid[1].m_pcsSelectedGridItem	= pcsGridItem;
...
BOOL	AgcmUIMain::CBQuickBeltUse_4(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl)
{
	...
	if(pThis->m_pcsAgcmUIOption->m_bUseNewSlot)
	{
		...
		if (pThis->m_apstQuickBelt && pThis->m_apstQuickBelt[nKeyCode*2] && pThis->m_apstQuickBelt[nKeyCode*2]->m_stUserData.m_pvData)
		{
			...
			AgpdGridItem	*pcsGridItem	= pThis->m_pcsAgpmGrid->GetItem(pstQuickBeltGrid, 0, 0, 3);
			if (pcsGridItem)
			{
				for(int i=0 ; i<AGCMUIMAIN_MAX_QUICKBELT*2 ; ++i)
					pThis->m_astQuickBeltGrid[i].m_pcsSelectedGridItem	= pcsGridItem;
	...
	else
	{
		if (pThis->m_apstQuickBelt && pThis->m_apstQuickBelt[0] && pThis->m_apstQuickBelt[0]->m_stUserData.m_pvData)
		{
			...
			AgpdGridItem	*pcsGridItem	= pThis->m_pcsAgpmGrid->GetItem(pstQuickBeltGrid, pThis->m_nNumOfActiveQuickbelt, 0, 3);
			if (pcsGridItem)
			{
				pThis->m_astQuickBeltGrid[0].m_pcsSelectedGridItem	= pcsGridItem;
				pThis->m_astQuickBeltGrid[1].m_pcsSelectedGridItem	= pcsGridItem;
...
BOOL	AgcmUIMain::CBQuickBeltUse_5(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl)
{
	...
	if(pThis->m_pcsAgcmUIOption->m_bUseNewSlot)
	{
		...
		if (pThis->m_apstQuickBelt && pThis->m_apstQuickBelt[nKeyCode*2] && pThis->m_apstQuickBelt[nKeyCode*2]->m_stUserData.m_pvData)
		{
			...
			AgpdGridItem	*pcsGridItem	= pThis->m_pcsAgpmGrid->GetItem(pstQuickBeltGrid, 0, 0, 4);
			if (pcsGridItem)
			{
				for(int i=0 ; i<AGCMUIMAIN_MAX_QUICKBELT*2 ; ++i)
					pThis->m_astQuickBeltGrid[i].m_pcsSelectedGridItem	= pcsGridItem;
	...
	else
	{
		if (pThis->m_apstQuickBelt && pThis->m_apstQuickBelt[0] && pThis->m_apstQuickBelt[0]->m_stUserData.m_pvData)
		{
			...
			AgpdGridItem	*pcsGridItem	= pThis->m_pcsAgpmGrid->GetItem(pstQuickBeltGrid, pThis->m_nNumOfActiveQuickbelt, 0, 4);
			if (pcsGridItem)
			{
				pThis->m_astQuickBeltGrid[0].m_pcsSelectedGridItem	= pcsGridItem;
				pThis->m_astQuickBeltGrid[1].m_pcsSelectedGridItem	= pcsGridItem;
...
BOOL	AgcmUIMain::CBQuickBeltUse_6(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl)
{
	...
	if(pThis->m_pcsAgcmUIOption->m_bUseNewSlot)
	{
		...
		if (pThis->m_apstQuickBelt && pThis->m_apstQuickBelt[nKeyCode*2+1] && pThis->m_apstQuickBelt[nKeyCode*2+1]->m_stUserData.m_pvData)
		{
			AgpdGrid *pstQuickBeltGrid = (AgpdGrid *) pThis->m_apstQuickBelt[nKeyCode*2+1]->m_stUserData.m_pvData;
	...
	{
		if (pThis->m_apstQuickBelt && pThis->m_apstQuickBelt[1] && pThis->m_apstQuickBelt[1]->m_stUserData.m_pvData)
		{
...
BOOL	AgcmUIMain::CBQuickBeltLayerUp(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl)
{
	...
	INT32 nNumofActive = 0;

	AgcdUI*	pUIBelt1 = pThis->m_pcsAgcmUIManager2->GetUI("UI_QuickBelt_1");
	AgcdUI*	pUIBelt2 = pThis->m_pcsAgcmUIManager2->GetUI("UI_QuickBelt_2");
	AgcdUI*	pUIBelt3 = pThis->m_pcsAgcmUIManager2->GetUI("UI_QuickBelt_3");

	if(!pUIBelt1||!pUIBelt2||!pUIBelt3)
		return FALSE;

	if(pThis->m_pcsAgcmUIOption->m_bUseNewSlot)
	{
		nNumofActive = pThis->m_nNumOfActiveQuickbelt + 1;
	if (nNumofActive < AGCMUIMAIN_MAX_QUICKBELT && (pUIBelt1->m_eStatus != AGCDUI_STATUS_CLOSING && pUIBelt2->m_eStatus != AGCDUI_STATUS_CLOSING && pUIBelt3->m_eStatus != AGCDUI_STATUS_CLOSING))
		{
		pThis->m_nNumOfActiveQuickbelt	= nNumofActive;
	pThis->OnAdjustHPMPBarPosition(pThis->m_nNumOfActiveQuickbelt);
		}

	switch(pThis->m_nNumOfActiveQuickbelt)
	{
	case 1:
		//일단 떠있는 다른 벨트를 닫자
		pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lEventHideAdditionalQuickBelt[1]);
		pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lEventHideAdditionalQuickBelt[2]);

		//그리고 띄워야하는 0번을 켜기 전에 위지조절을 해준다.
		pUIBelt1->m_pcsUIWindow->MoveWindow(pThis->GetCenterXPosOfUI(pUIBelt1->m_pcsUIWindow->w), pThis->GetYPosOfBelt(pUIBelt1->m_pcsUIWindow->h, pThis->m_nNumOfActiveQuickbelt));
		pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lEventShowAdditionalQuickBelt[0]);
		break;
	...
	case 2:
		...
		//그리고 띄워야하는 1번을 켜기 전에 위지조절을 해준다.
		pUIBelt2->m_pcsUIWindow->MoveWindow(pThis->GetCenterXPosOfUI(pUIBelt2->m_pcsUIWindow->w), pThis->GetYPosOfBelt(pUIBelt2->m_pcsUIWindow->h, pThis->m_nNumOfActiveQuickbelt));
		pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lEventShowAdditionalQuickBelt[1]);
		break;

	case 3:
		//띄워야하는 2번을 켜기 전에 위지조절을 해준다.
		pUIBelt3->m_pcsUIWindow->MoveWindow(pThis->GetCenterXPosOfUI(pUIBelt3->m_pcsUIWindow->w), pThis->GetYPosOfBelt(pUIBelt3->m_pcsUIWindow->h, pThis->m_nNumOfActiveQuickbelt));
		pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lEventShowAdditionalQuickBelt[2]);
		break;
		}
	}
	else
	{
		pThis->m_nNumOfActiveQuickbelt++;
		if (pThis->m_nNumOfActiveQuickbelt >= AGCMUIMAIN_MAX_QUICKBELT)
			pThis->m_nNumOfActiveQuickbelt	= 0;
			...
			if (pThis->m_apstQuickBelt[i]->m_ppControls)
			{
				for (INT32 lIndex = 0; lIndex < pThis->m_apstQuickBelt[i]->m_lControl; ++lIndex)
				{
					...
					if (pcsUserDataControl)
					{
						...
						if (pcsUIGrid)
						{
							pcsUIGrid->SetNowLayer(pThis->m_nNumOfActiveQuickbelt);
							pcsUIGrid->UpdateUIGrid();
						}
					}
				}
...
BOOL	AgcmUIMain::CBQuickBeltLayerDown(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl)
{
	...
	case 0:
		//다 닫자
		pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lEventHideAdditionalQuickBelt[0]);
		pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lEventHideAdditionalQuickBelt[1]);
		pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lEventHideAdditionalQuickBelt[2]);
		break;
		}
	...
	{
		pThis->m_nNumOfActiveQuickbelt--;
		if( pThis->m_nNumOfActiveQuickbelt < 0)
			pThis->m_nNumOfActiveQuickbelt	= AGCMUIMAIN_MAX_QUICKBELT - 1;
...
BOOL AgcmUIMain::RemoveGridItemFromQuickBelt(AgpdGridItem *pcsGridItem)
{
	...
	for(int i=0 ; i<AGCMUIMAIN_MAX_QUICKBELT*2 ; ++i)
	{
		m_pcsAgpmGrid->RemoveItem(&m_astQuickBeltGrid[i], pcsGridItem);
		m_pcsAgcmUIManager2->SetUserDataRefresh(m_apstQuickBelt[i]);
	}
...
{
	for(int i=0 ; i<=m_nNumOfActiveQuickbelt ; ++i)
	{
		int nLeft = i*2;
		int nRight = nLeft+1;

		m_pcsAgcmUIManager2->SetUserDataRefresh(m_apstQuickBelt[nLeft]);
		m_pcsAgcmUIManager2->SetUserDataRefresh(m_apstQuickBelt[nRight]);
	}
...
BOOL	AgcmUIMain::CBApplicationQuitEventCall(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl)
{
	...
	if( pThis->m_pcsAgpmCharacter->IsCombatMode( pThis->m_pcsAgcmCharacter->GetSelfCharacter() ) )
	{
		...
		if( pMessage )
		{
			AgcChatManager::OnAddSystemMessage( pMessage );
		}
...
BOOL AgcmUIMain::CBUpdateStackCount(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	if(pcsItem->m_eStatus == AGPDITEM_STATUS_SUB_INVENTORY || pcsItem->m_eStatus == AGPDITEM_STATUS_CHARGING_INVENTORY)
		return TRUE;
...
BOOL AgcmUIMain::CBReleaseSelfCharacter(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	for(int i=0 ; i<AGCMUIMAIN_MAX_QUICKBELT*2 ; ++i)
		pThis->m_pcsAgpmGrid->Reset(&pThis->m_astQuickBeltGrid[i]);
...
BOOL AgcmUIMain::RefreshQBeltGridItemStackCount(INT32 lLayer)
{
	...
	if(m_pcsAgcmUIOption->m_bUseNewSlot)
	{
	for(int i=0 ; i<lLayer*2 ; ++i)
	{
		if(!m_apstQuickBelt[i] || !m_apstQuickBelt[i]->m_stUserData.m_pvData)
			return FALSE;

		AgpdGrid* pstQuickBeltGrid = static_cast<AgpdGrid*>(m_apstQuickBelt[i]->m_stUserData.m_pvData);

			// Layer 에 있는 모든 GridItem 을 돌면서 Item 인 경우 스택카운트를 다시 그려준다.
			for(int i = 0; i < 9; ++i)
			{
			AgpdGridItem* pcsGridItem = m_pcsAgpmGrid->GetItem(pstQuickBeltGrid, m_nNumOfActiveQuickbelt, 0, i);
	...
	{
		if(!m_apstQuickBelt[0] || !m_apstQuickBelt[0]->m_stUserData.m_pvData)
			return FALSE;
...
BOOL AgcmUIMain::CBUpdateOnlineTime(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	if ( online >= 300 )
	{
		AgcChatManager::OnAddSystemMessage( ClientStr().GetStr(STI_RED_TEXT) );
	}
	else if ( online >= 180 )
	{
		_snprintf_s( buffer, 256, _TRUNCATE, ClientStr().GetStr(STI_YELLOW_TEXT), online / 60 );
		AgcChatManager::OnAddSystemMessage( buffer );
	}
	else if ( online >= 60 )
	{
		AgcChatManager::OnAddSystemMessage( ClientStr().GetStr(STI_GREEN_GREETING) );
...
BOOL AgcmUIMain::OnAdjustHPMPBarPosition( INT32 nLayer /*= 0 */ )
{
	AgcdUI* pUICharInfo = (AgcdUI*)m_pcsAgcmUIManager2->GetUI("UI_CharInfo");
	if(!pUICharInfo)
		return FALSE;

	AgcdUIControl* pUIControlBase = m_pcsAgcmUIManager2->GetControl(pUICharInfo, "CONTROL_CHARINFO_BASE");
	if(!pUIControlBase)
		return FALSE;

	AgcdUIControl* pUIControlHPMP = m_pcsAgcmUIManager2->GetControl(pUIControlBase, "CONTROL_CHARINFO_BASE_HPMP_TEXTURE");
	if(!pUIControlHPMP)
		return FALSE;

	static const int y	= pUIControlHPMP->m_pcsBase->y;
	int nAdjustedY		= y - (nLayer*52);

	pUIControlHPMP->m_pcsBase->MoveWindow(pUIControlHPMP->m_pcsBase->x, nAdjustedY);

	return TRUE;
}

INT32 AgcmUIMain::GetKeyCode()
{
...
UINT AgcmUIMain::GetCenterXPosOfUI( UINT nWidth )
{
	...
	float fHalfOfWidth = nWidth * 0.5f;
	float fHalfOfScreen = raster->width * 0.5f;

	return (UINT)(fHalfOfScreen - fHalfOfWidth);
}

UINT AgcmUIMain::GetYPosOfBelt( UINT nHeight /*= 0*/, UINT nLayer /*= 0 */ )
{
	RwRaster * raster = RwCameraGetRaster( g_pEngine->m_pCamera );
	...
	return (UINT)(raster->height - (nHeight*nLayer) - 63);
}

BOOL AgcmUIMain::CBChangedUseNewSlot( PVOID pData, PVOID pClass, PVOID pCustData )
{
	// 옵션상의 새로운 스킬 슬롯 관련 옵션을 건드릴 경우 본 콜백이 호출된다.
	// pData로 넘어오는 bool 값이 FALSE면 체크가 없는거고, TRUE면 체크가 있는 것
	AgcmUIMain* pThis	= (AgcmUIMain*)pClass;
	BOOL		bOption = (BOOL)pData;

	if(!pThis)
		return FALSE;

	// 일단 보이는 슬롯 초기화 시켜줌
	pThis->OnInitQuickBeltGrid();
