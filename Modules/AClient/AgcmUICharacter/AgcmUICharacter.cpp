...
, m_ulResurrectableTime(0)
, m_ulDisplayedTime(0)
, m_bDeathTimerFlag(FALSE)
, m_pcsAgcmShrine(NULL)
{
...
BOOL AgcmUICharacter::OnAddModule()
{
	...
	if( !m_pcsApmMap ||
		...
		!m_pcsAgpmItem || !m_pcsAgpmGrid || !m_pcsAgpmSkill || !m_pcsAgcmSkill ||
...
BOOL AgcmUICharacter::OnInit()
{
	...
	m_pcsAgpmShrineBattle		= (AgpmShrineBattle *) GetModule("AgpmShrineBattle");
	m_pcsAgcmUINotice			= (AgcmUINotice *)GetModule("AgcmUINotice");
	m_pcsAgcmShrine				= (AgcmShrine*)GetModule("AgcmShrine");
...
BOOL AgcmUICharacter::OnIdle(UINT32 ulClockCount)
{
	...
	if(pcsSelfCharacter)
	{
		if(m_bDeathTimerFlag)
		{
			m_ulResurrectableTime = ulClockCount + AGCM_UI_CHARACTER_RESURRECTION_DELAY_IN_SHRINE;
			m_ulDisplayedTime = ulClockCount - 1000;
			m_bDeathTimerFlag = FALSE;
		}

		if(m_ulResurrectableTime && m_pcsAgcmUINotice && m_pcsAgcmUIManager2)
		{
			CHAR	szBuffer[64];
			ZeroMemory(szBuffer, sizeof(CHAR) * 64);

			if(m_ulDisplayedTime + 1000 < ulClockCount)
			{
				sprintf(szBuffer, m_pcsAgcmUIManager2->GetUIMessage("Shrine_Resurrection_Delay"), (int)((m_ulResurrectableTime-ulClockCount)/1000)+1);
				m_pcsAgcmUINotice->AddMessage(szBuffer);
				m_ulDisplayedTime = ulClockCount;
			}

			if(m_ulResurrectableTime < ulClockCount)
			{
				m_ulResurrectableTime = 0;
				m_ulDisplayedTime = 0;
				m_pcsAgcmUIManager2->ThrowEvent(m_lEventOpenResurrectionUI);
			}
		}

		if ( pcsSelfCharacter->m_bIsTrasform || pcsSelfCharacter->m_bIsEvolution )
		{
			...
			{
				AgcChatManager::OnAddSystemMessage(m_pcsAgcmUIManager2->GetUIMessage(UI_MESSAGE_ID_TRANSFORM_RESTORE_NOTICE));
			...
			*plValue = lTemp;
			...
			if( pcsRegionTemplate && pcsRegionTemplate->nLevelLimit )
			{
				...
				if( nLevelLimit <= nLevelOriginal )
					pcsSourceControl->m_pcsBase->SetBlink( AcUIBase::TEXT_BLINK , 0xffff0000 , 1000 );
				else
					pcsSourceControl->m_pcsBase->SetBlink( AcUIBase::NONE_BLINK );
			}
			else
			{
				pcsSourceControl->m_pcsBase->SetBlink( AcUIBase::NONE_BLINK );
...
BOOL AgcmUICharacter::CBTargetCharName(PVOID pClass, PVOID pData, AgcdUIDataType eType, INT32 lID, CHAR *szDisplay, INT32 *plValue, AgcdUIControl *pcsSourceControl)
{
	...
	if( pThis->m_pcsAgcmBattleGround->IsOtherRace( pcsTargetCharacter ) ||
		...
		pThis->m_pcsAgcmShrine->IsOtherRace(pcsTargetCharacter))
	{
...
BOOL AgcmUICharacter::CharacterUpdateFactorMurdererPointCB(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	if (lDiff > 0)
	{
		...
		if (pszFormat)
		{
			...
			AgcChatManager::OnAddSystemMessage(szBuffer, 0xffcc00);
		}
	}
...
BOOL AgcmUICharacter::CharacterUpdateCriminalStatusCB(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	if (cOldStatus != pcsSelfCharacter->m_unCriminalStatus)
	{
		if (AGPDCHAR_CRIMINAL_STATUS_CRIMINAL_FLAGGED == pcsSelfCharacter->m_unCriminalStatus)
		{
			...
			if (psz)
				AgcChatManager::OnAddSystemMessage(psz, 0xFF0000);				
		}
		else
		{
			CHAR *psz = pThis->m_pcsAgcmUIManager2->GetUIMessage(UI_MESSAGE_ID_FREE_FIRST_ATTACKER);
			if (psz)
				AgcChatManager::OnAddSystemMessage(psz, 0x00FF00);
		}
	}
...
BOOL AgcmUICharacter::SetTargetCharacter(AgpdCharacter *pcsTargetCharacter)
{
	...
	if( m_pcsAgpmCharacter->IsPC( pcsTargetCharacter ) )
	{
		_UpdateTargetPlayerRaceAndClass( pcsTargetCharacter );
		 ...
		 //캐릭터고 선공, 악당이면 속성무시하고 뿌려준다. 쉬라인과 배틀그라운드가 아닐때만
		if( !m_pcsAgcmBattleGround->IsOtherRace( pcsTargetCharacter ) && !m_pcsAgcmShrine->IsOtherRace( pcsTargetCharacter ))
		{
...
BOOL AgcmUICharacter::CBUpdateActionStatus(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	if (pcsSelfCharacter == pcsCharacter)
	{
		if (pcsSelfCharacter->m_unActionStatus == AGPDCHAR_STATUS_DEAD)
		{
			...
			if(!pThis->m_pcsAgpmShrineBattle->IsInShrineRegionByIndex(pcsSelfCharacter))
				pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lEventOpenResurrectionUI);
			else
				pThis->m_bDeathTimerFlag = TRUE;
...
BOOL AgcmUICharacter::CBRefreshTargetName(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl)
{
	...
	if( pThis->m_pcsAgcmBattleGround->IsOtherRace( pThis->m_pcsTargetCharacter ) || 
		...
		pThis->m_pcsAgcmShrine->IsOtherRace(pThis->m_pcsTargetCharacter))
	{
		memset( szCharacterName, 0, 128 );
		for( unsigned int i=0; i<strlen(pThis->m_pcsTargetCharacter->m_szID); ++i )
			szCharacterName[i] = '*';
	...
	else
	{
		...
		{
			if(pThis->m_pcsAgpmCharacter->IsMonster(pThis->m_pcsTargetCharacter))
				bDisplayLevel = TRUE;
		}
...
BOOL AgcmUICharacter::CallBack_OnSelectPropertyAttack( void* pClass, void* pData1, void* pData2, void* pData3, void* pData4, void* pData5, ApBase* pBase, AgcdUIControl* pControl )
{
	...
	pcmUICharacter->OnSelectProperty( ePropertyType_Defense );
