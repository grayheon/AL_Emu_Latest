...
BOOL AgcmUIMailBox::CBAddMail(PVOID pData, PVOID pClass, PVOID pCustData)
	...
	if( pcmUIManager )
	{
		...
		{
			AgcChatManager::OnAddSystemMessage( pString );
		}
	}
...
#ifdef _AREA_GLOBAL_ // 글로벌에서는 메일박스 사용 블럭
	pThis->m_pAgcmUIManager2->ActionMessageOKDialog( ClientStr().GetStr( STI_DISABLE_AUCTION ) );
	AgcdUI * ui = pThis->m_pAgcmUIManager2->GetUI( "MailBox" );
	if( ui )
	{
		pThis->m_pAgcmUIManager2->CloseUI( ui );
	}
	return TRUE;
#endif
...
BOOL AgcmUIMailBox::CBMoveItemWrite(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl)
	...
	{
		if( !AgcmUIItem::IsCashItemByPPCard( pAgpdItem ) )
		{
