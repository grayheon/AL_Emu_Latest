...
BOOL AgcmUIRefinery::OnResult(INT32 eResult, INT32 lResultItemTID)
{
	...
	case AGPMREFINERY_RESULT_FAIL :
		...
		AgcChatManager::OnAddSystemMessage(GetMessageTxt(m_eCategory, AGCMUIREFINERY_MESSAGE_RESULT_FAIL_INSUFFICIENT_ITEM));
		break;

	case AGPMREFINERY_RESULT_FAIL_DIFFERENT_ITEM :
		m_eNotifyMsg = AGCMUIREFINERY_MESSAGE_RESULT_FAIL_DIFFRENT_ITEM;
		AgcChatManager::OnAddSystemMessage(GetMessageTxt(m_eCategory, AGCMUIREFINERY_MESSAGE_RESULT_FAIL_DIFFRENT_ITEM));
		break;
	...
	case AGPMREFINERY_RESULT_FAIL_INSUFFICIENT_MONEY :
		...
		AgcChatManager::OnAddSystemMessage(GetMessageTxt(m_eCategory, AGCMUIREFINERY_MESSAGE_RESULT_FAIL_INSUFFICIENT_MONEY));
		break;

	case AGPMREFINERY_RESULT_FAIL_FULL_INVENTORY :
		m_eNotifyMsg = AGCMUIREFINERY_MESSAGE_RESULT_FAIL_FULL_INVENTORY;
		AgcChatManager::OnAddSystemMessage(GetMessageTxt(m_eCategory, AGCMUIREFINERY_MESSAGE_RESULT_FAIL_FULL_INVENTORY));
		break;
...
void AgcmUIRefinery::OnSuccess(INT32 lResultItemTID)
{
	...
	if (psz)
	{
		...
		AgcChatManager::OnAddSystemMessage(sz);
	}
...
BOOL AgcmUIRefinery::OnMoveItem(AgpdItem *pAgpdItem, INT16 nLayer, INT16 nRow, INT16 nColumn)
{
	...
	if (m_pAgpmGrid->IsFullGrid(&m_AgpdGridSource))
	{
		...
		AgcChatManager::OnAddSystemMessage(GetMessageTxt(m_eCategory, AGCMUIREFINERY_MESSAGE_RESULT_FAIL_UNABLE_TO_LOAD));
		return FALSE;
	}
