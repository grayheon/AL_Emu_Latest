...
AgcmWater::AgcmWater()
{
	...
	memset(m_stWaterFInfo, 0, sizeof(m_stWaterFInfo));
	memset(m_stWaveFInfo, 0, sizeof(m_stWaveFInfo));
	memset(m_stHQBumpFInfo, 0, sizeof(m_stHQBumpFInfo));
	memset(m_stHQFresnelFInfo, 0, sizeof(m_stHQFresnelFInfo));

	//#pragma omp parallel for schedule(guided)
	for(int i=0;i<WMAP_HWATER_STATUS_NUM;++i)
	{
		// Default 값으로 Set
...
BOOL AgcmWater::OnIdle( UINT32 ulClockCount )
{
	...
	for(int i=0;i<m_iMaxWaterIDUsed;++i)
	{
...
void	AgcmWater::ChangeHWaterMode(BOOL bOn)
{
	...
	else 
	{
		if(m_bEnableHWater && !m_pReflectionCamera)			// m_pReflectionCamera 이 NULL일 경우 생성
		{
			...
			RwTextureSetFilterMode(m_pReflectionTexture,rwFILTERNEAREST );
...
void	AgcmWater::SetShaderCommonConstant()
{
	...
	D3DXVECTOR4 c8( ltm->pos.x, ltm->pos.y, ltm->pos.z, 1.0f);	
	//@} Jaewon
	D3DXVECTOR4 c9( -pAt->x,-pAt->y,-pAt->z,1.0f);
	D3DXVECTOR4 c10( 0.9855f, 0.00001f, 0.0f, 0.0f );
	//D3DXVECTOR4 c10( 0.9855f, m_stHWaterStatus[0].DirY3, 0.0f, 0.0f );
	D3DXVECTOR4 c16( (float)m_fTime*0.75f, sinf(m_fTime)*10.0f, 0.0f, 0.0f);
...
BOOL	AgcmWater::CB_HWUpdate ( PVOID pData, PVOID pClass, PVOID pCustData )
{
	...
	if(!pThis->m_bReflectionUpdate)
	{
		...
		pThis->m_matReflectionTrafoMatrix._42 = pHW->Height;
