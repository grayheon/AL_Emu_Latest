...
BOOL AgcmUIItem::OpenToolTip(AgpdItem *pcsItem, INT32 lX, INT32 lY, BOOL bShowPrice, BOOL bIsNPCTradeGrid, BOOL bEquipItemView)
{
	...
	if(!reservedTooltipItem_.empty())
	{
		pcsItem = reservedTooltipItem_.front();
		reservedTooltipItem_.pop_front();
	}

	AcUIToolTip*	pTempTooltip	=	m_pItemTooltip;
	...
	AgpdItemTemplate*	itemTemplate = (AgpdItemTemplate *)(pcsItem->m_pcsItemTemplate);

	if(itemTemplate->m_nType == AGPMITEM_TYPE_EQUIP ) // 착용 아이템이면
	{
		INT	lRestrictRace	=	0;
		...
		secondTooltipItem	=	m_pcsAgpmItem->GetEquipSlotItem( pcsSelfCharacter , ((AgpdItemTemplateEquip *)pcsItem->m_pcsItemTemplate)->m_nPart );

		// 같은 아이템 이라면 걍 스킵
		if( secondTooltipItem && secondTooltipItem->GetID() == pcsItem->GetID() )
		{
			secondTooltipItem = 0;
		}
		else
...
// 	}
	...
	if( secondTooltipItem )
	{
		...
		m_csItemToolTipSub.MoveWindow( pTempTooltip->x + pTempTooltip->w ,  pTempTooltip->y , m_csItemToolTipSub.w , m_csItemToolTipSub.h);
		...
		RwV2d	vPos		=	pTempTooltip->GetRenderPos( &unStatus );
		...
		if( pTempTooltip->pParent )
		{
			parentW = pTempTooltip->pParent->w;
			parentH = pTempTooltip->pParent->h;
		}

		// 착용툴팁을 오른쪽에 공간이 부족해서 표현 할 수 없다면 왼쪽으로 옮긴다.
		if( unStatus & ACUITOOLTIP_LEFT_TOOLTIP )
		{
			nLastPosX = posX - m_csItemToolTipSub.w;

			if( posX - m_csItemToolTipSub.w < 0 ) // 왼쪽으로 옮겨도 짤린다면 아래로 내린다.
			{
				if( posY + pTempTooltip->h + m_csItemToolTipSub.h > parentH ) // 아래로 내려도 짤린다면 위로 올린다.
				{
					nLastPosY = posY - m_csItemToolTipSub.h + pTempTooltip->h;
				}
				else
				{
					nLastPosY = posY + pTempTooltip->h;
				}
			}
			else if( m_csItemToolTipSub.y + m_csItemToolTipSub.h > parentH ) // 착용아이템 툴팁이 화면 밑으로 내려간다면 아래쪽으로 정렬해준다.
			{
				if(pTempTooltip->y + pTempTooltip->h < parentH)
					nLastPosY = pTempTooltip->y + pTempTooltip->h - m_csItemToolTipSub.h;
			}
			else
			{
				nLastPosY = posY;
		...
		{
			if( m_csItemToolTipSub.x + m_csItemToolTipSub.w > parentW ) // 짤린다면 아래로 내린다.
			{
				if( pTempTooltip->y + pTempTooltip->h + m_csItemToolTipSub.h > parentH ) // 아래로 내려도 짤린다면 위로 올린다.
				{
					if( posX < pTempTooltip->x )
						posX = pTempTooltip->x;

					if( posY - m_csItemToolTipSub.h < 0 ) // 위로 올려도 짤린다면 왼쪽으로 붙인다
					{
						nLastPosX = posX - m_csItemToolTipSub.w;
						nLastPosY = (posY + pTempTooltip->h - m_csItemToolTipSub.h < 0)?0:posY + pTempTooltip->h - m_csItemToolTipSub.h;
					}
					else
					{
						nLastPosX = posX;
			...
			else if( m_csItemToolTipSub.y + m_csItemToolTipSub.h > parentH ) // 착용아이템 툴팁이 화면 밑으로 내려간다면 아래쪽으로 정렬해준다.
			{
				...
				if(pTempTooltip->y + pTempTooltip->h < parentH)
				{
					nLastPosY = posY + pTempTooltip->h - m_csItemToolTipSub.h;
				}
			}
			else
			{
...
//			m_pItemTooltip->AddString( strTemp, 12, AGCMUIITEM_TOOLTIP_COLOR_NORMAL )		;
		...
		{
			bAddString = _SetItemToolTipDetailHeroicDamage( m_pcsAgcmUIManager2->GetUIMessage("UiMsg_HeroicDamage"), pcsItem, bShowPrice, bIsNPCTradeGrid );
		}
		else if (strcmp(m_aszTooptipTitle[pcsItemTooltip->m_alTooltipIndex[i]], AGCDUIITEM_TOOLTIP_NAME_HEROIC_DEFENSE) == 0)
		{
			bAddString = _SetItemToolTipDetailHeroicDefense( m_pcsAgcmUIManager2->GetUIMessage("UiMsg_HeroicDefense"), pcsItem, bShowPrice, bIsNPCTradeGrid );
		}
...
BOOL AgcmUIItem::SetItemToolTipPrice(AgpdItem *pcsItem, BOOL bShowPrice, BOOL bIsNPCTradeGrid)
{
	...
	char				strTemp[256];
	INT32				lItemPrice		=   0;
	INT32				lItemCharisma	=	0;
	INT32				lItemShrineCoin	=	0;
	...
	if (bIsNPCTradeGrid)
	{
		if( pcsItem->m_nCount > 0)
		{
			...
			lItemCharisma	=	(INT32)(m_pcsAgpmItem->GetCharismaPrice( pcsItem ) * pcsItem->m_nCount);
			lItemShrineCoin	=	(INT32)(m_pcsAgpmItem->GetShrineCoinPrice( pcsItem ) * pcsItem->m_nCount);
		}
		else
		{
			lItemPrice 		= (INT32)m_pcsAgpmItem->GetNPCPrice( pcsItem );
			lItemCharisma	= (INT32)m_pcsAgpmItem->GetCharismaPrice( pcsItem );
			lItemShrineCoin	= (INT32)m_pcsAgpmItem->GetShrineCoinPrice( pcsItem );
		}
		...
		sellTypeStr = m_pcsAgcmUIManager2->GetUIMessage("Buy_Price"); // 구매 가격
	}
	else
	{
		if( pcsItem->m_nCount > 0)
		...
		sellTypeStr = ClientStr().GetStr(STI_SELL_PRICE); // 판매 가격
	}
	
	if( lItemPrice || lItemCharisma || lItemShrineCoin )
	{
		sprintf( strTemp, "%s : ", sellTypeStr );
		m_pItemTooltip->AddString( strTemp, 12, dwColor );

		INT		nOffsetX	=	m_pcsAgcmFont->GetTextExtent( m_pItemTooltip->m_lFontType , strTemp , strlen(strTemp) );

		nOffsetX	+=	12;

		if( lItemPrice )
		{
			sprintf_s( strTemp , 256 , "%d %s" , lItemPrice, m_pcsAgcmUIManager2->GetUIMessage(UI_MESSAGE_ID_ITEM_MONEY_NAME) );
			m_pItemTooltip->AddString( strTemp, nOffsetX , dwColor );
			m_pItemTooltip->AddNewLine( 14 );
		...
		if( lItemShrineCoin )
		{
			sprintf_s( strTemp , 256 , "%d %s" , lItemShrineCoin, m_pcsAgcmUIManager2->GetUIMessage( "Shrine_Coin") );
			m_pItemTooltip->AddString( strTemp, nOffsetX , dwColor );
			m_pItemTooltip->AddNewLine( 14 );
		}
		...
		if( lItemCharisma )
		{
			sprintf_s( strTemp , 256 , "%d %s" , lItemCharisma, m_pcsAgcmUIManager2->GetUIMessage( "Charisma_Point" ) );
			m_pItemTooltip->AddString( strTemp, nOffsetX , dwColor );
			m_pItemTooltip->AddNewLine( 14 );
...
BOOL AgcmUIItem::SetItemToolTipCashInfo( AgpdItem* ppdItem, AgcdUIItemTooltip* pcdItemTooltip )
{
	...
	m_csItemToolTip.AddString( pText, 12, GetTooltipColor( AGCMUIITEM_TOOLTIP_COLOR_BOUND_TYPE ) );
	return TRUE;
}
#endif
// 2008.06.16. steeple
...
VOID AgcmUIItem::CalcTooltipPos( AcUIToolTip*	pTooltip )
{
	if( !pTooltip )
		return;

	INT	nX	=	(m_pcsAgcmUIManager2->m_lWindowWidth / 2) - (pTooltip->w/2);
	INT	nY	=	(m_pcsAgcmUIManager2->m_lWindowHeight / 2) - (pTooltip->h/2);

	pTooltip->MoveWindow( nX , nY , pTooltip->w , pTooltip->h );
}
