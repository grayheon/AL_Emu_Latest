...
, m_eChargingBagType(0)
{
	...
	m_pcsChargingInventoryALayerIndex = NULL;
	m_pcsChargingInventoryBLayerIndex = NULL;
	...
	m_lChargingInventoryALayerIndex = 0;
	m_lChargingInventoryBLayerIndex = 0;
	//m_lBankLayerIndex	= 0;
	...
	m_pcsChargingInventoryAGrid = NULL;
	m_pcsChargingInventoryBGrid = NULL;
	...
	m_lChargingInventoryAUIOpen = 0;
	m_lChargingInventoryAUIClose = 0;
	m_bUIOpenChargingInventoryB = FALSE;
	m_lChargingInventoryBUIOpen = 0;
	m_lChargingInventoryBUIClose = 0;
	...
	m_lNPCTradeMSGNotEnoughCharisma		=	0;
	m_lNPCTradeMSGNotEnoughShrineCoin	=	0;
	...
	m_pcsUIChargingInventoryA = NULL;
	m_pcsUIChargingInventoryB = NULL;
	m_pBtnAutoPickUp = NULL;

	SetTooltipUI(&m_csItemToolTip);
}
...
BOOL AgcmUIItem::OnAddModule()
{
	...
	m_pcsAgpmItem->SetCallbackItemInformationResult( CBTooltipRegisterItem , this );
	...
	// ChargingInventory
	if( !m_pcsAgpmItem->SetCallbackUIUpdateChargingInventory( CBUIUpdateChargingInventoryA, this ) )
		return FALSE;
	if( !m_pcsAgpmItem->SetCallbackUIUpdateChargingInventory( CBUIUpdateChargingInventoryB, this ) )
		return FALSE;
	...
	if(!m_pcsAgcmEventNPCTrade->SetCallbackNotEnoughCharisma( CBUINPCTradeMSGNotEnoughCharisma, this ) )
		return FALSE;
	if(!m_pcsAgcmEventNPCTrade->SetCallbackNotEnoughShrineCoin( CBUINPCTradeMSGNotEnoughShrineCoin, this ) )
		return FALSE;
...
BOOL AgcmUIItem::OnInit()
{
	...
	m_pcsAgcmFont				=	dynamic_cast< AgcmFont* >( GetModule( "AgcmFont" ) );

	if ( m_pcsAgcmUIManager2 != NULL )
	{
		m_csItemToolTip.m_Property.bTopmost = TRUE;
		m_csItemToolTipSub.m_Property.bTopmost = TRUE;
		m_pcsAgcmUIManager2->AddWindow( (AgcWindow*)(&m_csItemToolTip ) );
		m_pcsAgcmUIManager2->AddWindow( (AgcWindow*)(&m_csItemToolTipSub ) );
...
BOOL AgcmUIItem::OnIdle(UINT32 ulClockCount)
{
	...
	if(m_pcsAgcmCharacter->GetSelfCharacter())
	{
		CheckItemDurability(m_pcsAgcmCharacter->GetSelfCharacter());
	}
...
BOOL AgcmUIItem::AddEvent()
{
	...
	if( m_lInventoryMSGGetItem < 0 )
		return FALSE;
	m_lInventoryMSGDropItem = m_pcsAgcmUIManager2->AddEvent( "INV_MSG_Drop_ITEM", CBUIInventoryDestroyItem, this );
	...
	m_lChargingInventoryAUIOpen = m_pcsAgcmUIManager2->AddEvent("ExpandedInvA_OpenEvent");
	if( m_lChargingInventoryAUIOpen < 0 )
		return FALSE;
	m_lChargingInventoryAUIClose = m_pcsAgcmUIManager2->AddEvent("ExpandedInvA_CloseEvent");
	if( m_lChargingInventoryAUIClose < 0 )
		return FALSE;
	m_lChargingInventoryBUIOpen = m_pcsAgcmUIManager2->AddEvent("ExpandedInvB_OpenEvent");
	if( m_lChargingInventoryBUIOpen < 0 )
		return FALSE;
	m_lChargingInventoryBUIClose = m_pcsAgcmUIManager2->AddEvent("ExpandedInvB_CloseEvent");
	if( m_lChargingInventoryBUIClose < 0 )
		return FALSE;
	...
	m_lNPCTradeMSGNotEnoughCharisma = m_pcsAgcmUIManager2->AddEvent( "NPCTMSG_NotEnoughCharisma" );
	if( m_lNPCTradeMSGNotEnoughCharisma < 0 )
		return FALSE;

	m_lNPCTradeMSGNotEnoughShrineCoin = m_pcsAgcmUIManager2->AddEvent( "NPCTMSG_NotEnoughShrineCoin" );
	if( m_lNPCTradeMSGNotEnoughShrineCoin < 0 )
		return FALSE;
	...
	m_lShrineNPCTradeBuyConfirmOpen		=	m_pcsAgcmUIManager2->AddEvent( "Shrine_ItemBuyConfirmOpen_Event" );
	if( m_lShrineNPCTradeBuyConfirmOpen < 0 )
		return FALSE;

	m_lShrineNPCTradeBuyConfirmClose	=	m_pcsAgcmUIManager2->AddEvent( "Shrine_ItemBuyConfirmClose_Event" );
	if( m_lShrineNPCTradeBuyConfirmClose < 0 )
		return FALSE;
...
BOOL AgcmUIItem::AddFunction()
{
	...
	// Charging Inventory (확장 인벤토리)
	if (!m_pcsAgcmUIManager2->AddFunction(this, "ExpandedInventoryA_MoveItem", CBChargingInventoryAMoveItem, 1 ))
		return FALSE;

	if (!m_pcsAgcmUIManager2->AddFunction(this, "ExpandedInventoryA_MoveEndItem", CBChargingInventoryAMoveEndItem, 1 ))
		return FALSE;

	if (!m_pcsAgcmUIManager2->AddFunction(this, "ExpandedInventory_UIUpdate", CBChargingInventoryUIUpdate, 1 ))
		return FALSE;

	if (!m_pcsAgcmUIManager2->AddFunction(this, "ExpandedInventoryA_UIUpdateLayer", CBChargingInventoryAOpenInvenLayer, 1 ) )
		return FALSE;

	if (!m_pcsAgcmUIManager2->AddFunction(this, "ExpandedInventoryA_DragDropItem", CBDragDropItemToChargingInventoryA, 1 ) )
		return FALSE;

	if (!m_pcsAgcmUIManager2->AddFunction(this, "ExpandedInventoryB_MoveItem", CBChargingInventoryBMoveItem, 1 ))
		return FALSE;

	if (!m_pcsAgcmUIManager2->AddFunction(this, "ExpandedInventoryB_MoveEndItem", CBChargingInventoryBMoveEndItem, 1 ))
		return FALSE;

	if (!m_pcsAgcmUIManager2->AddFunction(this, "ExpandedInventoryB_UIUpdateLayer", CBChargingInventoryBOpenInvenLayer, 1 ) )
		return FALSE;

	if (!m_pcsAgcmUIManager2->AddFunction(this, "ExpandedInventoryB_DragDropItem", CBDragDropItemToChargingInventoryB, 1 ) )
		return FALSE;
...
BOOL AgcmUIItem::AddUserData()
{
	...
	*/
	...
	if (!(m_pcsSubInventoryLayerIndex = m_pcsAgcmUIManager2->AddUserData("SubInventoryLayerIndex", &m_lSubInventoryLayerIndex, sizeof(INT32), 1, AGCDUI_USERDATA_TYPE_INT32)))
		return FALSE;
	if (!(m_pcsChargingInventoryALayerIndex = m_pcsAgcmUIManager2->AddUserData("ExpandedInventoryALayerIndex", &m_lChargingInventoryALayerIndex, sizeof(INT32), 1, AGCDUI_USERDATA_TYPE_INT32)))
		return FALSE;
	if (!(m_pcsChargingInventoryBLayerIndex = m_pcsAgcmUIManager2->AddUserData("ExpandedInventoryBLayerIndex", &m_lChargingInventoryBLayerIndex, sizeof(INT32), 1, AGCDUI_USERDATA_TYPE_INT32)))
		return FALSE;
	/*
	...
	*/
	...
	if (!(m_pcsChargingInventoryA = m_pcsAgcmUIManager2->AddUserData("ExpandedInventoryAGrid", m_pcsChargingInventoryAGrid, sizeof(AgpdGrid), 1, AGCDUI_USERDATA_TYPE_GRID)))
		return FALSE;
	if (!(m_pcsChargingInventoryB = m_pcsAgcmUIManager2->AddUserData("ExpandedInventoryBGrid", m_pcsChargingInventoryBGrid, sizeof(AgpdGrid), 1, AGCDUI_USERDATA_TYPE_GRID)))
		return FALSE;
...
BOOL AgcmUIItem::SetSelfCharacterCB(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	pcsAgpdItemADChar = pThis->m_pcsAgpmItem->GetADCharacter( pcsAgpdCharacter );

	if( pcsAgpdItemADChar != NULL )
	{
		pThis->m_pcsInventoryGrid = &pcsAgpdItemADChar->m_csInventoryGrid;
		pThis->m_pcsInventoryQuestGrid = &pcsAgpdItemADChar->m_csQuestGrid;
		pThis->m_pcsAgpmCharacter->GetMoney(pcsAgpdCharacter, &pThis->m_llInventoryMoney);
		pThis->m_pcsEquipGrid = &pcsAgpdItemADChar->m_csEquipGrid;
		//pcsThis->m_pcsBankGrid = &pcsAgpdItemADChar->m_csBankGrid;

		pThis->m_pcsSalesBoxGrid = &pcsAgpdItemADChar->m_csSalesBoxGrid;
		//pcsThis->m_pcsNPCTradeGrid = ?? <-요놈은 어떻게 방법하지? 우울타~
		pThis->m_pcsPrivateTradeGrid = &pcsAgpdItemADChar->m_csTradeGrid;
		pThis->m_pcsPrivateTradeClientGrid = &pcsAgpdItemADChar->m_csClientTradeGrid;

		pThis->m_pcsSubInventoryGrid = &pcsAgpdItemADChar->m_csSubInventoryGrid;
		pThis->m_pcsChargingInventoryAGrid = &pcsAgpdItemADChar->m_csChargingInventoryGrid;
		pThis->m_pcsChargingInventoryBGrid = &pcsAgpdItemADChar->m_csChargingInventoryGrid;

		pThis->m_pcsInventory->m_stUserData.m_pvData				= pThis->m_pcsInventoryGrid;
		pThis->m_pcsInventoryQuest->m_stUserData.m_pvData			= pThis->m_pcsInventoryQuestGrid;
		pThis->m_pcsEquip->m_stUserData.m_pvData					= pThis->m_pcsEquipGrid;
		//pcsThis->m_pcsBank->m_stUserData.m_pvData					= pcsThis->m_pcsBankGrid;
		pThis->m_pcsSalesBox->m_stUserData.m_pvData				= pThis->m_pcsSalesBoxGrid;
		pThis->m_pcsPrivateTrade->m_stUserData.m_pvData			= pThis->m_pcsPrivateTradeGrid;
		pThis->m_pcsPrivateTradeEx->m_stUserData.m_pvData			= &pThis->m_PrivateTradeGridEx;
		pThis->m_pcsPrivateTradeClientEx->m_stUserData.m_pvData	= &pThis->m_PrivateTradeClientGridEx;
		pThis->m_pcsPrivateTradeClient->m_stUserData.m_pvData		= pThis->m_pcsPrivateTradeClientGrid;
		pThis->m_pcsSubInventory->m_stUserData.m_pvData			= pThis->m_pcsSubInventoryGrid;
		pThis->m_pcsChargingInventoryA->m_stUserData.m_pvData	= pThis->m_pcsChargingInventoryAGrid;
		pThis->m_pcsChargingInventoryB->m_stUserData.m_pvData	= pThis->m_pcsChargingInventoryBGrid;
		...
		pThis->m_pcsAgcmUIManager2->SetUserDataRefresh(pThis->m_pcsInventoryMoney);
		...
		pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lEventEquipStatusBodyOff);
		pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lEventEquipStatusHeadOff);
		pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lEventEquipStatusHandOff);
		pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lEventEquipStatusLegsOff);
		pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lEventEquipStatusFootOff);
		pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lEventEquipStatusShieldOff);
		pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lEventEquipStatusWeaponOff);

		pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lEventEquipStatusClose);

		for (int i = AGPMITEM_PART_BODY; i < AGPMITEM_PART_NUM; ++i)
		{
			AgpdItem	*pcsItem	= pThis->m_pcsAgpmItem->GetEquipSlotItem(pcsAgpdCharacter, (AgpmItemPart) i);
			if (pcsItem)
				pThis->CBEquipItem(pcsItem, pThis, NULL);
		}

		pThis->UpdateEquipSlotGrid();

		// 리버스, 레저력선 오브 확인한다. 2006.08.21. steeple
		pThis->CBUpdateActionStatus(pcsAgpdCharacter, pThis, NULL);
	}
	else
	{
		return FALSE;
...
BOOL AgcmUIItem::ReleaseSelfCharacterCB(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	*/
	...
	pcsThis->m_pcsChargingInventoryAGrid	= NULL;
	pcsThis->m_pcsChargingInventoryBGrid	= NULL;
	...
	pcsThis->m_pcsChargingInventoryA->m_stUserData.m_pvData	= NULL;
	pcsThis->m_pcsChargingInventoryB->m_stUserData.m_pvData	= NULL;
	...
	pcsThis->m_pcsAgcmUIManager2->SetUserDataRefresh(pcsThis->m_pcsChargingInventoryA);
	pcsThis->m_pcsAgcmUIManager2->SetUserDataRefresh(pcsThis->m_pcsChargingInventoryB);
...
BOOL AgcmUIItem::CBUseItemResult(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	switch (eResult)
	{
		case AGPMITEM_USE_RESULT_FAILED_BY_SIEGEWAR:
			{
			AgcChatManager::OnAddSystemMessage(pThis->m_pcsAgcmUIManager2->GetUIMessage(UI_MESSAGE_ID_USE_ITEM_RESULT_FAILED_BY_SIEGEWAR));
			}
			break;

		case AGPMITEM_USE_RESULT_CANNOT_TRANSFORM_CREATURE:
			{
			AgcChatManager::OnAddSystemMessage(pThis->m_pcsAgcmUIManager2->GetUIMessage(UI_MESSAGE_ID_USE_ITEM_RESULT_CANNOT_TRANSFORM_CREATURE));
			}
			break;
		...
		case AGPMITEM_USE_RESULT_FAILED_REVERSEORB:
			...
			break;
		case AGPMITEM_USE_RESULT_CANNOT_USE_TRANSFORM_POTION:
			{
				AgcChatManager::OnAddSystemMessage(pThis->m_pcsAgcmUIManager2->GetUIMessage(UI_MESSAGE_ID_USE_ITEM_RESULT_CANNOT_USE_TRANSFORM_POTION));
			}
			break;
	}
...
BOOL AgcmUIItem::CBUIUpdateChargingInventoryA(PVOID pData, PVOID pClass, PVOID pCustData)
{
	AgcmUIItem *pcsThis = (AgcmUIItem *) pClass;
	...
	if( pcsThis != NULL )
	{
		if( pcsThis->m_pcsChargingInventoryA != NULL )
		{
			pcsThis->m_pcsAgcmUIManager2->SetUserDataRefresh(pcsThis->m_pcsChargingInventoryA);
...
BOOL AgcmUIItem::CBUIUpdateChargingInventoryB(PVOID pData, PVOID pClass, PVOID pCustData)
{
	AgcmUIItem *pcsThis = (AgcmUIItem *) pClass;
	...
	if( pcsThis != NULL )
	{
		if( pcsThis->m_pcsChargingInventoryB != NULL )
		{
			pcsThis->m_pcsAgcmUIManager2->SetUserDataRefresh(pcsThis->m_pcsChargingInventoryB);
...
BOOL AgcmUIItem::CBRemoveItemFromInventory(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	{
		if(eNewStatus != AGPDITEM_STATUS_EQUIP)
...
BOOL AgcmUIItem::CheckItemDurability( AgpdCharacter * pcsCharacter)
{
	...
	INT32 nOpen = 0;
	if( m_pcsAgpmFactors->GetRace( &m_pcsAgcmCharacter->GetSelfCharacter()->m_csFactor ) == AURACE_TYPE_DRAGONSCION )
		nOpen += IsItemPartDurable( pcsCharacter, 20, m_lEventEquipStatusWeaponOn, m_lEventEquipStatusShieldOff, AGPMITEM_PART_HAND_RIGHT );	//방패나 왼손무기(양손잡이)
	else
...
BOOL AgcmUIItem::CBCheckUseItem(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	if (pcsTemplate->m_nType == AGPMITEM_TYPE_USABLE)
	{
		...
		{
			AgcChatManager::OnAddSystemMessage(pThis->m_pcsAgcmUIManager2->GetUIMessage(UI_MESSAGE_ID_INVALID_USE_AREA));
			return FALSE;
		}
		...
		if (NULL != pcsCharacter)
		{
			if ( pcsCharacter->m_bRidable || pcsCharacter->m_bIsTrasform)
			{
				...
				{
					AgcChatManager::OnAddSystemMessage(ClientStr().GetStr(STI_CANT_USE_MORPH));
					return FALSE;
				}
					...
					!pThis->m_pcsAgpmArchlord->IsArchlord(pcsCharacter->m_szID))
				{
					...
					{
						AgcChatManager::OnAddSystemMessage(ClientStr().GetStr(STI_CANT_USE_ATTACKP));
						return FALSE;
					}
					...
					{
						AgcChatManager::OnAddSystemMessage(ClientStr().GetStr(STI_CANT_USE_MOVEP));
						return FALSE;
					}
				}
			}
		...
		if (((AgpdItemTemplateUsable *) pcsTemplate)->m_nUsableItemType == AGPMITEM_USABLE_TYPE_TRANSFORM)
		{
			...
			if (pcsItem->m_pcsCharacter && pcsItem->m_pcsCharacter->m_bIsTrasform )
			{
				// UI 상에 뿌려준다.
				AgcChatManager::OnAddSystemMessage(pThis->m_pcsAgcmUIManager2->GetUIMessage(UI_MESSAGE_ID_ITEM_ALREADY_USE_TRANSFORM));
		...
		else if (((AgpdItemTemplateUsable *) pcsTemplate)->m_nUsableItemType == AGPMITEM_USABLE_TYPE_TELEPORT_SCROLL)
		{
			...
			{
				AgcChatManager::OnAddSystemMessage(pThis->m_pcsAgcmUIManager2->GetUIMessage(UI_MESSAGE_ID_ITEM_TRYUSETELEPORTSCROLLOnDead));
				return FALSE;
			}
			...
			{
				AgcChatManager::OnAddSystemMessage(pThis->m_pcsAgcmUIManager2->GetUIMessage(UI_MESSAGE_ID_ITEM_TRYUSETELEPORTSCROLLOnTransfrom));
				return FALSE;
			}
			...
			{
				AgcChatManager::OnAddSystemMessage(pThis->m_pcsAgcmUIManager2->GetUIMessage(UI_MESSAGE_ID_ITEM_TRYUSETELEPORTSCROLLOnCombat));
				return FALSE;
			}

			// 베틀 스퀘어 안에 있는지 체크해본다. 안에 있으면 사용 못한다. -> 보호귀환주문서는 사용가능으로 변경
			if (((AgpdItemTemplateUsableTeleportScroll *) pcsItem->m_pcsItemTemplate)->m_eTeleportScrollType != AGPMITEM_USABLE_TELEPORT_SCROLL_GO_TOWN_NOW &&
				pThis->m_pcsAgpmCharacter->IsInPvPDanger(pThis->m_pcsAgcmCharacter->GetSelfCharacter()))
			{
				// UI 상에 뿌려준다.
				AgcChatManager::OnAddSystemMessage(pThis->m_pcsAgcmUIManager2->GetUIMessage(UI_MESSAGE_ID_ITEM_TRYUSETELEPORTSCROLLINPVP));
		...
		else if (((AgpdItemTemplateUsable *) pcsTemplate)->m_nUsableItemType == AGPMITEM_USABLE_TYPE_REVERSE_ORB)
		{
			...
			// 단 배틀그라운드는 예외로 한다.(글로벌 한정) - Bischoff 2011-09-06
			AgcmBattleGround* pcsBattleGround = (AgcmBattleGround*)g_pEngine->GetModule("AgcmBattleGround");
			BOOL bIsInBattleGround = FALSE;
			
			if( pcsBattleGround && pcsBattleGround->IsBattleGournd(pcsItem->m_pcsCharacter) )
				bIsInBattleGround	= TRUE;
			...
			// 공성지역에서는 모든 부활아이템 사용 금지
			AgpmSiegeWar* pcsSiegeWar = (AgpmSiegeWar*)g_pEngine->GetModule("AgpmSiegeWar");
			if(pcsSiegeWar && pcsSiegeWar->GetSiegeWarInfo(pcsItem->m_pcsCharacter))
				return FALSE;
		}

		// 상자 열쇠 확인
		else if(((AgpdItemTemplateUsable *) pcsTemplate)->m_nUsableItemType == AGPMITEM_USABLE_TYPE_LOTTERY_BOX)
		{
				...
				|| pcsItem->m_pcsItemTemplate->m_nSubType == AGPMITEM_USABLE_LOTTERY_TYPE_PLATINUM)
			{
				...
				if (pcsGridItemLotteryKey == NULL)
				{
					// UI 상에 뿌려준다.
					if (pcsItem->m_pcsItemTemplate->m_nSubType == AGPMITEM_USABLE_LOTTERY_TYPE_GOLD)
						AgcChatManager::OnAddSystemMessage(pThis->m_pcsAgcmUIManager2->GetUIMessage(UI_MESSAGE_LOTTERYBOX_NEEDS_GOLD_KEY));
					else if (pcsItem->m_pcsItemTemplate->m_nSubType == AGPMITEM_USABLE_LOTTERY_TYPE_SILVER)
						AgcChatManager::OnAddSystemMessage(pThis->m_pcsAgcmUIManager2->GetUIMessage(UI_MESSAGE_LOTTERYBOX_NEEDS_SILVER_KEY));
					else if (pcsItem->m_pcsItemTemplate->m_nSubType == AGPMITEM_USABLE_LOTTERY_TYPE_BRONZE)
						AgcChatManager::OnAddSystemMessage(pThis->m_pcsAgcmUIManager2->GetUIMessage(UI_MESSAGE_LOTTERYBOX_NEEDS_BRONZE_KEY));
					else if (pcsItem->m_pcsItemTemplate->m_nSubType == AGPMITEM_USABLE_LOTTERY_TYPE_PLATINUM)
						AgcChatManager::OnAddSystemMessage(pThis->m_pcsAgcmUIManager2->GetUIMessage(UI_MESSAGE_LOTTERYBOX_NEEDS_PLATINUM_KEY));
		...
		else if (((AgpdItemTemplateUsable *) pcsTemplate)->m_nUsableItemType == AGPMITEM_USABLE_TYPE_SKILL_INIT)
		{
			...
			{
				AgcChatManager::OnAddSystemMessage(pThis->m_pcsAgcmUIManager2->GetUIMessage(UI_MESSAGE_SKILL_INITIALIZE_NO_OWN_SKILL));
				return FALSE;
			}
			...
			if (pcsItemTemplate)
			{
				...
				else
				{
					if (pcsItem)
					{
						...
						// Item Name
						dwColor |= 0xff000000;
						sprintf(szDisplay, "<H%x>%s", dwColor, ((AgpdItemTemplate *) pcsItem->m_pcsItemTemplate)->m_szName);
						...
						sprintf(szDisplay + strlen(szDisplay), " <Hffffff00>%s", ClientStr().GetStr(STI_ITEM_POST));
...
BOOL AgcmUIItem::CBUpdateActionStatus(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	if ( pcsSelfCharacter && pcsSelfCharacter == pcsCharacter )
	{
		if ( pcsSelfCharacter->m_unActionStatus == AGPDCHAR_STATUS_DEAD )
		{
			...
			if( pcsReverseOrb )
			{
				pcsRegionTemplate	= pThis->m_pcsApmMap->GetTemplate(pcsSelfCharacter->m_nBindingRegionIndex);
				if ( pcsRegionTemplate && pcsRegionTemplate->ti.stType.uSafetyType == ApmMap::ST_DANGER )
				{
					...
					if(IS_CASH_ITEM(pcsReverseOrb->m_pcsItemTemplate->m_eCashItemType))
					{
						...
						// 캐쉬템이라도 공성지역에서는 사용 불가
						// 글로벌 공성 지역에서는 부활 아이템을 모두 사용할 수 없게 했었으나 관련 오류 수정으로 원복 Bischoff 2012. 01. 10
// 						AgpmSiegeWar* pcsSiegeWar = (AgpmSiegeWar*)g_pEngine->GetModule("AgpmSiegeWar");
// 						if(pcsSiegeWar && pcsSiegeWar->GetSiegeWarInfo(pcsCharacter))
// 							pThis->m_bIsActiveReverseOrb	= FALSE;
					}
					else
						pThis->m_bIsActiveReverseOrb	= FALSE;

					// 배틀 그라운드에서 리버스오브는 사용 가능하다 (글로벌 한정)
					AgcmBattleGround* pcsBattleGround = (AgcmBattleGround*)g_pEngine->GetModule("AgcmBattleGround");
					if( pcsBattleGround && pcsBattleGround->IsBattleGournd(pcsSelfCharacter) )
						pThis->m_bIsActiveReverseOrb	= TRUE;
				}
				else
					pThis->m_bIsActiveReverseOrb	= TRUE;
...
BOOL AgcmUIItem::CBInventoryDoubleClick(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl)
{
	...
	// 착용가능한지 먼저 선체크 한번 간다.
	if( !pThis->OnCheckEquipEnable( pcsAgpdItem ) ) return TRUE;
	...
	if( pcsAgpdItem )
	{
		...
		{
			// 아이템 등록
			CHAR*		pItemName	=	pcsAgpdItem->m_pcsItemTemplate->GetName();
			pThis->m_pcsAgcmUIChatting2->OnReadyItemRegister( pItemName , pcsAgpdItem->m_lID );

		}

		else if ( (pcsAgpdItem->m_eStatus == AGPDITEM_STATUS_INVENTORY || pcsAgpdItem->m_eStatus == AGPDITEM_STATUS_SUB_INVENTORY || pcsAgpdItem->m_eStatus == AGPDITEM_STATUS_CHARGING_INVENTORY) )
		{
...
BOOL AgcmUIItem::CBTooltipRegisterItem( PVOID pData , PVOID pClass , PVOID pCustData )
{
	AgcmUIItem*	pThis	=	static_cast< AgcmUIItem* >(pClass);
	if( !pThis )
		return FALSE;

	AgpdItem*	ppdItem	=	static_cast< AgpdItem* >(pData);
	if( !ppdItem )
		return FALSE;

	eAgpmItemTooltipType tooltipType = (INT32)pCustData;

	pThis->AddTooltipRegisterItem( ppdItem );

	if( tooltipType.check( eAgpmItemTooltipType::none_grid ) )
	{
		pThis->OpenChattingItemTooltip( ppdItem );
	}
	else if( !tooltipType.check( eAgpmItemTooltipType::quest_grid ) )
	{
		BOOL isNPCTradeGrid = tooltipType.check( eAgpmItemTooltipType::npc_trade_grid );

		return pThis->OpenToolTip(ppdItem
			,(INT32)pThis->m_pcsAgcmUIManager2->m_v2dCurMousePos.x , (INT32)pThis->m_pcsAgcmUIManager2->m_v2dCurMousePos.y
			, isNPCTradeGrid, isNPCTradeGrid);
	}
...
BOOL AgcmUIItem::CBUpdateTradeOptionGrid( PVOID pData , PVOID pClass , PVOID pCustData )
{
	...
	if( pThis->m_pcsAgcmCharacter->GetSelfCharacter() == pcsCharacter )
	{
		...
		pThis->m_pcsAgpmGrid->Add( &pThis->m_PrivateTradeGridEx , 0, (INT16)dwCount, 0, pItem->m_pcsGridItem, 1, 1 );
	...
	else
	{
		...
		pThis->m_pcsAgpmGrid->Add( &pThis->m_PrivateTradeClientGridEx , 0 , (INT16)dwCount , 0 , pItem->m_pcsGridItem , 1 , 1 );
...
BOOL AgcmUIItem::AddTooltipRegisterItem( AgpdItem* ppdItem )
{
	if( !ppdItem )
		return FALSE;

	if( GetTooltipRegisterItem( ppdItem->GetID() ) )
		return FALSE;

	stTooltipItemRegister*	pTooltipItem	=	new stTooltipItemRegister;
	pTooltipItem->m_pItem				=	ppdItem;
	pTooltipItem->m_pTooltip			=	new AcUIToolTip;

	pTooltipItem->m_pTooltip->SetMoveWindow( TRUE );
	pTooltipItem->m_pTooltip->EnableExitButton( TRUE );
	pTooltipItem->m_pTooltip->ShowWindow( FALSE );
	m_pcsAgcmUIManager2->AddWindow( pTooltipItem->m_pTooltip );

	m_dequeTooltipRegisterItem.push_back( pTooltipItem );

	if( m_dequeTooltipRegisterItem.size() > CHAT_REGISTER_ITEM_MAX_TOOLTIP )
	{
		m_pcsAgpmItem->DestroyItem( m_dequeTooltipRegisterItem[0]->m_pItem );
		m_pcsAgcmUIManager2->RemoveWindow( m_dequeTooltipRegisterItem[0]->m_pTooltip , TRUE );

		DEF_SAFEDELETE( m_dequeTooltipRegisterItem[0]->m_pTooltip );
		DEF_SAFEDELETE( m_dequeTooltipRegisterItem[0] );

		m_dequeTooltipRegisterItem.pop_front();
	}

	return TRUE;
}

AgpdItem*	AgcmUIItem::GetTooltipRegisterItem( INT lID )
{
	INT	nCount	=	m_dequeTooltipRegisterItem.size();
	for( INT i = 0 ; i < nCount ; ++i )
	{
		if( m_dequeTooltipRegisterItem[i]->m_pItem->GetID()	==	lID )
			return m_dequeTooltipRegisterItem[i]->m_pItem;
	}

	return NULL;
}

AgpdItem*	AgcmUIItem::GetTooltipRegisterItemByTID( int tid )
{
	INT	nCount	=	m_dequeTooltipRegisterItem.size();
	for( INT i = 0 ; i < nCount ; ++i )
	{
		AgpdItem  * item = m_dequeTooltipRegisterItem[i]->m_pItem;
		if( item && item->m_pcsItemTemplate && item->m_pcsItemTemplate->GetID() == tid )
			return item;
	}
...
AcUIToolTip*	AgcmUIItem::GetTooltipRegisterItemIndex( AgpdItem* ppdItem )
{
	INT	nCount	=	m_dequeTooltipRegisterItem.size();
	for( INT i = 0 ; i < nCount ; ++i )
	{
		if( m_dequeTooltipRegisterItem[i]->m_pItem->GetID()	==	ppdItem->GetID() )
			return m_dequeTooltipRegisterItem[i]->m_pTooltip;
	}

	return NULL;
}

VOID			AgcmUIItem::ClearTooltipRegisterItem( VOID )
{
	INT	nCount	=	m_dequeTooltipRegisterItem.size();
	for( INT i = 0 ; i < nCount ; ++i )
	{
		m_pcsAgpmItem->DestroyItem( m_dequeTooltipRegisterItem[i]->m_pItem );

		m_dequeTooltipRegisterItem[i]->m_pTooltip->ShowWindow( FALSE );
		m_pcsAgcmUIManager2->RemoveWindow( m_dequeTooltipRegisterItem[i]->m_pTooltip , TRUE );

		DEF_SAFEDELETE( m_dequeTooltipRegisterItem[i]->m_pTooltip );
		DEF_SAFEDELETE( m_dequeTooltipRegisterItem[i] ) ;
	}

	m_dequeTooltipRegisterItem.clear();
}

BOOL AgcmUIItem::SendChatRegisterItemRequest( INT32 llID , INT lTID , INT32 lCID )
{
	if( m_pcsAgcmItem && m_pcsAgcmCharacter && m_pcsAgcmCharacter->GetSelfCharacter() )
	{
		return m_pcsAgcmItem->SendTooltipRegisterItemRequest( llID , lTID , m_pcsAgcmCharacter->GetSelfCharacter()->GetID(), eAgpmItemTooltipType::none_grid );
	}

	return FALSE;
}

BOOL AgcmUIItem::OpenChattingItemTooltip( AgpdItem* ppdItem )
{
	if( !ppdItem )
		return FALSE;

	AcUIToolTip*	pTooltip	=	GetTooltipRegisterItemIndex( ppdItem );
	if( !pTooltip )
		return FALSE;

	SetTooltipUI( pTooltip );

	BOOL	bResult	=	OpenToolTip( ppdItem , 0 , 0 , FALSE , FALSE , FALSE );

	CalcTooltipPos( pTooltip );
