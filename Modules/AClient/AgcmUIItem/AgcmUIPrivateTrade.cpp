...
//		if( pThis->m_bUIOpenPrivateTrade )
		...
		if( pThis->m_bUIOpenChargingInventoryA )
		{
			pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lChargingInventoryAUIClose);
			pThis->m_bUIOpenChargingInventoryA = FALSE;
		}

		if( pThis->m_bUIOpenChargingInventoryB )
		{
			pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lChargingInventoryBUIClose);
			pThis->m_bUIOpenChargingInventoryB = FALSE;
		}
...
BOOL AgcmUIItem::CBPrivateTradeMoveItem(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl)
{
	...
	if( IS_CASH_ITEM( pcsAgpdItem->m_pcsItemTemplate->m_eCashItemType ) )
	{
		...
		{
			// 내구도나 제한시간이 있는 아이템인데 내구도나 제한시간이 엥꼬인 물건은 거래할 수 없다.
			pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lEventTradeFailForUsingCashItem);
			return FALSE;
		}
	}
	...
	// 쉬라인 랭크의 아이템은 거래를 할 수 없다.
// 	if( pThis->m_pcsAgpmItem->GetItemRank( pcsAgpdItem ) == 4 )
// 	{
// 		SystemMessage.ProcessSystemMessage( pThis->m_pcsAgcmUIManager2->GetUIMessage( "Item_Not_Trade" ) , 0 , AGPMSYSTEMMESSAGE_PACKET_MODAL_MESSAGE );
// 	}
