...
BOOL AgcmUIItem::CBNPCTradeUIClose(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl)
{
	...
	if( pThis )
	{
		...
		pThis->m_pcsAgcmUIManager2->ThrowEvent( pThis->m_lShrineNPCTradeBuyConfirmClose );
...
BOOL AgcmUIItem::CBUINPCTradeMSGConfirmBuy( PVOID pData, PVOID pClass, PVOID pCustData )
{
	...
	if( pThis )
	{
		...
		// 거래가 가능한지 검사
		if( !pThis->_CheckEnableNPCTradeItem() )
			return FALSE;

		// 거래시 출력할 문구 생성
		std::string strMsg = pThis->_MakeConfirmMsgBuyNPCTradeItem();
		...
		bResult	=	pThis->m_pcsAgcmUIManager2->ThrowEvent( pThis->m_lShrineNPCTradeBuyConfirmOpen );
	}
	...
	if( pThis )
	{
		if( lTrueCancel )
		{
			...
			if( pcsAgpdItem != NULL )
			{
				...
				// PC 판매가격이 없는 아이템은 팔기가 불가능하다.
				if( pThis->m_pcsAgpmItem->GetPCPrice(pcsAgpdItem) <= 0 )
				{
					CHAR*	pMessage	=	pThis->m_pcsAgcmUIManager2->GetUIMessage( "Item_Not_Sell" );
					if( pMessage )
						pThis->m_pcsAgcmUIManager2->ActionMessageOKDialog( pMessage );
...
BOOL AgcmUIItem::CBUINPCTradeMSGNotEnoughCharisma( PVOID pData , PVOID pClass , PVOID pCustData )
{
	AgcmUIItem*		pThis	=	static_cast< AgcmUIItem* >(pClass);
	if( !pThis )
		return FALSE;

	pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lNPCTradeMSGNotEnoughCharisma);

	return TRUE;
}

BOOL AgcmUIItem::CBUINPCTradeMSGNotEnoughShrineCoin( PVOID pData , PVOID pClass , PVOID pCustData )
{
	AgcmUIItem*		pThis	=	static_cast< AgcmUIItem* >(pClass);
	if( !pThis )
		return FALSE;

	pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lNPCTradeMSGNotEnoughShrineCoin);
...
void AgcmUIItem::_NoticeBySystemMessage( char* pMsgName )
{
	...
	AgcChatManager::OnAddSystemMessage( pMsg );
}