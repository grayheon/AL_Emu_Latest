...
#define UI_MESSAGE_ID_USE_ITEM_RESULT_CANNOT_USE_TRANSFORM_POTION	"UseItemCannotTransformPotion"
...
#define CHAT_REGISTER_ITEM_MAX_TOOLTIP					5
...
struct stTooltipItemRegister
{
	AcUIToolTip*											m_pTooltip;
	AgpdItem*												m_pItem;
	stTooltipItemRegister()
	{
		m_pTooltip	=	NULL;
		m_pItem		=	NULL;
	}
...
class AgcmUIItem : public ApModule
{
	...
	AgcdUIUserData		*m_pcsChargingInventoryALayerIndex;
	AgcdUIUserData		*m_pcsChargingInventoryBLayerIndex;
	...
	AgcdUIUserData		*m_pcsChargingInventoryA;
	AgcdUIUserData		*m_pcsChargingInventoryB;

	AgcdUIUserData		*m_pcsActiveReverseOrb;

	INT32				m_lInventoryLayerIndex;
	INT32				m_lSubInventoryLayerIndex;
	INT32				m_lChargingInventoryALayerIndex;
	INT32				m_lChargingInventoryBLayerIndex;
	...
	AgpdGrid			*m_pcsChargingInventoryAGrid;		// 확장 인벤 그리드 TypeA
	AgpdGrid			*m_pcsChargingInventoryBGrid;		// 확장 인벤 그리드 TypeB

	AgcmUIItemMessageData	m_stMessageData;

	AcUIToolTip			m_csItemToolTip				;	// AgcmUIItem에 사용될 ToolTip!
	AcUIToolTip			m_csItemToolTipSub;			// 장착된 아이템 ToolTip
	...
	// Charging Inventory
	INT32				m_eChargingBagType;
	BOOL				m_bUIOpenChargingInventoryA;
	INT32				m_lChargingInventoryAUIOpen;
	INT32				m_lChargingInventoryAUIClose;
	BOOL				m_bUIOpenChargingInventoryB;
	INT32				m_lChargingInventoryBUIOpen;
	INT32				m_lChargingInventoryBUIClose;
	...
	INT32				m_lNPCTradeMSGNotEnoughCharisma;
	INT32				m_lNPCTradeMSGNotEnoughShrineCoin;
	//Shrine Trade UI Open
	INT32				m_lShrineNPCTradeBuyConfirmOpen;
	INT32				m_lShrineNPCTradeBuyConfirmClose;
	...
	// Charging Inventory UI
	AgcdUI*				m_pcsUIChargingInventoryA;
	AgcdUI*				m_pcsUIChargingInventoryB;

	typedef	std::deque<AgpdItem*> ReservedTooltipItem;

	ReservedTooltipItem		reservedTooltipItem_;
...
public:
	...
	INT						GetBuyItemTID		( VOID )	{	return m_lNPCTradeBuyItemID;	}
	INT						GetBuyItemStatus	( VOID )	{	return m_nNPCTradeBuyStatus;	}
	INT						GetBuyItemLayer		( VOID )	{	return m_lNPCTradeBuyLayer;		}
	INT						GetBuyItemRow		( VOID )	{	return m_lNPCTradeBuyRow;		}
	INT						GetBuyItemColumn	( VOID )	{	return m_lNPCTradeBuyColumn;	}
	...
	static BOOL	CBUIUpdateChargingInventoryA(PVOID pData, PVOID pClass, PVOID pCustData);
	static BOOL	CBChargingInventoryAMoveItem(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl);
	static BOOL	CBChargingInventoryAMoveEndItem(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl);
	static BOOL	CBChargingInventoryUIUpdate(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl);
	static BOOL	CBChargingInventoryAOpenInvenLayer(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl);
	static BOOL	CBDragDropItemToChargingInventoryA(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl);
	
	static BOOL	CBUIUpdateChargingInventoryB(PVOID pData, PVOID pClass, PVOID pCustData);
	static BOOL	CBChargingInventoryBMoveItem(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl);
	static BOOL	CBChargingInventoryBMoveEndItem(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl);
	static BOOL	CBChargingInventoryBOpenInvenLayer(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl);
	static BOOL	CBDragDropItemToChargingInventoryB(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl);
	...
	static BOOL CBUINPCTradeMSGNotEnoughCharisma( PVOID pData , PVOID pClass , PVOID pCustData );
	static BOOL	CBUINPCTradeMSGNotEnoughShrineCoin( PVOID pData , PVOID pClass , PVOID pCustData );
	...
	BOOL	OpenToolTip(AgpdItem *pcsItem, INT32 lX, INT32 lY, BOOL bShowPrice, BOOL bIsNPCTradeGrid, BOOL bEquipItemView = TRUE);
	BOOL	CloseToolTip();
...
#ifdef _AREA_GLOBAL_
	BOOL	SetItemToolTipCashInfo(AgpdItem* ppdItem, AgcdUIItemTooltip* pcdItemTooltip);
#endif
	BOOL	SetItemToolTipTimeInfo(AgpdItem* pcsItem, AgcdUIItemTooltip* pcsItemTooltip);

	VOID	SetChargingBagType(INT32 _eBagType) {m_eChargingBagType = _eBagType;}
	...
	BOOL	OpenChargingInventory();
	BOOL	CloseChargingInventory();
	...
	void EnableChargingInventoryA();
	void EnableChargingInventoryB();
	void DisableChargingInventory();
	...
	//
	//	채팅창 아이템 툴팁을 5개까지 저장한다.
	//
	static BOOL CBTooltipRegisterItem( PVOID pData , PVOID pClass , PVOID pCustData );

	BOOL		AddTooltipRegisterItem( AgpdItem*	ppdItem );
	AgpdItem*	GetTooltipRegisterItem( INT lID );
	AcUIToolTip* GetTooltipRegisterItemIndex( AgpdItem* ppdItem );
	AgpdItem*	GetTooltipRegisterItemByTID( int tid );

	VOID		CalcTooltipPos( AcUIToolTip*	pTooltip );

	VOID		ClearTooltipRegisterItem( VOID );

	// 채팅창 Item Tooltip 오픈
	BOOL		OpenChattingItemTooltip( AgpdItem*	ppdItem );

	VOID		SetTooltipUI			( AcUIToolTip*	pTooltipUI )	{	m_pItemTooltip	=	pTooltipUI;	}

	BOOL		SendChatRegisterItemRequest( INT32 llID , INT lTID , INT32 lCID );
private:
	std::deque< stTooltipItemRegister* >	m_dequeTooltipRegisterItem;
	AcUIToolTip*							m_pItemTooltip;
...
public :
#ifdef _AREA_GLOBAL_
	static BOOL	IsCashItemByPPCard( AgpdItem* ppdItem );
#endif
	BOOL		OnSendPrivateTradeCancel			( void );

	BOOL		OnOpenMsgBoxConfirmBuyNPCTradeItem	( int nLayer = -1, int nRow = -1, int nCol = -1 );
	BOOL		OnSendBuyNPCTradeItem				( int nItemCID, int nLayer = -1, int nRow = -1, int nCol = -1 );

	BOOL		OnCheckEquipEnable					( AgpdItem* ppdItem );
	BOOL		OnCheckUseEnable					( AgpdItem* ppdItem );
	BOOL		OnCheckUseEnable					( AgpdItem* ppdItem, AgpdCharacter* ppdCharacter );

	int			GetPriceNPCTradeItem				( AgpdItem* ppdItem );

	BOOL		IsHaveMurdererLevel					( int nMurdererLevel );
	int			GetMurdererLevel					( void );

private :
	BOOL		_CheckEnableNPCTradeItem			( void );
	BOOL		_CheckIsMurderer					( void );
	std::string	_MakeConfirmMsgBuyNPCTradeItem		( void );
	__int64		_CalcItemPriceByMurdererLevel		( int nItemCID, BOOL bIsMurderer );
	void		_NoticeBySystemMessage				( char* pMsgName );
};
