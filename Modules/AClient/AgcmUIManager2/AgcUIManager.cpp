...
BOOL AgcUIManager::OnKeyDown( RsKeyStatus *ks , AgcdUIHotkeyType eKeyType )
{
	if( !m_bLoginMode )
	{
		// 우선순위는 Shift/Ctrl/Alt
		...
		AgcdUIHotkey* pstHotkey = m_pcsAgcmUIManager2->GetHotkey( eKeyType==AGCDUI_HOTKEY_TYPE_NORMAL?m_eKeyStatus:eKeyType, ks->keyScanCode );
		if( !pstHotkey ) return FALSE;
...
BOOL AgcUIManager::SetCameraViewProcessHotkeys( void )
{
	...
	m_pHkF12	= m_pAgcmUIHotkey->FindCurrentHotkeyByDefault( AGCDUI_HOTKEY_TYPE_NORMAL, 88 );		// 단축키 F09	(g_aszHotkeyNameTable참고)
