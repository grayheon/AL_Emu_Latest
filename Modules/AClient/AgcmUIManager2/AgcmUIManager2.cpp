...
INT32 AgcmUIManager2::m_alControlMessages[ AcUIBase::TYPE_COUNT ] =
{
	...
	UICM_CUSTORM_MAX_MESSAGE,
	UICM_LIST_MAX_MESSAGE,
	UICM_LISTITEM_MAX_MESSAGE,
	UICM_CLOCK_MAX_MESSAGE,
	0,
	UICM_BUTTON_MAX_MESSAGE,			//Button_EX
};
...
:	m_csEffectWindow( this ), m_csEffectBGMWindow( this ), m_csEffectGuildBattleWindow1( this ),
	...
	m_csEffectGuildBattleWindow5( this ), m_csEffectGuildBattleWindow6( this ),	m_csEffectGuildBattleWindow7( this ), m_csShrineEffectMessage( this ), //JK_쉬라인머지
	...
	m_bRunFade(FALSE), m_bFinshFade(FALSE), m_FadeType(AGCMUI_FADETYPE_IN), m_ulFadeCurRunTime(0), m_ulFadeRunTime(0)
{
...
AgcmUIManager2::~AgcmUIManager2( void )
{
	...
	if( m_pcsMainWindow )
	{
		...
		m_pcsMainWindow->DeleteChild( &m_csShrineEffectMessage, 	TRUE, FALSE );		//JK_쉬라인머지
...
BOOL AgcmUIManager2::OnInit( void )
{
	...
	if( m_pcsMainWindow )
	{
		...
		m_pcsMainWindow->AddChild( &m_csShrineEffectMessage );	//JK_쉬라인머지
...
VOID AgcmUIManager2::ResizeManagerWindow( INT32 x, INT32 y, INT32 w, INT32 h )
{
	...
	m_csShrineEffectMessage.MoveWindow( x, y, w, h );	//JK_쉬라인머지
...
BOOL AgcmUIManager2::OpenUI( AgcdUI *pcsUI, BOOL bGroup, BOOL bEditMode, BOOL bSmooth )
{
	...
	{
		m_pcsAgcmResourceLoader->SetTexturePath( (CHAR*)m_strTexturePath.c_str() );
	}
	...
	for( pcsControl = GetSequenceControl( pcsUI, &lIndex ) ; pcsControl ; pcsControl = GetSequenceControl( pcsUI, &lIndex ) )
	{
		...
		if( pcsUI->m_bInitControlStatus )
		{
			...
			if( pcsControl->m_lType == AcUIBase::TYPE_BUTTON || pcsControl->m_lType == AcUIBase::TYPE_BUTTON_EX )
			{
...
AgcdUIControl* AgcmUIManager2::AddControl(AgcdUI *pcsUI, INT32 lType, INT32 lX, INT32 lY, AgcWindow *pcsWindow, AcUIBase *pcsBase, AgcdUICControl *pcsCustomControl)
{
	...
	else
	{
		...
		case AcUIBase::TYPE_BUTTON_EX:		pcsControlBase = ( AcUIBase* )new AcUIButtonEx;		break;
...
BOOL AgcmUIManager2::ThrowEvent( AgcdUIEventMap* pEventMap , INT32 lDataIndex /*= 0*/, BOOL bForce /*= FALSE*/, ApBase *pcsOwnerBase /*= NULL */ )
{
	if( !pEventMap )
		return FALSE;

	return ProcessAction( &pEventMap->m_listAction, NULL, pEventMap, lDataIndex, bForce, pcsOwnerBase );
}
			...
			case AGCDUI_ACTION_MESSAGE :
				{
					...
					if( pstActionMessage )
					{
						...
						case AGCDUI_ACTION_MESSAGE_CHATTING_WINDOW :
							{
								AgpdChatData chatData;
								ZeroMemory( &chatData , sizeof(chatData) );
								chatData.eChatType = AGPDCHATTING_TYPE_SYSTEM_LEVEL1;
								chatData.szSenderName = "[system]";
								chatData.szMessage = szMessage;
								chatData.lMessageLength = strlen( szMessage );

								EnumCallback( AGCMUIMANAGER2_CB_ID_ACTION_MESSAGE_CHAT, ( PVOID )( &chatData ), NULL );
							}
							break;
						}
					}
...
BOOL AgcmUIManager2::RemoveWindow( AgcWindow* pWindow , BOOL bForce )
{
	if( m_pcsMainWindow )
	{
...
VOID AgcmUIManager2::RefreshWindow( VOID )
{
	if( m_pcsMainWindow )
	{
		m_csManagerWindow.UpdateChildWindow();
...
INT32 AgcmUIManager2::ActionMessageEditOKCancelDialog( CHAR* szMessage , CHAR* pEditString , BOOL bOnlyNumber /* = FALSE */ )
{
	m_pcsAgcmUIControl->SetMessageDialogPosition( m_lWindowWidth / 2 - AGCMUICONTROL_MESSAGE_WIDTH / 2 ,
		m_lWindowHeight / 2 - AGCMUICONTROL_MESSAGE_HEIGHT / 2 );

	return g_pEngine->EditOKCancelMessageDialog( NULL , szMessage , pEditString , bOnlyNumber );
}
...
//fade panel 관련 함수 by psl
void AgcmUIManager2::RunFade( AgcdFadeCallbackFunc* pCallBack, AgcmUIFadeType type, UINT32 ulRunTime,  UINT32 ulDelayTime )
{
	stRenderFade	stFade;
	ZeroMemory( &stFade , sizeof(stFade) );

	if( pCallBack )
		stFade.m_data	=	*pCallBack;

	stFade.m_FadeType		=	type;
	stFade.m_ulDelayTime	=	ulDelayTime;
	stFade.m_ulRuntime		=	ulRunTime;

	m_queueRenderFade.push( stFade );
}

void AgcmUIManager2::OnRender()
{
	//fade
	if( !m_bRunFade && !m_queueRenderFade.empty() )
	{
		stRenderFade	stFade	=	m_queueRenderFade.front();

		m_FadeCallbackFunc.clear();
		m_FadeCallbackFunc = stFade.m_data;

		m_FadeType = stFade.m_FadeType;
		m_ulFadeRunTime = stFade.m_ulRuntime;
		m_ulFadeDelayTime = stFade.m_ulDelayTime;

		if( m_FadeType == AGCMUI_FADETYPE_OUT_IN  && m_ulFadeDelayTime == 0 )
			m_ulFadeDelayTime = 500;

		m_ulFadeCurRunTime = 0;

		m_bRunFade = TRUE;
		m_bFinshFade = FALSE;

		m_queueRenderFade.pop();
	}
...
void AgcmUIManager2::RenderFade()
{
	static INT32 ulT = 0;
	static INT32 ulT2 = 0;
	...
	if( m_bRunFade )
	{
		INT32 ulDeltaT = ulT - ulT2;
		m_ulFadeCurRunTime += ulDeltaT;

		switch( m_FadeType )
		{
		case AGCMUI_FADETYPE_IN:
			{
				if( m_bFinshFade == FALSE )
				{
					if( _DelayFade() )
						m_bFinshFade = TRUE;
				}
				...
				{
					if( _RunFade(AGCMUI_FADETYPE_IN) )
						m_bRunFade = FALSE;
				}
			}
			break;
		case AGCMUI_FADETYPE_OUT:
			{
				if( m_bFinshFade == FALSE )
				{
					_RunFade(AGCMUI_FADETYPE_OUT);
				}
				else
				{
					if( _DelayFade() )
		...
		case AGCMUI_FADETYPE_OUT_IN:
			{
				static AgcmUIFadeType type = AGCMUI_FADETYPE_OUT;
				if( m_bFinshFade == FALSE )
				{
					if( _RunFade( type ) && type == AGCMUI_FADETYPE_IN )
					{
						m_bRunFade = FALSE;
						type = AGCMUI_FADETYPE_OUT;
					}
				}
				else
				{
...
BOOL AgcmUIManager2::_RunFade(AgcmUIFadeType type)
{
	static float fAlpha = 0.0f;

	if( m_ulFadeCurRunTime < m_ulFadeRunTime )
	{
		float fFactor = (float)m_ulFadeCurRunTime / (float)m_ulFadeRunTime ;
		switch( type )
		{
		case AGCMUI_FADETYPE_IN:
			{
	...
	/*	char buff[32];
		sprintf( buff, "runTime %d, alpha %.2f\n", m_ulFadeCurRunTime, fAlpha );
		OutputDebugStr( buff );*/

		g_pEngine->Draw2DPanel( 0, 0, m_lWindowWidth, m_lWindowHeight , ARGB32_TO_DWORD( (int)(fAlpha*255), 0, 0, 0) );
	}
	else
	{
		if( type == AGCMUI_FADETYPE_IN )
			fAlpha	=	0.f;
		else
			fAlpha	=	1.f;
...
BOOL AgcmUIManager2::_DelayFade()
{
	//fade 처리시 딜레이 타임 처리
	if( m_ulFadeDelayTime != 0 )
	{
		g_pEngine->Draw2DPanel( 0, 0, m_lWindowWidth, m_lWindowHeight , 0Xff000000 );
		if( m_ulFadeCurRunTime > m_ulFadeDelayTime )
		{
			m_ulFadeDelayTime = 0;
			m_ulFadeCurRunTime =  0;
			return TRUE;
...
void AgcmUIManager2::_EventFadeFinsh(AgcmUIFadeType type)
{
	switch( type )
	{
	case AGCMUI_FADETYPE_IN:
		break;
	case AGCMUI_FADETYPE_OUT:
		if( m_FadeCallbackFunc.m_FadeFinishCBFunc )
			m_FadeCallbackFunc.m_FadeFinishCBFunc( &m_FadeCallbackFunc  );
		break;
	}
}

BOOL	AgcmUIManager2::_CalcPosition( BOOL bResizeUI , INT nX , INT nY , INT nWidth , INT nHeight , RwV2d& rPos )
{
	// UI 중간 위치를 구한다
	INT		nCenterX	=	nX + (nWidth/2);
	INT		nCenterY	=	nY + (nHeight/2);

	FLOAT	fWindowWidth	=	AGCMUIMANAGER2_DEFAULT_RES_X;
	FLOAT	fWindowHeight	=	AGCMUIMANAGER2_DEFAULT_RES_Y;

	if( (nX + nWidth) > (AGCMUIMANAGER2_DEFAULT_RES_X+10) )
	{
		fWindowWidth	=	(float)m_lWindowWidth;
	}

	if( (nY + nHeight) > (AGCMUIMANAGER2_DEFAULT_RES_Y+10) )
	{
		fWindowHeight	=	(float)m_lWindowHeight;
	}


	// 0 ~ 1 의 값으로 바꾼다
	FLOAT	fCenterX	=	nCenterX / fWindowWidth;
	FLOAT	fCenterY	=	nCenterY / fWindowHeight;
	...
	else if( fCenterX < 0.66 )
	{
		rPos.x	=	(RwReal)((fCenterX * m_lWindowWidth) - (nWidth/2));
	}

	else
	{
		rPos.x	= (RwReal)(( ( (nX+nWidth) / fWindowWidth ) * m_lWindowWidth ) - nWidth);
	}
	...
	else if( fCenterY < 0.66 )
	{
		rPos.y	=	(RwReal)((fCenterY * m_lWindowHeight) - (nHeight/2));
	}

	else
	{
		rPos.y	=	(RwReal)(( ( (nY+nHeight) / fWindowHeight ) * m_lWindowHeight ) - nHeight);
	}
...
BOOL	AgcmUIManager2::_CalcSize( INT nWidth , INT nHeight , RwV2d& rSize )
{
	FLOAT	fWidth	=	nWidth  / AGCMUIMANAGER2_DEFAULT_RES_X;
	FLOAT	fHeight	=	nHeight / AGCMUIMANAGER2_DEFAULT_RES_Y;

	rSize.x	=	fWidth *	m_lWindowWidth;
	rSize.y	=	fHeight *	m_lWindowHeight;

	return TRUE;
}

BOOL AgcmUIManager2::CalcRelativeToAbsolute( IN const float _fX, IN const float _fY, OUT int &_nX, OUT int &_nY )
{
	if(_fX < 0.0 || _fX > 1.0 || _fY < 0.0 || _fY > 1.0)
		return FALSE;

	_nX = (int)(_fX * m_lWindowWidth);
	_nY = (int)(_fY * m_lWindowHeight);

	return TRUE;
}

BOOL AgcmUIManager2::CalcAbsoluteToRelative( IN const int _nX, IN const int _nY, OUT float &_fX, OUT float &_fY )
{
	_fX = (float)(_nX / m_lWindowWidth);
	_fY = (float)(_nY / m_lWindowHeight);
