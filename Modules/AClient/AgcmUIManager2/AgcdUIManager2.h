...
typedef void ( *AgcUIFadeFinshCB )		( AgcdFadeCallbackFunc* pCallbackData );
...
public:
	...
	CHAR													m_szGroupName[ AGCDUIMANAGER2_MAX_NAME ];	//GroupName
	CHAR													m_szSubGroupName[ AGCDUIMANAGER2_MAX_NAME ];	//sub GroupName
	...
	INT32													m_alX[ AGCDUI_MAX_MODE ];	// 해상도별 X좌표 UI툴에 사용
	INT32													m_alY[ AGCDUI_MAX_MODE ];	// 해상도별 Y좌표 UI툴에 사용

	AgcdUIControl()
	{
		ZeroMemory(m_szName, AGCDUIMANAGER2_MAX_NAME);
		ZeroMemory(m_szGroupName, AGCDUIMANAGER2_MAX_NAME);
		ZeroMemory(m_szSubGroupName, AGCDUIMANAGER2_MAX_NAME);
		for(int i = 0; i < AGCDUI_MAX_MODE; ++i)
		{
			m_alX[i] = 0;
			m_alY[i] = 0;
		}
...
struct AgcdFadeCallbackFunc
{
	...
	AgcUIFadeFinshCB										m_FadeFinishCBFunc;
	void*															m_pData1;	
	//이후 필요한 파라메터 추가
	//...

	//디폴트 연산자로도 가능하나 만일에... 만일에....
	AgcdFadeCallbackFunc& operator =( const AgcdFadeCallbackFunc& cpy )
	{
		if( this == &cpy ) 
			return *this;

		m_pClassPointer				= cpy.m_pClassPointer;
		m_FadeFinishCBFunc			= cpy.m_FadeFinishCBFunc;
		m_pData1						= cpy.m_pData1;
	...
	void clear()
	{
		m_pClassPointer				= NULL;
		m_FadeFinishCBFunc			= NULL;
		m_pData1						= NULL;
	}
};
