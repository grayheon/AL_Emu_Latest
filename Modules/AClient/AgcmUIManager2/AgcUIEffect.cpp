...
BOOL			AgcUIEffect::OnIdle			( UINT32 ulClockCount	)
{
	...
	if (m_bDrawStaticString)
	{
		...
		if( 0x7FFFFFFF != m_ulTextEndTime )
		{
			...
			else if (m_ulTextEndTime < ulClockCount)
			{
				if (m_ulTextEndTime + AGCUIEFFECT_FADE_DELAY < ulClockCount)
				{
					...
					m_stRenderInfo.fOffsetX	=	0.0f;
					m_stRenderInfo.fOffsetY	=	0.0f;
				}
				else
		...
		m_stRenderInfo.cRenderAlpha	=	lAlpha;
...
BOOL			AgcUIEffect::SetEffectBG( CHAR* szImage , FLOAT	fOffsetY )
{
	m_strBGImageName	=	szImage;

	m_lBGImageID				=	m_csTextureList.AddOnlyThisImage(szImage, TRUE);

	RwTexture *	pstTexture	=	m_csTextureList.GetImage_ID(m_lBGImageID);
	if (!pstTexture)
		return FALSE;

	SetRednerTexture(m_lBGImageID);

	INT		nWidth	=	RwRasterGetWidth( RwTextureGetRaster(pstTexture) );
	m_stRenderInfo.fOffsetX	=	(w - nWidth) / 2.0f;
	m_stRenderInfo.fOffsetY =	fOffsetY - 46;

	return TRUE;
}

BOOL AgcUIEffect::RenderTexture( INT32 lRenderID )
{
