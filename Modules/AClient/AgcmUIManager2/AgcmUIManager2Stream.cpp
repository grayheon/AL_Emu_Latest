...
#define AGCMUIMANAGER2_NAME_CONTROL_GROUP		"CtrlGroup"
#define AGCMUIMANAGER2_NAME_CONTROL_SUB_GROUP   "CtrlSubGroup"
...
BOOL			AgcmUIManager2::StreamRead(CHAR *szFile, BOOL bDecryption)
{
	...
	while (csStream.ReadNextValue())
	{
		...
		//if(pstEventMap)
		//	AddEvent( pstEventMap->m_szName, pstEventMap->m_fnReturnCB, pstEventMap->m_pCBClass);
	}
	...
	CountUserData();
	//JK_쉬라인머지
	AgcmShrine* pShrine = (AgcmShrine*)GetModule("AgcmShrine");
	if( pShrine )
		pShrine->SetTapControl();

	AgcmUIWorldmap* pWorldmap = (AgcmUIWorldmap*)GetModule("AgcmUIWorldmap");
	if( pWorldmap )
		pWorldmap->SetGrouping();
...
BOOL			AgcmUIManager2::StreamReadCB(PVOID pData, ApModule *pClass, ApModuleStream *pStream)
{
	...
	while (pStream->ReadNextValue())
	{
		...
		if (!strcmp(szValueName, AGCMUIMANAGER2_NAME_POSITION_UI))
		{
			...
			else if(pcsUI && pcsControl)
			{
				if (pThis->m_eMode == lMode)
				{
					pcsControl->m_pcsBase->MoveWindow(x, y, pcsBase->w, pcsBase->h);
				}
			}
		}
		...
		else if (!strcmp(szValueName, AGCMUIMANAGER2_NAME_CONTROL_TYPE))
		{
			...
			if (lType == AcUIBase::TYPE_CUSTOM)
			{
				...
				{
					char strbuf[256] = {0,};
					sprintf(strbuf, "%s - %s\n", "Custom Control not exist!!!", szValue);
		...
		else if( !strcmp(szValueName, AGCMUIMANAGER2_NAME_CONTROL_GROUP))
		{
			if (!pcsControl)
				return FALSE;

			strcpy(pcsControl->m_szGroupName, szValue);
		}
		else if( !strcmp(szValueName, AGCMUIMANAGER2_NAME_CONTROL_SUB_GROUP))
		{
			if (!pcsControl)
				return FALSE;

			strcpy(pcsControl->m_szSubGroupName, szValue);
		}
		...
		{
			if (!pcsControl || !(pcsControl->m_lType == AcUIBase::TYPE_BUTTON || pcsControl->m_lType == AcUIBase::TYPE_BUTTON_EX || pcsControl->m_lType == AcUIBase::TYPE_SCROLL_BUTTON) )
				return FALSE;
		...
		{
			if (!pcsControl ||
				!(pcsControl->m_lType == AcUIBase::TYPE_BUTTON || pcsControl->m_lType == AcUIBase::TYPE_BUTTON_EX || pcsControl->m_lType == AcUIBase::TYPE_SCROLL_BUTTON) )
				return FALSE;
