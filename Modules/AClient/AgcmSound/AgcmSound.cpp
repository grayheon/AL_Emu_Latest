	...
	for( int nCount = 0 ; nCount < nDeviceCount ; nCount++ )
	{
		...
		if( SoundManager::Inst().GetDeviceInfo( nCount, NewEntry.m_strDeviceName, AGCMSOUND_SOUND_DEVICENAME_LENGTH, &NewEntry.m_stGUID ) )
		{
			if( strlen( NewEntry.m_strDeviceName ) >= 0 )
			{
				...
				//memcpy_s( &m_stDevice[ nIndex ].m_stGUID, sizeof( FMOD_GUID ), &NewEntry.m_stGUID, 1 ); ?
				m_stDevice[ nIndex ].m_stGUID = NewEntry.m_stGUID;
...
void AgcmSound::_AddPreset( eSoundEffectRoomType eType, FMOD_REVERB_PROPERTIES* pProperty )
{
	if( !pProperty ) return;
	 ...
	 if( pEntry )
	 {
		 //memcpy_s( &pEntry->m_Property, sizeof( FMOD_REVERB_PROPERTIES ), pProperty, 1 ); ?
	 ...
	 else
	 {
		 ...
		 //memcpy_s( & NewEntry.m_Property, sizeof( FMOD_REVERB_PROPERTIES ), pProperty, 1 ); ?
