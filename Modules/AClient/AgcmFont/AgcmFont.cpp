...
AgcmFont::AgcmFont()
{
	...
	m_mbcsLetters = g_eServiceArea == AP_SERVICE_AREA_GLOBAL ? 1 : 2;
...
BOOL AgcmFont::InitFuncPtr()
{
	...
	if (AP_SERVICE_AREA_GLOBAL == g_eServiceArea )
	{
		...
		GetTextExtentPtr = &AgcmFont::GetTextExtentCn; //xiaomi support new chinses
		GetCharacterWidthPtr = &AgcmFont::GetCharacterWidthCn;
		IsLeadBytePtr = &AgcmFont::IsLeadByteCn;
	}
...
BOOL AgcmFont::OnInit()
{
	...
	case AP_SERVICE_AREA_GLOBAL :
		FontName = "FZCuYuan-M03S"; //xiaomi support chinese
		break;
	...
	m_astFonts[lFontMaxType].lID = 8;
	strcpy(m_astFonts[lFontMaxType].szType, FontName);
	strcpy(m_astFonts[lFontMaxType].szFileName, "Texture/etc/af1.grp");
	m_astFonts[lFontMaxType].lSize = 10;
	++lFontMaxType;

	m_astFonts[lFontMaxType].lID = 9;
	strcpy(m_astFonts[lFontMaxType].szType, FontName);
	strcpy(m_astFonts[lFontMaxType].szFileName, "Texture/etc/af1.grp");
	m_astFonts[lFontMaxType].lSize = 14;
	++lFontMaxType;
#else
	m_astFonts[lFontMaxType].lID = lFontMaxType;
	strcpy(m_astFonts[lFontMaxType].szType, FontName);
	strcpy(m_astFonts[lFontMaxType].szFileName, "Texture/NotPacked/af1.grp");
	//m_astFonts[lFontMaxType].lSize = g_eServiceArea == AP_SERVICE_AREA_GLOBAL ? 11 : 13;
	m_astFonts[lFontMaxType].lSize = 13; //xiaomi change font size
	...
	strcpy(m_astFonts[lFontMaxType].szType, FontName);
	strcpy(m_astFonts[lFontMaxType].szFileName, "Texture/NotPacked/af1.grp");
	//m_astFonts[lFontMaxType].lSize = g_eServiceArea == AP_SERVICE_AREA_GLOBAL ? 11 : 12;
	m_astFonts[lFontMaxType].lSize = 12;//xiaomi change font size2
	++lFontMaxType;

	m_astFonts[lFontMaxType].lID = 2;
	strcpy(m_astFonts[lFontMaxType].szType, FontName);
	strcpy(m_astFonts[lFontMaxType].szFileName, "Texture/NotPacked/af1.grp");
	//m_astFonts[lFontMaxType].lSize = g_eServiceArea == AP_SERVICE_AREA_GLOBAL ? 10 : 11;
	m_astFonts[lFontMaxType].lSize = 11;//xiaomi change font size3
	++lFontMaxType;
	...
	for( int fontIndex = 0; fontIndex < lFontMaxType; ++fontIndex )
	{
		...
		else
		{
			...
			m_iXLH[fontIndex] = (INT32)((FLOAT)m_astFonts[fontIndex].lSize * 1.1f); //xiaomi support chinese
		}
	...
	else
	{
			...
			(length != -1 && s < length))
		{
			...
			if( IsLeadByte( pstr[s] ) ) // DBCS
			{
				...
				xoffset = GetCharacterWidth( ftype, pstr + s ); //xiaomi 俚竟쇌몰
				++s;
			}
			else
			{
...
void	AgcmFont::DrawTextIM2DScale(float x,float y,float size,char* pstr,UINT32 ftype,UINT8 alpha,DWORD color, bool bDrawShadow, INT32 length, BOOL bClipping)
{
		...
		(length != -1 && s < length))
	{
		...
		if( IsLeadByte( pstr[s] ) ) // DBCS
		{
			...
			RenderIMMode2DSCALE(ChatPt.x,ChatPt.y,str,m_mbcsLetters,ftype,alpha,bDrawShadow,size);

			xoffset = (int)(GetCharacterWidth( ftype, str )* size);
			++s;
		}
		else
		{
			...
			RenderIMMode2DSCALE(ChatPt.x,ChatPt.y,numstr,1,ftype,alpha,bDrawShadow,size);
...
std::wstring AgcmFont::CharToWchar(const char* c, size_t m_encode = CP_ACP)
{
	std::wstring str;
...
void	AgcmFont::PrepareIMMode2D(float x,float y,char* pstr,INT32 len,UINT32 ftype,UINT8 alpha,bool bShadow)
{
	...
	std::wstring wStr = CharToWchar(pstr);// xiaomi fix chinese

	UINT16	ccode = (len == 2)? (*(wStr.c_str()+1) << 8) + *(wStr.c_str()) : *(wStr.c_str()); // xiaomi fix chinese
...
void	AgcmFont::RenderIMMode2DSCALE(float x,float y,char* pstr,INT32 len,UINT32 ftype,UINT8 alpha,bool bShadow,float size)
{
	...
	std::wstring wStr = CharToWchar(pstr);

	UINT16	ccode = (len == 2)? (*(wStr.c_str()+1) << 8) + *(wStr.c_str()) : *(wStr.c_str());
