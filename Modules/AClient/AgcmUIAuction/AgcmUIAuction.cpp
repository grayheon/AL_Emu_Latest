...
BOOL AgcmUIAuction::OnAddModule( void )
{
	...
	if( !m_pcsAgpmGrid || !m_pcsAgpmFactors || !m_pcsAgpmCharacter || !m_pcsAgcmCharacter || !m_pcsAgpmItem || 
		...
		!m_pcsAgcmUIManager2 || !m_pcsAgcmUISplitItem )
	{
		return FALSE;
	}
	...
	if( pClass )
	{
		...
		else if( lDepth == AGCM_UI_AUCTION_DEPTH_3 )
		{
            ...
            if( pcsItemTemplate )
			{
				...
				if(AGPMITEM_USABLE_TYPE_RUNE == ((AgpdItemTemplateUsable *) pcsItemTemplate)->m_nUsableItemType )
					sprintf_s( strDisplayData, sizeof( CHAR ) * 128, "%s", pcsItemTemplate->m_szName );
				else
