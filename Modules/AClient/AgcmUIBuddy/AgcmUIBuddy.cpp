...
BOOL AgcmUIBuddy::OnAddModule( void )
{
	...
	if( !m_pcsAgpmBuddy || !m_pcsAgcmCharacter || !m_pcsAgcmUIManager2 || !m_pcsAgpmChatting || 
...
BOOL AgcmUIBuddy::OperationMsgCodeBuddy( EnumAgpmBuddyMsgCode eMsgCode )
{
	...
	case AGPMBUDDY_MSG_CODE_REFUSE:					AgcChatManager::OnAddSystemMessage( ClientStr().GetStr( STI_MAX_CANNOT_REGISTER_BUDDY ) );						break;
	case AGPMBUDDY_MSG_CODE_INVALID_PROCESS:		AgcChatManager::OnAddSystemMessage( m_pcsAgcmUIManager2->GetUIMessage( UIMESSAGE_MENTOR_INVALID_PROCESS ) );		break;
	case AGPMBUDDY_MSG_CODE_NOT_INVITE_BUDDY:		AgcChatManager::OnAddSystemMessage( m_pcsAgcmUIManager2->GetUIMessage( UIMESSAGE_MENTOR_NOT_INVITE_BUDDY ) );		break;
...
BOOL AgcmUIBuddy::CBBtnDeleteBuddyClick( PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl )
{
	...
	if( pThis->m_ICurrentBuddySelectIndex >= AGPD_BUDDY_MAX || pThis->m_ICurrentBuddySelectIndex < 0 )
	{
		...
		AgcChatManager::OnAddSystemMessage( pThis->m_pcsAgcmUIManager2->GetUIMessage( UI_MESSAGE_BUDDYLIST_SELECT_NONE ) );
		return FALSE;
	}
...
BOOL AgcmUIBuddy::CBBtnBlockWhisperClick( PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl )
{
	...
	if( pThis->m_ICurrentBanSelectIndex >= AGPD_BUDDY_MAX || pThis->m_ICurrentBanSelectIndex < 0 )
	{
		...
		AgcChatManager::OnAddSystemMessage( pThis->m_pcsAgcmUIManager2->GetUIMessage( UI_MESSAGE_BANLIST_SELECT_NONE ) );
		pThis->m_bBlockWhisper = FALSE;
		pThis->RefreshBlockOptions();
		return FALSE;
	}
...
VOID AgcmUIBuddy::OperationMentorDelete( AgpdCharacter* pcsCharacter , AgpdBuddyElement* pcsBuddyElement )
{
	...
	AgcChatManager::OnAddSystemMessage( szMessage );
