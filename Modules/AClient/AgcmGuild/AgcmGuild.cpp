...
BOOL AgcmGuild::SendGuildBuyGuildMark(INT32 lGuildMarkTID, INT32 lGuildMarkColor, BOOL bForce )
{
	...
	PVOID pvPacket = NULL;
	if (bForce)
		pvPacket = m_pcsAgpmGuild->MakeGuildBuyGuildMarkForcePacket(&nPacketLength ,lCID, szGuildID, &lGuildMarkTID, &lGuildMarkColor);
	else
		pvPacket = m_pcsAgpmGuild->MakeGuildBuyGuildMarkPacket(&nPacketLength ,lCID, szGuildID, &lGuildMarkTID, &lGuildMarkColor);
