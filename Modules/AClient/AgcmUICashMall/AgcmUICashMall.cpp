...
BOOL AgcmUICashMall::AddFunction()
{
	if (!m_pcsAgcmUIManager2->AddFunction(this, "CashMallGetEdit", CBGetEditControl, 1, "cash edit control"))
		return FALSE;
#ifdef _AREA_GLOBAL_
	if (!m_pcsAgcmUIManager2->AddFunction(this, "CashMallGetEdit2", CBGetEditControl2, 1, "cash edit control2"))
		return FALSE;
	if (!m_pcsAgcmUIManager2->AddFunction(this, "CashMallGetEdit3", CBGetEditControl3, 1, "cash edit control3"))
		return FALSE;
#endif
...
BOOL AgcmUICashMall::AddEvent()
{
	...
	if (m_lEventGetEditControl < 0)
		return FALSE;
#ifdef _AREA_GLOBAL_
	m_lEventGetEditControl2	= m_pcsAgcmUIManager2->AddEvent("CashMallGetCashEditControl2");
	if (m_lEventGetEditControl2 < 0)
		return FALSE;

	m_lEventGetEditControl3	= m_pcsAgcmUIManager2->AddEvent("CashMallGetCashEditControl3");
	if (m_lEventGetEditControl3 < 0)
		return FALSE;
#endif
...
#ifdef _AREA_GLOBAL_
	m_lEventBuyResultNotPremiumService = m_pcsAgcmUIManager2->AddEvent("CashMallBuyResultNotPremiumService");
	if (m_lEventBuyResultNotPremiumService < 0)
		return FALSE;
#else
...
#ifdef _AREA_GLOBAL_
	// Edit cash info update
	pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lEventGetEditControl);
	pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lEventGetEditControl2);
	pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lEventGetEditControl3);
	pThis->RefreshCashEdit();

	pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lEventOpenCashMallUI);
...
#ifdef _AREA_GLOBAL_
		case AGPMCASH_BUY_RESULT_NOT_GPREMIUM :
			{
				pThis->m_pcsAgcmUIManager2->ThrowEvent(pThis->m_lEventBuyResultNotPremiumService);
			}
			break;

		case AGPMCASH_BUY_RESULT_PPCARD_ONLY:
			{
				AgcmUIManager2* pcmUIManager = ( AgcmUIManager2* )g_pEngine->GetModule( "AgcmUIManager2" );
				if( !pcmUIManager ) return FALSE;

				char* pMessage = pcmUIManager->GetUIMessage( "CashItem_PurchaseByPPCardOnly" );
...
BOOL AgcmUICashMall::RefreshCashEdit()
{
	...
	m_pcsEditCash->SetText(szBuffer.GetBuffer());

	szBuffer.Format("%s : %d", m_pcsAgcmUIManager2->GetUIMessage(AGCMUIMESSAGE_CASHMALL_P_CASH), (INT64)pCash.m_PCoin);
	m_pcsEditCash2->SetText(szBuffer.GetBuffer());
	
	szBuffer.Format("%s : %d", m_pcsAgcmUIManager2->GetUIMessage(AGCMUIMESSAGE_CASHMALL_KEEP_CASH), (INT64)pCash.m_WCoin+(INT64)pCash.m_PCoin);
	m_pcsEditCash3->SetText(szBuffer.GetBuffer());
...
#ifdef _AREA_GLOBAL_
BOOL AgcmUICashMall::CBGetEditControl2(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl)
{
	if (!pClass || !pData1)
		return FALSE;
	...
	pThis->m_pcsEditCash2			= (AcUIEdit *) ((AgcdUIControl *)	pData1)->m_pcsBase;

	return TRUE;
}

BOOL AgcmUICashMall::CBGetEditControl3(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl)
{
	if (!pClass || !pData1)
		return FALSE;
	...
	pThis->m_pcsEditCash3			= (AcUIEdit *) ((AgcdUIControl *)	pData1)->m_pcsBase;
...
#ifdef _AREA_GLOBAL_
	switch(g_eChannelingService)
	{
	case AP_SERVICE_CHANNELING_ORIGINAL_PATH:
		::ShellExecute( NULL, "open", pThis->m_pcsAgcmUIManager2->GetUIMessage(AGCMUIMESSAGE_CASHMALL_CHARGE_URL), NULL, NULL, SW_SHOW );
		break;
	case AP_SERVICE_CHANNELING_TAHADI_PATH:
		::ShellExecute( NULL, "open", pThis->m_pcsAgcmUIManager2->GetUIMessage(AGCMUIMESSAGE_CASHMALL_CHARGE_FOR_TAHADI_URL), NULL, NULL, SW_SHOW );
		break;
	}
#else
	::ShellExecute( NULL, "open", pThis->m_pcsAgcmUIManager2->GetUIMessage(AGCMUIMESSAGE_CASHMALL_CHARGE_URL), NULL, NULL, SW_SHOW );
...
BOOL AgcmUICashMall::CBBuyProduct(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl)
{
	...
	return pThis->m_pcsAgcmCashMall->SendPacketBuyProduct(pThis->m_pcsBuyItem->m_ulCID, pThis->m_iCashType);
}
...
#else
	llCash			= pThis->m_pcsAgpmCharacter->GetCash(pThis->m_pcsAgcmCharacter->GetSelfCharacter());
#endif
...
INT64	llCash = 0;
	...
	if( AGPMCASHMALL_TYPE_WCOIN == pThis->m_iCashType )
		llCash = (INT64)pCash.m_WCoin;
	else if( AGPMCASHMALL_TYPE_WCOIN_PPCARD == pThis->m_iCashType )
		llCash = (INT64)pCash.m_PCoin;
#else
	llCash			= pThis->m_pcsAgpmCharacter->GetCash(pThis->m_pcsAgcmCharacter->GetSelfCharacter());
#endif
