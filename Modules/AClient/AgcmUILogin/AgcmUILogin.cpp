				...
				)
			{
				...
				if( pcmUIEventPointLight )
				{
					if (m_LoadingWindow.GetIsPostLoadingWindow() && !pcmUIEventPointLight->m_bReturnLoginLobby)
					{
						...
						AgcdUIEventMap*	pEventMap	=	pcmUIManager->GetEventMap( "Chat_InputNormalUIOpen" );
						if( pEventMap )
						{
							pcmUIManager->ThrowEvent( pEventMap );
							AgcdUIControl *pInputControl = pcmUIManager->GetControl(pcmUIManager->GetUI("Chat_Input_Normal"), "Input_Message");
							if(pInputControl)
							{
								if(pInputControl->m_pcsBase->m_nType == AcUIBase::TYPE_EDIT)
								{
									AcUIEdit *pUIEdit = (AcUIEdit*)pInputControl->m_pcsBase;
									pUIEdit->ReleaseMeActiveEdit();
								}
							}
						}

						AgcmUIQuestJournal*	pcmUIQuestJournal	=	(AgcmUIQuestJournal*)GetModule( "AgcmUIQuestJournal" );
						if( pcmUIQuestJournal )
						{
							pcmUIQuestJournal->OpenJournalUI();
						}
...
#ifndef USE_MFC
	#ifdef _AREA_KOREA_
		// 국내에서는 리얼에서는 Yes/No 만 묻고 테스트용은 패스워드 입력
		if( g_cHanAuthKor.IsExcute() )
...
BOOL AgcmUILogin::CBReturnLoginCharacterSelectMode(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl)
{
	...
	AgcmUIChatting2*	pcsUIChatting2	=	static_cast< AgcmUIChatting2* >(pThis->GetModule("AgcmUIChatting2") );
	...
	pcsUIChatting2->AllHideChatUI();
	pcsUIChatting2->SetEnableIdle( FALSE );
	pcsUIChatting2->AllClearMessage();

	if( pcmUIItem )
	{
		pcmUIItem->ClearTooltipRegisterItem();
	}

	//월드맵에 모든 마크 삭제
	AgcmUIWorldmap* pWorldMap = (AgcmUIWorldmap*)pThis->GetModule( "AgcmUIWorldmap" );
	if( pWorldMap )
	{
		pWorldMap->DeleteAllMark();
	}
...
BOOL AgcmUILogin::CBEnterGameEnd(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	AgcmUIChatting2* pcmUIChatting2	= (AgcmUIChatting2*)pThis->GetModule( "AgcmUIChatting2" );
...
ApChannelingService AgcmUILogin::ParseChannelingCodeString( char* pString )
{
	// 아래의 파일이 존재하면 채널링검사를 건너뛴다.
	FILE* pFile = fopen( "Archlord.ini", "rb" );
	if( pFile )
	{
		fclose( pFile );
		return AP_SERVICE_CHANNELING_MAX;
	}

	//이곳에서 인증 스트링을 구분하여 채널링인지 아닌지 확인한다.
	if(strcmp(pString, CHANNEL_TAHADI_COMMAND_STRING))
		return AP_SERVICE_CHANNELING_TAHADI_PATH;

	return AP_SERVICE_CHANNELING_ORIGINAL_PATH;
}
