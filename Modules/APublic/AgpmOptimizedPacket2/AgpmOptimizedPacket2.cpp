...
BOOL AgpmOptimizedPacket2::OnReceiveCharMove(UINT32 ulType, PVOID pvPacket, INT16 nSize, UINT32 ulNID, DispatchArg *pstCheckArg)
{
	...
	ApdObject		*pObject = NULL;
	if(NULL == pcsMoveTargetChar)
		pObject = m_pcsApmObject->GetObject(lFollowTargetID);
	...
	*/
	...
	m_pcsAgpmCharacter->ProcessMovePacket(pcsCharacter, pcsMoveTargetChar, pObject, (INT32) unFollowDistance,
...
BOOL AgpmOptimizedPacket2::OnReceiveCharAction(UINT32 ulType, PVOID pvPacket, INT16 nSize, UINT32 ulNID, DispatchArg *pstCheckArg)
{
	...
	UINT8	cHitIndex		= 0;
	INT32	lWantedPinchCID	=	0;
	AgpdCharacter*	pTempChar	=	NULL;
	...
	if( lTargetID	!= AP_INVALID_CID )
		pcsTargetCharacter	= m_pcsAgpmCharacter->GetCharacter(lTargetID);
...
PVOID AgpmOptimizedPacket2::MakePacketCharView(AgpdCharacter *pcsCharacter, INT16 *pnPacketLength, BOOL bIsNewChar)
{
	...
	for (i = 0; i < AGPMITEM_PART_NUM; ++i)
	{
		...
		AgpdGridItem	*pcsAgpdGridItem = m_pcsAgpmItem->GetEquipItem( pcsCharacter, i );

		if (pcsAgpdGridItem)
		{
			pcsItem = m_pcsAgpmItem->GetItem( pcsAgpdGridItem->m_lItemID );
		}
...
PVOID AgpmOptimizedPacket2::MakePacketCharMove(INT32 lCID, INT8 cMoveFlag, INT8 cMoveDirection,  AuPOS* pcsCurrentPOS, AuPOS *pcsDestPos, INT16 *pnPacketLength)
{
	if (!pnPacketLength)
		return NULL;
...
INT32 AgpmOptimizedPacket2::MakeBufferCharAllItem(AgpdCharacter *pcsCharacter, CHAR *szItemBuffer, INT32 *plItemBufferSize, CHAR *szItemConvertBuffer, INT32 *plItemConvertBufferSize)
{
	...
	i = 0;
	for (pcsGridItem = m_pcsAgpmGrid->GetItemSequence(&pcsItemADChar->m_csChargingInventoryGrid, &i);
		pcsGridItem;
		pcsGridItem = m_pcsAgpmGrid->GetItemSequence(&pcsItemADChar->m_csChargingInventoryGrid, &i))
	{
		AgpdItem *pcsItem = m_pcsAgpmItem->GetItem(pcsGridItem);
		if (!pcsItem)
			continue;
...
PVOID AgpmOptimizedPacket2::MakePacketCharAllItemExceptBankCash(AgpdCharacter *pcsCharacter, INT16 *pnPacketLength)
{
	if (!pcsCharacter || !pnPacketLength)
		return NULL;
	...
	{
		char strCharBuff[256] = { 0, };
		sprintf_s(strCharBuff, sizeof(strCharBuff), "[%s] packet length is %d in MakePacketCharAllItemExceptBankCash()\n", pcsCharacter->m_szID, *pnPacketLength);
		AuLogFile_s("LOG\\OptimizedPacketError.txt", strCharBuff);
	}
...
PVOID AgpmOptimizedPacket2::MakePacketCharAllSkill(AgpdCharacter *pcsCharacter, INT16 *pnPacketLength)
{
	if (!pcsCharacter || !pnPacketLength)
		return NULL;
