...
const int	AGPMCHAR_TYPE_SHRINE_TYPIFIER			= 0x00000040;	//JK_쉬라인머지
...
typedef enum _AgpdCharacterJanusTalismanType
{
	AGPDCHAR_JANUS_TALISMAN_NONE			= 0,		// 타입 없음
	AGPDCHAR_JANUS_TALISMAN_EXP,						// EXP만 지급되는 타입
	AGPDCHAR_JANUS_TALISMAN_DROP,						// 일반 아이템만 지급되는 타입
	AGPDCHAR_JANUS_TALISMAN_DROP2,						// 레어 아이템만 지급되는 타입
	AGPDCHAR_JANUS_TALISMAN_GHELD,						// 겔드만 지급되는 타입
	AGPDCHAR_JANUS_TALISMAN_MAX,
} AgpdCharacterJanusTalismanType;
...
const int	AGPMCHAR_MAX_LEVEL				= 121;	// 헤븐즈헬 적용(최고레벨 120으로 상승)
...
const int	AGPMCHAR_MAX_RESIST_OPTION_PERCENT	= 80;					// 저항력이나 저항력무시 스탯의 최대값
...
typedef struct _AgpdCharacterIgnoreStatus
{
	INT32		m_lIgnorePhysicalAttackResist;	
	INT32		m_lIgnoreBlockRate;				
	INT32		m_lIgnoreSkillBlockRate;		
	INT32		m_lIgnoreEvadeRate;				
	INT32		m_lIgnoreDodgeRate;				
	INT32		m_lIgnoreCriticalDefenseRate;	
	INT32		m_lIgnoreStunDefenseRate;		

	_AgpdCharacterIgnoreStatus()
	{
		m_lIgnorePhysicalAttackResist	= 0;	
		m_lIgnoreBlockRate				= 0;		
		m_lIgnoreSkillBlockRate			= 0;		
		m_lIgnoreEvadeRate				= 0;				
		m_lIgnoreDodgeRate				= 0;				
		m_lIgnoreCriticalDefenseRate	= 0;	
		m_lIgnoreStunDefenseRate		= 0;		
	}

	_AgpdCharacterIgnoreStatus operator +(_AgpdCharacterIgnoreStatus& data)
	{
		m_lIgnorePhysicalAttackResist		+= data.m_lIgnorePhysicalAttackResist;
		m_lIgnoreBlockRate					+= data.m_lIgnoreBlockRate;
		m_lIgnoreSkillBlockRate				+= data.m_lIgnoreSkillBlockRate;
		m_lIgnoreEvadeRate					+= data.m_lIgnoreEvadeRate;
		m_lIgnoreDodgeRate					+= data.m_lIgnoreDodgeRate;
		m_lIgnoreCriticalDefenseRate		+= data.m_lIgnoreCriticalDefenseRate;
		m_lIgnoreStunDefenseRate			+= data.m_lIgnoreStunDefenseRate;
	...
	VOID RegulateData()
	{
		...
		if (m_lIgnorePhysicalAttackResist > lMax)			m_lIgnorePhysicalAttackResist = lMax;
		else if (m_lIgnorePhysicalAttackResist < lMin)		m_lIgnorePhysicalAttackResist = lMin;

		if (m_lIgnoreBlockRate > lMax)						m_lIgnoreBlockRate = lMax;
		else if (m_lIgnoreBlockRate < lMin)					m_lIgnoreBlockRate = lMin;

		if (m_lIgnoreSkillBlockRate > lMax)					m_lIgnoreSkillBlockRate = lMax;
		else if (m_lIgnoreSkillBlockRate < lMin)			m_lIgnoreSkillBlockRate = lMin;

		if (m_lIgnoreEvadeRate > lMax)						m_lIgnoreEvadeRate = lMax;
		else if (m_lIgnoreEvadeRate < lMin)					m_lIgnoreEvadeRate = lMin;

		if (m_lIgnoreDodgeRate > lMax)						m_lIgnoreDodgeRate = lMax;
		else if (m_lIgnoreDodgeRate < lMin)					m_lIgnoreDodgeRate = lMin;

		if (m_lIgnoreCriticalDefenseRate > lMax)			m_lIgnoreCriticalDefenseRate = lMax;
		else if (m_lIgnoreCriticalDefenseRate < lMin)		m_lIgnoreCriticalDefenseRate = lMin;

		if (m_lIgnoreStunDefenseRate > lMax)				m_lIgnoreStunDefenseRate = lMax;
		else if (m_lIgnoreStunDefenseRate < lMin)			m_lIgnoreStunDefenseRate = lMin;
	}

} AgpdCharacterIgnoreStatus;
...
//	AuBLOCKING					m_astWorldBlockInfo[AGPDCHARACTER_MAX_BLOCK_INFO];
	...
	AgpdCharacterJanusTalismanType		m_eJanusType;			// 야누스 탈리스만 타입

	BOOL						m_bShrineEffect;	//JK_쉬라인머지

	AgpdCharacterIgnoreStatus	m_stStatusIgnore;

	UINT32						m_ulLastAddAgroDate;			// 어그로를 마지막으로 줬던 시간

	UINT32						m_ulLastUseEscapeDate;
...
typedef std::map<INT32, INT32>				MapPenaltyCharismaPoint;
typedef MapPenaltyCharismaPoint::iterator	IterPenaltyCharismaPoint;
typedef std::map<INT32, std::vector<INT32>>	MapPenaltySkill;
typedef MapPenaltySkill::iterator			IterPenaltySkill;
typedef std::map<INT32, INT32>				MapPenaltySkillDuration;
typedef MapPenaltySkillDuration::iterator	IterPenaltySkillDuration;

class AgpdMurderPenalty
{
public:
	MapPenaltyCharismaPoint		m_mapPenaltyCharismaPoint;		// PK시에 악당레벨에 따라 감소하는 카리스마 포인트 map<악당레벨, 포인트>
	MapPenaltySkill				m_mapPenaltySkill;				// PK시에 악당레벨에 따라 부여하는 페널티 스킬 map<악당레벨, 스킬들>
	MapPenaltySkillDuration		m_mapSkillDurationMinute;		// 페널티 스킬들의 지속시간 map<악당레벨, 분단위시간>
};
