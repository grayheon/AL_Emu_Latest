...
//	}
	...
	if (bStopMove)
	{
		...
		{
			StopCharacter(pcsCharacter, pstPos , TRUE);
	...
	// 일단 마고자가 주석 처리.
//	if (m_pcsApmMap && m_pcsApmMap->CheckBlockingInfo(*pstPos))
//	{
//		return FALSE;
//	}
	...
	BOOL bNoSendSelf = TRUE;
	if ( TRUE == IsHitHeroic(pcsCharacter) )
	{
		// 히로익 상태이상이 걸려있음
		bNoSendSelf = FALSE;
	}

	return EnumCallback(AGPMCHAR_CB_ID_MOVE, pcsCharacter, (PVOID)((ULONG_PTR)bNoSendSelf));
}
...
BOOL AgpmCharacter::MoveCharacterFollow(AgpdCharacter *pcsCharacter, AgpdCharacter *pcsTargetCharacter, INT32 lFollowDistance, BOOL bMoveFirst, FLOAT *pfDeltaMove , BOOL bCheckBlock )
{
...
BOOL AgpmCharacter::RestoreEvolutionCharacter(AgpdCharacter *pcsCharacter, AgpdCharacterTemplate *pcsTemplate)
{
	...
	BOOL bIsTeleportBlock = FALSE;
	EnumCallback(AGPMCHAR_CB_ID_CHECK_TELEPORTBLOCK, pcsCharacter, &bIsTeleportBlock);
	if ( TRUE == bIsTeleportBlock )
		return FALSE;
...
BOOL AgpmCharacter::StopCharacter(INT32 lCID, AuPOS *pstPos , BOOL bNoSendSelf )
{
	return StopCharacter(GetCharacter(lCID), pstPos , bNoSendSelf );
}

BOOL AgpmCharacter::StopCharacter(AgpdCharacter *pcsCharacter, AuPOS *pstPos , BOOL bNoSendSelf )
{
	...
	if (pcsCharacter->m_stNextAction.m_eActionType != AGPDCHAR_ACTION_TYPE_NONE)
	{
		...
		if (!ActionCharacter(stAction.m_eActionType, pcsCharacter, (ApBase *) &stAction.m_csTargetBase, &stAction.m_stTargetPos, stAction.m_bForceAction, (INT8) stAction.m_lUserData[0], (AgpdFactor *) stAction.m_lUserData[1], (INT8) stAction.m_lUserData[2]))
			EnumCallback(AGPMCHAR_CB_ID_STOP, pcsCharacter, (PVOID)((ULONG_PTR)bNoSendSelf));
	}
	else
	{
		EnumCallback(AGPMCHAR_CB_ID_STOP, pcsCharacter, (PVOID)((ULONG_PTR)bNoSendSelf));
