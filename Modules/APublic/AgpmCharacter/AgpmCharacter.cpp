...
AgpmCharacter::AgpmCharacter()
{
	...
	m_csPacket.SetFieldType(
							...
							AUTYPE_UINT32,			1,											// ESCAPE 명령어 최근 사용 날짜.
							AUTYPE_END,				0
							);
	...
	m_csPacketIgnoreStatus.SetFlagLength(sizeof(INT16));
	m_csPacketIgnoreStatus.SetFieldType(
							AUTYPE_INT32,	1,		// IgnorePhysicalAttackResist;	
							AUTYPE_INT32,	1,		// IgnoreBlockRate;		
							AUTYPE_INT32,	1,		// IgnoreSkillBlockRate;		
							AUTYPE_INT32,	1,		// IgnoreEvadeRate;			
							AUTYPE_INT32,	1,		// IgnoreDodgeRate;			
							AUTYPE_INT32,	1,		// IgnoreCriticalDefenseRate;
							AUTYPE_INT32,	1,		// IgnoreStunDefenseRate;	
							AUTYPE_END,		0
							);				
		...
		*/
		...
		if (pcsCharacter->m_eMoveDirection)
		{
			...
			else
			{
				...
				StopCharacter(pcsCharacter, NULL, TRUE);
			}
		}
		else if (pcsCharacter->m_bMoveFollow)
		{
			...
			else
			{
				...
				{
					ApdObject* pcsObject = m_pApmObject->GetObject(pcsCharacter->m_lFollowTargetID);
					if (pcsObject)
					{
						if (m_ulProcessMoveFollowInterval == 0 ||
							m_ulPrevProcessMoveFollow + m_ulProcessMoveFollowInterval < ulClockCount)
						{
							MoveCharacterFollow(pcsCharacter, pcsObject, pcsCharacter->m_lFollowDistance);
						}
						...
						if (!pcsCharacter->m_ulCheckBlocking || bDestUpdate )
						{
							...
							if (CheckStopFollow( &pcsCharacter->m_stPos, &pcsObject->m_stPosition, pcsCharacter->m_lFollowDistance ))
								StopCharacter(pcsCharacter, NULL);
						}
						else
						{
...
//		ZeroMemory(pcsCharacter->m_szBindingAreaName, AGPMCHAR_MAX_TOWN_NAME + 1);
		...
		pcsCharacter->m_ulLastAddAgroDate = 0;
		pcsCharacter->m_ulLastUseEscapeDate = 0;
	}
...
//JK_쉬라인머지
BOOL AgpmCharacter::SetShrineTypifier(AgpdCharacterTemplate *pcsTemplate)
{
	if (!pcsTemplate)
		return FALSE;

	pcsTemplate->m_ulCharType |= AGPMCHAR_TYPE_SHRINE_TYPIFIER;
...
//JK_쉬라인머지
BOOL AgpmCharacter::IsShrineTypifier(AgpdCharacter *pcsAgpdCharacter)
{
	UINT32	ulCharType	= GetCharacterType(pcsAgpdCharacter);

	if (ulCharType & AGPMCHAR_TYPE_SHRINE_TYPIFIER)
		return TRUE;
...
BOOL AgpmCharacter::SetCallBackCheckTeleportBlock( ApModuleDefaultCallBack pfCallback, PVOID pClass )
{
	return SetCallback(AGPMCHAR_CB_ID_CHECK_TELEPORTBLOCK, pfCallback, pClass );
}

BOOL AgpmCharacter::SetCallBackUpdateIgnoreStatus( ApModuleDefaultCallBack pfCallback, PVOID pClass )
{
	return SetCallback(AGPMCHAR_CB_ID_UPDATE_IGNORE_STATUS, pfCallback, pClass );
}
...
BOOL AgpmCharacter::OnReceive(UINT32 ulType, PVOID pvPacket, INT16 nSize, UINT32 ulNID, DispatchArg *pstCheckArg)
{
	...
	UINT32			lLastUsedEscapeCommandDate = 0;

	{
		...
		m_csPacket.GetField(TRUE, pvPacket, nSize,														
							...
							&lLastUsedEscapeCommandDate							// ESCAPE 명령어 최근 사용 날짜.
							);
	}

	// 구조체용 Packet은 CID를 의무적으로 넣어서 오지 않는다.
	...
	case AGPMCHAR_PACKET_OPERATION_ADD:
		{
			if (pstCheckArg->bReceivedFromServer)
			{
				if (OnOperationAdd(pstCheckArg, lCID, lTID, szName, cStatus, cActionStatus, cCriminalStatus, pvMovePacket, pvActionPacket, pvFactorPacket, llMoneyCount, llBankMoney, llCash, cIsNewChar, ucRegionIndex, ulSpecialStatus, cIsTransform, szSkillInit, cFaceIndex, cHairIndex, cBankSize, unEventStatusFlag, lRemainedCriminalTime, lRemainedMurdererTime, lWantedCriminal, szNickName , 1 , 1, lLastKilledTimeInBattleSquare, lLastUsedEscapeCommandDate))
				{
	...
	case AGPMCHAR_PACKET_OPERATION_UPDATE_IGNORE_STATUS:
		{
			//EnumCallback(AGPMCHAR_CB_ID_UPDATE_IGNORE_STATUS, pvIgnoreStatus, NULL );
			PACKET_CHARACTER_IGNORE_STATUS_UPDATE* pPacket2 = (PACKET_CHARACTER_IGNORE_STATUS_UPDATE*) pvPacket;
			AgpdCharacterIgnoreStatus stIgnoreStatus;
			stIgnoreStatus.m_lIgnorePhysicalAttackResist	= pPacket2->m_lIgnorePhysicalAttackResist;
			stIgnoreStatus.m_lIgnoreBlockRate				= pPacket2->m_lIgnoreBlockRate;
			stIgnoreStatus.m_lIgnoreSkillBlockRate			= pPacket2->m_lIgnoreSkillBlockRate;
			stIgnoreStatus.m_lIgnoreEvadeRate				= pPacket2->m_lIgnoreEvadeRate;
			stIgnoreStatus.m_lIgnoreDodgeRate				= pPacket2->m_lIgnoreDodgeRate;
			stIgnoreStatus.m_lIgnoreCriticalDefenseRate		= pPacket2->m_lIgnoreCriticalDefenseRate;
			stIgnoreStatus.m_lIgnoreStunDefenseRate			= pPacket2->m_lIgnoreStunDefenseRate;

			EnumCallback(AGPMCHAR_CB_ID_UPDATE_IGNORE_STATUS, (PVOID)&stIgnoreStatus, NULL );
		}
		break;
...
BOOL AgpmCharacter::OnOperationAdd(DispatchArg *pstCheckArg, INT32 lCID, INT32 lTID, CHAR *szName, INT8 cStatus, INT8 cActionStatus, INT8 cCriminalStatus, PVOID pvMovePacket, PVOID pvActionPacket, PVOID pvFactorPacket, INT64 llMoney, INT64 llBankMoney, INT64 llCash, INT8 cIsNewChar, UINT8 ucRegionIndex, UINT64 ulSpecialStatus, INT8 cIsTransform, CHAR *szSkillInit, INT8 cFaceIndex, INT8 cHairIndex, INT8 cBankSize, UINT16 unEventStatusFlag, INT32 lRemainedCriminalTime, INT32 lRemainedMurdererTime, INT32 lWantedCriminal, CHAR *szNickName ,BOOL bNPCDisplayForMap , BOOL bNPCDisplayForNameBoard, UINT32 ulLastKilledTimeInBattleSquare, UINT32 ulLastUseEscapeTime)
{
...

	{
		...
		SetLastUseEscapeTime(pcsCharacter, ulLastUseEscapeTime);
...
BOOL AgpmCharacter::OnOperationUpdate(DispatchArg *pstCheckArg, INT32 lCID, INT32 lTID, CHAR *szName, INT8 cStatus, INT8 cActionStatus, INT8 cCriminalStatus, PVOID pvMovePacket, PVOID pvActionPacket, PVOID pvFactorPacket, INT64 llMoney, INT64 llBankMoney, INT64 llCash, INT8 cIsNewChar, UINT8 ucRegionIndex, UINT64 ulSpecialStatus, INT8 cIsTransform, INT8 cFaceIndex, INT8 cHairIndex, INT8 cBankSize, UINT16 unEventStatusFlag, INT32 lRemainedCriminalTime, INT32 lRemainedMurdererTime, INT32 lWantedCriminal, CHAR *szNickName , INT32 lSpecialPart )
{
	...
	{
		if (pstCheckArg ->bReceivedFromServer)
			return OnOperationAdd(pstCheckArg, lCID, lTID, szName, cStatus, cActionStatus, cCriminalStatus, pvMovePacket, pvActionPacket, pvFactorPacket, llMoney, llBankMoney, llCash, cIsNewChar, ucRegionIndex, ulSpecialStatus, cIsTransform, NULL, cFaceIndex, cHairIndex, cBankSize, unEventStatusFlag, lRemainedCriminalTime, lRemainedMurdererTime, lWantedCriminal, NULL , 1 , 1, 0, 0);
		else
			return FALSE;
	}
	else if (!bLockResult)
	...
	if (pvMovePacket)
	{
		...
		bRet	= ProcessMovePacket(pcsCharacter, pcsMoveTargetChar, NULL, lMoveFollowDistance,
...
BOOL AgpmCharacter::ProcessMovePacket(AgpdCharacter *pcsCharacter, AgpdCharacter *pcsMoveTargetChar, ApdObject *pcsMoveTargetObject, INT32 lMoveFollowDistance,
									  ...
									  INT8 cMoveFlag, INT8 cMoveDirection, BOOL bReceivedFromServer)
{
	...
	if (bReceivedFromServer || !IsActionBlockCondition(pcsCharacter))
	{
		...
		else if (cMoveFlag & AGPMCHARACTER_MOVE_FLAG_FOLLOW)
		{
			...
			if(pcsMoveTargetChar)
				bRet = MoveCharacterFollow(pcsCharacter, pcsMoveTargetChar, lMoveFollowDistance, TRUE , FALSE , FALSE );
			else
				bRet = MoveCharacterFollow(pcsCharacter, pcsMoveTargetObject, lMoveFollowDistance, TRUE, FALSE, FALSE );
...
UINT32 AgpmCharacter::GetLastUseEscapeTime(AgpdCharacter *pcsCharacter)
{
	if(!pcsCharacter)
		return 0;

	return pcsCharacter->m_ulLastUseEscapeDate;
}
...
UINT32 AgpmCharacter::SetLastUseEscapeTime(AgpdCharacter *pcsCharacter, UINT32 ulLastUseEscapeTime)
{
	if(!pcsCharacter)
		return 0;
	if(ulLastUseEscapeTime > 0)
		pcsCharacter->m_ulLastUseEscapeDate = ulLastUseEscapeTime;
...
BOOL AgpmCharacter::IsHitHeroic( AgpdCharacter *pcsCharacter )
{
	if(!pcsCharacter)
		return FALSE;

	if( pcsCharacter->m_ulSpecialStatus & AGPDCHAR_SPECIAL_STATUS_FEAR ||
		pcsCharacter->m_ulSpecialStatus & AGPDCHAR_SPECIAL_STATUS_CONFUSION ||
		pcsCharacter->m_ulSpecialStatus & AGPDCHAR_SPECIAL_STATUS_HOLD ||
		pcsCharacter->m_ulSpecialStatus & AGPDCHAR_SPECIAL_STATUS_BERSERK ||
		pcsCharacter->m_ulSpecialStatus & AGPDCHAR_SPECIAL_STATUS_SHRINK		)
		return TRUE;

	return FALSE;
}

BOOL AgpmCharacter::IsHitHeroic( INT32 lCID )
{
	return IsHitHeroic(GetCharacter(lCID));
}
...
BOOL AgpmCharacter::StreamReadChatFilterText(CHAR* szFile, BOOL bEncryption)
{
	...
	if (!csExcelTxtLib.OpenExcelFile(szFile, TRUE, bEncryption))
	{
		...
		OutputDebugString("AgpmCharacter::StreamReadChatFilterText() Error (1) !!!\n");
		return FALSE;
	}
...
BOOL AgpmCharacter::MoveCharacterFollow( AgpdCharacter *pcsCharacter, ApdObject *pTargetObject, INT32 lFollowDistance, BOOL bMoveFirst /*= FALSE*/, FLOAT *pfDeltaMove /*= NULL */, BOOL bCheckBlock /*= TRUE */ )
{
	PROFILE("AgpmCharacter::MoveCharacterFollow");

	if (!pcsCharacter || !pTargetObject || lFollowDistance <= 0)
		return FALSE;

	if (pTargetObject->m_eType != APBASE_TYPE_OBJECT)
		return FALSE;
...
//	}
	...
	pstPos	= &pTargetObject->m_stPosition;
...
//	pcsCharacter->m_ulEndMoveTime = (UINT32) (ulClockCount + fDiv / /*0.4*/ (lMoveSpeed / 10000.0) );		// 초당 4타일을 간다고 가정한다.
	...
	pcsCharacter->m_lFollowTargetID	= pTargetObject->m_lID;
...
BOOL AgpmCharacter::CheckStopFollow( AuPOS *pCharacterPos, AuPOS *pTargetPos, INT32 lFollowDistance )
{
	PROFILE("AgpmCharacter::CheckStopFollow");

	if (!pCharacterPos || !pTargetPos)
		return TRUE;

	if (lFollowDistance == 0)
		return TRUE;

	if (m_pcsAgpmFactors->IsInRange(pCharacterPos, pTargetPos, lFollowDistance, 0, NULL))
		return TRUE;

	return FALSE;
}

BOOL AgpmCharacter::StreamReadMurderPenalty( CHAR *pszFile, BOOL bDecryption )
{
	...
	const INT32			nRow			= csExcelTxtLib.GetRow();
	const INT32			nColumn			= csExcelTxtLib.GetColumn();

	for (INT32  nCurRow = 1; nCurRow <= nRow; nCurRow++)
	{
		for (INT32 nCurColumn = 1; nCurColumn <= nColumn; nCurColumn++)
		{
			CHAR* pszColumnName	= csExcelTxtLib.GetData(nCurColumn, 0);
			CHAR* pszData		= csExcelTxtLib.GetData(nCurColumn, nCurRow);
			
			if (!pszData)
			{
				TRACE("AgpmCharacter::StreamReadMerderPenalty() Error : <%d, %d> 에 값이 없습니다\n", nCurColumn, nCurRow);
				continue;
			}

			CHAR*	pszMerderLevel		= csExcelTxtLib.GetData(0, nCurRow);
			INT32	lMurderLevel		= 0;
			if (!pszMerderLevel)
			{
				TRACE("AgpmCharacter::StreamReadMerderPenalty() Error : MerderLevel이 없습니다\n");
				return FALSE;
			}
			else
			{
				lMurderLevel = atoi(pszMerderLevel);
			}

			if ( !strcmp(AGPMCHAR_IMPORT_MURDER_PENALTY_SUB_CHARISMA_POINT, pszColumnName) )
			{
				INT32 lSubCharismaPoint = atoi(pszData);

				m_AgpdMurderPenalty.m_mapPenaltyCharismaPoint.insert( make_pair(lMurderLevel, lSubCharismaPoint) );
			}
			else if ( !strcmp(AGPMCHAR_IMPORT_MURDER_PENALTY_CAST_SKILL_TID, pszColumnName) )
			{
				std::vector<INT32> vtSkill;
				...
				szToken = strtok(pszData, szSeps);
				while(szToken)
				{
					INT32 lValue = atoi(szToken);
					vtSkill.push_back(lValue);

					szToken = strtok(NULL, szSeps);
				}

				m_AgpdMurderPenalty.m_mapPenaltySkill.insert( make_pair(lMurderLevel, vtSkill) );
			}
			else if ( !strcmp(AGPMCHAR_IMPORT_MURDER_PENALTY_SKILL_DURATION, pszColumnName) )
			{
				INT32 lSkillDuration = atoi(pszData);

				m_AgpdMurderPenalty.m_mapSkillDurationMinute.insert( make_pair(lMurderLevel, lSkillDuration) );
			}
			else
			{
				TRACE("AgpmCharacter::StreamReadMerderPenalty() Error : Column 이름이 맞지 않습니다\n");
				continue;
			}
		}
	}
