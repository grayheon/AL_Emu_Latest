...
BOOL AgpmEventNPCDialog::OnOperationRequest(ApdEvent *pcsEvent, AgpdCharacter *pcsCharacter)
{
	...
	else
	{
		...
		if( pcsEvent->m_pcsSource->m_eType == APBASE_TYPE_CHARACTER )
		{
			AgpdCharacter*		pcsNPCCharacter		=	(AgpdCharacter*)pcsEvent->m_pcsSource;
			if( pcsNPCCharacter )
				m_pcsAgpmCharacter->MoveCharacterFollow(pcsCharacter, pcsNPCCharacter, AGPMEVENT_NPCDIALOG_MAX_USE_RANGE );
		}
	}
