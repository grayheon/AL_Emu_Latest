...
IterTitle AgpdTitle::FindByTitleID(INT32 lTitleID)
{
	IterTitle iter;

	for(iter = m_vtTitle.begin(); iter != m_vtTitle.end(); ++iter)
	{
		if ((*iter).lTitleID == lTitleID)
			return iter;
	}
...
INT32 AgpdTitle::GetUseTitle()
{
	...
	for(iter = m_vtTitle.begin(); iter != m_vtTitle.end(); ++iter)
	{
		if ((*iter).bUsingTitle == TRUE)
			return iter->lTitleID;
	}

	return 0;
}

IterTitle AgpdTitle::GetUsingTitle()
{
	IterTitle iter;
