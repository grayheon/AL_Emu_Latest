...
BOOL AgpmArchlord::SetCallbackEventNoticeNewArchlord(ApModuleDefaultCallBack pfCallback, PVOID pClass)
{
	return SetCallback(AGPMARCHLORD_CB_NOTICE_NEW_ARCHLORD, pfCallback, pClass);
}
...
BOOL AgpmArchlord::OnReceive(UINT32 ulType, PVOID pvPacket, INT16 nSize, UINT32 ulNID, DispatchArg *pstCheckArg)
{
	...
	case AGPMARCHLORD_OPERATION_EVENT_GRANT :
		...
		break;
	case AGPMARCHLORD_OPERATION_NOTICE_NEW_ARCHLORD:
		EnumCallback(AGPMARCHLORD_CB_NOTICE_NEW_ARCHLORD, 0, 0);
		break;
	};
...
PVOID AgpmArchlord::MakePacketNoticeNewArchlord(INT16 *pnPacketLength)
{
	INT8 cOperation = AGPMARCHLORD_OPERATION_NOTICE_NEW_ARCHLORD;
