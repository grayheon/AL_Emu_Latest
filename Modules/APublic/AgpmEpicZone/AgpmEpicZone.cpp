...
BOOL AgpmEpicZone::IsEpicBossZone( INT16 nBindingRegionindex )
{
	switch(nBindingRegionindex)
	{
	case REGIONINDEX_EPICZONE_HEAVEN_HELL:	// ÇìºìÁîÇï ¿ÁÁÂ
		return TRUE;
	}
...
BOOL AgpmEpicZone::IsInEpicBossZone( AgpdCharacter* pcsCharacter )
{
	if(NULL == pcsCharacter)
		return FALSE;

	return IsEpicBossZone(pcsCharacter->m_nBindingRegionIndex);
}
