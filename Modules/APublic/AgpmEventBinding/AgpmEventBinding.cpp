...
BOOL AgpmEventBinding::OnInit()
{
	...
	if(!m_pcsAgpmShrineBattle) return FALSE;
...
BOOL AgpmEventBinding::GetBindingPositionForResurrection(AgpdCharacter *pcsCharacter, AuPOS *pstDestPos)
{
	...
	if(m_pcsAgpmShrineBattle && m_pcsAgpmShrineBattle->IsInShrineRegionByIndex(nLastExistBindingIndex))
	{
		nLastExistBindingIndex = m_pcsAgpmShrineBattle->FindResurrectionIndexInShrineArea(pcsCharacter);
	}
...
//JK_쉬라인 추가..재 접속시 부활 위치에서 스폰되도록
BOOL AgpmEventBinding::GetBindingPositionForShrineBattleArea(AgpdCharacter *pcsCharacter, AuPOS *pstDestPos, INT16 nLastExistBindingIndex)
{
	if (!pcsCharacter || !pstDestPos)
		return FALSE;
	...
	nBindingIndex = m_pcsAgpmShrineBattle->FindResurrectionIndexInShrineArea(pcsCharacter, nLastExistBindingIndex);


	ApmMap::RegionTemplate	*pstRegionTemplate	= m_pcsApmMap->GetTemplate(nBindingIndex);
	if (!pstRegionTemplate || !pstRegionTemplate->pStrName || !pstRegionTemplate->pStrName[0])
		return FALSE;
