...
eAgpmSummonsSummonsCheckResult AgpmSummons::IsSummonsEnable(AgpdCharacter* pcsCharacter, BOOL bForce)
{
	...
	INT32 lItemSummonsCount = GetItemSummonsCount(pcsCharacter);
...
//	ASSERT(lSummonsCount <= lMaxSummonsCount);
	...
	if(!bForce)
	{
		if(lSummonsCount - lItemSummonsCount >= lMaxSummonsCount)
			return AGPMSUMMONS_SUMMONS_CHECK_RESULT_FAILURE_MAX_COUNT;
	}
	...
	while(iter != tmpVector.end())
	{
		...
		if(pcsSummons)
		{
			...
			if ( !m_pagpmShrineBattle->IsInShrineRegionByIndex(pcsCharacter)
				&& iter->m_eType == AGPMSUMMONS_TYPE_ITEM )
			{
			...
			if(eCheckResult != AGPMSUMMONS_SUMMONS_CHECK_RESULT_SUCCESS)
			{
				...
				else if ( eCheckResult == AGPMSUMMONS_SUMMONS_CHECK_RESULT_FAILURE_SUMMON_ITEM && m_pagpmSystemMessage)
				{
					m_pagpmSystemMessage->ProcessSystemMessage( 0 , AGPMSYSTEMMESSAGE_CODE_SUMMONS_FAILURE_UPDATE_REGION,
						-1, -1, NULL, NULL, NULL);
				}
...
INT32 AgpmSummons::GetItemSummonsCount( AgpdCharacter* pcsCharacter )
{
	ASSERT(NULL != pcsCharacter);
	...
	for ( unsigned int i = 0; i < pcsSummonsADChar->m_SummonsArray.m_pSummons->size() ; i++ )
	{
		if ( (*(pcsSummonsADChar->m_SummonsArray.m_pSummons))[i].m_eType == AGPMSUMMONS_TYPE_ITEM )
			lCount++;
	}
