...
BOOL AgpmSiegeWar::CBInitCharacter(PVOID pData, PVOID pClass, PVOID pCustData)
{
		...
		eMonsterType == AGPD_SIEGE_MONSTER_ATTACK_RESURRECTION_TOWER)
	{
		...
		if (pcsAttachData->m_pcsAttackObjectGrid)
		{
			pThis->m_pcsAgpmGrid->Init(
				pcsAttachData->m_pcsAttackObjectGrid
				, Siegewar_AttackObject_Grid::layer
				, Siegewar_AttackObject_Grid::row
				, Siegewar_AttackObject_Grid::colum
			);
		}
	}
...
BOOL AgpmSiegeWar::OnOperationUpdate(PVOID pvPacketCastleInfo, DispatchArg *pstCheckArg)
{
	...
	if (pvPacketCastleInfo)
	{
		...
		if (pcsSiegeWar)
		{
			// 성주 길드와 성주에 대한 문자열 길이 검사를 할 경우
			// 같은 계정으로 서버를 옮겨 접속할 때 비어있는 성에 대해
			// 기존 정보가 갱신되지 않는 문제가 있어서 수정
...
BOOL AgpmSiegeWar::OnOperationUpdateAttackObject(INT32 lCID, PVOID pvPacketAttackObjectInfo)
{
	...
	if (pvPacketAttackObjectInfo)
	{
		...
		if (pcsAttachData->m_pcsAttackObjectGrid)
		{
			for (int i = 0; i < Siegewar_AttackObject_Grid::max_count; ++i)
			{
				if (alItemID[i] == (-1))
					continue;
				...
				if (pcsItem)
				{
					m_pcsAgpmGrid->Add(
						pcsAttachData->m_pcsAttackObjectGrid
						, 0
						, i / Siegewar_AttackObject_Grid::colum
						, i % Siegewar_AttackObject_Grid::colum
...
PVOID AgpmSiegeWar::MakePacketUpdateAttackObject(AgpdCharacter *pcsCharacter, INT16 *pnPacketLength, BOOL bSendGridInfo)
{
	...
	ApSafeArray< INT32, Siegewar_AttackObject_Grid::max_count > alGridItemID;

	if (bSendGridInfo && pcsAttachData->m_pcsAttackObjectGrid)
	{
		AgpdGridItem *	pcsGridItem = 0;

		for( INT16 i=0; i < Siegewar_AttackObject_Grid::max_count; ++i )
		{
			INT16 row	= (INT16)(i / Siegewar_AttackObject_Grid::colum);
			INT16 colum	= (INT16)(i % Siegewar_AttackObject_Grid::colum);

			pcsGridItem	= m_pcsAgpmGrid->GetItem( pcsAttachData->m_pcsAttackObjectGrid, 0, row, colum );
			
			if (pcsGridItem)
				alGridItemID[i] = pcsGridItem->m_lItemID;
		}
...
PVOID AgpmSiegeWar::MakePacketSyncMessage(AgpmSiegeMessage eMessageType, UINT16 unTimeSec, INT16 *pnPacketLength)
{
	...
	PVOID	pvPacketMessage	= m_csPacketSyncMessage.MakePacket(FALSE, NULL, 0,
								&cMessageType,
								(unTimeSec > 0) ? &unTimeSec : NULL, NULL, NULL, NULL);
	if (!pvPacketMessage)
		return NULL;
...
BOOL AgpmSiegeWar::CheckCarveASeal(AgpdCharacter *pcsCharacter, AgpdCharacter *pcsThrone)
{
	...
	*/
	...
	// 투명상태에서 각인시 투명상태를 풀어준다 [ 2011-07-22 silvermoo ]
	if (m_pcsAgpmCharacter->IsStatusTransparent(pcsCharacter) == TRUE)
		m_pcsAgpmCharacter->UpdateUnsetSpecialStatus(pcsCharacter, AGPDCHAR_SPECIAL_STATUS_TRANSPARENT);

	// 반투명 상태도 체크해서 풀어준다 [ 2011-07-22 silvermoo ]
	if (m_pcsAgpmCharacter->IsStatusHalfTransparent(pcsCharacter) == TRUE)
...
BOOL AgpmSiegeWar::IsSiegeWarRegion( AgpdCharacter *pcsCharacter )
{
	AgpdSiegeWar	*pcsSiegeWar = GetSiegeWarInfo(pcsCharacter);
