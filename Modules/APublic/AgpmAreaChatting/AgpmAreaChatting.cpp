...
AgpmAreaChatting::AgpmAreaChatting()
{
	...
	m_csPacket.SetFieldType(
							...
							AUTYPE_UINT32,			1,		// 텍스트 칼라 -> 채팅창 리뉴얼 되면서 채팅타입으로 변경
	...
	case AGPMAREACHATTING_OPERATION_SEND_MESSAGE:
		{
			...
			pvBuffer[4]	= (PVOID) ((ULONG_PTR)(ulTextColorRGB));

			AgpdChatData stChatData;
			stChatData.eChatType = (AgpdChattingType)ulTextColorRGB;
			stChatData.lMessageLength = unMessageLength;
			stChatData.lSenderID = lCID;
			stChatData.szMessage = szMessage;
			stChatData.ulCharType = 0;
			stChatData.szSenderName = NULL;

			return EnumCallback(AGPMAREACHATTING_CB_RECV_SEND_MESSAGE, &stChatData, (PVOID)0);
		}
		break;
	}
