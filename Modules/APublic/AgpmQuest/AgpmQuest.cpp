...
// AGSDQUEST_EXPAND_BLOCK
	m_csPacket.SetFieldType(
							...
							AUTYPE_INT32,			1, // SelectItem Index
...
BOOL AgpmQuest::OnReceive(UINT32 ulType, PVOID pvPacket, INT16 nSize, UINT32 ulNID, DispatchArg *pstCheckArg)
{
	...
	Param.lSelectItemIndex	= lSelectItemIndex;
	...
	case AGPMQUEST_PACKET_OPERATION_INIT_QUEST:
		{
			...
			GetFieldEmbeddedPacket(pvPacketQuest, lQuestID, lParam1, lParam2, lItemTID, szNPCName, bRegisterJournal);

			AgpdQuest *pAgpdQuest = GetAttachAgpdQuestData(pcsAgpdCharacter);
			if (!pAgpdQuest) break;

			pAgpdQuest->AddNewQuest(lQuestID, lParam1, lParam2, bRegisterJournal);

			EnumCallback( AGPMQUEST_INIT_QUEST , (PVOID)((ULONG_PTR)lQuestID) , (PVOID)((ULONG_PTR)bRegisterJournal) );
		}
		break;
...
BOOL AgpmQuest::SetCallbackQuestRegisterJournalChange(ApModuleDefaultCallBack pfCallback, PVOID pClass)
{
	return SetCallback(AGPMQUEST_PACKET_OPERATION_QUEST_REGISTER_JOURNAL_CHANGE, pfCallback, pClass);
}

BOOL AgpmQuest::SetCallbackInitQuest( ApModuleDefaultCallBack pfCallback, PVOID pClass )
{
	return SetCallback( AGPMQUEST_INIT_QUEST , pfCallback, pClass);
}
