...
BOOL AgpdQuest::AddNewQuest(INT32 lQuestTID, INT32 lParam1, INT32 lParam2, BOOL bRegisterJournal)
{
	// 새로운 퀘스트 과제 추가
	...
	if (iter == m_vtQuest.end())
	{
		...
		CurrentQuest.bRegisterJournal	= bRegisterJournal;
...
AgpdCurrentQuest*	AgpdQuest::GetQuest( INT32 lQuestTID )
{
	IterQuest	Iter	=	FindByQuestID( lQuestTID );
	if( Iter != m_vtQuest.end() )
	{
