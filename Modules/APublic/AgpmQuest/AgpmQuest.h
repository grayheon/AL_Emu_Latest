...
enum EnumAgpmQuestOperation
{
	...
	AGPMQUEST_PACKET_OPERATION_DISABLE_FORAWHILE,
	AGPMQUEST_PACKET_OPERATION_QUEST_REGISTER_JOURNAL_CHANGE,
	AGPMQUEST_PACKET_OPERATION_MAX
};

#define AGPMQUEST_INIT_QUEST		AGPMQUEST_PACKET_OPERATION_MAX+1
...
struct CBQuestParam
{
	...
	CBQuestParam() : pcsAgpdCharacter(NULL), pcsEvent(NULL), lQuestTID(0), lCheckPointIndex(0), 
		...
		pBuffer2(NULL), nBufferLength2(0), pBuffer3(NULL), nBufferLength3(0), lSelectItemIndex(-1)
	{}
};
...
public:
	...
	// Stream Functions
	BOOL StreamReadTemplateFileList(CHAR *pszFile, BOOL bDecryption);
	...
	BOOL SetCallbackQuestRegisterJournalChange(ApModuleDefaultCallBack pfCallback, PVOID pClass);
	BOOL SetCallbackInitQuest( ApModuleDefaultCallBack pfCallback, PVOID pClass );
	...
	BOOL IsEnableQuest() { return m_bUseQuest; }

	VOID SetUseQuest(BOOL bUse) { m_bUseQuest = bUse; } 
	...
	PVOID MakePacketQuestComplete(INT16 *pnPacketLength, ApdEvent *pEvent, INT32 lCID, INT32 lQuestTID, INT32 lSelectItemIndex = -1);
	...
	PVOID MakePacketEmbeddedQuest(INT32 lQuestID, INT32 lParam1, INT32 lParam2, INT32 lItemTID, CHAR* szNPCName, BOOL bRegisterJournal);
	BOOL GetFieldEmbeddedPacket(PVOID pvPacket, INT32 &lQuestID, INT32 &lParam1, INT32 &lParam2, INT32 &lItemTID, CHAR*& szNPCName, BOOL &bRegisterJournal);
...
//	PVOID MakePacketCheckPointResult(INT16 *pnPacketLength, INT32 lCID, INT32 lQuestTID, INT32 lCheckPointIndex);
	...
	PVOID MakePacketQuestRegisterJournal(INT16 *pnPacketLength, INT32 lCID, INT32 lQuestTID, BOOL bRegisterJournal);
