...
CHAR GeldName[5][5] =
{
	...
	{ 0xbd, 0xf0, 0xb1, 0xd2, 0x00 }	// taiwan
};
...
BOOL AgpmItem::CharTemplateWriteCB(PVOID pData, ApModule *pClass, ApModuleStream *pStream)
{
	...
	{
		if( !pcsItem->m_vtEquipItemSetInfo.size() )
			break;

		if (pcsItem->m_vtEquipItemSetInfo[0].m_lDefaultEquipITID[nIndex])
		{
			INT32	lStrlen	= sprintf(szName, "%s%d", AGPMITEM_INI_NAME_DEFAULT_ITEM, nIndex);
			ASSERT(lStrlen < 128);
			lStrlen	= sprintf(szValue, "%d:%d", nIndex, pcsItem->m_vtEquipItemSetInfo[0].m_lDefaultEquipITID[nIndex]);
			ASSERT(lStrlen < 128);
...
BOOL AgpmItem::ParseADCharTemplate(AgpdItemADCharTemplate *pcsItem, CHAR *szValue)
{
	...
	if( !pcsItem->m_vtEquipItemSetInfo.size() )
		pcsItem->m_vtEquipItemSetInfo.push_back( AgpdItemADCharTemplate::EquipItemSetInfo() );
	...
	for (; nIndex < nLength; nIndex++)
	{
		if ((szValue[nIndex] >= '0' && szValue[nIndex] <= '9'))
		{
			pcsItem->m_vtEquipItemSetInfo[0].m_lDefaultEquipITID[nPartIndex] = atoi(szValue + nIndex);
			break;
		}
	}
...
//	UINT32					ulTemp1, ulTemp2;
	...
	while (pStream->ReadNextValue())
	{
		...
		else if (!strcmp(szValueName, AGPMITEM_INI_NAME_NAME))
		{
			...
			if (g_eServiceArea == AP_SERVICE_AREA_CHINA || g_eServiceArea == AP_SERVICE_AREA_TAIWAN)
			{
			...
			{
				//if ( strcmp(pcsTemplate->m_szName, GeldName[AP_SERVICE_AREA_GLOBAL]) == 0 )
				//	pThis->m_lItemMoneyTID = pcsTemplate->m_lID;
				if ( strcmp(pcsTemplate->m_szName, GeldName[AP_SERVICE_AREA_CHINA]) == 0 )
					pThis->m_lItemMoneyTID = pcsTemplate->m_lID;//xiaomi change
			}
			else
			{
...
BOOL AgpmItem::StreamWriteOneTemplate( AgpdItemTemplate* ppdItemTemplate, char* pFileName, BOOL bEncryption )
{
	...
	sprintf_s( strSectionName, "%d", ppdItemTemplate->m_lID );
...
BOOL AgpmItem::StreamReadTemplates( char* pPathName, char* pEntryFileName, char* pErrorString, BOOL bDecryption )
{
	...
	int nItemTemplateCount = (int)m_mapItemTemplates.size();
...
BOOL AgpmItem::StreamReadImportData(CHAR *szFile, CHAR *szDebugString, BOOL bDecryption)
{
	...
	enum	IMPORT_VALUE
	{
		...
		IMVINDEX_ITEM_NPC_CHARISMA_PRICE	,	//JK_쉬라인상점
		IMVINDEX_ITEM_NPC_SHRINE_COIN		,	//JK_쉬라인상점
		IMVINDEX_MAX					= 255
	};
	...
	for(INT16 nCol = 1; nCol < pExcel->GetColumn(); ++nCol)
	{
		...
		else if(!strcmp(pszData,	AGPMITEM_IMPORT_ITEM_NPC_CHRISMA_PRICE		)) aColumn[ nCol] = IMVINDEX_ITEM_NPC_CHARISMA_PRICE	;//JK_쉬라인상점
		else if(!strcmp(pszData,	AGPMITEM_IMPORT_ITEM_NPC_SHRINE_COIN		)) aColumn[ nCol] = IMVINDEX_ITEM_NPC_SHRINE_COIN		;
		...
		else if(!strcmp(pszData,	AGPMITEM_IMPORT_ITEM_SHRINE_TYPE			)) aColumn[ nCol] = IMVINDEX_ITEM_SHRINE_TYPE			;
		...
		else if(!_stricmp(pszData,	AGPMITEM_IMPORT_GACHA_POINTTYPE				)) aColumn[ nCol] = IMVINDEX_GACHA_POINTTYPE			;
	...
	for(INT16 nRow = 1; nRow < pExcel->GetRow(); ++nRow)
	{
		...
		for(INT16 nCol = 1; nCol < pExcel->GetColumn(); ++nCol)
		{
			...
			case IMVINDEX_ITEM_NPC_PRICE				:
				...
				break;
			//JK_쉬라인상점
			case IMVINDEX_ITEM_NPC_CHARISMA_PRICE		:
				{
					if(!m_pagpmCharacter->SetStreamFactor(pExcel, &pcsAgpdItemTemplate->m_csFactor, nRow, nCol, AGPD_FACTORS_TYPE_PRICE, AGPD_FACTORS_PRICE_TYPE_CHRISMA_PRICE))
					{
						TRACE("AgpmItem::StreamReadImportData() Error (8-1) !!!\n");
						pExcel->CloseFile();
						return FALSE;
			...
			case IMVINDEX_ITEM_NPC_SHRINE_COIN			:
				{
					if(!m_pagpmCharacter->SetStreamFactor(pExcel, &pcsAgpdItemTemplate->m_csFactor, nRow, nCol, AGPD_FACTORS_TYPE_PRICE, AGPD_FACTORS_PRICE_TYPE_SHRINE_COIN))
					{
						TRACE("AgpmItem::StreamReadImportData() Error (8-1) !!!\n");
						pExcel->CloseFile();
						return FALSE;
			...
			*/
			...
			case IMVINDEX_CLASSIFY_ID					:
				{
					StreamReadClassfyID(pcsAgpdItemTemplate, *pExcel, nCol, nRow);
				}
				break;
			...
			case IMVINDEX_GACHATYPE					:
				...
				break;
			case IMVINDEX_GACHA_POINTTYPE:
				{
					pcsAgpdItemTemplate->m_bIsGachaPointType = pExcel->GetDataToInt(nCol, nRow) ? TRUE : FALSE;
				}
				break;
	...
	/*					if (pcsAgpdItemTemplate->m_nType != AGPMITEM_TYPE_EQUIP)
						...
						else if (pcsAgpdItemTemplate->m_nType == AGPMITEM_TYPE_OTHER)
						{
							...
							switch(eOtherType)
							{
								...
								case AGPMITEM_OTHER_TYPE_SHRINE_COIN:
									((AgpdItemTemplateOther *) pcsAgpdItemTemplate)->m_eOtherItemType = AGPMITEM_OTHER_TYPE_SHRINE_COIN;
									break;
			...
			case IMVINDEX_ITEM_SUB_TYPE				:
				{
					...
					if (pszData)
					{
						if (pcsAgpdItemTemplate->m_nType == AGPMITEM_TYPE_USABLE)
						{
							...
							else if (((AgpdItemTemplateUsable *) pcsAgpdItemTemplate)->m_nUsableItemType == AGPMITEM_USABLE_TYPE_JANUS_TALISMAN)
							{
								((AgpdItemTemplateUsableJanusTalisman *) pcsAgpdItemTemplate)->m_eJanusTalismanType	= (AgpmItemUsableJanusTalismanType) atoi(pszData);
							}
							else
							{
			...
			case IMVINDEX_ITEM_BOUND_TYPE				:
				...
				break;
			case IMVINDEX_ITEM_SHRINE_TYPE:
				{
					pszData	=	pExcel->GetData( nCol , nRow );
					if( pszData )
					{
						if( atoi(pszData) )
						{
							SetShrineType( pcsAgpdItemTemplate );
						}
...
BOOL AgpmItem::CBInitTemplateDefaultValue(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	//if (pcsAgpdCharacterTemplate->m_ulCharType & AGPMCHAR_TYPE_PC)
	{
		...
		if (pcsItemADCharTemplate)
		{
			...
			pcsItemADCharTemplate->m_vtInvenItemSetInfo.clear();
		}
	}
...
BOOL AgpmItem::SetDefaultEquipments(AgpdCharacterTemplate *pstAgpdCharacterTemplate, AgpdItemADCharTemplate::EquipItemSetInfo& stEquipSetInfo)
{
	if(NULL == pstAgpdCharacterTemplate)
		return FALSE;
	...
	pstAgpdItemADCharTemplate->m_vtEquipItemSetInfo.push_back(stEquipSetInfo);
...
BOOL AgpmItem::SetDefaultEquipments(AgpdCharacterTemplate *pstAgpdCharacterTemplate, INT32 lITID, INT32 lPart)
{
	...
	if( !pstAgpdItemADCharTemplate->m_vtEquipItemSetInfo.size() )
		pstAgpdItemADCharTemplate->m_vtEquipItemSetInfo.push_back( AgpdItemADCharTemplate::EquipItemSetInfo() );


	pstAgpdItemADCharTemplate->m_vtEquipItemSetInfo[0].m_lDefaultEquipITID[ lPart ]	=	lITID;

	return TRUE;
}

BOOL AgpmItem::SetDefaultInventoryItems(AgpdCharacterTemplate *pstAgpdCharacterTemplate, AgpdItemADCharTemplate::InvenItemSetInfo& stInvenSetInfo)
{
	if(NULL == pstAgpdCharacterTemplate)
		return FALSE;
	...
	pstAgpdItemADCharTemplate->m_vtInvenItemSetInfo.push_back(stInvenSetInfo);
...
// 	if( !pstAgpdItemADCharTemplate->m_vtInvenItemSetInfo.size() )
// 		pstAgpdItemADCharTemplate->m_vtInvenItemSetInfo.push_back( AgpdItemADCharTemplate::InvenItemSetInfo() );

// 	pstAgpdItemADCharTemplate->m_vtInvenItemSetInfo[0].m_aalDefaultInvenInfo[lIndex][AGPDITEM_DEFAULT_INVEN_INDEX_TID]				= lITID;
// 	pstAgpdItemADCharTemplate->m_vtInvenItemSetInfo[0].m_aalDefaultInvenInfo[lIndex][AGPDITEM_DEFAULT_INVEN_INDEX_STACK_COUNT]		= lNum;

	return TRUE;
}

BOOL AgpmItem::SetDefaultMoney(AgpdCharacterTemplate *pstAgpdCharacterTemplate, INT32 lGheld)
{
	if(lGheld == 0 || NULL == pstAgpdCharacterTemplate)
		return TRUE;
	...
	pstAgpdItemADCharTemplate->m_lDefaultMoney = lGheld;

// 	if (bJumpingEvent)
// 	{
// 		AgsmServerManager2* pAgsmServerManager2 = (AgsmServerManager2*)GetModule("AgsmServerManager2");
// 		if (!pAgsmServerManager2)
// 			return FALSE;
// 
// 		AgsdServer *pThisServer = pAgsmServerManager2->GetThisServer();
// 		pThisServer->m_llJumpingStartGheld = (INT64)lGheld;
// 	}
...
BOOL AgpmItem::StreamReadOptionData(CHAR *szFile, BOOL bDecryption)
{
	...
	for (int i = 1; i < lNumRow; ++i)
	{
		...
		for (int j = 1; j < lNumColumn; ++j)
		{
			...
			else if (strcmp(szColumnName, AGPMITEM_OPTION_IGNORE_PHYSICAL_ATTACK_RESIST) == 0)
			{
				pcsOptionTemplate->m_stSkillData.m_stIgnoreStatus.m_lIgnorePhysicalAttackResist = atoi(szValue);
			}
			else if (strcmp(szColumnName, AGPMITEM_OPTION_REINFORCEMENT_RATE) == 0)
			{
				pcsOptionTemplate->m_stSkillData.m_lReinforcementRate = atoi(szValue);
			}
			else if (strcmp(szColumnName, AGPMITEM_OPTION_IGNORE_BLOCK_RATE) == 0)
			{
				pcsOptionTemplate->m_stSkillData.m_stIgnoreStatus.m_lIgnoreBlockRate = atoi(szValue);
			}
			else if (strcmp(szColumnName, AGPMITEM_OPTION_IGNORE_SKILL_BLOCK_RATE) == 0)
			{
				pcsOptionTemplate->m_stSkillData.m_stIgnoreStatus.m_lIgnoreSkillBlockRate = atoi(szValue);
			}
			else if (strcmp(szColumnName, AGPMITEM_OPTION_IGNORE_EVADE_RATE) == 0)
			{
				pcsOptionTemplate->m_stSkillData.m_stIgnoreStatus.m_lIgnoreEvadeRate = atoi(szValue);
			}
			else if (strcmp(szColumnName, AGPMITEM_OPTION_IGNORE_DODGE_RATE) == 0)
			{
				pcsOptionTemplate->m_stSkillData.m_stIgnoreStatus.m_lIgnoreDodgeRate = atoi(szValue);
			}
			else if (strcmp(szColumnName, AGPMITEM_OPTION_IGNORE_CRITICAL_DEFENSE_RATE) == 0)
			{
				pcsOptionTemplate->m_stSkillData.m_stIgnoreStatus.m_lIgnoreCriticalDefenseRate = atoi(szValue);
			}
			else if (strcmp(szColumnName, AGPMITEM_OPTION_IGNORE_STUN_DEFENSE_RATE) == 0)
			{
				pcsOptionTemplate->m_stSkillData.m_stIgnoreStatus.m_lIgnoreStunDefenseRate = atoi(szValue);
			}
		}
	}
...
BOOL AgpmItem::SaveItemTemplateEntryINI( char* pFileName, BOOL bEncryption )
{
	...
	while( Iter != m_mapItemTemplates.end() )
	{
		...
		// TID로 Section을 설정하고
		char strSectionName[ 32 ] = { 0, };
		sprintf_s( strSectionName, "%d", pEntry->m_nTID );
		csStream.SetSection( strSectionName );
...
BOOL AgpmItem::ClearModifiedTemplate( void )
{
	m_mapModifiedTemplates.clear();
	return TRUE;
}

BOOL AgpmItem::AddModifiedTemplate( int nTID )
{
	m_mapModifiedTemplates.insert( std::map< int, int >::value_type( nTID, nTID ) );
	return TRUE;
}

BOOL AgpmItem::StreamWriteModifiedTemplates( char* pTemplatePath, char* pEntryFileName, BOOL bIsEncrypt )
{
	if( !pTemplatePath || strlen( pTemplatePath ) <= 0 ) return FALSE;
	if( !pEntryFileName || strlen( pEntryFileName ) <= 0 ) return FALSE;


	std::map< int, int >::iterator Iter = m_mapModifiedTemplates.begin();
	while( Iter != m_mapModifiedTemplates.end() )
	{
		int nTID = Iter->second;

		AgpdItemTemplate* ppdItemTemplate = GetItemTemplate( nTID );
		if( !ppdItemTemplate ) continue;

		char strSaveFileName[ 256 ] = { 0, };
		sprintf_s( strSaveFileName, sizeof( char ) * 256, "%s\\%08d.ini", pTemplatePath, nTID );

		if( !StreamWriteOneTemplate( ppdItemTemplate, strSaveFileName, bIsEncrypt ) )
		{
#ifdef _DEBUG
			char strDebug[ 256 ] = { 0, };
			sprintf_s( strDebug, sizeof( char ) * 256, "-- Error -- : Cannot Save Item Template File, FileName = %s\n", strSaveFileName );
			OutputDebugString( strDebug );
#endif
		}
	...
	// 엔트리 파일 저장
	if( !SaveItemTemplateEntryINI( pEntryFileName, bIsEncrypt ) ) return FALSE;

	ClearModifiedTemplate();
	return TRUE;
}
