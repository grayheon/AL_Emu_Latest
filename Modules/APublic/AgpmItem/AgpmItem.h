...
enum eAGPM_PRIVATE_TRADE_RESULT
{
	AGPM_PRIVATE_TRADE_RESULT_SUCCESS			,
	AGPM_PRIVATE_TRADE_RESULT_BOTH_FAILED		,
	AGPM_PRIVATE_TRADE_RESULT_REQUESTOR_FAILED	,
	AGPM_PRIVATE_TRADE_RESULT_ACCEPTOR_FAILED	,
};
...
//JK_쉬라인상점
#define AGPMITEM_SHRINECOIN_TID						53000//21044 //쉬라인 코인 TID 변경
...
typedef enum AgpmItemPacketOperation
{
	...
	AGPMITEM_PACKET_OPERATION_ITEMINFORMATION,
	AGPMITEM_PACKET_OPERATION_ITEMINFORMATION_RESULT,
...
typedef enum _AgpmItemUseResult
{
	...
	AGPMITEM_USE_RESULT_CANNOT_USE_TRANSFORM_POTION,
} AgpmItemUseResult;
...
//	ITEM_CB_ID_CHAR_GETITEM,
	...
	*/
	...
	ITEM_CB_ID_UI_UPDATE_CHARGING_INVENTORY,		// 유료 확장인벤 갱신
	...
	ITEM_CB_ID_CHARGING_INVENTORY_CHECK_ADD,
	ITEM_CB_ID_CHARGING_INVENTORY_ADD,
	ITEM_CB_ID_CHARGING_INVENTORY_UPDATE,
	ITEM_CB_ID_CHARGING_INVENTORY_REMOVE,
	...
	ITEM_CB_ADD_GACHA_ITEM_UPDATE,

	ITEM_CB_ITEM_INFORMATION,
	ITEM_CB_ITEM_INFORMATION_RESULT,
...
struct eAgpmItemTooltipType
{
	enum eAgpmItemTooltipType_Impl
	{
		inventroy_grid	= 0x00000001,
		npc_trade_grid	= 0x00000002,
		quest_grid		= 0x00000004,
		none_grid		= 0x00000008,
		equip			= 0x00000010,
		refinary_book	= 0x00000020,
		etc				= 0x00000040,
	};
	int m_eType;

	void add( int flag )	{ m_eType |= flag; }
	void remove( int flag )	{ m_eType &= ~flag; }
	bool check( int flag )	{ return (m_eType & flag) ? true : false; }

	eAgpmItemTooltipType() : m_eType( inventroy_grid | etc ) {}
	eAgpmItemTooltipType( int type ) : m_eType( type ) {}
	eAgpmItemTooltipType & operator= ( int type ) { m_eType = type; return *this; }
	operator int() { return m_eType; }
};
...
public:
	...
	BOOL				OnOperationItemInformation(AgpdCharacter* pcsCharacter, INT32 llID, INT32 lTID, INT32 lType);
	BOOL				OnOperationItemInformationResult(BOOL bReceivedFromServer, INT8 cStatus, INT32 lIID, INT32 lTID, INT32 lCID, INT32 lItemCount, PVOID pField, PVOID pInventory, PVOID pBank, PVOID pEquip, PVOID pFactor, PVOID pFactorPercent, PVOID pRestrictFactor, PVOID pConvert, PVOID pQuest, INT32 lSkillTID, INT32 lStatusFlag, PVOID pOption, PVOID pSkillPlus, PVOID pvCashInformation, PVOID pvExtraInformation, INT32 tooltipType);
...
//	BOOL                    PreUpdateItem(AgpdItem *pItem, AgpdItemStatus eCurrentStatus);
	...
	BOOL					CheckUseItem(AgpdCharacter *pcsCharacter, AgpdItem *pcsItem, BOOL bExceptLevel = FALSE, BOOL bLogin = FALSE);
	...
	bool					IsTradeBlockItem( AgpdItem * item );

	void					InitTradeBlockItems();
...
//	2005.11.16. By SungHoon
	...
	AgpdGrid				*GetChargingInventory( INT32 lCID );
	AgpdGrid				*GetChargingInventory( AgpdCharacter *pcsAgpdCharacter );

	// ************************************************************************/
	// ChargingInventory
	// 
	// 1. ChargingInventory는 Type에 따라서 사용할 수 있는 영역이 정해져 있어서
	//	  Type에 따라 할당된 Layer, Row, Column을 따로 알려준다.
	//
	// 2. Return값이 -1은 Error
	//--------------------------------------------------------------------------
	INT32					GetChargingInvectoryLayer( AgpdCharacter *pcsCharacter );
	INT32					GetChargingInvectoryRow( AgpdCharacter *pcsCharacter );
	INT32					GetChargingInvectoryColumn( AgpdCharacter *pcsCharacter );
	...
	*/
	...
	AgpdItem				*GetCashInventoryItemByType(AgpdCharacter *pcsCharacter, AgpmItemUsableType eItemType, BOOL bInUse = FALSE);
	...
	*/
	...
	BOOL					AddItemToBank(INT32 lCID, AgpdItem *pcsAgpdItem, INT32 lLayer, INT32 lRow, INT32 lColumn, BOOL bStackMerge = TRUE, BOOL bRecieveFromServer = FALSE);
	BOOL					AddItemToBank(AgpdCharacter *pcsCharacter, AgpdItem *pcsAgpdItem, INT32 lLayer, INT32 lRow, INT32 lColumn, BOOL bStackMerge = TRUE, BOOL bRecieveFromServer = FALSE);
	...
	*/
	...
	BOOL					AddItemCheckToChargingInventory(INT32 lCID, AgpdItem *pcsAgpdItem, INT32 lLayer, INT32 lRow, INT32 lColumn, BOOL bStackMerge = TRUE);
	BOOL					AddItemCheckToChargingInventory(AgpdCharacter *pcsCharacter, AgpdItem *pcsAgpdItem, INT32 lLayer, INT32 lRow, INT32 lColumn, BOOL bStackMerge = TRUE);
	BOOL					AddItemToChargingInventory(AgpdCharacter *pcsCharacter, AgpdItem *pcsAgpdItem, INT32 lLayer, INT32 lRow, INT32 lColumn, BOOL bStackMerge = TRUE);
	BOOL					AddItemToChargingInventory(INT32 lCID, AgpdItem *pcsAgpdItem, INT32 lLayer, INT32 lRow, INT32 lColumn, BOOL bStackMerge = TRUE);
	BOOL					AddItemToChargingInventory(AgpdCharacter *pcsCharacter, AgpdItem *pcsAgpdItem, BOOL bIsMergeStack = TRUE);
	BOOL					AddItemToChargingInventoryResult(AgpdItemGridResult eResult, AgpdItem *pcsAgpdItem, AgpdCharacter *pcsCharacter);
	BOOL					UpdateItemInChargingInventory(INT32 lCID, AgpdItem *pcsAgpdItem, INT32 lLayer, INT32 lRow, INT32 lColumn);
	BOOL					UpdateItemInChargingInventory(AgpdCharacter *pcsAgpdCharacter, AgpdItem *pcsAgpdItem, INT32 lLayer, INT32 lRow, INT32 lColumn);
	BOOL					RemoveItemFromChargingInventory(INT32 lCID, AgpdItem *pcsAgpdItem);
	BOOL					RemoveItemFromChargingInventory(AgpdCharacter *pcsAgpdCharacter, AgpdItem *pcsAgpdItem);
	...
	//JK_쉬라인상점
	INT32					GetCharismaPrice(AgpdItem *pcsItem);
	INT32					GetShrineCoinPrice(AgpdItem *pcsItem);

	// 아이템의 등급 
	// 1-노멀 , 2-레어 , 3-유니크 , 4-쉬라인
	INT32					GetItemRank( AgpdItem* pcsItem );

	// 캐릭터의 쉬라인코인 갯수 얻어오기
	INT32					GetCharShrineCoinCount( AgpdCharacter* pcsCharacter );
	...
	// 상점에서 NPC가 파는 가격을 세팅
	BOOL					SetNPCTradeGheld			( AgpdItem* pcsItem , INT32 nGheldPrice );
	BOOL					SetNPCTradeCharismaPoint	( AgpdItem* pcsItem , INT32 nCharismaPoint );
	BOOL					SetNPCTradeShrineCoin		( AgpdItem* pcsItem , INT32 nShrineCoin );
	...
	BOOL					StreamReadGachaDropPointTable(CHAR *szFile, BOOL bDecryption);
	...
	BOOL					StreamReadClassfyID(AgpdItemTemplate* pcsItemTemplate, AuExcelLib& csExcelTxtLib, INT32 lCurCol, INT32 lCurRow);

	BOOL					SetDefaultEquipments(AgpdCharacterTemplate *pstAgpdCharacterTemplate, INT32 lITID, INT32 lPart = 0);
	BOOL					SetDefaultEquipments(AgpdCharacterTemplate *pstAgpdCharacterTemplate, AgpdItemADCharTemplate::EquipItemSetInfo& stEquipSetInfo);
	BOOL					SetDefaultInventoryItems(AgpdCharacterTemplate *pstAgpdCharacterTemplate, INT32 lITID, INT32 lNum, INT32 lIndex);
	BOOL					SetDefaultInventoryItems(AgpdCharacterTemplate *pstAgpdCharacterTemplate, AgpdItemADCharTemplate::InvenItemSetInfo& stInvenSetInfo);
	BOOL					SetDefaultMoney(AgpdCharacterTemplate *pstAgpdCharacterTemplate, INT32 lGheld);
	...
	*/
	...
	PVOID					MakePacketItemInforMation(INT32 lIID, INT32 lTID, INT32 lCID , INT32 lType, INT16 *pnPacketLength);
	PVOID					MakePacketItemInforMationResult(AgpdItem *pcsItem, INT32 lType, INT16 *pnPacketLength);
	PVOID					MakePacketItemInforMationResult(AgpdItemTemplate *pcsItemTemplate, INT32 lType, INT16 *pnPacketLength);
	...
	BOOL					SetCallbackChargingInventoryCheckAdd(ApModuleDefaultCallBack pfCallback, PVOID pClass);
	BOOL					SetCallbackChargingInventoryAdd(ApModuleDefaultCallBack pfCallback, PVOID pClass);
	BOOL					SetCallbackChargingInventoryUpdate(ApModuleDefaultCallBack pfCallback, PVOID pClass);
	BOOL					SetCallbackChargingInventoryRemove(ApModuleDefaultCallBack pfCallback, PVOID pClass);
	...
	BOOL					SetCallbackUIUpdateChargingInventory(ApModuleDefaultCallBack pfCallback, PVOID pClass);
	...
	BOOL					SetCallbackItemInformation(ApModuleDefaultCallBack pfCallback, PVOID pClass);
	BOOL					SetCallbackItemInformationResult(ApModuleDefaultCallBack pfCallback, PVOID pClass);

	BOOL					SetCallbackAddGachaItemUpdate(ApModuleDefaultCallBack pfCallback, PVOID pClass);
...
public:	// For Admin Client. 2004.04.01. steeple
	...
	BOOL				SetShrineType(AgpdItemTemplate *pcsItemTemplate);
	BOOL				SetShrineType(AgpdItem *pcsItem);

	BOOL				IsShrineType(AgpdItemTemplate* pcsItemTemplate);
	BOOL				IsShrineType(AgpdItem* pcsItem);
...
public:
	...
	AgpdGridItem*		GetUsingItemInvolveChargingInventory(AgpdCharacter* pcsCharacter);
	BOOL				IsEnableChargingInventory(AgpdCharacter* pcsCharacter) { return GetUsingItemInvolveChargingInventory(pcsCharacter) != NULL ? TRUE : FALSE; }
	...
	BOOL				IsSameClassfyID(AgpdItemTemplate* pcsItemTemplate, AgpdItemTemplate* pcsGridItemTemplate);

	void				Report(FILE *fp);

	// 인벤 상태 체크
	// pcsRequestorChar	- 거래를 요청한 사람
	// pcsAcceptorChar	- 거래를 수락한 사람
	eAGPM_PRIVATE_TRADE_RESULT		CheckTradeCondition	( AgpdCharacter*	pcsRequestorChar	,	AgpdCharacter* pcsAcceptorChar );
...
private :
	// CheckTradeCondition 함수 내부에서 사용
	BOOL							_CheckInvenCondition ( AgpdCharacter*	pcsCharacter );
	...
	std::map< int, int >								m_mapModifiedTemplates;
...
public :
	...
	BOOL				ClearModifiedTemplate			( void );
	BOOL				AddModifiedTemplate				( int nTID );
	BOOL				StreamWriteModifiedTemplates	( char* pTemplatePath, char* pEntryFileName, BOOL bIsEncrypt );
...
private :
	...
	typedef std::set< int > TradeBlockItems;
	TradeBlockItems tradeBlockItems_;
};
