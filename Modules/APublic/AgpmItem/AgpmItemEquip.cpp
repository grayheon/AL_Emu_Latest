...
BOOL AgpmItem::EquipItem(AgpdCharacter *pcsCharacter, AgpdItemADChar *pcsItemADChar, AgpdItem *pcsItem, BOOL bCheckUseItem, BOOL bLogin)
{
	...
	if( bCheckUseItem && !CheckUseItem( pcsCharacter, pcsItem, FALSE, bLogin ) )	return FALSE;
	...
	if ( (pcsCharacter->m_bIsTrasform || pcsCharacter->m_bIsEvolution) && 
		...
		!CheckUseItem(pcsCharacter, pcsItem, FALSE, bLogin))	// 변신중에도 변경가능한 무기는 변경할 수 있어야 한다
		return FALSE;		
	...
	// Disarmament형 상태에서 해당 Parts를 Equip하거나 UnEquip할때 사용못한다.
	if(pcsCharacter->m_ulSpecialStatus & AGPDCHAR_SPECIAL_STATUS_DISARMAMENT)
	{
		if(pcsCharacter->m_ulSpecialStatusReserved == pcsAgpdItemTemplate->m_nPart)
			return FALSE;
	}
	...
	if(((AgpdItemTemplateEquip*)pcsAgpdItemTemplate)->m_nKind == AGPMITEM_EQUIP_KIND_RIDE && m_pagpmCharacter->IsCombatMode(pcsCharacter)
		&& pcsAgpdItemTemplate->m_nPart != AGPMITEM_PART_V_RIDE /* 전투중에도 아바타형 탈것은 탈 수 있다. */)
	{
		if( m_pagpmSystemMessage )
...
/*
	...
	if (pcsWeapon &&
		...
		return TRUE;
*/
	//JK_보우마스터리 탈것 탑승후 수치 돌아오지 않는 현상 수정
	AgpdItem* pcsWeaponItem = GetEquipSlotItem(pcsCharacter, AGPMITEM_PART_HAND_LEFT);
	if(pcsWeaponItem && pcsWeaponItem->m_pcsItemTemplate)
	{
		if(((AgpdItemTemplateEquip*)pcsWeaponItem->m_pcsItemTemplate)->m_nKind == AGPMITEM_EQUIP_KIND_WEAPON &&
			((AgpdItemTemplateEquipWeapon*)pcsWeaponItem->m_pcsItemTemplate)->m_nWeaponType == AGPMITEM_EQUIP_WEAPON_TYPE_TWO_HAND_BOW)
			return TRUE;
	}
	return FALSE;
}
...
/*
	...
	if (pcsWeapon &&
		...
		return TRUE;
*/
	//JK_보우마스터리 탈것 탑승후 수치 돌아오지 않는 현상 수정 크로스보우의 경우
	AgpdItem* pcsWeaponItem = GetEquipSlotItem(pcsCharacter, AGPMITEM_PART_HAND_RIGHT);
	if(pcsWeaponItem && pcsWeaponItem->m_pcsItemTemplate)
	{
		if(((AgpdItemTemplateEquip*)pcsWeaponItem->m_pcsItemTemplate)->m_nKind == AGPMITEM_EQUIP_KIND_WEAPON &&
