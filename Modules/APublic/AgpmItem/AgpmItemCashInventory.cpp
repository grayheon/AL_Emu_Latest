	...
	for (int i = 0; i < pcsItemADChar->m_csCashInventoryGrid.m_nRow; i++)
	{
		...
		if (pcsGridItem)
		{
			...
			if ((pcsTmpItem->m_nInUseItem == AGPDITEM_CASH_ITEM_INUSE || pcsTmpItem->m_nInUseItem == AGPDITEM_CASH_ITEM_PAUSE)
				...
				( 
				( IsSameClassfyID(pcsItem->m_pcsItemTemplate, pcsTmpItem->m_pcsItemTemplate) && pcsItem->m_pcsItemTemplate->m_vClassifyID.size() != 0 )
				||
...
AgpdGridItem* AgpmItem::GetUsingItemInvolveChargingInventory(AgpdCharacter* pcsCharacter)
{
	if(!pcsCharacter)
		return NULL;
		...
		pcsGridItem = m_pagpmGrid->GetItemSequence(pcsCashInventoryGrid, &lIndex))
	{
		...
		if( !pcsTemplateUsable || pcsTemplateUsable->m_nUsableItemType != AGPMITEM_USABLE_TYPE_CHARGING_BAG)
			continue;
