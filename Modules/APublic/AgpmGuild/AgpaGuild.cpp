...
AgpdGuild* AgpaGuild::AddGuild(AgpdGuild* pcsGuild)
{
	...
	strncpy_s(lwrGuildID, _countof(lwrGuildID), pcsGuild->m_szID, _TRUNCATE);
...
BOOL AgpaGuild::RemoveGuild(CHAR* szGuildID)
{
	...
	char lwrGuildID[AGPMGUILD_MAX_GUILD_ID_LENGTH+1] = {0, };
	strncpy_s(lwrGuildID, _countof(lwrGuildID), szGuildID, _TRUNCATE);
...
/////////////////////////////////////////////////////////////
			...
			if(lCount < 10 || IsGuildMasterVisitMaxTermOver(pcsGuild) == TRUE) 
			{
				pcsGuild->m_Mutex.Release();
				continue;
			}
...
BOOL AgpaGuild::IsGuildMasterVisitMaxTermOver(AgpdGuild *pcsGuild)
{
	ASSERT(pcsGuild != NULL);
	if(pcsGuild == NULL) return TRUE;
	INT32 lCurrentTime = AuTimeStamp::GetCurrentTimeStamp();
	if(GUILD_MASTER_VISIT_MAX_TERM < lCurrentTime - pcsGuild->m_lMasterLastVisitDate)
		return TRUE;
	return FALSE;
}
