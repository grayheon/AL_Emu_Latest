...
BOOL AgpmGuild::OnOperationForcedLeave(INT32 lCID, CHAR* szGuildID, CHAR* szMasterID, CHAR* szMemberID)
{
	...
	bool bCheck = EnumCallback(AGPMGUILD_CB_CHECK_LEAVE_POSSIBLE, szGuildID, szMemberID);

	if(bCheck && ForcedLeaveMember(szGuildID, szMasterID, szMemberID))
	{
		// 성공
		PVOID pBuffer[2];
		pBuffer[0] = szGuildID;
		...
		if (Lock.Result())
		{
			...
			if (lPage > 0)
			{
					...
					pcsGuild = m_csGuildAdmin.m_ApGuildSortList.GetNext())
				{
					...
					if(m_csGuildAdmin.IsGuildMasterVisitMaxTermOver(pcsGuild) == TRUE) continue;
...
BOOL AgpmGuild::OnOperationLeaveAllow(INT32 lCID, CHAR* szGuildID, CHAR* szCharID, BOOL bNotifyMaster, BOOL bAdmin)
{
	...
	bool bCheck = EnumCallback(AGPMGUILD_CB_CHECK_LEAVE_POSSIBLE, szGuildID, szCharID);

	if(bCheck && LeaveMember(szGuildID, szCharID, bNotifyMaster, bAdmin))
	{
		// 성공
