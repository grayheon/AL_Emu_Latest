...
int AuLogConsole_s(char* fmt , ...)
{
	...
	::ZeroMemory(strLog, sizeof(char) * MAX_BUF);

	va_start(ap, fmt);
	bufcnt = vsprintf_s(strLog, fmt, ap);
	va_end(ap);

	CTime cur = CTime::GetCurrentTime();

	std::string strOutput;
	std::cout << "[" << cur.GetHour() << ":" << cur.GetMinute() << ":" << cur.GetSecond() << "] " << strLog << std::endl;
	//#endif
	return 0;
}
