...
enum ePacketType
{
	...
	AGPM_WARINFO_PACKET_TYPE						= 0x61,
	//JK_쉬라인머지
	AGPM_SHRINEBATTLE_PACKET_TYPE					= 0x62,		
...
typedef enum
{
	AP_SERVICE_CHANNELING_ORIGINAL_PATH	= 0,
	AP_SERVICE_CHANNELING_TAHADI_PATH	= 1,
	AP_SERVICE_CHANNELING_MAX,
} ApChannelingService;

#define		CHANNEL_TAHADI_COMMAND_STRING							"TAHADI"

extern ApChannelingService g_eChannelingService;
// 각각 국가 플래그 사용의 경우
...
#define _create_functor__( thisType, return_type, name, args , code ) \
struct _##name##_functor__ { \
	_##name##_functor__ ( thisType * _this ) : this_(_this) {} \
	typedef return_type result_type; \
	return_type operator() args const { code }; \
	thisType* this_; \
}; \
	_##name##_functor__ name(this)
