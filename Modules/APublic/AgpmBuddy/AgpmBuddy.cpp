...
BOOL AgpmBuddy::IsFullBuddyList(AgpdCharacter *pcsCharacter, BOOL bIsFriend)
{
	ASSERT(NULL != pcsCharacter);
	AgpdBuddyADChar* pcsBuddyADChar = GetAttachAgpdBuddyData(pcsCharacter);
	ASSERT(NULL != pcsBuddyADChar);
	if(pcsBuddyADChar == NULL) return TRUE;
	
	if(bIsFriend == TRUE)
	{
		return pcsBuddyADChar->m_vtFriend.full();
	}
	else
	{
		return pcsBuddyADChar->m_vtBan.full();
	}
	return TRUE;
