...
#define AGPMDROPITEM_MAX_DROP_ITEM_COUNT		10
...
BOOL AgpmDropItem2::ProcessDropItem(AgpdCharacter *pcsDropCharacter, INT32 lAttackerLevel, ApBase *pcsFirstLooter)
{
	...
	ApSafeArray<AgpdItemTemplate*, AGPMDROPITEM_MAX_DROP_ITEM_COUNT>		arrDropItemTemplate;
	arrDropItemTemplate.MemSetAll();
	AgpdDropItemsIter		iter;
		...
		ppcsDropGroup = (AgpdDropItemGroup **) m_csDropGroups.GetObjectSequence(&lIndex))
	{
		...
		if(lRank > 0)
		{
			...
			if (!GetDropItemList(&vtDropList, &arrDropItemTemplate[0], lDropRateTotal))
			{
...
// 			}
			...
			INT32 lNum = 0;
			while(NULL != arrDropItemTemplate[lNum])
			{
				EnumCallback(AGPM_DROPITEM_CB_DROP_ITEM, arrDropItemTemplate[lNum], &csDropInfo);
				++lNum;

				if (AGPMDROPITEM_MAX_DROP_ITEM_COUNT <= lNum)
					break;
			}
...
BOOL AgpmDropItem2::ProcessDropItem2(AgpdCharacter *pcsDropCharacter, INT32 lAttackerLevel, ApBase *pcsFirstLooter)
{
	...
	while(GroupEntryIter != pAgpdDropItemGroup2->m_vtGroupEntries.end())
	{
		...
		AgpdDropItems2 RandomSample;
		iterator_traits<AgpdDropItemGroupEntryIter>::difference_type nCount = GroupEntryIter->m_lDropCount;
...
BOOL AgpmDropItem2::GetDropItemList( AgpdDropItems *pvtDropList, AgpdItemTemplate **ppDropItemTemplate, INT32 lTotalRate, const INT32 lItemNum /*= 1*/ )
{
	if ( AGPMDROPITEM_MAX_DROP_ITEM_COUNT < lItemNum )
		return FALSE;
	...
	ApSafeArray<INT32, AGPMDROPITEM_MAX_DROP_ITEM_COUNT>		arrRandomValue;
	arrRandomValue.MemSetAll();

	for (lIndex = 0; lIndex < lItemNum; ++lIndex)
	{
		arrRandomValue[lIndex]	= m_csRand.randInt(lTotalRate - 1);
	}
	...
	while (itItemList != pvtDropList->end())
	{
		...
		for (lIndex = 0; lIndex < lItemNum; ++lIndex)
		{
			if (lDropRate >= arrRandomValue[lIndex])
			{
				// OK, Drop 한다.
				ppDropItemTemplate[lIndex] = itItemList->m_pcsItemTemplate;
				arrRandomValue[lIndex]	= lTotalRate + 100;
