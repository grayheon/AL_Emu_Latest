...
AgpmPvP::AgpmPvP()
{
	...
	m_csPacket.SetFieldType(
							...
							AUTYPE_MEMORY_BLOCK,	1,	// PvP Detail
							AUTYPE_END,				0
							);
...
BOOL AgpmPvP::OnInit()
{
	...
	if(!m_pagpmBattleGround || !m_pagpmConfig || !m_pagpmShrineBattle)
		return FALSE;
...
BOOL AgpmPvP::AddRaceWinResult(AgpdCharacter *pcsCharacter, AuRaceType eType)
{
	if (NULL == pcsCharacter)
		return FALSE;

	if (eType <= AURACE_TYPE_NONE || eType > AURACE_TYPE_MAX)
		return FALSE;

	AgpdPvPADChar *pcsPvPADChar = GetADCharacter(pcsCharacter);
	if (NULL == pcsPvPADChar)
		return FALSE;

	InterlockedIncrement(&pcsPvPADChar->m_arrRaceScore[eType].m_lWin);

	return TRUE;
}

BOOL AgpmPvP::AddRaceLoseResult(AgpdCharacter* pcsCharacter, AuRaceType eType)
{
	if (NULL == pcsCharacter)
		return FALSE;
	...
	InterlockedIncrement(&pcsPvPADChar->m_arrRaceScore[eType].m_lLose);

	return TRUE;
}

BOOL AgpmPvP::SetRaceWinResult(AgpdCharacter* pcsCharacter, AuRaceType eType, LONG lPoint)
{
	if (NULL == pcsCharacter)
		return FALSE;
	...
	pcsPvPADChar->m_arrRaceScore[eType].m_lWin = lPoint;

	return TRUE;
}

BOOL AgpmPvP::SetRaceLoseResult(AgpdCharacter* pcsCharacter, AuRaceType eType, LONG lPoint)
{
	if (NULL == pcsCharacter)
		return FALSE;
	...
	pcsPvPADChar->m_arrRaceScore[eType].m_lLose = lPoint;
...
BOOL AgpmPvP::OnReceive(UINT32 ulType, PVOID pvPacket, INT16 nSize, UINT32 ulNID, DispatchArg *pstCheckArg)
{
	...
	CHAR* szGuildID = NULL;
	CHAR* szMessage = NULL;
	AgpdPvPADChar::AgpdRacePvPResult *pcsRacePvPResult = NULL;
	INT32 lLength = -1;
	...
	switch(cOperation)
	{
		case AGPMPVP_PACKET_PVP_INFO:
			OnReceivePvPInfo(lCID, lTargetCID, lWin, lLose, cPvPMode, cPvPStatus, cWin, pcsRacePvPResult);
			break;
...
BOOL AgpmPvP::OnReceivePvPInfo( INT32 lCID, INT32 lTargetCID, INT32 lWin, INT32 lLose, INT8 cPvPMode, INT8 cPvPStatus, INT8 cWin, PVOID pcsRacePvPResult )
{
	...
	if(pcsRacePvPResult)
	{
		for(int i = AURACE_TYPE_HUMAN ; i < AURACE_TYPE_MAX ; ++i)
		{
			if(((AgpdPvPADChar::AgpdRacePvPResult*)pcsRacePvPResult)[i].m_lWin != -1)
				pcsAttachedPvP->m_arrRaceScore[i].m_lWin = ((AgpdPvPADChar::AgpdRacePvPResult*)pcsRacePvPResult)[i].m_lWin;
			if(((AgpdPvPADChar::AgpdRacePvPResult*)pcsRacePvPResult)[i].m_lLose != -1)
				pcsAttachedPvP->m_arrRaceScore[i].m_lLose = ((AgpdPvPADChar::AgpdRacePvPResult*)pcsRacePvPResult)[i].m_lLose;
		}
	}
...
{
	if(lCID == AP_INVALID_CID || !szGuildID || strlen(szGuildID) <= 0 || strlen(szGuildID) > AGPMGUILD_MAX_GUILD_ID_LENGTH)
		return FALSE;
...
{
	if(!pcsCharacter || !szGuildID || strlen(szGuildID) <= 0 || strlen(szGuildID) > AGPMGUILD_MAX_GUILD_ID_LENGTH)
		return FALSE;
...
BOOL AgpmPvP::AddEnemyGuild(AgpdCharacter* pcsCharacter, CHAR* szGuildID, INT8 cPvPMode, UINT32 ulLastCombatClock)
{
	...
	ZeroMemory(csPvPGuildInfo.m_szGuildID, AGPMGUILD_MAX_GUILD_ID_LENGTH+1);
	strncpy(csPvPGuildInfo.m_szGuildID, szGuildID, AGPMGUILD_MAX_GUILD_ID_LENGTH);
...
BOOL AgpmPvP::IsAttackable(AgpdCharacter* pcsCharacter, AgpdCharacter* pcsTarget, BOOL bForce)
{
	...
	if(!pcsCharacter || !pcsTarget || !pcsCharacter->m_pcsCharacterTemplate || !pcsTarget->m_pcsCharacterTemplate)
		return FALSE;
	...
	//JK_쉬라인머지
	if( m_pagpmShrineBattle->IsInShrineRegionByIndex(pcsCharacter) && m_pagpmShrineBattle->IsInShrineRegionByIndex(pcsTarget))
	{
		return (!m_pagpmCharacter->IsSameRace(pcsCharacter, pcsTarget));
	}
	...
	if (!bIsInSiegeWarIngArea)
	{
		...
		if(IsEnemyCharacter(pcsCharacter, pcsTarget))
			return TRUE;
		if(!m_pagpmShrineBattle->IsInShrineRegionByIndex(pcsCharacter))//JK_쉬라인머지
		{
...
BOOL AgpmPvP::IsSkillEnable(AgpdSkill* pcsSkill, AgpdCharacter* pcsTarget, AgpdCharacter* pcsSummonsOwner)
{
	...
	if(bAttackSkill)
	{
		...
		if( m_pagpmShrineBattle->IsInShrineRegionByIndex(pcsCharacter) && m_pagpmShrineBattle->IsInShrineRegionByIndex(pcsTarget))
		{
			if (m_pagpmCharacter->IsSameRace(pcsCharacter, pcsTarget))
				return FALSE;
		}
