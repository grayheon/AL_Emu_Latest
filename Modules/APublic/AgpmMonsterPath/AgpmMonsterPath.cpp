...
BOOL	AgpmMonsterPath::SavePath( const string &strINIFolder , BOOL bUpdateListSave )
{
	...
	for( ; Iter != m_MonsterPath.end() ; ++Iter )
	{
		...
		sprintf_s( szCurrentPath , "%s//%s" , szCurrentPath , strINIFolder.c_str() );

		CreateDirectory( szCurrentPath , NULL );
...
{
	return AddMonsterPath( pMonsterPath , (INT)Position.x , (INT)Position.z );
}
