...
BOOL AgpmGrid::Init( AgpdGrid *pcsAgpdGrid, EnumGridType eGridType )
{
	...
	case AGPDGRID_TYPE_CHARGING_INVENTORY:
		{
			return CreateByMemoryPool(eGridType, AGPMITEM_CHARGING_TYPE_LAYER, AGPMITEM_CHARGING_TYPE_ROW, AGPMITEM_CHARGING_TYPE_COLUMN,
				pcsAgpdGrid, &m_ChargingTypeInventoryGridItem, &m_ChargingInventoryLockTable);
		}
		break;
...
void AgpmGrid::InitGridMemoryPool(INT32 lPoolCount)
{
	...
	lGridCount = AGPMITEM_CHARGING_TYPE_LAYER * AGPMITEM_CHARGING_TYPE_ROW * AGPMITEM_CHARGING_TYPE_COLUMN;
	m_ChargingTypeInventoryGridItem.Initialize(lGridCount * sizeof(AgpdGridItem*), lPoolCount, "m_ChargingTypeInventoryGridItem");
	m_ChargingInventoryLockTable.Initialize(lGridCount * sizeof(BOOL), lPoolCount, "m_ChargingInventoryLockTable");
...
BOOL AgpmGrid::Remove( AgpdGrid *pcsAgpdGrid )
{
	...
	case AGPDGRID_TYPE_CHARGING_INVENTORY: return DeleteByMemoryPool(pcsAgpdGrid, &m_ChargingTypeInventoryGridItem, &m_ChargingInventoryLockTable);
...
BOOL AgpmGrid::AddToLayer( AgpdGrid *pcsAgpdGrid, INT16 nLayer, AgpdGridItem *pcsGridItem, INT16 nHeight, INT16 nWidth, INT16 *nRow, INT16 *nColumn )
{
	...
	for (int i = 0; i < pcsAgpdGrid->m_nRow; ++i)
	{
		...
		{
			if (iStartOfLayer + i * pcsAgpdGrid->m_nColumn + j >= pcsAgpdGrid->m_lGridCount)
				return FALSE;
...
{
	AgpdGridItem* pcsItemGrid = (AgpdGridItem*) CreateModuleData<AgpdGridItem>( AGPMGRID_DATA_GRIDITEM );
