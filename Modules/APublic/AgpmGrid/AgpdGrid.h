...
enum EnumGridType
{
	...
	AGPDGRID_TYPE_CHARGING_INVENTORY,	
...
protected:
	...
	BOOL				m_bIsDurabilityUnder20;	// 내구도가 20%이하임을 표시하는것 (0과 동일시 하면 캐릭터 속성에 이상하게 출력됨)
	BOOL				m_bIsLevelLimited	;	// 레벨 제한이 걸린놈인지.
public:

	BOOL				SetLevelLimited( BOOL bValue ) { return m_bIsLevelLimited = bValue; }
	BOOL				SetDurabilityUnder20( BOOL bValue ) { return m_bIsDurabilityUnder20 = bValue; }
	...
	BOOL				IsDurabilityUnder20() const	{ return m_bIsDurabilityUnder20; }
	BOOL				IsLevelLimited() const		{ return m_bIsLevelLimited; }

	BOOL				IsDisabled() const			{ return IsDurabilityZero() || IsLevelLimited() || IsDurabilityUnder20(); }
