...
FLOAT AgpmCombat::GetPhysicalResistance(AgpdCharacter* pcsAttacker, AgpdCharacter* pcsTarget)
{
	if(!pcsAttacker || !pcsAttacker->m_pcsCharacterTemplate || !pcsTarget || !pcsTarget->m_pcsCharacterTemplate)
		return 0.0f;

	FLOAT fPhysicalResistance = 0.0f;
	m_pcsAgpmFactors->GetValue(&pcsTarget->m_csFactor, &fPhysicalResistance, AGPD_FACTORS_TYPE_RESULT, AGPD_FACTORS_TYPE_DEFENSE, AGPD_FACTORS_DEFENSE_TYPE_DEFENSE_RATE, AGPD_FACTORS_ATTRIBUTE_TYPE_PHYSICAL);

	fPhysicalResistance -= pcsAttacker->m_stStatusIgnore.m_lIgnorePhysicalAttackResist;

	// 2008.03.24. steeple
	// 0일때 1일 리턴하는 거 기렬이가 지우라고 했음.
	if(fPhysicalResistance > 80.0f)
		fPhysicalResistance = 80.0f;
	else if(fPhysicalResistance < 0.0f)
		fPhysicalResistance = 0.0f;

	return fPhysicalResistance;
}

// 2007.07.31. steeple
INT32 AgpmCombat::GetPhysicalBlockRate(AgpdCharacter* pcsAttacker, AgpdCharacter* pcsTarget)
{
	if(!pcsAttacker || !pcsAttacker->m_pcsCharacterTemplate || !pcsTarget || !pcsTarget->m_pcsCharacterTemplate)
		return 0;

	INT32 lPhysicalBlockRate = 0;
	m_pcsAgpmFactors->GetValue(&pcsTarget->m_csFactor, &lPhysicalBlockRate, AGPD_FACTORS_TYPE_RESULT, AGPD_FACTORS_TYPE_DEFENSE, AGPD_FACTORS_DEFENSE_TYPE_DEFENSE_RATE, AGPD_FACTORS_ATTRIBUTE_TYPE_PHYSICAL_BLOCK);

	lPhysicalBlockRate -= pcsAttacker->m_stStatusIgnore.m_lIgnoreBlockRate;

	if(lPhysicalBlockRate > 80)
		lPhysicalBlockRate = 80;
	else if(lPhysicalBlockRate < 0)
		lPhysicalBlockRate = 0;

	return lPhysicalBlockRate;
}

// 2007.07.31. steeple
INT32 AgpmCombat::GetSkillBlockRate(AgpdCharacter* pcsAttacker, AgpdCharacter* pcsTarget)
{
	if(!pcsAttacker || !pcsAttacker->m_pcsCharacterTemplate || !pcsTarget || !pcsTarget->m_pcsCharacterTemplate)
		return 0;

	INT32 lSkillBlockRate = 0;
	m_pcsAgpmFactors->GetValue(&pcsTarget->m_csFactor, &lSkillBlockRate, AGPD_FACTORS_TYPE_RESULT, AGPD_FACTORS_TYPE_DEFENSE, AGPD_FACTORS_DEFENSE_TYPE_DEFENSE_RATE, AGPD_FACTORS_ATTRIBUTE_TYPE_SKILL_BLOCK);

	lSkillBlockRate -= pcsAttacker->m_stStatusIgnore.m_lIgnoreSkillBlockRate;

	if(lSkillBlockRate > 80)
		lSkillBlockRate = 80;
	else if(lSkillBlockRate < 0)
		lSkillBlockRate = 0;
...
// 2005.12.22. steeple
INT32 AgpmCombat::GetAdditionalEvadeRate(AgpdCharacter* pcsAttacker, AgpdCharacter* pcsTarget)
{
	if(!pcsAttacker || !pcsAttacker->m_pcsCharacterTemplate || !pcsTarget || !pcsTarget->m_pcsCharacterTemplate)
		return 0;

	INT32 lEvadeRate = 0;
	m_pcsAgpmFactors->GetValue(&pcsTarget->m_csFactor, &lEvadeRate, AGPD_FACTORS_TYPE_RESULT, AGPD_FACTORS_TYPE_ATTACK, AGPD_FACTORS_ATTACK_TYPE_EVADE_RATE);

	lEvadeRate -= pcsAttacker->m_stStatusIgnore.m_lIgnoreEvadeRate;
	
	if(lEvadeRate > 80)
		lEvadeRate = 80;
	else if(lEvadeRate < 0)
		lEvadeRate = 0;
...
// 2005.12.22. steeple
INT32 AgpmCombat::GetAdditionalDodgeRate(AgpdCharacter* pcsAttacker, AgpdCharacter* pcsTarget)
{
	if(!pcsAttacker || !pcsAttacker->m_pcsCharacterTemplate || !pcsTarget || !pcsTarget->m_pcsCharacterTemplate)
		return 0;

	INT32 lDodgeRate = 0;
	m_pcsAgpmFactors->GetValue(&pcsTarget->m_csFactor, &lDodgeRate, AGPD_FACTORS_TYPE_RESULT, AGPD_FACTORS_TYPE_ATTACK, AGPD_FACTORS_ATTACK_TYPE_DODGE_RATE);

	lDodgeRate -= pcsAttacker->m_stStatusIgnore.m_lIgnoreDodgeRate;

	if(lDodgeRate > 80)
		lDodgeRate = 80;
	else if(lDodgeRate < 0)
		lDodgeRate = 0;
...
//	m_pcsAgpmFactors->GetValue(&pcsItem->m_csFactor, &lShieldDefenseRatePhysical, AGPD_FACTORS_TYPE_DEFENSE, AGPD_FACTORS_DEFENSE_TYPE_DEFENSE_RATE, AGPD_FACTORS_ATTRIBUTE_TYPE_PHYSICAL);
	...
	INT32	lShieldDefenseRatePhysical	= GetPhysicalBlockRate(pcsAttacker, pcsTargetChar) - lPanelty;
...
INT32 AgpmCombat::CalcPhysicalAttack(AgpdCharacter *pcsAttackChar, AgpdCharacter *pcsTargetChar, BOOL bIsEquipWeapon, BOOL bIsSpec, BOOL bIsMin, int nCombatPreCheck)
{
	...
	FLOAT		fPhysicalResistance	= GetPhysicalResistance(pcsAttackChar, pcsTargetChar) - lPenalty;
...
AgpdCharacterActionResultType AgpmCombat::ProcessEvade(AgpdCharacter* pcsAttackChar, AgpdCharacter* pcsTargetChar)
{
	...
	{
		// Dodge 체크
		INT32 lDodgeRate = GetAdditionalDodgeRate(pcsAttackChar, pcsTargetChar) - lPenalty;
	...
	{
		// Evade 체크
		INT32 lEvadeRate = GetAdditionalEvadeRate(pcsAttackChar, pcsTargetChar) - lPenalty;
