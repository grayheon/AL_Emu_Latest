...
BOOL	__CBCustomCommandCallBack (PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	if( !strcmp( pStrCommand , "/PS" ) )
	{
		if( pArg->GetArgCount() < 2 )
		{
			...
			AgcChatManager::OnAddSystemMessage( strMessage , 0xffff0000 );
		}
		else
		{
...
BOOL MainWindow::LoadTemplates()
{
	...
	{
		BOOL bResult = FALSE;
		FILE* pFile = fopen(".\\ArchlordTest.ini", "r");	//테스트 파일 있는지 확인하고
		if(!pFile)		// 없으면
		{
			// 분할된 파일만 읽는다
...
//#endif // _DEBUG
	...
	if (g_pcsAgcmWarInfo)
	{
		g_pcsAgcmWarInfo->ReadStreamWarInfo("Ini\\WarInfoData.txt", m_bDecryption);
#ifdef _DEBUG
		GlobalMemoryStatus(&stMemory);
		TRACE("Loaded ... TipText.txt - Allocation : %d\n", stMemory.dwTotalVirtual - stMemory.dwAvailVirtual);
#endif // _DEBUG
	...
	if( g_pcsAgcmQuest )
	{
		g_pcsAgcmQuest->ReadXMLData( "ini\\QuestNameColor.xml" );
	}

	if( g_pcsAgcmQuestJournal )
	{
		g_pcsAgcmQuestJournal->ReadJournalText( "ini\\Quest_Aid_Data.txt", m_bDecryption );
	}
...
//#endif // _BIN_EXEC_
	...
	if (g_pcsAgcmUIChatting2)
	{
		g_pcsAgcmUIChatting2->OnLoadFilterFile( "INI\\Base.dat", m_bDecryption );
		g_pcsAgcmUIChatting2->OnLoadFilterFile( "INI\\CustomTextFilter.txt", m_bDecryption );
	}
...
BOOL MainWindow::OnIdle			( UINT32 ulClockCount )
{
	...
	*/
	...
	if( g_eMiniDumpType == MiniDumpWithFullMemory && g_pEngine->GetDebugFlag() & AgcEngine::OBJECT_LOADING_ERROR )
	{
		...
		AgcChatManager::OnAddSystemMessage( "Object Loading Failed!" );
