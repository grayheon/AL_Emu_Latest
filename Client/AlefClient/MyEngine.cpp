...
AgpmEventGuide*			g_pcsAgpmEventGuide = NULL;
...
AgcmUIWorldmap			*g_pcsAgcmUIWorldmap = NULL;

AgcmUIQuestJournal		*g_pcsAgcmUIQuestJournal	=	NULL;
AgcmQuestJournal		*g_pcsAgcmQuestJournal		=	NULL;

AgcmShrine					*g_pcsAgcmShrine				= NULL;//JK_쉬라인머지
AgpmShrineBattle*		g_pcsAgpmShrineBattle			= NULL;


AgcmUIGuideMessage		*g_pcsAgcmUIGuideMessage = NULL;

AgcmUIShrineShop*		g_pcsAgcmUIShrineShop	=	NULL;

AgpmWarInfo				*g_pcsAgpmWarInfo = NULL;
AgcmWarInfo				*g_pcsAgcmWarInfo = NULL;
AgcmUIWarInfo			*g_pcsAgcmUIWarInfo = NULL;

AgcmUINotice			*g_pcsAgcmUINotice = NULL;
// exe, dat
...
{
	m_bEmulationFullscreen =  (g_eServiceArea == AP_SERVICE_AREA_CHINA || g_eServiceArea == AP_SERVICE_AREA_TAIWAN) ? TRUE : FALSE;	//중국은 IME때문에 강제 풀스크린
...
BOOL MyEngine::OnRegisterModule()
{
	...
	*/
	...
	REGISTER_MODULE( g_pcsAgpmEventGuide, AgpmEventGuide )
	...
	REGISTER_MODULE( g_pcsAgcmUIWarInfo, AgcmUIWarInfo);
	REGISTER_MODULE( g_pcsAgpmWarInfo, AgpmWarInfo);
	REGISTER_MODULE( g_pcsAgcmWarInfo, AgcmWarInfo);

	REGISTER_MODULE( g_pcsAgpmShrineBattle, AgpmShrineBattle );//JK_쉬라인머지
	REGISTER_MODULE( g_pcsAgcmShrine, AgcmShrine );
	...
	REGISTER_MODULE( g_pcsAgcmUIWorldmap, AgcmUIWorldmap );
	REGISTER_MODULE( g_pcsAgcmQuestJournal , AgcmQuestJournal );
	REGISTER_MODULE( g_pcsAgcmUIQuestJournal , AgcmUIQuestJournal );

	REGISTER_MODULE( g_pcsAgpmTitle, AgpmTitle );

	REGISTER_MODULE( g_pcsAgcmUIGuideMessage, AgcmUIGuideMessage );

	REGISTER_MODULE( g_pcsAgcmUIShrineShop , AgcmUIShrineShop );

	REGISTER_MODULE( g_pcsAgcmUINotice , AgcmUINotice );
		...
		case AP_SERVICE_AREA_GLOBAL :
			{
				...
				g_pcsAgpmResourceInfo->SetMinorVersion(3);
			}
		break;
#endif
	...
	if (g_pcsApmObject)
	{
		g_pcsApmObject->		SetMaxObjectTemplate	( 6000							);
	...
	{
		g_pcsAgpmEventTeleport->SetMaxTeleportPoint(AGPMTELEPORT_MAX_TELEPORT_POINT);
		g_pcsAgpmEventTeleport->SetMaxTeleportGroup(AGPMTELEPORT_MAX_TELEPORT_GROUP);
	}
...
BOOL MyEngine::OnInit()
{
	...
	{
		char* szString = "2007.11.22 부터 아크로드가 [한게임]으로 서비스 전환되었습니다.\n\
아크로드 접속은 공식홈페이지를 통해 가능합니다.\n\
또한, 아직 한게임 전환을 하지 않으신 분께서는 반드시 '한게임 전환 신청'을 해주셔야 아크로드 접속이 가능합니다.";

		if( MessageBox( NULL, szString, "아크로드", MB_OK ) )
	...
	case WM_ALEFPOSITIONWARP:
		{
			...
			if( pcmUIChatting )
			{
				pcmUIChatting->OnSendMessage( AGPDCHATTING_TYPE_NORMAL , strMessage , strlen( strMessage ) , NULL );
			}
		}
		break;
	case WM_DROPFILES:
...
VOID MyEngine::SetupLanguage()
{
	...
	else if (!stricmp(szTemp, "Turkish"))
	{
		strcpy(m_szLanguage, "turk\\");
		g_INIFileName = "ini\\turk\\sysstr.txt";
	}
...
void	MyEngine::LuaErrorMessage( const char * pStr )
{
	...
	AgcChatManager::OnAddSystemMessage( ( char * ) pStr );
}
