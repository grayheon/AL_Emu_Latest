...
BOOL CModelToolApp::CopyCharacterTemplate(INT32 lTID)
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		pcsDestADCharTemplate->EquipItemSetInfo::m_lDefaultEquipITID.MemCopy(0, &pcsSrcADCharTemplate->EquipItemSetInfo::m_lDefaultEquipITID[0], AGPMITEM_PART_NUM);

		///////////////////////////////////////////////////////////////////////
		//. 2005. 09. 12 Nonstopdj
		for( int nIndex = 0; nIndex < AGPMITEM_DEFAULT_INVEN_NUM; ++nIndex )
		{
			if(pcsSrcADCharTemplate->InvenItemSetInfo::m_aalDefaultInvenInfo[nIndex][AGPDITEM_DEFAULT_INVEN_INDEX_TID])
			{
				pcsDestADCharTemplate->InvenItemSetInfo::m_aalDefaultInvenInfo[nIndex][AGPDITEM_DEFAULT_INVEN_INDEX_TID] = 
					pcsSrcADCharTemplate->InvenItemSetInfo::m_aalDefaultInvenInfo[nIndex][AGPDITEM_DEFAULT_INVEN_INDEX_TID];

				pcsDestADCharTemplate->InvenItemSetInfo::m_aalDefaultInvenInfo[nIndex][AGPDITEM_DEFAULT_INVEN_INDEX_STACK_COUNT] = 
					pcsSrcADCharTemplate->InvenItemSetInfo::m_aalDefaultInvenInfo[nIndex][AGPDITEM_DEFAULT_INVEN_INDEX_STACK_COUNT];
		...
		{
			if ( !pcsDestADCharTemplate->EquipItemSetInfo::m_lDefaultEquipITID[lPartID] )		continue;

			AgpdItemTemplate* ppDefItem	= m_csAgcEngine.GetAgpmItemModule()->GetItemTemplate(pcsDestADCharTemplate->EquipItemSetInfo::m_lDefaultEquipITID[lPartID]);
...
BOOL CModelToolApp::CopyItemTemplate(INT32 lTID)
{
	...
	{		
		...
		strcpy(pstAgpdItemTemplate2->m_szName, szTName);
