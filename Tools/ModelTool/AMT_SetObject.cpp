...
AgpdCharacter *CModelToolApp::SetCharacter(INT32 lTID, HTREEITEM hSelected, BOOL bResetCamera, BOOL bReleaseAll, INT32 lForceCID, RwV3d *pv3dForcePos)
{
	...
	if (m_csAgcEngine.GetAgpmItemModule())
	{
		...
		{
			if( pcsAgpdItemADCharTemplate->EquipItemSetInfo::m_lDefaultEquipITID[ nPart + nSpecialOffset ] )
			{
				if( !EquipItem(lCID, pcsAgpdItemADCharTemplate->EquipItemSetInfo::m_lDefaultEquipITID[ nPart + nSpecialOffset ] ) )
					continue;
			}
			else if( pcsAgpdItemADCharTemplate->EquipItemSetInfo::m_lDefaultEquipITID[ nPart ] )
			{
				if( !EquipItem(lCID, pcsAgpdItemADCharTemplate->EquipItemSetInfo::m_lDefaultEquipITID[ nPart ] ) )
					continue;
			}
		}
	}
	...
	if( pcsAgcdCharacter && pcsAgcdCharacter->m_pvTextboard )
		pIDBoard	=	((sBoardDataPtr)pcsAgcdCharacter->m_pvTextboard)->pIDBoard;
...
BOOL CModelToolApp::GenerateAllItemTemplateBSphere()
{
	...
	char pMsgBuff[256];
	int nCurCount = 0;
	int nTotalCount = m_csAgcEngine.GetAgpmItemModule()->csTemplateAdmin.size();
	//@}
