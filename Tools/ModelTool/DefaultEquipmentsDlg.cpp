	...
	: CDialog(CDefaultEquipmentsDlg::IDD, pParent)
{
	...
	if( CModelToolApp::GetInstance() )
	{
		...
		if( pcsAgpmItem )
		{
			...
			{
				if( pcsAgpdItemADCharTemplate->EquipItemSetInfo::m_lDefaultEquipITID[lPartID] )
				{
					AgpdItemTemplate* pcsAgpdItemTemplate = pcsAgpmItem->GetItemTemplate( pcsAgpdItemADCharTemplate->EquipItemSetInfo::m_lDefaultEquipITID[lPartID]) ;
...
{
	CItemTemplateListDlg dlg( &m_csTempData.EquipItemSetInfo::m_lDefaultEquipITID[ lEqipID ] );
	...
	AgpdItemTemplate *pcsAgpdItemTemplate = pcsAgpmItem->GetItemTemplate( m_csTempData.EquipItemSetInfo::m_lDefaultEquipITID[lEqipID] );
...
{
	m_csTempData.EquipItemSetInfo::m_lDefaultEquipITID[lEquipID] = 0;
