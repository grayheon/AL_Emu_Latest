...
{
	// 테스트중이면 무조건 어드민으로
	if( AuIgnoringHelper::IgnoreToFile( "TestAdminDll.arc" ) )
		return AGPMADMIN_LEVEL_5;
	else
...
BOOL AlefAdminManager::ConvertTimeStampToString(UINT32 ulTimeStamp, CHAR* szTmp)
{
	...
	return strlen(szTmp) > 0 ? TRUE : FALSE;
}
...
BOOL AlefAdminManager::AddItemTemplate(stAgpdAdminItemTemplate* pstItemTemplate)
{
	...
	//hash_map<string, stAgpdAdminItemTemplate*>::const_iterator iterData = m_hashMapItemTemplate.find(pstItemTemplate->m_szItemName);
	...
	//	hash_map<string, stAgpdAdminItemTemplate*>::value_type(
...
BOOL AlefAdminManager::ClearItemTemplateData()
{
	...
	//hash_map<string, stAgpdAdminItemTemplate*>::const_iterator iterData = m_hashMapItemTemplate.begin();
