...
BOOL AlefAdminCharacter::SetCharData(AgpdCharacter* pcsCharacter)
{
	...
	// Attack Rate
	m_stCharData.m_stCombat.m_lAttackRate[0] = (INT32)pcsAgpmCombat->GetAR(pcsCharacter, pcsAgpmFactors->GetClass(&pcsCharacter->m_pcsCharacterTemplate->m_csFactor));
	m_stCharData.m_stCombat.m_lAttackRate[1] = (INT32)pcsAgpmCombat->GetBaseAR(pcsCharacter, pcsAgpmFactors->GetClass(&pcsCharacter->m_pcsCharacterTemplate->m_csFactor));
	...
	// Block
	m_stCharData.m_stCombat.m_lBlock[0] = (INT32)pcsAgpmCombat->GetDR(pcsCharacter, pcsAgpmFactors->GetClass(&pcsCharacter->m_pcsCharacterTemplate->m_csFactor));
	m_stCharData.m_stCombat.m_lBlock[1] = (INT32)pcsAgpmCombat->GetBaseDR(pcsCharacter, pcsAgpmFactors->GetClass(&pcsCharacter->m_pcsCharacterTemplate->m_csFactor));
...
BOOL AlefAdminCharacter::EditCharToDB(LPCTSTR szCharName, LPCTSTR szEditField, LPCTSTR szNewValue)
{
	...
	if(stCharEdit.m_lEditField == AGPMADMIN_CHAREDIT_DB)
	{
		...
		sprintf(stCharEdit.m_szNewValueChar, "%ld,%ld,%ld", 
												...
												(INT32)m_stCharData.m_stStatus.m_stPos.y,
												(INT32)m_stCharData.m_stStatus.m_stPos.z);
		stCharEdit.m_lNewValueCharLength = (INT16)strlen(stCharEdit.m_szNewValueChar);
	...
	else if(stCharEdit.m_lEditField == AGPMADMIN_CHAREDIT_DB2)
	{
		...
		stCharEdit.m_lNewValueFloat = (FLOAT)m_stCharData.m_stStatus.m_lMurdererPoint;
	...
	{
		stCharEdit.m_lNewValueFloat = (FLOAT)atof(szNewValue);
		bEditEnableField = TRUE;
	}
	else if(strcmp(szEditField, (LPCTSTR)AlefAdminManager::Instance()->GetStringManager().GetResourceMessage(IDS_RCF)) == 0)
	{
