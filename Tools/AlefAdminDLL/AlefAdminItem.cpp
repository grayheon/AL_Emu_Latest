...
enum eAlefAdminItemIndex
{
	...
	ALEFADMINITEM_INDEX_CHARGING_INVEN		= 12,
	ALEFADMINITEM_INDEX_BANK_START			= 13,
...
AlefAdminItem::~AlefAdminItem()
{
	...
	OnClearInventory( Inventory_ChargingInven );
}
...
BOOL AlefAdminItem::InitPosCB()
{
	...
	m_csItemPosCB.AddString(AlefAdminManager::Instance()->GetStringManager().GetResourceMessage(IDS_ITEM_CHARGING_INVEN));
...
BOOL AlefAdminItem::InitExpireDate()
{
	...
	m_csExpireDateDefaultCB.AddString(AlefAdminManager::Instance()->GetStringManager().GetResourceMessage(IDS_ITEM_EXPIRE_ENDDATE));
	m_csExpireDateDefaultCB.AddString(AlefAdminManager::Instance()->GetStringManager().GetResourceMessage(IDS_ITEM_EXPIRE_DEFAULT));
...
BOOL AlefAdminItem::SetDefaultRemainTime(INT64 lRemainTime)
{
	...
	{
		SetDlgItemInt(IDC_E_R_TIME_DAY, (UINT)(lRemainTime / ONE_DAY));
		SetDefaultRemainTime(lRemainTime % ONE_DAY);
	}
	else if(lRemainTime >= ONE_HOUR)
	{
		SetDlgItemInt(IDC_E_R_TIME_HOUR, (UINT)(lRemainTime / ONE_HOUR));
		SetDefaultRemainTime(lRemainTime % ONE_HOUR);
	}
	else if(lRemainTime >= ONE_MINUTE)
	{
		SetDlgItemInt(IDC_E_R_TIME_MIN, (UINT)(lRemainTime / ONE_MINUTE));
		SetDefaultRemainTime(lRemainTime % ONE_MINUTE);
	}
	else
	{
		SetDlgItemInt(IDC_E_R_TIME_SEC, (UINT)(lRemainTime / 1000));
...
BOOL AlefAdminItem::OnReceive(stAgpdAdminItem* pstItem)
{
	...
	case AGPDITEM_STATUS_INVENTORY :
		OnAddItemToInventory( Inventory_Normal, pstItem );
		break;
	case AGPDITEM_STATUS_EQUIP :
		OnAddItemToInventory( Inventory_Equip, pstItem );
		break;
	case AGPDITEM_STATUS_SUB_INVENTORY :
		OnAddItemToInventory( Inventory_Pet, pstItem );	
		break;
	case AGPDITEM_STATUS_BANK :
		OnAddItemToInventory( Inventory_Bank, pstItem );
		break;
	case AGPDITEM_STATUS_SALESBOX_GRID :
	case AGPDITEM_STATUS_SALESBOX_BACKOUT :
		OnAddItemToInventory( Inventory_Sales, pstItem );
		break;
	case AGPDITEM_STATUS_CASH_INVENTORY :
		OnAddItemToInventory( Inventory_Cash, pstItem );
		break;
	case AGPDITEM_STATUS_MAILBOX :
		OnAddItemToInventory( Inventory_MailBox, pstItem );
		break;
	case AGPDITEM_STATUS_CHARGING_INVENTORY :
		OnAddItemToInventory( Inventory_ChargingInven, pstItem );
		break;
	}
...
INT32 AlefAdminItem::GetListCtrlIndex(stAgpdAdminItem* pstItem)
{
	...
	else if(pstItem->m_lPos == AGPDITEM_STATUS_CHARGING_INVENTORY)
	{
		lIndex = (pstItem->m_lRow * AGPMITEM_CHARGING_TYPE_COLUMN) + pstItem->m_lCol + pstItem->m_lRow;
	}
...
BOOL AlefAdminItem::GetItemPosByIndex(stAgpdAdminItem* pstItem, int iLVIndex)
{
	...
	else if(iCurSel == ALEFADMINITEM_INDEX_CHARGING_INVEN)
	{
		pstItem->m_lPos = AGPDITEM_STATUS_CHARGING_INVENTORY;
...
BOOL AlefAdminItem::ShowData(stAgpdAdminItem* pstItem)
{
	...
	if(iTypeIndex == ALEFADMINITEM_INDEX_EQUIP && pstItem->m_lPos == AGPDITEM_STATUS_EQUIP)
		...
		bShowData = TRUE;
	else if(iTypeIndex == ALEFADMINITEM_INDEX_CHARGING_INVEN && pstItem->m_lPos == AGPDITEM_STATUS_CHARGING_INVENTORY)
		bShowData = TRUE;
...
BOOL AlefAdminItem::ShowData(INT32 lTypeIndex)
{
	...
	switch( lTypeIndex )
	{
	case ALEFADMINITEM_INDEX_SALESBOX :
		return OnShowInventory( Inventory_Sales );		break;
	case ALEFADMINITEM_INDEX_CASH_ITEM :
		return OnShowInventory( Inventory_Cash );		break;
	case ALEFADMINITEM_INDEX_MAILBOX :
		return OnShowInventory( Inventory_MailBox );	break;
	case ALEFADMINITEM_INDEX_CHARGING_INVEN	:
		return OnShowInventory( Inventory_ChargingInven ); break;
	default :
		{
...
BOOL AlefAdminItem::InitItemList(INT32 lTypeIndex)
{
	...
	else if(lTypeIndex == ALEFADMINITEM_INDEX_CHARGING_INVEN)
	{
		CHAR szTmp[32];
		memset(szTmp, 0, sizeof(CHAR) * 32);

		for(int i = 0; i < AGPMITEM_CHARGING_TYPE_ROW; i++)
		{
			for(int j = 0; j < AGPMITEM_CHARGING_TYPE_COLUMN; j++)
			{
				sprintf(szTmp, "%d, %d", i, j);
				m_csItemList.InsertItem(iIndex++, szTmp);
			}
...
void AlefAdminItem::OnCbnSelchangeCbPos()
{
	...
	if(iIndex == ALEFADMINITEM_INDEX_CASH_ITEM &&
		AlefAdminManager::Instance()->GetAdminLevel() < AGPMADMIN_LEVEL_ADMIN)
	{
...
void AlefAdminItem::OnNMClickLcItem(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	{
	case AGPDITEM_STATUS_INVENTORY :
		pstItem = GetItem( Inventory_Normal, &stItem );
		break;
	case AGPDITEM_STATUS_EQUIP :
		pstItem = GetItem( Inventory_Equip, &stItem );
		break;
	case AGPDITEM_STATUS_BANK :
		pstItem = GetItem( Inventory_Bank, &stItem );
		break;
	case AGPDITEM_STATUS_SALESBOX_GRID :
		pstItem = GetItem( Inventory_Sales, &stItem );
		break;
	case AGPDITEM_STATUS_SUB_INVENTORY :
		pstItem = GetItem( Inventory_Pet, &stItem );
		break;
	case AGPDITEM_STATUS_CASH_INVENTORY :
		pstItem = GetItem( Inventory_Cash, &stItem );
		break;
	case AGPDITEM_STATUS_MAILBOX :
		pstItem = GetItem( Inventory_MailBox, &stItem );
		break;
	case AGPDITEM_STATUS_CHARGING_INVENTORY :
		pstItem = GetItem( Inventory_ChargingInven, &stItem );
		break;
	}
...
BOOL AlefAdminItem::OnAddItemToInventory( eInventoryType eInventory, stAgpdAdminItem* ppdItem )
{
	...
	{
		if( AlefAdminManager::Instance()->GetAdminLevel() < AGPMADMIN_LEVEL_ADMIN )
		{
			return FALSE;
		}
	}
...
void* AlefAdminItem::_SelectInventory( eInventoryType eInventory )
{
	...
	case Inventory_ChargingInven :	return &m_vecItemInventory_ChargingInven; break;
	};
...
BOOL AlefAdminItem::_UpdateItemPosition_ChargingInven( stAgpdAdminItem* ppdItem )
{
	AFX_MANAGE_STATE( AfxGetStaticModuleState() );
	if( !ppdItem ) return FALSE;
...
INT16 AlefAdminItem::_GetInventoryMaxLayer( eInventoryType eInventory )
{
	...
	case Inventory_ChargingInven :	return AGPMITEM_CHARGING_TYPE_LAYER; break;
...
INT16 AlefAdminItem::_GetInventoryMaxRow( eInventoryType eInventory )
{
	...
	case Inventory_ChargingInven :	return AGPMITEM_CHARGING_TYPE_ROW;	break;
...
INT16 AlefAdminItem::_GetInventoryMaxCol( eInventoryType eInventory )
{
	...
	case Inventory_ChargingInven :	return AGPMITEM_CHARGING_TYPE_COLUMN; break;
