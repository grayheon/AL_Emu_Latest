...
{
	// 마고�?(2003-11-13 오후 3:41:44) : 맵이 커져�?톰앱 설정�?오류가 생겼�?.
	int	dx , dz;
...
BOOL MainWindow::OnInit()		// 초기�?, 데이타 수집 & 그래�?데이타 로딩.	WindowUI 의경�?Open돼는 시점, Full UI�?경우 Setting돼는 시점.
{
	char	path[ 1024 ];
	...
	{
		::MessageBox( NULL , "�޷���ȡ����ģ�塣ģ�����δ��ȷ����" , "��ʾ" , MB_ICONERROR | MB_OK );
	}

	// netong 추가 (AgcmEventBindingDlg 관�?
	theApp.SetStartUpProcess( 42 );
	TRACE( "캐릭�?타�?로딩\n" );
	if( g_pcsAgpmFactors && !g_pcsAgpmFactors->CharacterTypeStreamRead(".\\Ini\\CharType.ini", FALSE ) )
	{
		::MessageBox( NULL , "��ɫ���Ͷ�ȡʧ�ܡ���ɫ���Ϳ���û���������ء�." , "��ʾ" , MB_ICONERROR | MB_OK );
	}
	// netong 추가
	...
	if (g_pcsAgcmWater)
	{
		try
		{
			...
			{
				::MessageBox( NULL , "./Ini/WaterStatusT1.ini �ļ���ȡʧ�ܡ���ȷ��" , "맵툴" , MB_OK | MB_ICONERROR );
			...
			{
				::MessageBox( NULL , "./Ini/HWaterStatus.ini �ļ���ȡʧ�ܡ���ȷ��" , "맵툴" , MB_OK | MB_ICONERROR );
		...
		{
			if( IDYES == ::MessageBox( NULL , "./Ini/WaterStatusT1.ini ���ڿ��ļ���ʱ����������ˡ���Ҫɱ�����ڵĳ�����" , "맵툴" , MB_YESNOCANCEL | MB_ICONERROR ) )
			{
				throw;
			}
		}
	...
	if (g_pcsAgcmGrass)
	{
		try
		{
			...
			{
				::MessageBox( NULL , "./Ini/GrassTemplate.ini �ļ���ȡʧ�ܡ���ȷ��" , "맵툴" , MB_OK | MB_ICONERROR );
		...
		{
			if( IDYES == ::MessageBox( NULL , "./Ini/GrassTemplate.ini ���ڿ��ļ���ʱ����������ˡ���Ҫɱ�����ڵĳ�����" , "맵툴" , MB_YESNOCANCEL | MB_ICONERROR ) )
			{
				throw;
			}
		}
	...
	*/
	...
	if( g_pcsAgpmQuest )
	{
		...
		{
			MD_SetErrorMessage( "AgpmQuest �������ȡʧ��" );
		}
	}
...
RpAtomic *	_GetWorldSectorGeometryCallback( RpIntersection* intersection, RpWorldSector* sector, RpAtomic* pAtomic, RwReal distance, void* data )
{
	...
	INT32	index;
	if( ACUOBJECT_TYPE_WORLDSECTOR != AcuObject::GetAtomicType( pAtomic, &index ) )		// 아토믹의 타입이 월드섹터 일때�?등록..
		return pAtomic;
...
BOOL MainWindow::OnLButtonDown	( RsMouseStatus *ms	)
{
			...
			case	BRUSHTYPE_EDGETURN		:
				{
					...
					{
						// 마고�?(2004-06-15 오후 3:42:16) : 엣지�?처리.
						...
						// 마고�?(2004-06-25 오전 11:32:42) : 업데이트 통보..
				...
				if( m_pCurrentGeometry && ISBUTTONDOWN( VK_MENU ) )
				{
					...
					for( j = z1 ; j <= z2 ; j ++ )
					{
						for( i = x1 ; i <= x2 ; i ++ )
						{
							...
							{
								// 아자.;; 
								// 이제 각각 세그먼트�?둘러�? �?범위 내에 들어있는 세그먼트라면 Height�?적용한다.
	...
	// 지�?선택.
	// �?m_pFoundSector�?Idle Time 마다 체크해서 저장돼�?녀석이�?
...
//	MTRACE( "KEY DOWN = '%c'\n" , ks->keyCharCode );
	...
	case	'.'	:
		{
			// 마고�?(2004-05-29 오후 9:54:05) : 타�?모드 스케�?변�?�?.
			switch( GetCurrentMode() )
			{
			case EDITMODE_TILE:
				{
	...
	// 마고�?(2004-06-22 오후 8:23:18) : 프로파일링~
	#ifdef _PROFILE_
	case rsPGUP:
				...
				if( m_bCopyObject )
				{
					...
					while( pNode )
					{
...

						{
							...
							if( !pstNewAgcdObject )
							{
								MD_SetErrorMessage( "g_pcsAgcmObject->GetObjectData 오브젝트 클라이언�?정보 얻기 실패" );
								continue;
							}
							...
							{
								MD_SetErrorMessage( "g_pcsApmObject->UpdateInit 클럼�?설정 실패" );
								continue;
							}
						}
...
//				DisplayMessage( AEM_NOTIFY , "Fill 기능은 Undo�?지원하지 않습니다. 참고하세�?.;;" );
				m_UndoManager.StartActionBlock( CUndoManager::TILE );
...
//    RwInt32					matIndex2	;
	...
	*/
    ...
    {
		for (j = 0; j < MAP_WIDTH ; j++, k += 6 ) // 폴리�?두개�?처리�?
		{
			RwV3d normal = {0, 1, 0};
...
RpWorld *		MainWindow::CreateWorld			( void )
{
	...
	params.numTexCoordSets		= 2		; // 멀티텍스쳐 지�?
			...
			case DWSECTOR_ALLROUGH			:
				{
					...
					else
					{
						...
						//else
						//{
						//	DisplayMessage( AEM_ERROR , "티테일맵�?업는 곳은 러프맵을 생성할수 없습니다." );
						//	return;
						//}
					}
				}
...
void		MainWindow::MoveToSectorCenter( ApWorldSector * pSector )
{
	...
	// 마고�?(2003-11-25 오후 1:01:12) : 카메�?빠지는현상제�?.
...
int	MainWindow::GetCurrentMode()	// 현재 작업중인 모드�?얻어�?
{
	// 모드�?찾아�?리턴
	// 해당 모드�?대�?정보�?MapToolConst.h �?들어있다.
	return g_pMainFrame->m_Document.m_nSelectedMode;
}

int	MainWindow::GetCurrentLayer()	// 현재 작업중인 모드�?얻어�?
{
	// 모드�?찾아�?리턴
	// 해당 모드�?대�?정보�?MapToolConst.h �?들어있다.
...
ApDetailSegment * MainWindow::GetCurrentPolygonsTileIndex( ApWorldSector *pWorldSector , int polygonindex ,
													  ...
													  int *pPosDetailedX , int *pPosDetailedY )// 세부좌표 , 나중�?사용�?
{
// 월드섹터와 폴리�?인덱스로 , 해당 폴리건이 속한 섹터와 , 그의 좌표�?얻어낸다.
...
//	// lefttop , rightbottom 에는 y값은 의미가 없고 , 각각 좌표�?가진다.
//
...
{
	// 마고�?(2004-06-16 오후 2:53:11) : 처리변경으�?.. agcmmap 으로 통합..
	return AGCMMAP_THIS->GetWorldSectorInfo( pMapInfo , pSector , x , z , detailedX , detailedZ );
}


float		MainWindow::SetSectorHeight( stFindMapInfo * pFindMapInfo , float height ) // 9개의 배열�?입력으로..
{
...
void		MainWindow::DrawHeightGrid				()
{
	...
	if( m_pCurrentGeometry && m_pCurrentGeometry->GetCurrentDetail() > SECTOR_EMPTY )
	{
		...
		// 마고�?(2002-08-02 오후 12:19:27) : 조만�?처리하자..
...
//				fZ = m_pCurrentGeometry->GetZStart() + ( FLOAT ) j * m_pCurrentGeometry->GetStepSizeZ() ;
				...
				if( i < nDepth && j < nDepth )
				{
					...
					if( bShowPolygonSplit )
					{
						// 마고�?(2004-06-16 오전 10:31:39) : 엣지�?표시..
...
void MainWindow::TileIdleAdjustment()
{
	...
	// 마고�?(2004-04-16 오후 3:31:32) : 러프�?전혀 처리하지 않는 것을 기본으로 한다.
		...
		case TILELAYER_TONGMAP		:
			{
				if( GetAsyncKeyState( VK_MENU ) >= 0 && m_pCurrentGeometry->GetCurrentDetail() == SECTOR_HIGHDETAIL )
				{
					...
					{
						DisplayMessage( AEM_ERROR , "선택?타일이 없어�?타일을 출력�?�?없습니다." );
						m_bTileAdjust = FALSE;
						m_UndoManager.EndActionBlock();
						break;
					}
			...
			case IDM_OBJECT_PROPERTY:
				{
					...
					catch(...)
					{
						...
						char	str[] = "Dialog가 정상적으�?종료돼지 않았습니�?. 후딱 쎄이브하�?다시켜세�? 세이브가 위험할수�?있습니다! -_-; 그래�?쎄이�?하시겠습니까?.";
			...
			case IDM_OBJECT_EVENT:
				{
					...
					#endif // _DEBUG
					{
						...
						{
							// 마고�?(2004-06-10 오전 11:46:44) : 변화상황을 모듈�?통보�?.
					...
					{
						char	str[] = "Dialog가 정상적으�?종료돼지 않았습니�?. 후딱 쎄이브하�?다시켜세�? 세이브가 �?위험�?�?있습니다! -_-; 그래�?쎄이�?하시겠습니까?.";
						if( IDYES == g_pMainFrame->MessageBox( str , "에러" , MB_ICONERROR | MB_YESNO ) ) 
						{
							g_pMainFrame->Save();
						}
...
//	}
	...
	// DWSector Setting�?따라�?.. 주위 디테�?섹터 설정�?변경해준�?
	switch( g_pMainFrame->m_Document.m_nDWSectorSetting )
	{
	case DWSECTOR_EACHEACH			:
	case DWSECTOR_ALLROUGH			:
...
//	{
//		// 에러표시
//		DisplayMessage( AEM_NOTIFY , "현재 모드에서�?작동하지 않습니다. 타�?알파찍기 모드�?바꿔주세�?" );
//		return;
//	}
//
...
//
//	CProgressDlg	dlg;
//	dlg.StartProgress( "알파타�?계산�?.." , m_listTileSelect.GetCount() , g_pMainFrame );
//	int	count = 0;
//
...
//	dlg.EndProgress();
//
//	DisplayMessage( AEM_NORMAL , "알파타�?계산�?끝났습니�?" );
}
...
//		pMapInfo->pSegment->pIndex[ TD_THIRD ] != ALEF_TEXTURE_NO_TEXTURE )
	{
		...
		// 마고�?(2004-05-28 오후 9:47:46) : 한개�?4타�?알파 처리�?.임시 버퍼.
...
//						TilePattern[ j ].Empty();
						...
						// 마고�?(2004-06-01 오후 12:37:48) : �?없에버림..
...
BOOL		MainWindow::ApplyFill					(stFindMapInfo *pMapInfo, int basetile , int replaceindex , BOOL bReculsive , BOOL bFirst )
{
	...
	for ( i = 0; i < 9; i++ )
	{
		...
		if (	pFindMapSelf[ i ].pSector 
			&&	pFindMapSelf[ i ].pSector == pMapInfo->pSector // 섹터 범위�?넘어가�?못하�?.
			)
		{
			// �?타일이 아니�?..
...
BOOL MainWindow::OnApplyFill( )
{
	...
	if( bFirst )
	{
		DisplayMessage( AEM_NOTIFY , "참고 : 필기능은 섹터�?넘지 못합니다." );
		bFirst = false;
	}
...
/*	
		오브젝트 브러싱은 일단 브러시의 범위�?체크해서 ,
		어느어느 타일에 오브젝트가 삽입�?�?것인가를판단한�?
		그리�?�?타일들�?검사하면서 , 해당타�?안에 있는오브젝트�?
		밀도를 구한�? 그리�?밀도가 일정 이하�?, 렌덤�?조건�?생성하여
		�?위치�?오브젝트�?삽입한다..

		ALT�?누르�?있는경우�?, 오브젝트 삭제 기능�?실행한다..
		역시 타일단�?검사를 중심으로하며 , 해당타일안�?있는 오브젝트�?삭제한다..
*/
...
BOOL	GetWorldPosToScreenPos		( RwV3d * pWorldPos , POINT * pPoint )
{
	...
	if( OutV.z < 0 )	return FALSE; // 카메�?뒤에 있다.
	else				return TRUE;
}
...
static RpClump *   __CallBackObjectSelection ( RpClump * pClump , void *pData )
{
	...
	case	ACUOBJECT_TYPE_OBJECT		:
		// 오브젝트�?. 등록�?.		
		if( // 마고�?(2004-11-30 오전 11:14:57) : 던젼 오브젝트이면 튕겨�?..
...
void	MainWindow::SetObjectSelection			( POINT p1 , POINT p2 , BOOL bGeometryCheck	)
{
	// 두점 사이�?있는 오브젝트�?구한�?.
	// 올라가있는 모든 오브젝트에서..
	// GetWorldpoToScreenPos�?호출, 위의 범위�?들어가는지 점검한다..
	// 그리곤리스트�?�?내용을추가한다.
...
{
	// 리스트를 초기화하�?, pClump만을 선택 리스트에 등록한다.
	m_listObjectSelected.RemoveAll();
...
void	MainWindow::AddObjectSelection			( RpClump * pClump, BOOL bSelecting 	)
{
	...
	{
		// 마고�?(2004-05-31 오후 9:02:44) : 범위 체크�?한다..범위 밖에 있는 녀석들은
		// 편집�?돼지 않도�?
...
void	MainWindow::DrawObjectSelectionWire		()
{
	...
	// 리스트에 등록되어있는 오브젝트에관�?WireFrame�?그린�?
	static
...
void	MainWindow::DrawGridInfo	()
{
	...
	// 마고�?(2005-12-15 오후 12:12:27) : 테스�?서버데이타 표시..
	if( m_pCurrentGeometry )
	{
...
void	MainWindow::OnWindowRender	()					 // Im2D�?Render하기
{
...
//	g_pcsAgcmEventNature->SetFogSetting();
	...
	if( g_pMainFrame->m_Document.m_bToggleShadingMode )
	{
		...
		case	rwSHADEMODEFLAT		:
			...
			DisplayMessage( AEM_NOTIFY , "셰이�?모드 변�?: Gouraud" );
			break;
		case	rwSHADEMODEGOURAUD	:
			RwRenderStateSet( rwRENDERSTATESHADEMODE , ( void * ) rwSHADEMODEFLAT );
			DisplayMessage( AEM_NOTIFY , "셰이�?모드 변�?: Flat" );
			break;
		}
	}
	...
	// 마고�?(2004-04-21 오후 7:04:16) : 백골아저�?폰트�?버그�?블러�?
	if( CMainFrame::MT_VM_NORMAL == g_pMainFrame->m_bFullScreen )
	{
		...
		if( g_pcsAgcmFont )
		{
			...
			// 마고�?(2004-04-02 오후 7:04:58) : 로테이트 값으�?화면 출력..
			...
			case EDITMODE_OBJECT:
				{
					if( IsObjectSelected() )
					{
						sprintf( str , "Total Object Count = ( %d / %d ) , 선택�?오브젝트 %d �?	, 
...
BOOL MainWindow::LockSector			( ApWorldSector * pSector	, RwInt32 nLockMode	)	// 현재 섹터�?락함.
{
	return AGCMMAP_THIS->LockSector( pSector , nLockMode );
}

BOOL MainWindow::UnlockSectors		( BOOL bUpdateCollision  , BOOL bUpdateRough )	// 락한 모든 섹터�?언락�?
{
