...
BOOL MainWindow::OnInit()		// 초기화 , 데이타 수집 & 그래픽 데이타 로딩.	WindowUI 의경우 Open돼는 시점, Full UI의 경우 Setting돼는 시점.
{
	...
	{
		::MessageBox( NULL , "无法读取技能模板。模板可能未正确加载。" , "맵툴" , MB_ICONERROR | MB_OK );
	}
	...
	{
		::MessageBox( NULL , "无法读取项目模板。模板可能未正确加载。" , "맵툴" , MB_ICONERROR | MB_OK );
	}
	...
	if (g_pcsAgcmGrass)
	{
		...
		{
			if( IDYES == ::MessageBox( NULL , "./Ini/GrassTemplate.ini 我在看文件的时候程序死掉了。你要杀了现在的程序吗？" , "맵툴" , MB_YESNOCANCEL | MB_ICONERROR ) )
			{
				throw;
			}
		}
	...
	*/
	...
	if( g_pcsAgpmQuest )
	{
		...
		{
			MD_SetErrorMessage( "AgpmQuest 任务组读取失败" );
		}
	}
		...
		case TILELAYER_TONGMAP		:
			{
				if( GetAsyncKeyState( VK_MENU ) >= 0 && m_pCurrentGeometry->GetCurrentDetail() == SECTOR_HIGHDETAIL )
				{
					...
					{
						DisplayMessage( AEM_ERROR , "선택?타일이 없어서 타일을 출력할 수 없습니다." );
						m_bTileAdjust = FALSE;
						m_UndoManager.EndActionBlock();
						break;
					}
