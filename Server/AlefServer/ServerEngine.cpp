...
//JK_쉬라인머지
AgpmShrineBattle		*g_pcsAgpmShrineBattle;
AgsmShrineBattle		*g_pcsAgsmShrineBattle;
...
AgpmWarInfo				*g_pcsAgpmWarInfo;
AgsmWarInfo				*g_pcsAgsmWarInfo;
...
//#endif	//_PROFILE_
	...
	REGISTER_MODULE(g_pcsAgsmWarInfo, AgsmWarInfo);
	...
	//JK_쉬라인머지
	REGISTER_MODULE(g_pcsAgpmShrineBattle, AgpmShrineBattle);
	REGISTER_MODULE(g_pcsAgsmShrineBattle, AgsmShrineBattle);
...
//#endif	//_PROFILE_
	...
	printf("Processing - 30-2\n");
	if (!g_pcsAgpmCharacter->		StreamReadMurderPenalty("Ini\\MurderPenalty.txt", ENC_SVR_DATA))
	{
		ASSERT( !"g_csAgpmCharacter StreamReadMerderPenalty 실패" );
		return FALSE;
	}
	...
	printf("Processing - 42.6\n");

	if (!g_pcsAgpmItem->StreamReadGachaDropPointTable("Ini\\GachaDropTablePoint.txt", ENC_SVR_DATA))
	{
...
*/
	...
	if (!g_pcsAgpmCharacter->StreamReadChatFilterText("Ini\\FilterCharacName.txt", ENC_SVR_DATA))
	{
	...
	if(!g_pcsAgsmChatting->ReadFlagImoticon("Ini\\FlagImoticon.xml"))
	{
		ASSERT(!".\\Ini\\FlagImoticon.xml");
		return FALSE;
	}

	printf("Processing - 86\n");
	...
	printf("Processing - 87\n");

	if(!g_pcsAgsmAdmin->StreamReadAdminTypeSpecial("ini\\AdminTypeSpecial.xml"))
	{
		ASSERT(!".\\ini\\AdminTypeSpecial.xml");
		return FALSE;
	}
	...
	//JK_쉬라인머지
	printf("Processing - 85\n");
	if(!g_pcsAgpmShrineBattle->LoadShrineBattleData())
	{
		ASSERT(!".\\ini\\ShrineTable.txt");
		return FALSE;
	}
