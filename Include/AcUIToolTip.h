...
#define	ACUITOOLTIP_LEFT_TOOLTIP								0x00000001
#define ACUITOOLTIP_RIGHT_TOOLTIP								0x00000002
...
public:		// New Tool Tip UI System 
	...
	// Window를 움직일 수 있도록 한다.
	VOID						SetMoveWindow					( BOOL bMoveWindow );

	VOID						EnableExitButton				( BOOL bEnable );	//	Exit 버튼을 만든다
	VOID						SetSubTooltipType				( eSubTooltipType type )	{ subTooltipType_	=	type;	}
	...
	eSubTooltipType				GetSubToolTipType()				{ return subTooltipType_; }

	static	BOOL												m_bViewToolTip;			// Option창에서 설정

private :
	VOID						_InitExitButton					( VOID );
	VOID						_UpdateExitButtonPosition		( VOID );
	...
	BOOL						m_bEnableExit;
	AcUIBase*					m_pExitButton;

	eSubTooltipType				subTooltipType_;
