...
private:
	...
	AgcdUIUserData *	m_apstQuickBelt[AGCMUIMAIN_MAX_QUICKBELT*2];
...
//	AgcdUIUserData *	m_pstSkill;
	...
	INT32				m_lEventShowAdditionalQuickBelt[3];
	INT32				m_lEventHideAdditionalQuickBelt[3];
...
public:
	AgpdGrid			m_astQuickBeltGrid[AGCMUIMAIN_MAX_QUICKBELT*2];
//	AgpdGrid			m_astVQuickBeltGrid[2];
//	AgpdGrid			m_stSkillGrid;
	INT32				m_nNumOfActiveQuickbelt;
...
public:
	...
	BOOL			AddShortcut(AgpdGrid *pcsGrid, AgpdGridItem *pcsGridItem, INT32 lLayer, INT32 lRow, INT32 lColumn, BOOL bIsThrowEvent = TRUE, INT32 lRealLayer = 0);
	BOOL			RemoveShortcut(AgpdGrid *pcsGrid, INT32 lLayer, INT32 lRow, INT32 lColumn, BOOL bIsThrowEvent = TRUE, INT32 lRealLayer = 0);
...
#ifdef _AREA_GLOBAL_
	BOOL			LoadSystemMessageGridToolTip( char* pFileName, BOOL bIsEncrypt );
	BOOL			AddSystemMessageGridItem(AgpdGridItem *pcsGridItem, ApModuleDefaultCallBack pfUseCallback = NULL, PVOID pvCallbackClass = NULL , BOOL bFrontAdd = FALSE, UINT32 nExpireTime = 0 );
#else
...
#ifdef _AREA_GLOBAL_
	UINT32			GetGridItemExpireTime( void* pGridItem );
#endif
	...
	BOOL			UseRideItemInQuickBelt(AgpdItem* pcsItem);
...
//	static BOOL		CBRemoveItem(PVOID pData, PVOID pClass, PVOID pCustData);
	...
	static BOOL		CBChangedUseNewSlot(PVOID pData, PVOID pClass, PVOID pCustData);
...
public :
#ifdef _AREA_GLOBAL_
	BOOL			OnAddSystemGridControl				( AgpdGridItem* ppdGridItem, ApModuleDefaultCallBack fnCallBack, void* pClass , BOOL bFrontAdd = FALSE, UINT32 nExpireTime = 0 );
#else
	...
	BOOL			OnAdjustHPMPBarPosition				( INT32 nLayer = 0 );
	UINT			GetCenterXPosOfUI					( UINT nWidth = 0 );
	UINT			GetYPosOfBelt						( UINT nHeight = 0, UINT nLayer = 0 );
