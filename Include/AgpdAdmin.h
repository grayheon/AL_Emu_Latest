...
#define AGPMADMIN_MAX_REGION_INDEX				300//256	//JK_쉬라인머지 리전 추가로 최대값 변경
...
typedef std::vector<string> AdminNameVector;
typedef std::vector<string> AdminCommandVector;

typedef struct _stAgpdAdminTypeSpecialGroup
{
	INT32	m_lGroupID;

	AdminNameVector			m_vtAdminName;
	AdminCommandVector		m_vtAdminCommand;

	_stAgpdAdminTypeSpecialGroup()
		: m_lGroupID(0)
	{
		m_vtAdminName.clear();
		m_vtAdminCommand.clear();
	}

} stAgpdAdminTypeSpecialGroup, *pstAgpdAdminTypeSpecialGroup;
