...
#define AGSMCHARACTER_ESCAPE_COMMAND_COOL_TIME			60 * 60
...
typedef enum _eAgsmCharacterCB {
	...
	AGSMCHARACTER_CB_UPDATE_USE_ESCAPE_DATE
...
typedef enum  AgsmCharacterPacketOperation {
	...
	AGSMCHAR_PACKET_OPERATION_COUPON_INFO
...
private:
	...
	AgpmShrineBattle*	m_pagpmShrineBattle;//JK_쉬라인추가
...
private:
	...
	// coupon
	BOOL				OnOperationCouponList(PVOID pvCharDetailInfo, UINT32 ulNID);
...
public:
	...
	*/
	...
	static BOOL CBRequestCoupon(PVOID pData, PVOID pClass, PVOID pCustData);
	static BOOL CBRequestCouponUse(PVOID pData, PVOID pClass, PVOID pCustData);

	static BOOL CBSetTeleportBlock(PVOID pData, PVOID pClass, PVOID pCustData);
	static BOOL CBCheckTeleportBlock(PVOID pData, PVOID pClass, PVOID pCustData);

	static BOOL CBAddGachaItemUpdate(PVOID pData, PVOID pClass, PVOID pCustData);
	...
	*/
	...
	BOOL CalcCharacterIgnoreStatus(AgpdCharacter *pcsCharacter, AgpdCharacterIgnoreStatus *pcsIgnoreStatus, BOOL bSendPacket = TRUE);
	...
	BOOL SendPacketCharPosition(AgpdCharacter *pcsCharacter , BOOL bSendSelf = TRUE );
	BOOL SendPacketCharPosition(AgpdCharacter *pcsCharacter, DPNID dpnid, BOOL bGroupNID = FALSE , BOOL bSendSelf = TRUE );
	BOOL SendPacketSyncCharPos(AgpdCharacter *pcsCharacter);
	...
	*/
	...
	// coupon
	BOOL SendCouponInfo(AgpdCharacter *pcsCharacter, UINT32 ulNID);
...
//	BOOL				StreamUpdateDBMajorData(AgpdCharacter *pcsCharacter, ApModuleDefaultCallBack pfCallback, PVOID pClass);
	...
	*/
	...
	BOOL				SetCallbackUseEscapeDate(ApModuleDefaultCallBack pfCallback, PVOID pClass);
	...
	BOOL				WriteExpLog(AgpdCharacter* pcsCharacter, INT32 lExp, BOOL bIsQeuest);
	...
	BOOL				AddExp(AgpdCharacter *pcsCharacter, INT64 llInputAddExp, BOOL bCheckLevelup = TRUE);
	BOOL				SubExp(AgpdCharacter *pcsCharacter, INT64 llSubExp);

	BOOL				AddExpPercent(AgpdCharacter *pcsCharacter, INT64 llInputAddExpPercent, BOOL bCheckLevelup = TRUE);
...
public:
	...
	BOOL	ProcessSavedSpecialStatus(AgpdCharacter *pcsCharacter, UINT64 ulSpecialStatus, BOOL bAdd);
	BOOL	LoadSavedSpecialStatus(AgpdCharacter *pcsCharacter);
	...
	BOOL	UseEscapeCommand(AgpdCharacter *pcsCharacter);

	// read
	BOOL	StreamReadBaseCharacterInfo(CHAR *szFile);
};
