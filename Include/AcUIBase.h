...
#define	ACUIBASE_STATUS_MAX_NUM							100
#define	ACUIBASE_STATUS_NAME_LENGTH						32
#define	ACUIBASE_STATUS_ID_BIT_CONSTANT				0x00080	// 반드시 ACUIBASE_STATUS_MAX_NUM 보다는 큰 값이 어야 한다 
#define	ACUIBASE_STATUS_ID_MAX								1000

#define RESIZE_BOX_SIZE									5
...
enum	eDefaultRenderType
{
	UICM_BASE_DEFAULT_RENDER		,		//	기본 렌더방식
	UICM_BASE_RGBA_RENDER			,		//	RGBA 값을 이용해 배경을 그린다
	UICM_BASE_SMOOTH_RGBA_RENDER	,		//	Smooth하게 배경이 그려진다
};
...
public:
	...
	{
		NONE_BLINK		= 0,
		TEXT_BLINK		= 0x00000001,
		TEXTURE_BLINK	= 0x00000002,
	};
	...
	BOOL												m_bBold;				// Static String Style Bold
	...
	BOOL												m_bAutoClippingString;	//minasang: static String이 범위를 벗어나면 자르기.

	BOOL												m_bBlink;			// 블링크 설정.
	...
	BOOL												m_bPreLoad;		//미리 로드해놓는 타입의 ui
	BOOL												m_bEnablePrevLoad;
	...
	eDefaultRenderType									m_eDefaultRenderType;
	DWORD												m_dwDefaultRenderColor;

	DWORD												m_dwSmoothTime;
	DWORD												m_dwStartTime;

	// Wheel이 돌아갔을때 호출될 콜백
	AcUIDefalutCallback									m_pfnMouseWheelCB;
	PVOID												m_pMouseWheelClass;
...
public:
	...
	VOID				SetDefaultRenderType			( eDefaultRenderType eType );
	VOID				SetRenderRGBAColor				( DWORD dwColor );

	VOID				SetRenderSmoothTime				( DWORD	dwTick );
	VOID				SetRenderSmoothStartTime		( DWORD dwTick );

	VOID				SetMouseWheelCallBack			( PVOID pClass , AcUIDefalutCallback pfnCallBack )	{ m_pMouseWheelClass = pClass;	m_pfnMouseWheelCB = pfnCallBack;	}

	DWORD				GetRenderSmoothStartTime		( VOID )	{	return m_dwStartTime;	}
	...
	DWORD				GetColor						( void );
	UINT8				GetAlpha						();
	INT					SetBlink						( eBlinkType bBlink , DWORD dColor = 0 , DWORD dGap = 300 );

	INT					GetStringWidth					( VOID ) { return m_lStringWidth;	}
	INT					GetStringHeight					( VOID ) { return m_lStringHeight;	}
...
protected:
	...
	VOID				_RenderRGBARender				( VOID );
	VOID				_RenderSmoothRGBARender			( VOID );
	...
	BOOL				_CheckMouseReSizeArea			(const RsMouseStatus *ms);
	// Status 관련
public:
