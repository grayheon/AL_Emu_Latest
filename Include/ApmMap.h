...
public:
	...
	struct RegionTemplate
	{
		...
		struct PropertyList
		{
			...
			BYTE	bShrinePotion	: 1;	// 쉬라인 포션 사용 여부
...
public:
	...
	struct	WorldMap
	{
		...
		//월드맵 UI에서 쓸 텍스쳐
		char	szRegionTexture[WORLDMAP_COMMENT_LENGTH];					
		char	szRegionRoadTexture[WORLDMAP_COMMENT_LENGTH];

		//월드맵 그룹
		char szParentGroupNmae[WORLDMAP_COMMENT_LENGTH];
		char	szGroupName[WORLDMAP_COMMENT_LENGTH];
		...
		WorldMap() :	nMID ( 0 ) , xStart ( 0.0f ) , zStart ( 0.0f ) ,
			xEnd ( 0.0f ) , zEnd ( 0.0f ) , nMapItemID ( -1 )	, iGroupID(0)
			,iSX(0), iSY(0), iEX(0), iEY(0)	
		{
			strcpy( strComment , "No Map" );

			memset( szRegionTexture, 0, WORLDMAP_COMMENT_LENGTH * sizeof( char ) );
			memset( szRegionRoadTexture, 0, WORLDMAP_COMMENT_LENGTH * sizeof( char ) );

			memset( szParentGroupNmae, 0, WORLDMAP_COMMENT_LENGTH * sizeof( char ) );
			memset( szGroupName, 0, WORLDMAP_COMMENT_LENGTH * sizeof( char ) );
		}
	};

	std::vector< WorldMap > m_arrayWorldMap;
	BOOL	LoadWorldMap( char * pFileName , BOOL bDecryption = FALSE );
	BOOL	ReloadWorldMap( char * pFileName , BOOL bDecryption = FALSE );
	BOOL	SaveWorldMap( char * pFileName , BOOL bEncryption = FALSE );

	WorldMap *	GetWorldMapInfo( INT32 nMID );
	WorldMap *	GetWorldMapInfoByGroupName( const char* pszGroupName );
