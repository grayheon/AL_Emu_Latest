...
enum	eRatioType
{
	RESOLUTION_RATIO_0403	,
	RESOLUTION_RATIO_1609	,
	RESOLUTION_RATIO_1610	,
};
...
public:
	...
	VOID		InitRatio( VOID );
	eRatioType	GetRatio( VOID )	{	return m_eRatioType;		}
	...
	VOID	ToggleShowCrossFilter( VOID )	{	m_bCrossFilter	=	!m_bCrossFilter;	}
	...
	UINT32		GetPrevProcessClock() { return m_ulPrevProcessClock; }
	...
	virtual void			OnDrawScreen( RwCamera*	pCamera )	{			}
	...
	//  Texture Folder찾기
	virtual VOID		FindTextureDirectory( CONST CHAR*	szFindDirectory , string& strTexturePath );
...
public:
	...
	AgcModalWindow	*m_pEditOKCancelDialog	;	// Edit + OK + Cancel Dialog
#ifdef _AREA_GLOBAL_
	...
	INT32	EditMessageDialog( AgcWindow* pWindow, char* pszmessage, CHAR* pEditString , BOOL bOnlyNumber = FALSE )	;	// Messsage Dialog width EditBox
	INT32	EditMessageDialog2( AgcWindow* pWindow, char* pszmessage, CHAR* pEditString , BOOL bOnlyNumber = FALSE )	;	// Messsage Dialog width EditBox
...
#endif
	INT32	EditOKCancelMessageDialog( AgcWindow* pWindow , CHAR* pszMessage , CHAR* pEditString , BOOL bOnlyNumber = FALSE );
	...
	bool		DrawIm2D( RwTexture* pTex, 
		...
		DWORD color = 0xffffffff, UINT8 alpha = 255 , RECT* pClipping = NULL );
	BOOL		DrawIm2DPixel( RwTexture* pTexture,
		...
		DWORD uColor = 0xffffffff, UINT8 uAlpha = 255 , RECT* pClipping = NULL );
...
#endif
	VOID	SetEditOKCancelDialog( AgcModalWindow* pWindow ){	m_pEditOKCancelDialog = pWindow;}
