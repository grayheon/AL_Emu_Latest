...
struct MapLinkPos
{
	...
	MapLinkPos() { point.x = 0; point.y = 0; }

	MapLinkPos( float x, float y ) { point.x = x; point.y = y; }

	MapLinkPos( RwV2d & pos ) : point(pos) {}

	bool operator==( MapLinkPos const & other ) const {
	...
	inline int GetNID( size_t len ) const {
		float nID = point.x + point.y + (float)len;
		return (int)abs(nID);
	}
};
...
{
private:
	typedef std::map< MapLinkPos, std::string >				QuestPostions;
	typedef std::map< INT, QuestPostions >					QuestPositionsByQuestID;

	// 해당 퀘스트의 포지션 ID정보를 저장한다.( 나중에 퀘스트 완료,삭제시에 위치정보 삭제에 이용한다 )
	QuestPositionsByQuestID	m_qeustPositionsByID;

	AgcUIQuestText		*acceptQuestText_;
	AgcUIQuestText		*mainQuestText_;
	...
	AgcmUIWorldmap		*m_pcsAgcmUIWorldmap;
	AgcmQuestJournal	*m_pcsAgcmQuestJournal;
	AgcmUIQuestJournal	*m_pcsAgcmUIQuestJournal;
...
//	void SetTestData();
	...
	BOOL OpenWorldMap	( INT	nRegionIndex	=	-1 );

	VOID SetScrollButton			( VOID );
	...
	static BOOL	CBRemoveItemInventory(PVOID pData, PVOID pClass, PVOID pCustData);
	...
	INT32 ComposeAcceptItem(AgpdElementItem *pItem, char* pBuffer, INT32 lBufferSize , BOOL bSelect = FALSE );
	...
	INT32 ComposeMainSummaryText(CHAR *pBuffer, INT32 lBufferSize, AgpdQuestTemplate *pQuestTemplate, AgpdQuest *pAgpdQuest , BOOL bOnlyIncomplete = FALSE , BOOL bOneSummary = FALSE );	// return value : The number of characters written
	INT32 ComposeCheckPointText(Enum_AGPDQUEST_SUMMARY_FACTOR eCheckPointFactor, char *pBuffer, AgpdQuestTemplate *pQuestTemplate, AgpdItemADChar* pcsAgpdItemADChar , BOOL bOnlyIncomplete , BOOL *pbOneSummary );
	BOOL		AddQuestPositionInfo		( INT nQuestID , MapLinkPos pos, std::string & name );
	BOOL		DelQuestPositionInfo		( INT nQuestID , MapLinkPos pos );
	std::string GetQuestPosName				( INT nQuestID, MapLinkPos pos );

	BOOL		ClearQuestPositionInfo		( INT nQuestID );
	...
	static BOOL CBDisplayQuestMainTreeCheck(PVOID pClass, PVOID pData, AgcdUIDataType eType, INT32 lID, CHAR *szDisplay, INT32 *plValue, AgcdUIControl *pcsSourceControl);
	...
	static BOOL	CBCheckJournal( PVOID pClass , PVOID pData1 , PVOID pData2 , PVOID pData3 , PVOID pData4 , PVOID pData5 , ApBase* pcsTarget , AgcdUIControl* pcsSourceControl );
	static BOOL	CBClickJournalClose( PVOID pClass , PVOID pData1 , PVOID pData2 , PVOID pData3 , PVOID pData4 , PVOID pData5 , ApBase* pcsTarget , AgcdUIControl* pcsSourceControl );
	static BOOL CBClickQuestDel( PVOID pClass , PVOID pData1 , PVOID pData2 , PVOID pData3 , PVOID pData4 , PVOID pData5 , ApBase* pcsTarget , AgcdUIControl* pcsSourceControl );
	static BOOL	CBClickMapOpen( PVOID pClass , PVOID pData1 , PVOID pData2 , PVOID pData3 , PVOID pData4 , PVOID pData5 , ApBase* pcsTarget , AgcdUIControl* pcsSourceControl );


	static BOOL	CBReturnCancelQuest(PVOID pClass, INT32 lTrueCancel, CHAR* szInputMessage);

	BOOL		OpenMainQuest( VOID );

private:
	VOID				_RefreshTextList		( eQuestTextType eType , AcUIBase* pUIBase , BOOL bReParse = FALSE );

	// 유효성 체크 후에 정상이면 퀘스트템플릿 반납
	AgpdQuestTemplate*	_CheckQuest				( eQuestTextType eType );

	INT32				_GetTopQuestTID			( INT nTID );
};
