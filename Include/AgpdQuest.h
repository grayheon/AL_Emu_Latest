...
public:
	...
	AgpdCurrentQuest() : lQuestID(0), lParam1(0), lParam2(0), bRegisterJournal(FALSE)
	{
...
public:
	...
	// 새로운 진행중인 퀘스트 추가
	BOOL AddNewQuest(INT32 lQuestTID, INT32 lParam1 = 0, INT32 lParam2 = 0, BOOL bRegisterJournal = FALSE);

	// 진행중인 퀘스트 취소
	BOOL CancelQuest(INT32 lQuestTID);
	AgpdCurrentQuest*	GetQuest( INT32 lQuestTID );
