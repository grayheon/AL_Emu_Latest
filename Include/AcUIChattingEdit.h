#ifndef			_ACUI_CHATTING_EDIT_H_
#define			_ACUI_CHATTING_EDIT_H_

#include "AcUIEdit.h"
#include "ApMemory.h"

#include <list>

using namespace std;

enum	ChattingEditInputType
{
	CHAT_INPUT_NORMAL		= 0	,
	CHAT_INPUT_WHISPER			,
	CHAT_INPUT_PARTY			,
	CHAT_INPUT_GUILD			,
	CHAT_INPUT_SHOUT			,
	CHAT_INPUT_REPLY			,
};

enum	ChattingEditInputStatus
{
	CHAT_INPUT_MESSAGE		= 0	,
	CHAT_INPUT_WHISPER_ID	= 1,
};

#define	NUMBER_OF_CHAT_HISTORY	8
#define	NUMBER_OF_ID_HISTORY	4

struct	ChatHistory
{
	char			szBuffer[512];				// 512 size의 문자열만 기록 -_-a
	ChatHistory*	bef;
	ChatHistory*	next;
};

struct	WhisperIDHistory
{
	char				szBuffer[32];
	WhisperIDHistory*	bef;
	WhisperIDHistory*	next;
};

class AcUIChattingEdit : public AcUIEdit
{
private:
	struct stEventKey : public ApMemory< stEventKey , 20 >
	{
		INT			m_nIndex;
		string		m_strEventName;
		string		m_strName;
	};

	typedef		std::list< stEventKey* >					ListStringEvent;
	typedef		std::list< stEventKey* >::iterator			ListStringEventIter;

	BOOL			m_bChangedByMenu;

public:
	AcUIChattingEdit();
	~AcUIChattingEdit();

public:
	virtual	VOID	OnClose();
	virtual BOOL	OnChar( CHAR * pChar		, UINT lParam );
	virtual	BOOL	OnKeyDown( RsKeyStatus *ks	);
	virtual BOOL	OnIdle(UINT32 ulClockCount);
	
	virtual	VOID	OnWindowRender();
	virtual VOID	OnEditActive();

	VOID			SetCurrentInputType	( ChattingEditInputType eType );

	void			UpdateCurrentStatus	( VOID );		// 현재 입력 상태를 체크하여 갱신한다.
	void			ClearStatus			( VOID );		// 상태 초기화
	VOID			UpdateEvent			( VOID );		// 이벤트 업데이트

	VOID			SetCallbackRefresh	( PVOID pClass , AcUIDefalutCallback pfnCB );

	VOID			RefreshCustomData	( VOID );

	ChattingEditInputType	GetCurrentInputType( VOID )	{	return m_eInputType;	}

// Datas
	DWORD						m_ulCurTick;	

	ChattingEditInputType		m_ePrevInputType;
	ChattingEditInputType		m_eInputType;
	ChattingEditInputStatus		m_eInputStatus;

	// 2005.2.18 gemani .. 채팅 메시지를 역순으로 기억하고 같은 메시지는 추가X하고 환형큐 갱신만 한다..(ID역시 마찬가지)
	ChatHistory*			m_listChat;
	ChatHistory*			m_pCurChat;
	INT32					m_iChatNum;
	INT32					m_iChatStartPos;					// 메시지가 시작되는 포지션

	WhisperIDHistory*		m_listWhisperID;
	WhisperIDHistory*		m_pCurWhisperID;
	INT32					m_iWhisperIDNum;
	INT32					m_iWhisperIDStartPos;

	RwTexture*				m_pInputInfoTex;

	HKL						m_hKeyboardLayOut;

	AcUIDefalutCallback		m_pfnRefresh;
	PVOID					m_pRefreshClass;

public :
	BOOL					IsDoubleByteCode_Kr	( char* pString, int nIndex );

private:
	VOID					_EventParse			( string&	strString , INT* nIndex , INT nOffset = 0  );
	string					_KeyString			( string&	strString );
	VOID					_ProcessEvent		( VOID );

	ListStringEvent			m_listEvent;			// 이벤트를 저장해 둔다
};

#endif			//_ACUI_CHATTING_EDIT_H_

