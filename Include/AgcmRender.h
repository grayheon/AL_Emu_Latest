...
struct stBlendAtomicInfo
{
	...
	INT32						GetCount					( void ) { return (INT32)m_mapAtomics.size(); }
};
...
public:
	...
	BOOL	AddToRwFrameMapTable(const RwFrame* pFrame)
	{
		...
		m_UsableRwFrameTable.insert(pFrame);
	...
	BOOL	RemoveFromRwFrameMapTable(const RwFrame* pFrame)
	{
		...
		m_UsableRwFrameTable.erase( pFrame );
	...
	BOOL	FindFromRwFrameMapTable(const RwFrame* pFrame)
	{
		...
		std::set<const RwFrame*>::iterator Itr = m_UsableRwFrameTable.find(pFrame);
	...
	std::set<const RwFrame*>		m_UsableRwFrameTable;		//. 유효한 RwFrame 포인터인지 전역 적으로 관리가능한 stl map을 이용.
};
...
public:
	...
	RwRGBAReal						m_lightColorPicking;
	//. 2006. 9. 14. Nonstopdj
