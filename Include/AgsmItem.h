...
typedef enum _eAgsmItemCB {
	...
	AGSMITEM_CB_PRE_CHECK_ENABLE_JANUS_TALISMAN,
	AGSMITEM_CB_USE_ITEM_JANUS_TALISMAN,

	AGSMITEM_CB_ADD_GACHA_ITEM_UPDATE,
...
//	static BOOL CallbackApplyBonusFactor(PVOID pData, PVOID pClass, PVOID pCustData);
	...
	static BOOL CBItemInformation(PVOID pData, PVOID pClass, PVOID pCustData);
...
//	BOOL SendPacketRemoveConvertHistory(AgpdItem *pcsItem, INT32 lIndex, UINT32 dpnid);
	...
	BOOL SendPacketItemInformation(AgpdCharacter *pcsCharacter, AgpdItem* pcsItem, AgpdItemTemplate *pcsItemTemplate, INT32 lType);
...
//	PVOID MakePacketItemView(AgpdItem *pcsItem, INT16 *pnPacketLength, BOOL IsStaticPacket = FALSE);
	...
	*/
	...
	BOOL				SetCallbackPreCheckEnableJanusTalisman(ApModuleDefaultCallBack pfCallback, PVOID pClass);
	BOOL				SetCallbackUseItemJanusTalisman(ApModuleDefaultCallBack pfCallback, PVOID pClass);
	...
	*/
	...
	BOOL				UnuseChargingInventory(AgpdItem* pcsItem);
	...
	BOOL				UseChargingInventory(AgpdItem *pcsItem);	// ChargingInventory
	...
	BOOL				UseItemJanusTalisman(AgpdItem *pcsItem, AgpdCharacter *pcsTargetChar, BOOL bIsWriteLog = TRUE);
	...
	BOOL				GiveItem(AgpdCharacter *pcsCharacter, INT32 lItemTID, INT32 lCount, eAGPDLOGTYPE_ITEM eReason);
...
//	BOOL				ParseStringItemConvertHistory(AgpdItem *pcsItem, CHAR *szStringBuffer);
	...
	BOOL				CalcItemOptionSkillData(AgpdItem* pcsItem, BOOL bAdd = TRUE, INT32 lPrevLevel = 0, BOOL bEvolution = FALSE);
	BOOL				UpdateItemOptionSkillData(AgpdCharacter* pcsCharacter, BOOL bFirst = FALSE, INT32 lPrevLevel = 0, BOOL bEquip = TRUE);
	static BOOL			CBAddItemOption(PVOID pData, PVOID pClass, PVOID pCustData);
	BOOL CalcItemOptionIgnoreStatus(AgpdCharacter *pcsCharacter, AgpdItem* pcsItem, BOOL bAdd);
	...
	BOOL				SetCharacterJanusTypeFromItem(AgpdCharacter *pcsCharacter, AgpmItemUsableJanusTalismanType eType);

	BOOL				AddItemToCharacterPointForGacha( AgpdCharacter *pcsCharacter, AgpdItem *pcsItem );
};
