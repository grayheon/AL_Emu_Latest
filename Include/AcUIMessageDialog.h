...
public :
	...
	VOID					SetButtonName				( CHAR* szOK );
	VOID					EditControlImage			( CHAR* szFileName );

public :
	AcUIEdit											m_clEdit;
};

class AcUIEditOKCancelDialog : public AcUIEditOKDialog
{
public :
	AcUIEditOKCancelDialog( VOID );
	~AcUIEditOKCancelDialog( VOID );

	virtual BOOL			OnInit						( void );
	virtual BOOL			OnCommand					( INT32 nID , PVOID pParam );

	VOID					SetCancelButtonName			( CHAR* szCancel );

public :
	AcUIButton											m_clCancelButton;
	string												m_strCancelName;
};
