...
#define AGPMITEM_OPTION_MAX_DESCRIPTION				75
...
struct stOptionSkillData
{
	...
	INT32				m_lReinforcementRate;

	// 저항력 무시 관련 신규 옵션
	AgpdCharacterIgnoreStatus		m_stIgnoreStatus;
	...
	void init()
	{
		...
		m_lSkillLevelUp = m_lBonusExp = m_lBonusMoney = m_lBonusDropRate = m_lReinforcementRate = 0;
	...
	stOptionSkillData& operator += (const stOptionSkillData& data)
	{
		...
		m_lReinforcementRate += data.m_lReinforcementRate;
