...
#define AGPMSIEGEWAR_OBJECT_MAX_USE_RANGE		1100

#define GUILD_PVP_SEND_TERM_MIN					3
...
public:
	...
	UINT32	m_ulLastGuildPvPSendTime;
...
private:
	...
	ApAdmin	m_acsSiegeWarRegionAdminCID[AGPMSIEGEWAR_MAX_CASTLE];
...
public:
	...
	static BOOL CBReadyArchlordSiegewar(PVOID pData, PVOID pClass, PVOID pCustData);
...
public:
	...
	BOOL	SendWarInfoToSiegeWarRegion(AgpdSiegeWar *pcsSiegeWar, eAgpmWarInfoType eWarType, INT8 cWarStep, INT8 cMessageStep);
	BOOL	SendWarInfoToChar(AgpdCharacter *pcsCharacter, eAgpmWarInfoType eWarType, INT8 cWarStep, INT8 cMessageStep);
	BOOL	SendPacketInSiegeWar(AgpdSiegeWar *pcsSiegeWar, PACKET_HEADER& pPacket);

	BOOL	SendFactor(AgpdSiegeWar *pcsSiegeWar, CHAR *szFactor1, CHAR *szFactor2, CHAR *szFactor3);
	BOOL	SendEndWar(AgpdSiegeWar *pcsSiegeWar);
	BOOL	SendEndWarToChar(AgpdSiegeWar *pcsSiegeWar, AgpdCharacter *pAgpdCharacter);

	BOOL	OnPvPDead(AgpdCharacter *pAgpdCharacterKill, AgpdCharacter *pAgpdCharacterDead);

	BOOL	CheckGuildPvPSend(UINT32 ulClockCount);
	BOOL	SendGuildPvPScore();

	BOOL	SendRemainTime(AgpdSiegeWar *pcsSiegeWar, UINT32 ulClockCount);

	BOOL	IsOnGoingSiegeWarTime();
};